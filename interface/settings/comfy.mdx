...
- **Default Value**: Disabled
- **Experimental**: Yes
- **Function**: Restores widget values by matching their names instead of their position when loading a workflow, keeping values attached to the same widget even if the graph layout changed.

### Deduplicate subgraph node IDs
- **Default Value**: Enabled
- **Experimental**: Yes
- **Deprecated**: Yes
- **Function**: Automatically reassigns duplicate node IDs in subgraphs when loading a workflow.
