...
- `gpu-only`: GPU-only mode, keeps models on GPU without offloading to CPU
...
**Options**:
- `none` (Default): No preview images displayed, only shows progress bar during generation
...
### Enable legacy Manager UI
- **Default**: `false`
- **Type**: boolean

**Function**: Uses the old ComfyUI-Manager UI instead of the new one. Enable this if you prefer the classic Manager interface or encounter compatibility issues with the new UI.
