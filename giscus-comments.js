...
(function() {
  ...
  function showGiscusNotice(container, noticeType = 'rate_limit') {
    ...
    const noticeMessages = {
      ...
      unauthenticated: {
        en: {
          ...
          message: 'You are viewing comments as a guest. Sign in to GitHub to post comments and enjoy higher rate limits (5000 requests/hour vs 60 for guests).',
          suggestion: 'After signing in, you can post comments directly here. You can also check existing discussions or start a new one.',
          discussionLink: 'Find Related Discussions',
          newDiscussionLink: 'Start New Discussion',
        ...
        zh: {
          title: '🔐 登录 GitHub',
          message: '您当前以访客身份查看评论。登录 GitHub 后可以发表评论，并享受更高的 API 限额（每小时 5000 次请求，访客为 60 次）。',
          suggestion: '登录后，您可以直接在此发表评论。您也可以查看现有讨论或发起新讨论。',
          discussionLink: '查找相关讨论',
          newDiscussionLink: '发起新讨论',
    ...
    // For unauthenticated users, add sign-in button
  ...
  function setupGiscusErrorListener() {
    window.addEventListener('message', function(event) {
      ...
      if (message && message.giscus) {
        ...
        // viewer === null but discussion !== null means user is not logged in but can view comments
        // We only show notice if there's an error, not for normal viewing
        if (message.giscus.viewer === null && message.giscus.discussion !== null) {
          console.log('Giscus: User is not authenticated (viewer is null)');
          // Don't show error for unauthenticated users who can view comments normally
          // Only show notice if there's an actual error
          return;
        }
        ...
        if (message.giscus.error) {
          ...
          if (errorType === 'rate_limit' || errorType === 'rate-limit' || errorType === 'RATE_LIMITED') {
            // Check if rate limit is due to unauthenticated user
            if (message.giscus.viewer === null) {
              // Unauthenticated user hit rate limit, suggest login
              showGiscusNotice(container, 'unauthenticated');
            } else {
              // Authenticated user hit rate limit
              showGiscusNotice(container, 'rate_limit');
            }
          } else if (errorType === 'unauthorized' || errorType === 'UNAUTHORIZED') {
            ...
            if (container.querySelector('.giscus-frame')) {
              ...
              if (iframe && iframe.contentDocument) {
                ...
                if (errorElements.length > 0) {
                  ...
                  // Check error content to determine if it's authentication-related
                  const errorText = iframe.contentDocument.body?.textContent || '';
                  if (errorText.includes('sign in') || errorText.includes('login') || errorText.includes('authenticate')) {
                    showGiscusNotice(container, 'unauthenticated');
                  } else {
                    showGiscusNotice(container, 'rate_limit');
                  }
      ...
      if (typeof message === 'string') {
        if (message.includes('rate limit')) {
          ...
          showGiscusNotice(container, 'rate_limit');
        } else if (message.includes('sign in') || message.includes('login') || message.includes('authenticate')) {
          console.warn('Giscus: Authentication required detected in string message');
          showGiscusNotice(container, 'unauthenticated');
        }
      }
    });
          ...
          setTimeout(() => {
            ...
            if (iframeDoc) {
              ...
              if (bodyText.includes('rate limit') || bodyText.includes('API rate limit') || bodyText.includes('exceeded')) {
                ...
                showGiscusNotice(container, 'rate_limit');
              } else if (bodyText.includes('sign in') || bodyText.includes('login') || bodyText.includes('authenticate')) {
                console.warn('Giscus: Authentication required detected in iframe content');
                showGiscusNotice(container, 'unauthenticated');
      ...
      if (container.classList.contains('loading') || (iframe && !hasError)) {
        ...
        try {
          if (iframe && iframe.contentDocument) {
            ...
            if (bodyText.includes('sign in') || bodyText.includes('login') || bodyText.includes('authenticate')) {
              console.warn('Giscus: Authentication required detected in iframe after timeout');
              showGiscusNotice(container, 'unauthenticated');
            } else if (bodyText.includes('error occurred') || bodyText.includes('rate limit') || bodyText.includes('exceeded')) {
  ...
  window.giscusDebug = {
    ...
    // Force show unauthenticated notice
    showUnauthenticatedNotice: () => {
      const container = document.querySelector('.giscus-container');
      if (container) {
        showGiscusNotice(container, 'unauthenticated');
      } else {
  ...
  function init() {
    ...
    console.log('- giscusDebug.showUnauthenticatedNotice() - Show unauthenticated notice');
