...
|`convertWidgetToInput`|Convert a widget to an input if allowed by `isConvertableWidget` (in `widgetInputs.js`). Deprecated in v1.16 frontend as widget and socket co-exist. Conversion is no longer necessary. |
