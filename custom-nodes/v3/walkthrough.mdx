...
description: "Getting Started with Custom Nodes: a step-by-step ComfyUI walkthrough covering backend node setup, defining a schema, the execute method, registering nodes, adding options, and building a client-side JS extension (V3 schema)."
---
...
[frontend code](/custom-nodes/js/javascript_overview).
...
<Info>The basic structure of a custom node is described in detail [here](/custom-nodes/v3/backend/server_overview). </Info>
...
(see [later](/custom-nodes/v3/backend/server_overview#define_schema) for details of the schema fields),
...
<Info>For a detailed explanation of how ComfyUI discovers and loads custom nodes, see the [node lifecycle documentation](/custom-nodes/v3/backend/lifecycle).</Info>
