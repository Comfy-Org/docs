...
The Custom Nodes 2.0 frontend SDK is the published JavaScript API for extending ComfyUI node behavior, widgets, graphs, and interface features.
...
For an existing pack, the recommended migration path is MAGIC PATCH, which uses Claude Code or Codex installed on your computer with the same conversion skills Comfy uses for its pack catalog. See [Migrate legacy frontend code](/custom-nodes/v2/javascript/migration-recipes) for the frontend mappings it produces.
