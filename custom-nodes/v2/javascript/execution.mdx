---
title: "Control execution and resolution"
description: "Queue workflows, observe execution, define frontend-only nodes, supply values, and customize prompt serialization."
---
...
with cycle detection. A resolver must be pure: it cannot edit the graph or a
prompt draft. It may return a promise, and prompt construction awaits it. The
synchronous readers cannot wait, so `input.resolvedSource()` and
