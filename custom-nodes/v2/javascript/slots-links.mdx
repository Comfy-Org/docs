---
title: "Work with slots and links"
description: "Inspect, connect, add, reorder, modify, and resolve slots and links through stable handles."
---
...
- `{ kind: 'output', nodeId, output }`;
- `{ kind: 'literal', value }`;
- `{ kind: 'omitted', reason }`.

Resolution is read-only and scoped to the input's graph. See
[Execution and resolution](/custom-nodes/v2/javascript/execution) for defining resolvers and suppliers.
