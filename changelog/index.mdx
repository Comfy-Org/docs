...
- **Sora2 API Node**: Added support for OpenAI's Sora2 video generation API, bringing state-of-the-art text-to-video capabilities to ComfyUI workflows
- **PixVerse Enhancement**: Improved PixVerse API nodes to support multiline negative prompts, providing better control over video generation quality

**Enhanced Text Encoding Support**
- **Gemma 3 Integration**: Implemented Gemma 3 as a text encoder, expanding language model options for text-to-image and multimodal workflows

**Node Schema Migration (V3)**
- **Pika and Kling Updates**: Converted Pika and Kling video generation nodes to V3 schema for improved compatibility and consistency across video workflows

**Custom Node Compatibility**
- **LTXV Node Fix**: Applied temporary fix for LTXV custom nodes to resolve workflow interruptions and maintain stable video generation pipelines

**API Infrastructure Improvements**
- **ReCraft API Enhancement**: Fixed custom multipart parser in ReCraft API node to properly return FormData, improving image editing workflow reliability
- **Code Quality**: Enhanced pylint rules for API nodes, ensuring better code consistency and maintainability

**Frontend Updates**
- **Version Bump**: Updated frontend to version 1.27.10 with latest UI improvements and performance optimizations
</Update>
