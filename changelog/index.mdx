...
- Improved AMD GPU support by only disabling cudnn on newer AMD GPUs, enhancing performance for older AMD hardware
- Fixed Windows-specific network issues in API nodes by capturing general OSError for better retry handling
- Added warning for torch-directml usage to help users identify potential compatibility issues

**API Nodes & External Services**
- **Network Client V2**: Major upgrade with asynchronous operations, cancellation support, and download capabilities for improved workflow responsiveness
- **LTXV API Integration**: Added new LTXV API nodes for expanded video generation capabilities
- **Schema Migration**: Converted Tripo and Gemini API nodes to V3 schema for better consistency and maintainability

**Execution & Caching Improvements**  
- Enhanced dependency-aware caching system that fixes --cache-none behavior with loops and lazy evaluation workflows
- Added support for multi-dimensional latents, opening possibilities for more complex workflow architectures

**Development & Distribution**
- Updated portable dependencies to PyTorch CUDA 130 with Python 3.13.9 for better performance and compatibility
- Added batch file to run ComfyUI portable without API nodes for simplified deployment scenarios
- Added custom node published subgraphs endpoint for better workflow sharing and discovery
- Frontend bumped to version 1.28.8 with various UI improvements
- Template updates to version 0.2.4 with enhanced functionality
