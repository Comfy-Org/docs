...
**Hunyuan Image Model Ecosystem**
- **Hunyuan Image Distilled Model**: Full support for the new distilled model with faster generation capabilities
- **Hunyuan Image Refiner**: New refiner model implementation with noise augmentation support for enhanced image quality
- **Fast Preview Support**: Added quick preview functionality for Hunyuan Image workflows
- **VAE Compatibility**: Hunyuan refiner now works seamlessly with tiled VAE processing, enabling high-resolution refinement
- **Color Correction**: Fixed blown-out colors issue in refiner at noise augmentation levels below 0.25

**New Model Support**
- **HuMo Models**: Added support for both 1.7B and 17B HuMo models for advanced multimodal workflows
- **Chroma Radiance**: New model support for enhanced color processing capabilities
- **Qwen Edit Plus**: Support for Qwen edit plus model with new TextEncodeQwenImageEditPlus node
- **Omnigen2 UMO LoRA**: Added compatibility for Omnigen2 UMO LoRA models
- **Wav2vec Base Models**: Extended audio processing capabilities with wav2vec base model support
- **Whisper Large V3**: Added encoder part support for advanced audio encoding workflows

**WAN Animation Enhancements**
- **WAN Animate Model**: Basic support for WAN animate model with video generation capabilities
- **WanAnimateToVideo Node**: New node for easy video extension workflows with character replacement inputs
- **Memory Optimization**: Reduced peak VRAM usage for WAN inference, enabling better performance on consumer hardware
- **Experimental Status**: WAN nodes are no longer marked as experimental, indicating stable production-ready status

**API Node Expansions**
- **Kling v2.1 Support**: Added kling-v2-1 model to KlingStartEndFrame node for enhanced video frame control
- **Timeout Improvements**: Reduced default API timeout from 7 days to 2 hours for better reliability
- **Seedream4 Enhancements**: Added flag to ignore errors on partial success for more robust workflows
- **WAN API Nodes**: New text-to-image, text-to-video, and image-to-video API nodes for cloud-based generation
...
- **Enhanced Combo Types**: Added Output to V3 Combo type to match V1 capabilities for better workflow compatibility
- **Moonvalley API**: Converted Moonvalley API nodes to V3 schema for improved functionality

**Performance & Technical Improvements**
- **Runtime Attention Selection**: Enabled dynamic selection of attention functions for optimized performance
...
- **Differential Diffusion**: Added strength input parameter for more precise control over differential diffusion effects

**UI & Frontend Updates**
- **Frontend Bump**: Updated to version 1.26.13 with various interface improvements
- **Image Sharpen Fix**: Corrected issues with the image sharpen node for reliable image enhancement
- **Template Updates**: Updated workflow templates to version 0.1.86 with latest best practices
