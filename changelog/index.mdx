...
- Enabled async offloading by default on Nvidia GPUs for improved performance. Enhanced VRAM management with in-place weight dequantization to significantly reduce Flux2 memory usage and account for weight offloading costs in memory calculations.

**New Model Support & Features**
- Added comprehensive Z Image support including LoRA training, format compatibility, and ScaleRope node functionality. Integrated 3D animation nodes for enhanced creative workflows and added cheap latent preview support for Flux 2 models.

**API Node Enhancements**
- Added Veo3 First-Last-Frame node and Kling O1 model support for video generation workflows. Improved batch upload UX for image processing and enhanced Gemini integration with optimized file handling (first 10 images as URLs, remaining as base64).

**Training & Dataset Processing**
- Introduced dataset processing nodes with improved LoRA trainer nodes supporting multi-resolution training. Enhanced LoRA compatibility with Z Image models and removed redundant training nodes for streamlined workflows.

**Video Processing**
- Added support for video tiny VAEs to enable more efficient video processing workflows with reduced memory requirements.

**Developer Tools & UX**
- Added BlockInfo support for Flux models to improve node organization. Updated Qwen tokenizer with Qwen 3 tokens for better language model compatibility. Improved security with System User protection and fixed various UI/UX issues.

**Bug Fixes & Stability**
- Fixed CSP offline feature compatibility with latest frontend, resolved quantized tensor contiguity issues, and improved memory management with proper stream context handling. Fixed user data management validation and disabled offload stream conflicts with torch compile.
