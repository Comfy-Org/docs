...
**New 3D Generation Support**
- **Rodin3D Gen-2**: Added new API node for Rodin3D Gen-2 3D model generation with improved file naming using UUID system instead of timestamps for better workflow organization

**Enhanced Audio Capabilities**
- **New Audio Nodes**: Expanded audio processing capabilities with additional nodes for enhanced audio-driven workflows and multimodal content creation

**API Node Expansions**
- **WAN Image-to-Image**: Added new WanImageToImageApi node for enhanced image transformation workflows, with improved VRAM usage optimization reducing peak memory consumption during inference

**Ascend NPU Hardware Support**
- **NPU Compatibility**: ComfyUI can now run on specified Ascend NPU hardware, expanding deployment options for specialized AI hardware environments

**Node Schema Migration Progress (V3)**
- **Extensive V3 Updates**: Migrated numerous node categories to V3 schema including rebatch, Fresca, SD upscale, TCFG, SAG, post-processing, CLIP text encoding, HiDream, BFL, Luma, PixArt, PhotoMaker, Qwen, Lotus, Lumina2, HyperTile, Mochi, PerpNeg, and Mahiro nodes for improved performance and consistency

**Video Processing Improvements**
- **LatentCompositeMasked**: Enhanced compatibility with basic video latents, improving video workflow flexibility and compositing capabilities
...
**Development and Workflow Updates**
- **Template Tracking**: Added workflow templates version tracking to system stats response for better debugging and version management
- **PyTorch Updates**: Updated commands for installing latest nightly PyTorch builds
- **Locale Template Caching**: Improved locale-based template entry point file caching behavior
