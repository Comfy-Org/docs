...
**New Model Support**
- Qwen Image Layered diffusion model with dedicated empty latents node
- Improved VAE compatibility for Wan/Qwen models with automatic channel handling
- Enhanced support for Jina CLIP v2 and NewBie dual CLIP text encoders

**New Features**
- LatentCutToBatch node for batch processing workflows
- Flux2MaxImage API node for high-resolution image generation
- Unified jobs API with /api/jobs endpoints for workflow monitoring
- Sliding attention in Gemma3 models for better long-sequence processing

**Improvements**
- Refactored trainer with resolution bucketing for better training efficiency
- Enhanced AMD GPU support with optional MIOpen acceleration
...
- Fixed Context Windows logging errors
- Corrected Gemma quantization configuration
