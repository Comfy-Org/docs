...
**Node Schema Evolution (V3)**
- **Comprehensive V3 Migration**: Migrated core node categories to V3 schema including model downscale, LoRA extraction, compositing, latent operations, SD3/SLG, Flux, upscale models, and HunyuanVideo nodes. Enhanced Enum class support for Combo options provides better type safety and workflow reliability.

**Audio Workflow Enhancements**
- **MMaudio 16K VAE Support**: Implemented the new MMaudio 16K VAE for high-fidelity audio generation workflows, expanding multimodal content creation capabilities. Fixed mono audio incorrectly saving as stereo in audio save nodes.

**Model Sampling and Performance Improvements**
- **Refactored Model Sampling**: Streamlined model sampling sigmas code for better performance and maintainability. Fixed FP8 scaled LoRA application issues when FP8 operations are unavailable, ensuring consistent model behavior across different hardware configurations.

**AMD GPU Optimizations**
- **Enhanced AMD Performance**: Significant performance improvements for AMD users including better memory estimation for SD/Flux VAE operations. Enabled RDNA4 PyTorch attention on ROCm 7.0 and higher for improved inference speeds.

**API Node Expansions**
- **Enhanced External Integrations**: Added price extractor functionality and improvements to Kling/Pika API nodes. Enhanced Gemini Image API node with aspect_ratio support and latest model versions for more flexible AI image generation workflows.

**Model Compatibility Fixes**
- **Legacy Support**: Fixed loading of older Stable Diffusion checkpoint files on newer NumPy versions, ensuring backward compatibility with existing model collections. Improved diffusion model evaluation mode consistency across workflows.

**Developer Experience**
- **Code Quality Improvements**: Implemented lazy formatting in logging functions and added JSON pretty printing with proper indentation in "Preview Any" nodes. Fixed WAN2.2 cache VRAM leak issues and updated model path examples for better clarity.

**Template and Documentation Updates**
- Updated template to version 0.1.95 and node documentation to 0.3.0, providing the latest workflow examples and node reference materials for creators.
</Update>
