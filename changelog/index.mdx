...
**Major Media Loading Improvements**
* **Unified audio/video loading:** Video loader node now reads audio and video simultaneously for streamlined media workflows
* **PyAV integration:** Replaced Pillow with PyAV for image loading, reducing memory usage for JPEG formats and supporting tRNS PNG transparency
* **Metadata handling:** Added automatic metadata rotation handling in PyAV code for proper image orientation

**New Model Support**
* **CogVideoX:** Added support for CogVideoX video generation models
* **Gemma4:** Introduced text generation support for Gemma4 models
* **Void model:** New support for Void models with pass 1 & 2 capabilities
* **BiRefNet:** Added background removal model support for advanced masking workflows
* **Wan-Dancer:** Implemented support for Wan-Dancer models

**Performance Enhancements**
* **Block prefetch & LoRA async loading:** Significant speedup improvements, especially for LTX models
* **Dynamic VRAM optimizations:** Enhanced memory management with --cache-ram 2 support and reduced peak VRAM usage for video processing
* **Triton ComfyUI kitchen:** Added CLI argument to enable advanced optimization features

**Video Generation Features**
* **Auto-regressive video generation:** New capability for extended video sequence creation
* **I2V for causal forcing models:** Image-to-video support for causal model architectures
* **Context windows:** Added causal_window_fix for improved blending in long video sequences
* **Frame interpolation improvements:** Better memory estimation and reduced processing overhead

**Partner Node Additions**
* **Luma UNI-1:** Partner support for Luma's Uni-1 unified image model (create and edit workflows)
* **Topaz Astra 2:** Topaz video enhancement with the Astra 2 creative upscaling model
* **Tripo3D 3.1:** Updated 3D generation capabilities
* **GPT-5.5 models:** Support for GPT-5.5 and GPT-5.5-pro LLM variants
* **NanoBanana2:** New node with DynamicCombo/Autogrow functionality

**Node Improvements**
* **Flux2 latent previews:** High-quality preview support for Flux2 model latents
* **Image blending fixes:** Resolved device mismatch issues with --gpu-only flag and improved channel count handling
* **Batch broadcasting:** Enhanced JoinImageWithAlpha node with batch processing capabilities
* **Math Expression:** Added Boolean support for more complex calculations
* **Tile processing:** Fixed vertical stride calculations in SplitImageToTileList and ImageMergeTileList nodes

**Developer & API Updates**
* **Feature flags system:** Added generic --feature-flag CLI argument and registry for extensibility
* **OpenAPI enhancements:** Cloud-runtime operations and experiment endpoints added to specification
* **Blueprint support:** Added description fields to blueprint subgraphs for better documentation
* **ControlNet determinism:** Made ControlNet load order deterministic for consistent results

**Bug Fixes & Stability**
* Fixed CPU TE crashes by handling uninitialized meta-tensors
...
* Improved graceful handling of port-in-use errors on server startup
* Fixed save filename support without trailing underscores
