...
**Major Model Support & VAE Improvements**
- Added support for Qwen Image Layered diffusion model with dedicated empty latents node for enhanced image layering workflows
- Improved VAE compatibility for Wan/Qwen models with automatic channel trimming/padding, enabling seamless integration of diverse model architectures
- Enhanced nested tensor support for denoise masks and custom sampler nodes, providing better control over complex denoising operations

**New Nodes & Workflow Tools**
- Added LatentCutToBatch node for advanced batch processing workflows
- Introduced Flux2MaxImage API node for high-resolution image generation
- Implemented unified jobs API with /api/jobs endpoints for better workflow monitoring and management

**Text Encoder & Attention Enhancements**
- Implemented Jina CLIP v2 and NewBie dual CLIP support for improved text encoding capabilities
- Added sliding attention in Gemma3 models for better long-sequence processing
- Disabled prompt weights on NewBie text encoder to prevent compatibility issues in existing workflows

**Performance & Memory Optimizations**
- Refactored trainer implementation with resolution bucketing for more efficient training workflows
- Changed skip_load_model parameter to force_full_load for clearer model loading behavior
- Enhanced AMD GPU support with optional MIOpen acceleration via environment variable

**API & External Service Updates**
- Extended Kling O1 StartEndFrame node duration range for more flexible video generation
- Updated Topaz 4k video upscaling functionality
- Disabled watermark generation by default in API nodes for cleaner outputs
- Integrated new Nano Banana custom endpoint for improved processing capabilities

**Bug Fixes & Stability**
- Fixed mask concatenation issues in ZImageFunControlNet when running in --gpu-only mode
- Resolved batch processing errors with NewBie model integration
- Fixed Context Windows logging errors that could interrupt workflow execution
- Corrected Gemma quantization configuration to apply only to relevant models

**Workflow Templates**
- Updated workflow templates to v0.7.63 with latest node configurations and best practices
...
<Update label="v0.5.0" description="December 17, 2024">
