...
**Enhanced Model Support & ControlNet Integration**
...
- **Future-Ready Architecture**: Streamlined implementation framework makes adding new Qwen ControlNets easier for future releases

**Node Schema & API Evolution**
- **V3 Schema Migration**: String nodes, Google Veo API, and Ideogram API nodes upgraded to V3 schema for better performance and consistency
...
- **3D Latent Conditioning**: Fixed conditioning masks on 3D latents, enabling proper depth-aware conditioning in advanced workflows
...
- **Navigation Mode**: Disabled annoying default navigation mode for better user experience

**Workflow Benefits**
- **Precision Control**: Expanded Qwen ControlNet options provide artists with more precise control over image generation and editing
- **Stable Video Workflows**: LTXV and 3D conditioning fixes ensure reliable operation of complex video generation pipelines
- **Enhanced Performance**: New caching systems and elementwise fusions reduce execution time for compute-intensive workflows
- **Better UX**: Improved API node interfaces and filename handling create smoother workflow creation experience
- **Future-Proof**: V3 schema upgrades and Python 3.13 support keep workflows compatible with evolving standards
