...
- **Nodes 2.0 Public Beta**: Introducing a completely redesigned node interface system for improved workflow creation
- **Linear Mode Beta**: New linear workflow execution mode (enable via keybinding settings hotkey)
- **Workflow Progress Panel**: Real-time visualization of workflow execution progress
- **Assets Sidebar**: New dedicated sidebar for managing workflow assets
- **Subgraphs Improvements**: Enhanced subgraph functionality with bug fixes and stability improvements
- **Missing Node UX**: Improved user experience when handling workflows with missing nodes

**3D Feature Enhancements**
- Added panoramic image support in 3D nodes for immersive 360° content workflows
- Merged 3D animation functionality directly into the main 3D node for streamlined workflows

**LoRA Training Capabilities**
- **Multi-Resolution Training**: Native LoRA trainer now supports training at multiple resolutions simultaneously
- **Z Image LoRA Training**: Added comprehensive Z Image LoRA training support with format compatibility

**Performance & Model Support**
- **Video Tiny VAEs**: Added support for video tiny VAEs to enable more efficient video processing with reduced memory requirements
- **Z Image Format Support**: Full support for Z Image LoRA formats and model compatibility
- **Async Offloading**: Enabled async offloading by default on Nvidia GPUs for improved performance
- **Memory Optimizations**: Enhanced VRAM management with in-place weight dequantization to significantly reduce Flux2 memory usage

**API Node Enhancements**
- **Veo3 First-Last-Frame Node**: New node for advanced video generation control with Veo3
- **Kling V2.5 Turbo**: Added Kling v2.5 turbo model to Kling FirstLastFrame node for faster processing
- **Kling O1 Model Support**: Integrated Kling O1 model for enhanced video generation workflows
- **Batch Upload UX**: Improved batch upload experience for image processing
- **Gemini Integration**: Enhanced Gemini integration with optimized file handling (first 10 images as URLs, remaining as base64)

**Developer Tools & System Improvements**
- Added BlockInfo support for Flux models to improve node organization
- Updated Qwen tokenizer with Qwen 3 tokens for better language model compatibility
- Improved security with System User protection
- Added ScaleRope node functionality for Z Image models
- Added cheap latent preview support for Flux 2 models

**Bug Fixes & Stability**
- Fixed CSP offline feature compatibility with latest frontend
- Resolved quantized tensor contiguity issues
- Improved memory management with proper stream context handling
- Fixed user data management validation
- Disabled offload stream conflicts with torch compile
