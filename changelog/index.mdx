...
- **Pinned Memory Enabled by Default** for NVIDIA and AMD GPUs, providing significant performance improvements for model loading and inference
- **Reduced VRAM Usage** for LTXV, Qwen, and Flux models through optimized memory management
- Enhanced model unloading with intelligent VRAM monitoring that automatically frees memory when usage increases between runs
- Fixed async offload black screen issues and improved weight casting performance on offload streams

**Node & Workflow Improvements**
- **ScaleROPE Node Now Works with Flux Models**, expanding rope scaling capabilities across more architectures
- Unified RoPE function implementation across all models for better consistency and performance
- Fixed Qwen ControlNet regression that was affecting workflow reliability
- Resolved bad outputs in Upscaler and LoRA workflows through improved model reloading logic

**API Node Updates**
- Updated Rodin3D nodes to use new V3 client architecture with removal of legacy API client
- Deprecated OpenAI DALL-E 2 and DALL-E 3 nodes (temporarily reverted for compatibility)
- Added create_time field to prompt data in /history and /queue endpoints for better workflow tracking

**Development & Stability**
- Enhanced quantized operations with better stability and performance
- Improved custom node import error handling for more reliable third-party integrations
- Added left padding support to tokenizers for advanced text processing workflows
- Better memory pinning guards to prevent double pin/unpin operations

**Platform Updates**
- Updated Intel Arc GPU installation instructions with IPEX removal
- Added Python 3.14 compatibility notes
- Changed ROCm installation to use version 7.1 for better AMD GPU support
