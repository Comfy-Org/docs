...
**New Model Support**
- Ovis image model implementation for advanced image understanding workflows
- Kandinsky5 model support with enhanced generation capabilities
- Support for Z Image Alipai ControlNets for improved control over image generation
- Newbie diffusion model loader for streamlined model integration

**Major VRAM Optimizations**
- Temporal rolling VAE implementation delivering significant VRAM reductions for Hunyuan and Kandinsky models
- Reduced massive LoRA reservations, especially beneficial for Flux2 workflows
- Fixed Flux2 out-of-memory errors with improved dequantization offload accounting
- Optimized text encoder GPU utilization to prevent unnecessary CPU fallbacks
- Enhanced LoRA computation speed with lower memory usage through FP16 processing

**V3 Schema Migration**
- Converted multiple node libraries to V3 schema including 3D nodes, audio nodes, freelunch nodes, and mask nodes
- Added MatchType, DynamicCombo, and Autogrow support to V3 Schema for improved workflow flexibility

**Workflow Enhancements**
- ComfyUI-Manager support for pip installation version
- Context windows fixes and new features for improved temporal processing
- Z-Image FP16 compatibility improvements for better performance
- Enhanced @image reference format support in Kling Omni API nodes

**API & Integration**
- Added PATCH method to CORS headers for better API compatibility
- System instruction support and visual enforcement for Banana API nodes
- Support for seedance-1-0-pro-fast model in API workflows
- Chroma-radiance-x0 mode addition for expanded creative options

**Model Compatibility**
- Transformer LoRA prefix support for Z-Image workflows
- Qwen Image LoRA training fixes for improved fine-tuning workflows
- HunyuanVideo1.5 meanflow distil compatibility
- Fixed scaled FP8 format compatibility with mixed quantization operations

**Bug Fixes & Stability**
- Fixed text encoder regression when loaded directly on GPU
- Resolved transformer_options clearing issue in Lumina models
- Fixed portable updater functionality
