...
- Significantly improved workflow cancellation speed for better responsiveness during development and experimentation
- Fixed critical issue where VAEs consume 3x more memory on PyTorch 2.9 with NVIDIA GPUs, providing workarounds for improved memory efficiency on modern setups
- Enhanced chroma radiance processing speed and fixed batch size issues above 1 for more reliable multi-batch operations

**Hardware Compatibility**
- Added Python 3.14 support with updated installation instructions for developers wanting to use the latest Python features
- Disabled FP8 operations for AMD gfx942 GPUs to prevent compatibility issues
- Improved CUDA memory management when using --fast autotune mode, with CUDA malloc disabled by default for better performance

**New Nodes & API Updates**
- Added TemporalScoreRescaling node for advanced temporal control in video generation workflows
- Introduced Veo 3.1 model support in API nodes, expanding video generation capabilities
- Fixed dynamic pricing format and renamed comfy_io to IO in API nodes for better consistency

**Node Schema Migration (V3)**
- Converted ControlNet nodes to V3 schema, improving workflow compatibility and node standardization

**Caching & Execution Improvements**
- Enhanced EasyCache with proper batch_slice handling in apply_cache_diff for more reliable caching behavior
- Improved merge_nested_dicts functionality with proper input ordering and refactored manual patch merging

**Frontend Updates**
- Updated to frontend version 1.28.7 with deprecated API alerts to help users transition to newer APIs
- Enhanced user experience with better feedback on API deprecation

**Developer Experience**
- Added deprecation warnings for unused files to help maintain cleaner codebases
- Improved torch compiler compatibility by disabling compilation for specific functions that caused issues
- Updated to latest PyTorch stable with CUDA 13.0 support for better GPU acceleration
