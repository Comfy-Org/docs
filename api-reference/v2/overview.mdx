...
**Comfy API deployments (Developer Platform).** A workflow you deploy through the [Developer Platform](https://platform.comfy.org) gets its own dedicated endpoint at `https://{deployment}.run.comfy.app`, serving the same v2 API with the same API key. A Comfy API deployment runs one pinned workflow, so it scales independently and `GET /workflow` returns the executed graph. See the [Comfy API guide](/development/serverless/overview) for building and deploying.
