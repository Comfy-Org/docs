...
4. **No cost on the response.** Usage is reported through your workspace's billing, not per call. See [limitations](/api-reference/comfy-router/limitations).

## Next

- [Quickstart](/api-reference/comfy-router/quickstart): typed error handling in Python and TypeScript, reading the `422`, retrying safely with your own key.
- [API reference](/api-reference/comfy-router/reference): the generated contract these headers are defined in.
- [Limitations](/api-reference/comfy-router/limitations): what Router does not do today, and what to use instead.
