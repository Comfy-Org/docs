...
A standalone OpenAPI document describing ONE Comfy Router model's input - the body `POST /v2/models/{provider}/{model}` accepts for that model. It is what `GET /v2/models/{provider}/{model}/openapi.json` returns.
