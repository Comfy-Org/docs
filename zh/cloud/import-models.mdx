...
2. 点击 **Add Secret**，选择密钥对应的 **Provider**（例如 Hugging Face 或 Civitai），为密钥命名，并把密钥粘贴到 **Secret Value** 中。Provider 是必选项：它告诉 Comfy Cloud 该密钥属于哪个平台，这样从该平台导入模型时就会自动使用对应的 token。
...
<Note>
密钥保存后会被加密，且无法再次查看，请自行留存备份。每个 Provider 只能保存一个密钥。需要更换时，打开该密钥并填入新的值。
</Note>
