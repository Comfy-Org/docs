...
[**comfy-local-mcp**](https://github.com/Comfy-Org/comfy-local-mcp) 是 Comfy 的**第一方本地 MCP 服务器**——从 AI 代理（Claude Code、Claude Desktop、Cursor 及其他 MCP 客户端）驱动**本地** ComfyUI 安装的官方方式。它是在 [comfy-cli](https://github.com/Comfy-Org/comfy-cli) 之上的薄封装：每个工具都会调用外部的 `comfy` 指令，因此 `comfy-cli` 是引擎，并且与 [Comfy Cloud MCP](/zh/agent-tools/mcp) 没有共享任何代码。
...
- [Comfy Cloud MCP](/zh/agent-tools/mcp)：托管MCP服务器，无需本地安装或GPU
- [Comfy CLI](/zh/agent-tools/cli)：从终端驱动本地ComfyUI和合作伙伴生成
