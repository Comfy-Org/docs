...
```
  ...
  <Tab title="先排队，稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/gemini-interactions/gemini-omni-1.1-flash/requests`。Router 在运行被接纳后立即返回 `201` 和 `request_id`，结果就绪后再进行收集，可以在此进程或其他进程中完成。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消和收集的完整流程。
...
with Comfy() as client:
    ...
    print("request_id:", handle.request_id)  # 加上模型 ID，就是另一个进程所需要的全部信息
...
console.log("requestId:", handle.requestId); // 加上模型 ID，就是另一个进程所需要的全部信息
...
print("requestId:", handle.requestId)  // 加上模型 ID，就是另一个进程所需要的全部信息
