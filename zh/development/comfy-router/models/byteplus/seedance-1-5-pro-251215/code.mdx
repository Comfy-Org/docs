...
<ParamField body="return_last_frame" type="boolean" default="false">
  是否返回生成视频的最后一帧图像。
</ParamField>true：返回已生成视频的最后一帧图像。将此参数设置为 true 后，可通过调用“查询视频生成任务信息”获取最后一帧图像。最后一帧图像为 PNG 格式，其像素宽度和高度与已生成视频一致，且不包含水印。使用此参数可以生成多个连续视频：将前一个已生成视频的最后一帧用作下一个视频任务的第一帧，从而快速生成多个连续视频。
