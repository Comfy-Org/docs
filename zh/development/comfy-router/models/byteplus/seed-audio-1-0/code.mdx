...
同样的请求体，发送到 `POST https://api.comfy.org/v2/models/byteplus/seed-audio-1.0/requests`。Router 会在运行被接受后立即返回 `201` 和一个 `request_id`，结果就绪后可从本次进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue)介绍了状态查询、取消与收集的完整流程。
