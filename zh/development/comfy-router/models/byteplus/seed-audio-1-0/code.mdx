...
同样的请求体，发送到 `POST https://api.comfy.org/v2/models/byteplus/seed-audio-1.0/requests`。Router 会在运行被接受后立即返回 `201` 和一个 `request_id`，结果就绪后可从本次进程或其他进程收集。[排队投递](/zh/development/comfy-router/queue)介绍了状态查询、取消与收集的完整流程。
...
<ParamField body="audio_config.enable_subtitle" type="boolean">
  是否启用字幕服务。启用后，响应会包含句级和词级时间戳（默认：false）。
</ParamField>
...
<ParamField body="model" type="string">
  模型标识符。支持的模型：seed-audio-1.0 和 seed-audio-1.0-multilingual。直接向 POST /proxy/byteplus/api/v3/tts/create 发起的 v1 调用必须提供它：代理会拒绝任何其他值，也会以 400 拒绝省略该字段的请求。它不在本模式的 `required` 列表中，因为 Comfy Router 会从 /v2/models/byteplus/\{model} 的 `{model}` 路径段填充它，所以 Router 调用方会省略它。
</ParamField>

<ParamField body="references" type="object[]">
  参考资源。纯文本生成时请省略。最多 3 个音频参考（每个最长 30 秒、解码后最大 10 MB；wav、mp3、pcm 或 ogg_opus），或者恰好 1 个图像参考（解码后最大 10 MB；jpeg、png 或 webp）。图像参考不能与音频参考混用。这些上限由 BytePlus 设定，并以解码后的字节数表示：base64 会使资源膨胀约 4/3，而 Comfy Router 调用对整个 JSON 文档的限制为 20 MiB，因此发送到 /v2/models/byteplus/\{model} 的内联 `audio_data`/`image_data` 参考，解码后必须保持在约 15 MB 以下。更大的内容请使用 `audio_url`/`image_url`：如果内联，三个达到 BytePlus 自身 10 MB 上限的音频参考会在验证运行之前被以 413 拒绝。
</ParamField>
...
<ParamField body="text_prompt" type="string" required>
  要合成的提示词或文本（1 至 3,000 个字符；空字符串会被拒绝）。提供音频参考时，可按顺序使用 @Audio1、@Audio2 和 @Audio3 引用它们。
</ParamField>
...
由 Router 在 `GET /v2/models/byteplus/seed-audio-1.0/openapi.json` 处提供的模式生成，这也是它在请求到达提供商之前用于验证调用的同一份文档。
