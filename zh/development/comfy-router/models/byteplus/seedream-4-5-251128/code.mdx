...
<ParamField body="guidance_scale" type="number">
  控制输出图像与输入提示词的贴合程度。范围 [1, 10]。值越高，对提示词的遵循越强。seedream-5.0-pro、5.0-lite、4.5 和 4.0 不支持。
...
Comfy Router 将整个 JSON 请求限制为 100 MiB，其中包括 base64 展开部分和所有参考图像。对于会超过此传输限制的输入，请使用 URL。
...
在图层分离场景中（启用 layer_decomposition），image 为必填，且仅支持单张输入图像（传入多张图像会返回错误）。输入图像必须为 png、jpeg、webp、bmp、tiff 或 gif（不支持 heic 和 heif），最大 30 MB，总像素须在 [512x512, 6000x6000] 范围内，宽高比须在 [1/16, 16] 范围内。
</ParamField>
...
否：标准图像生成模式；不执行图层分离。
关于图层分离模式的说明：仅支持单张输入图像（传入多张图像会返回错误）；如果其中任一单个图层生成失败，整个请求都会失败，不支持部分成功；最多返回 17 张图像（1 张基础图像 + 16 个图层）。传入 sequential_image_generation、sequential_image_generation_options、tools 和 stream 会返回错误。
</ParamField>

<ParamField body="model" type="string">
  模型标识符。支持的模型：seedream-4-0-250828、seedream-4-5-251128、seedream-5-0-260128 和 seedream-5-0-pro-260628。直接对 POST /proxy/byteplus/api/v3/images/generations 发起 v1 调用时必须提供该参数：代理会拒绝任何其他值，若省略该参数则返回 400。它不在本架构的 `required` 列表中，因为 Comfy Router 会从 /v2/models/byteplus/\{model} 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。
</ParamField>
...
<ParamField body="prompt" type="string">
  用于图像生成或变换的文本描述。
在图层分离场景中为可选（seedream-5.0-pro 启用 layer_decomposition）：如果提供了提示词，模型会按照提示词的意图识别并分离你指定的元素；如果未提供提示词，模型会自动检测图像中的所有主要元素，并将它们分离为独立图层。
</ParamField>
...
<ParamField body="sequential_image_generation_options" type="object">
  仅 seedream-5.0-lite、4.5 和 4.0 支持此参数（seedream-5.0-pro 不支持）。Configuration for the batch image generation feature. This parameter is only effective when sequential_image_generation is set to auto.

批量图像生成功能的配置。仅当 sequential_image_generation 设置为 auto 时，此参数才生效。
</ParamField>
...
<ParamField body="stream" type="boolean" default="false">
  ...
  Comfy Router 会将显式提供的 stream 标志固定为 false，因为它捕获的是完整的 JSON 结果。在 v1 代理上，此字段控制是否启用流式输出模式。仅 seedream-5.0-lite、4.5 和 4.0 支持此参数（seedream-5.0-pro 不支持）。false = 所有输出图像一次性返回。true = 每张输出图像在生成后立即返回。
</ParamField>
...
Generated from the schema Router serves at `GET /v2/models/byteplus/seedream-4-5-251128/openapi.json`, the same document it validates a call against before the request reaches the provider.

本文档根据 Router 在 `GET /v2/models/byteplus/seedream-4-5-251128/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。
...
<ResponseField name="data[].bounding_box.normalized" type="integer[]">
  图层边界框的千分位量化（归一化）坐标，根据基础图像尺寸按比例映射到 [0, 1000] 的离散整数范围，最大截断至 1000。坐标格式：[left, top, right, bottom]。
</ResponseField>

<ResponseField name="data[].description" type="string">
  当前分离元素的详细描述，相比 name 提供更丰富的图层特征（如颜色、状态、材质）。仅图层会返回此字段；基础图像不返回。仅在 layer_decomposition 为 true 时返回。
</ResponseField>
...
<ResponseField name="data[].z_index" type="integer">
  图层的堆叠顺序，从下往上递增：0 为最底层（基础图像）；数值越大位置越高。可用其按正确的堆叠顺序将各图层重新合成为完整图像。仅在 layer_decomposition 为 true 时返回。
</ResponseField>
