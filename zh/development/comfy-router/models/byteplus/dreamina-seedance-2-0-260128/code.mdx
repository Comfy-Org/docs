...
<ParamField body="content[].audio_url" type="object">
  输入音频对象。仅 Seedance 2.5、2.0 和 2.0 fast 支持音频输入。Seedance 2.0 和 2.0 fast 不能单独使用音频，必须包含至少 1 张图像或 1 个视频；Seedance 2.5 支持纯音频输入。
</ParamField>
...
Comfy 侧护栏，并非 BytePlus 的限制。BytePlus 未公布文本长度上限，在测试中（2026-09-17）接受了 40,000 个字符。
它统计的是字符数而非字节数，因此多字节提示词在网络传输时可能达到该数值的数倍。它限制的只是这一个字段，而不是整个请求：
`content` 是一个数组，整份文档受每请求正文大小限制的约束。执行该限制的是 Comfy Router（/v2/models/byteplus/\{model}）；
直接调用 v1 /proxy 时，则由 BytePlus 自带的校验器作出应答。该值设置得远高于真实流量，因此永远不会对真实提示词做出判定。如果调用方需要更大上限，可将其调高。
</ParamField>
...
<ParamField body="return_last_frame" type="boolean" default="false">
  是否返回所生成视频的最后一帧图像。
是：返回所生成视频的最后一帧图像。将该参数设置为是后，可通过调用查询视频生成任务信息来获取最后一帧图像。最后一帧图像为 PNG 格式，其像素宽度和高度与所生成视频一致，且不含水印。使用该参数可以生成多个连续的视频：将前一个已生成视频的最后一帧作为下一个视频任务的第一帧，从而快速生成多个连续的视频。
否：不返回所生成视频的最后一帧图像。
</ParamField>
...
<ResponseField name="content" type="object">
  视频生成任务完成后的输出内容，其中包含输出视频的下载 URL，以及在 BytePlus 返回时，其最后一帧的下载 URL。`video_url` 和 `last_frame_url` 都会被重新托管到 Comfy 存储上；这里的其他字段均为 BytePlus 自身的字段。可为空：BytePlus 会在任务结束 24 小时后清除这些 URL，之后轮询到的 succeeded 文档可能不带 `content` 字段，或其值为 null。
</ResponseField>

<ResponseField name="content.last_frame_url" type="string">
  已生成视频最后一帧的下载 URL，仅在请求中设置了 `return_last_frame` 时返回。不要根据此 URL 推断图像格式：BytePlus 在请求侧将最后一帧记录为 PNG，Router 会重新托管它收到的任意字节，并根据上游的 Content-Type 或内容嗅探来确定其类型，只有在两者都失败时才会作为最后的兜底使用 `image/jpeg`。Router 会将最后一帧重新托管到 Comfy 存储上并改写此字段，因此它通常是一个有效期最长 24 小时的 Comfy 签名 URL：签出时有效期为 24 小时，并从 23 小时的备忘中重放，因此稍后轮询拿到的链接可能只剩一小时有效期。当无法执行重新托管时，该字段会保留 BytePlus 自身的 URL，BytePlus 会在任务结束 24 小时后清除它。无论哪种情况，链接都会过期，因此请下载该帧，而不要保存 URL。
</ResponseField>

<ResponseField name="content.output_format" type="string">
  已生成视频的封装格式（mp4 或 mov），当 BytePlus 将其嵌套在 `content` 内时返回。Seedance 系列模型更常将其作为 `content` 的顶层同级字段返回，详见顶层的 `output_format` 字段，Router 会读取两者中存在的那个。
</ResponseField>

<ResponseField name="content.video_url" type="string">
  输出视频的下载 URL。Router 会将视频重新托管到 Comfy 存储上并改写此字段，因此它通常是一个有效期最长 24 小时的 Comfy 签名 URL：签出时有效期为 24 小时，并从 23 小时的备忘中重放，因此稍后轮询拿到的链接可能只剩一小时有效期。当无法执行重新托管时，该字段会保留 BytePlus 自身的 URL，BytePlus 会在任务结束 24 小时后清除它，并在部分模型上将其限制为 100 次下载。无论哪种情况，链接都会过期，因此请下载该视频，而不要保存 URL。
</ResponseField>
...
<ResponseField name="duration" type="number">
  已生成视频的时长，单位为秒。此处声明为数字而非整数，是因为 BytePlus 在这点上并不一致：视频任务曾被观察到返回整秒，而同属 BytePlus 的其他接口会报告带小数的时长，因此客户端不能假定它是整数值。这是 BytePlus 自身的字段，在视频任务成功时返回并原样转发。
</ResponseField>
...
<ResponseField name="output_format" type="string">
  已生成视频的封装格式（mp4 或 mov），在顶层作为 `content` 的同级字段返回，Seedance 视频任务查询正是在此返回该字段。这是 BytePlus 自身的字段，会原样转发。
</ResponseField>

<ResponseField name="resolution" type="string">
  已生成视频的分辨率，例如 `1080p`。这是 BytePlus 自身的字段，在视频任务成功时返回并原样转发。
</ResponseField>

<ResponseField name="seed" type="integer">
  该任务实际使用的生成种子。这是 BytePlus 自身的字段，在视频任务成功时返回并原样转发。
