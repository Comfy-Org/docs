...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/byteplus/dreamina-seedance-2-0-mini/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [Queued delivery](/zh/development/comfy-router/queue)。
...
这是 Comfy 侧的防护限制，而非 BytePlus 的限制。BytePlus 未公布文本长度上限，测试中接受了 40,000 个字符（2026-09-17）。
它统计的是字符数而非字节数，因此多字节提示词在实际传输时可能达到该大小的数倍。它约束的只是这一个字段，而非整个请求：
`content` 是一个数组，整个文档受每请求正文大小限制约束。Comfy Router（/v2/models/byteplus/\{model}）是执行该限制的接口；
直接进行 v1 /proxy 调用时，则由 BytePlus 自身的校验器负责。该值设置得远高于实际流量，因此绝不会裁决正常提示词；如果调用方需要更大的值，可以提高它。
</ParamField>
...
<ParamField body="return_last_frame" type="boolean" default="false">
  是否返回生成视频的最后一帧图像。
true: 返回生成视频的最后一帧图像。将此参数设置为 true 后，可通过调用查询视频生成任务信息来获取最后一帧图像。最后一帧图像为 PNG 格式，其像素宽度和高度与生成的视频一致，且不含水印。使用该参数可以生成多个连续视频：将前一个已生成视频的最后一帧用作下一个视频任务的第一帧，从而快速生成多个连续视频。
false: 不返回生成视频的最后一帧图像。
</ParamField>
...
由 Router 在 `GET /v2/models/byteplus/dreamina-seedance-2-0-mini/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。

### 输出

<ResponseField name="content" type="object">
  视频生成任务完成后返回的输出，包含输出视频的下载 URL，以及当 BytePlus 返回最后一帧时所附带的最后一帧下载 URL。`video_url` 和 `last_frame_url` 都会被重新托管到 Comfy 存储；此处的其他所有字段均为 BytePlus 自有字段。可为 null：BytePlus 会在任务结束后 24 小时清除这些 URL，因此在此之后轮询到的已成功文档可能缺失 `content` 或其值为 null。
</ResponseField>

<ResponseField name="content.last_frame_url" type="string">
  已生成视频最后一帧的下载 URL，当请求设置了 `return_last_frame` 时返回。不要根据此 URL 推断图像格式：BytePlus 在请求侧将最后一帧记录为 PNG，Router 会重新托管它收到的任意字节，并根据上游的 Content-Type 或内容嗅探来确定其类型，而 `image/jpeg` 只有在两者都失败时才是最后的兜底。Router 会将最后一帧重新托管到 Comfy 存储并改写此字段，因此它通常是有效期最长 24 小时的 Comfy 签名 URL：签发时按 24 小时签名，并从 23 小时的备忘中重放，所以较晚的轮询可能返回只剩一小时有效期的链接。当无法执行重新托管时，该字段会保留 BytePlus 自己的 URL，而 BytePlus 会在任务结束后 24 小时清除它。无论哪种情况链接都会过期，因此请下载该帧，而不要只保存 URL。
</ResponseField>
...
<ResponseField name="content.video_url" type="string">
  输出视频的下载 URL。Router 会将视频重新托管到 Comfy 存储并改写此字段，因此它通常是有效期最长 24 小时的 Comfy 签名 URL：签发时按 24 小时签名，并从 23 小时的备忘中重放，所以较晚的轮询可能返回只剩一小时有效期的链接。当无法执行重新托管时，该字段会保留 BytePlus 自己的 URL，而 BytePlus 会在任务结束后 24 小时清除它，并且在某些模型上会将其限制为 100 次下载。无论哪种情况链接都会过期，因此请下载视频，而不要只保存 URL。
</ResponseField>
...
<ResponseField name="duration" type="number">
  已生成视频的时长，单位为秒。声明为 number 而不是 integer，是因为 BytePlus 对此并不一致：已观察到视频任务返回整秒数，而与之同类的其他 BytePlus 接口会反馈带小数的时长，因此客户端不得假定其为整数值。这是 BytePlus 自有字段，在视频任务成功时返回并原样转发。
</ResponseField>
...
<ResponseField name="output_format" type="string">
  已生成视频的容器格式（mp4 或 mov），在顶层作为 `content` 的同级字段返回：Seedance 视频任务查询正是在此返回它。BytePlus 自有字段，原样转发。
</ResponseField>

<ResponseField name="resolution" type="string">
  已生成视频的分辨率，例如 `1080p`。BytePlus 自有字段，在视频任务成功时返回并原样转发。
</ResponseField>

<ResponseField name="seed" type="integer">
  任务实际使用的生成种子。BytePlus 自有字段，在视频任务成功时返回并原样转发。
