...
Comfy 侧的防护限制，而非 BytePlus 的约束。BytePlus 未公布文本长度上限，在测试中接受过 40,000 个字符（2026-09-17）。它按字符计数而非字节，因此多字节提示词在传输时占用的体积可能达到该值的数倍。它限制的是这一个字段，而不是整个请求：`content` 是数组，整个文档由每个请求的正文大小上限约束。执行该限制的是 Comfy Router（/v2/models/byteplus/\{model}）；直接调用 v1 /proxy 时则由 BytePlus 自身的校验器负责。该值设置得远高于真实流量，因此不会对正常提示词做出裁决。如果调用方需要更多，可以将其调高。
</ParamField>
...
<ParamField body="return_last_frame" type="boolean" default="false">
  是否返回已生成视频的最后一帧图像。
true：返回已生成视频的最后一帧图像。将此参数设置为 true 后，你可以通过调用查询视频生成任务信息来获取最后一帧图像。该最后一帧图像为 PNG 格式，其像素宽度和高度与已生成的视频一致，且不包含水印。使用此参数可以生成多个连续的视频：将前一个已生成视频的最后一帧用作下一个视频任务的第一帧，从而快速生成多个连续的视频。
...
由 Router 在 `GET /v2/models/byteplus/dreamina-seedance-2-0-mini/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前 Router 用来校验调用的文档相同。

### 输出

<ResponseField name="content" type="object">
  视频生成任务完成后返回的输出内容，其中包含输出视频的下载 URL，以及当 BytePlus 返回时其最后一帧的下载 URL。`video_url` 和 `last_frame_url` 都会重新托管到 Comfy 存储上；此处的其他所有字段均来自 BytePlus 本身。可为空。BytePlus 会在任务完成 24 小时后清除这些 URL，因此在该时间之后轮询到的成功文档可能 `content` 缺失或为 null。
</ResponseField>

<ResponseField name="content.last_frame_url" type="string">
  生成视频最后一帧的下载 URL，仅当请求中设置了 `return_last_frame` 时返回。不要根据此 URL 推断图像格式：BytePlus 在请求侧将最后一帧记录为 PNG，Router 会重新托管其收到的任意字节内容，并根据上游的 Content-Type 或内容嗅探结果来标注类型，只有当两者都失败时才会回退到 `image/jpeg`。Router 会把最后一帧重新托管到 Comfy 存储上并重写此字段，因此它通常是 Comfy 签名的 URL，有效期最长 24 小时。签发时有效期为 24 小时，并且会从 23 小时的缓存记录中重放，因此稍后轮询时可能拿到的 URL 只剩不到一小时的有效期。当无法执行重新托管时，该字段会保留 BytePlus 自己的 URL，而 BytePlus 会在任务完成 24 小时后清除它。无论哪种情况，链接都会过期，因此请下载该帧，而不要保存 URL。
</ResponseField>

<ResponseField name="content.output_format" type="string">
  生成视频的容器格式（mp4 或 mov），当 BytePlus 将其嵌套在 `content` 内时出现。Seedance 系列模型更常将其作为 `content` 的顶层同级字段返回，参见顶层的 `output_format` 字段。Router 会读取两者中存在的那个。
</ResponseField>

<ResponseField name="content.video_url" type="string">
  输出视频的下载 URL。Router 会把视频重新托管到 Comfy 存储上并重写此字段，因此它通常是 Comfy 签名的 URL，有效期最长 24 小时。签发时有效期为 24 小时，并且会从 23 小时的缓存记录中重放，因此稍后轮询时可能拿到的 URL 只剩不到一小时的有效期。当无法执行重新托管时，该字段会保留 BytePlus 自己的 URL，而 BytePlus 会在任务完成 24 小时后清除它，并且某些模型还会将下载次数限制为 100 次。无论哪种情况，链接都会过期，因此请下载该视频，而不要保存 URL。
</ResponseField>
...
<ResponseField name="duration" type="number">
  生成视频的时长，单位为秒。之所以声明为数字而非整数，是因为 BytePlus 对此并不一致：视频任务曾被观察到返回整秒数，而 BytePlus 的其他同类接口会报告带小数的时长，因此客户端不能假定它是整数值。这是 BytePlus 自己的字段，在成功的视频任务中返回并原样转发。
</ResponseField>
...
<ResponseField name="output_format" type="string">
  生成视频的容器格式（mp4 或 mov），在顶层作为 `content` 的同级字段返回，这正是 Seedance 视频任务查询接口返回它的位置。这是 BytePlus 自己的字段，原样转发。
</ResponseField>
