...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 代码片段使用 Comfy SDK（`pip install comfy-sdk` 和 `npm install @comfyorg/sdk`）；cURL 代码片段则是通过原始 HTTP 发起的同一个调用。
...
```
  ...
  <Tab title="入队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/openai/gpt-5-mini/requests`。只要运行被受理，Router 就会立即返回 `201` 和 `request_id`；结果就绪后即可收集，可以由本进程收集，也可以由另一个进程收集。状态、取消与收集的完整流程详见[队列投递](/zh/development/comfy-router/queue)。
