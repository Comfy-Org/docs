---
title: "将 GPT 6 Astra 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/gpt-6-astra：端点、请求结构，以及 Router 返回的响应。"
...
`openai/gpt-6-astra` 的 API 参考文档，由 Comfy Router 从 OpenAI 提供。
...
**端点：** `POST https://api.comfy.org/v2/models/openai/gpt-6-astra`
...
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/gpt-6-astra/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [Queued delivery](/zh/development/comfy-router/queue)。
...
<ParamField body="input" type="string | object[]" required>
  提供给模型的文本、图像或文件输入，用于生成响应。这是本契约中 Router 无法提供的唯一字段，也是下面 `required` 中唯一的条目。
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  响应所生成 token 数量的上限，包括可见的输出 token 和 reasoning token。在 reasoning id 上，这一上限与隐藏的 reasoning token 共用，因此一个较小的值可能在产生任何可见文本之前就耗尽整个预算，这就是 reasoning 冒烟用例发送 1024、而 chat 用例发送 16 的原因。
...
<ParamField body="reasoning" type="object">
  仅限 REASONING 层级。reasoning 模型的配置，例如 `{"effort": "medium"}`。原样转发；有关可接受的键，请参阅 OpenAI 的 reasoning 指南。chat 层级的 id 会忽略它。
</ParamField>

<ParamField body="store" type="boolean">
  OpenAI 是否存储生成的响应以便日后检索。
</ParamField>

<ParamField body="stream" type="boolean">
  声明此字段是为了让发送它的调用方不被拒绝，但它在此接口上无效：Router 会在分发之前将其固定为 `false`，因为它捕获的是提供商的响应，而不是转发 `text/event-stream`，而 openAiResponsesProxy 的 ModifyResponse 无法解码该格式，因此流式生成会被 OpenAI 计费，却无人计量。如果需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限 CHAT 层级：o 系列 reasoning id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 处会拒绝此参数。Router 不会替它们拒绝，请参阅此组件中关于两个层级为何共用一个 schema 的说明，因此发送该参数的 reasoning 调用会由 OpenAI 自身的错误来回应。
...
<ParamField body="tool_choice" type="string | object">
  模型应如何选择要使用哪个工具。可以是一个字符串模式，也可以是一个指定某个工具的对象。
</ParamField>

<ParamField body="tools" type="object[]">
  模型可以调用的工具定义。Router 不会缩小工具的分类范围；有关可接受的形状，请参阅 OpenAI 的 Responses API 参考。
</ParamField>
...
<ParamField body="truncation" type="string">
  上下文超出模型窗口时的截断策略。与上面三个词表不同，这里的枚举会被强制执行，因为这两个值是 OpenAI 记录的完整集合，且一直没有增长。显式的 `null` 仍然被接受，条件与其上方的字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  Token 使用情况封装。此契约上存在该字段，是因为 v1 操作在请求体上声明了它；OpenAI 会在响应中填充它，因此调用方没有理由发送它。
</ParamField>

根据 Router 在 `GET /v2/models/openai/gpt-6-astra/openapi.json` 提供的 schema 生成，这也是它在请求到达提供商之前用于校验调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将一条系统（或开发者）消息插入模型上下文的第一个位置。

当与 `previous_response_id` 一起使用时，先前响应中的 instructions 不会延续到下一个响应。这使得在新响应中替换系统（或开发者）消息变得很简单。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  模型响应所使用的截断策略。
- `auto`：如果本次响应以及先前响应的上下文超出
  模型的上下文窗口大小，模型将通过丢弃对话
  中间部分的输入项来截断响应，以适应该上下文窗口。
...
约束[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理强度。
目前支持的值为 `low`、`medium` 和 `high`。降低
推理强度可以使响应更快，并减少响应中用于
推理的 token 数量。
...
从而确保模型会匹配你提供的 JSON schema。在
[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)中了解更多。

默认格式为 `{ "type": "text" }`，没有任何额外选项。

**不推荐用于 gpt-4o 及更新的模型：**

设置为 `{ "type": "json_object" }` 会启用较旧的 JSON 模式，
它确保模型生成的消息是有效的 JSON。对于支持
`json_schema` 的模型，优先使用 `json_schema`。
</ResponseField>
...
<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  模型在生成响应时应如何选择要使用的一个或多个工具。
请参阅 `tools` 参数，了解如何指定模型可以
调用的工具。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并假定它是包含模型生成内容的 `assistant` 消息，
...
<ResponseField name="prompt_cache_key" type="string">
  OpenAI 用它为相似的请求缓存响应，以优化缓存命中率。它取代了 `user` 字段。
</ResponseField>
...
<ResponseField name="user" type="string">
  已弃用的终端用户标识符。已由 `safety_identifier` 和 `prompt_cache_key` 替换。
</ResponseField>
