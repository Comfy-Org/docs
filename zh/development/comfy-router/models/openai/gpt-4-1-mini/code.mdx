---
title: "将 GPT 4.1 Mini 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/gpt-4.1-mini：端点、请求格式以及 Router 返回的响应。"
...
`openai/gpt-4.1-mini` 的 API 参考文档，由 Comfy Router 提供，模型来自 OpenAI。
...
**端点：** `POST https://api.comfy.org/v2/models/openai/gpt-4.1-mini`
...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/openai/gpt-4.1-mini/requests`。运行一经受理，Router 就会立即返回 `201` 和 `request_id`；结果就绪后，可从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 详细介绍了状态、取消和收集。
...
<ParamField body="input" type="string | object[]" required>
  提供给模型的文本、图像或文件输入，用于生成响应。这是 Router 无法代为提供的唯一字段，也是下方 `required` 中唯一的条目。
</ParamField>

<ParamField body="instructions" type="string">
  插入一条系统（或开发者）消息，作为模型上下文中的第一项。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  响应所生成 token 数量的上限，包括可见输出 token 和推理 token。在 reasoning id 上，这一上限与隐藏的推理 token 共享，因此一个较小的值可能在出现任何可见文本之前就耗尽整个预算，这正是 reasoning 冒烟用例发送 1024、而 chat 用例只发送 16 的原因。

  范围：`1` 到 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上该字段为可选，Router 会从 `{model}` 路径段填充它；显式的 `null` 也以同样方式被替换。发送与路径不一致的值会被拒绝。
</ParamField>
...
<ParamField body="reasoning" type="object">
  仅限 REASONING 层级。推理模型的配置，例如 `{"effort": "medium"}`。原样转发；接受的键请参阅 OpenAI 的推理指南。chat 层级的 id 会忽略它。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明该字段是为了让发送它的调用方不会被拒绝，但在此接口上它不起作用：Router 会在派发之前将其置为 `false`，因为它捕获的是提供商响应，而不是转发 `text/event-stream`。openAiResponsesProxy 的 ModifyResponse 无法解码该流，因此流式生成会被 OpenAI 计费，却无人计量。如果你需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限 CHAT 层级：o 系列 reasoning id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 端会拒绝此参数。Router 不会替它们拒绝，关于两个层级为何共用同一 schema 的说明见本组件的注释，因此发送该参数的 reasoning 调用会由 OpenAI 自身的错误来应答。
...
<ParamField body="tool_choice" type="string | object">
  模型应如何选择要使用的工具。可以是字符串模式，也可以是命名某个工具的对象。
</ParamField>
...
<ParamField body="truncation" type="string">
  当上下文超出模型窗口时采用的截断策略。与上面三种词汇表不同，这里的枚举确实会被强制执行，因为这两个值是 OpenAI 文档中记载的完整集合，且至今没有增长。显式的 `null` 仍然被接受，条件与上面的字段相同。

  可选值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  token 用量封装结构。它出现在此契约中，是因为 v1 操作在请求体上声明了它；OpenAI 是在响应中填充它，因此调用方没有理由发送它。
</ParamField>

由 Router 在 `GET /v2/models/openai/gpt-4.1-mini/openapi.json` 提供的 schema 生成，也就是在请求到达提供商之前用于校验调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将一条 system（或 developer）消息插入到模型上下文的第一项。

与 `previous_response_id` 一起使用时，上一个 response 中的 instructions 不会延续到下一个 response。这使得在新的 response 中替换 system（或 developer）消息变得非常简单。
</ResponseField>

<ResponseField name="max_output_tokens" type="integer">
  一个 response 可生成的 token 数量上限，包含可见的输出 token 和[推理 token](https://platform.openai.com/docs/guides/reasoning)。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型 response 的截断策略。
- `auto`：如果当前 response 及之前 response 的上下文超出模型的上下文窗口大小，模型将通过丢弃对话中间部分的输入项来截断 response，使其适应上下文窗口。
- `disabled`（默认）：如果模型 response 将超出模型的上下文窗口大小，请求将失败并返回 400 错误。

  可选值：`auto`、`disabled`
</ResponseField>

<ResponseField name="previous_response_id" type="string">
  发送给模型的上一 response 的唯一 ID。使用它来创建多轮对话。了解更多关于[对话状态](https://platform.openai.com/docs/guides/conversation-state)的信息。
</ResponseField>
...
<ResponseField name="reasoning.context" type="string">
  控制在后续轮次中将哪些推理项回传给模型，例如 `auto`、`current_turn` 或 `all_turns`。
</ResponseField>

<ResponseField name="reasoning.effort" type="string" default="&quot;medium&quot;">
  **仅支持 o 系列模型**

约束[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理力度。
当前支持的值为 `low`、`medium` 和 `high`。降低推理力度可以让响应更快，并减少 response 中用于推理的 token 数量。
...
模型所执行推理的摘要。这对于调试和理解模型的推理过程很有帮助。
...
配置 `{ "type": "json_schema" }` 会启用结构化输出（Structured Outputs），
确保模型匹配你提供的 JSON schema。了解更多信息请参阅[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)。
...
确保模型生成的消息是合法的 JSON。对于支持 `json_schema` 的模型，优先使用 `json_schema`。
</ResponseField>

<ResponseField name="text.verbosity" type="string">
  约束模型 response 的详细程度。取值为 `low`、`medium` 或 `high` 之一。
</ResponseField>

<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  模型在生成 response 时应如何选择使用哪个（或哪些）工具。参阅 `tools` 参数了解如何指定模型可以调用的工具。
</ResponseField>
...
<ResponseField name="completed_at" type="number">
  该 Response 完成时的 Unix 时间戳（以秒为单位）。仅当状态为 `completed` 时存在。
</ResponseField>

<ResponseField name="created_at" type="number">
  该 Response 创建时的 Unix 时间戳（以秒为单位）。
</ResponseField>
...
<ResponseField name="prompt_cache_key" type="string">
  供 OpenAI 用于缓存相似请求的响应，以优化缓存命中率。取代 `user` 字段。
</ResponseField>
...
<ResponseField name="status" type="string">
  响应生成的状态。可取 `completed`、`failed`、`in_progress`、`cancelled`、`queued` 或 `incomplete` 之一。
...
<ResponseField name="top_logprobs" type="integer">
  在每个 token 位置返回的最可能 token 的最大数量，每个 token 都附带相应的对数概率。
</ResponseField>
