---
title: "将 GPT 4.1 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/gpt-4.1：端点、请求结构以及 Router 返回的响应。"
...
{/* 已生成的文件。由 `pnpm code-pages:gen` 从 router-schemas/openai/gpt-4.1.json 生成。 */}

import RouterCodeFooter from "/snippets/zh/comfy-router/model-code-footer.mdx";

`openai/gpt-4.1` 的 API 参考，由 Comfy Router 从 OpenAI 提供。
...
**模型 ID：** `openai/gpt-4.1`

**端点：** `POST https://api.comfy.org/v2/models/openai/gpt-4.1`
...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/openai/gpt-4.1/requests`。运行一经受理，Router 就会返回 `201` 和 `request_id`；结果就绪后即可收集，可以由本进程收集，也可以由另一个进程收集。状态、取消与收集的流程详见[队列投递](/zh/development/comfy-router/queue)。
...
# 3. 收集。返回 200 并带有模型的原始输出；仍在运行时返回 202 和状态响应体。
...
<ParamField body="input" type="string | object[]" required>
  传递给模型的文本、图像或文件输入，用于生成响应。这是本契约中 Router 无法提供的字段，也是下方 `required` 中唯一的条目。
</ParamField>

<ParamField body="instructions" type="string">
  将一条系统（或开发者）消息插入到模型上下文的第一个位置。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  为响应生成的 token 数量上限，包括可见的输出 token 和推理 token。在推理类 id 上，该上限与隐藏的推理 token 共享，因此一个较小的值可能在任何可见文本产生之前就耗尽整个预算，这也是推理冒烟用例发送 1024 而聊天用例发送 16 的原因。

  范围：`1` 到 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上此字段是可选的，Router 会用 `{model}` 路径段填充它；显式的 `null` 也会以同样方式被替换。发送与路径不一致的值会被拒绝。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明该字段是为了让发送它的调用者不被拒绝，但它在此接口上是无效的：Router 会在分发之前将其确定为 `false`，因为它捕获的是提供商响应，而不是转发 `text/event-stream`，而 openAiResponsesProxy 的 ModifyResponse 无法解码后者，因此流式生成会被 OpenAI 计费而无人计量。如果需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限聊天层级：o 系列推理 id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 会拒绝此参数。Router 不会替它们拒绝，原因见本组件关于两个层级共用同一 schema 的说明，因此发送它的推理调用会由 OpenAI 自身的错误来应答。
...
<ParamField body="tool_choice" type="string | object">
  模型应如何选择使用哪个工具。可以是字符串模式，或是指定某个工具的对象。
</ParamField>
...
<ParamField body="truncation" type="string">
  当上下文超出模型窗口时的截断策略。与上面三种取值集合不同，这里的枚举确实会被强制执行，因为这两个值是 OpenAI 文档记载的完整集合，且并未增长。显式的 `null` 仍会被接受，条件与上面各字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  Token 用量封装。此契约中存在该字段，是因为 v1 操作在请求体上声明了它；OpenAI 会在响应中填充它，因此调用者没有理由发送它。
</ParamField>

由 Router 在 `GET /v2/models/openai/gpt-4.1/openapi.json` 提供的 schema 生成，也就是它在请求到达提供商之前用于验证调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将系统（或开发者）消息作为第一项插入模型的上下文中。

当与 `previous_response_id` 一起使用时，上一个响应的 instructions 不会延续到下一个响应。这使得在新响应中替换系统（或开发者）消息变得非常简单。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型响应的截断策略。
- `auto`：如果本次响应以及之前响应的上下文超出模型的上下文窗口大小，模型将通过丢弃对话中间位置的输入项来截断响应，以适应该上下文窗口。
...
<ResponseField name="previous_response_id" type="string">
  模型上一个响应的唯一 ID。用它来创建多轮对话。了解更多关于[会话状态](https://platform.openai.com/docs/guides/conversation-state)的信息。
</ResponseField>
...
约束[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理投入程度。目前支持的值为 `low`、`medium` 和 `high`。降低推理投入可以加快响应速度，并减少响应中用于推理的 token 数量。
...
模型所执行推理的摘要。这对于调试和理解模型的推理过程很有用。可为 `auto`、`concise` 或 `detailed` 之一。
...
配置 `{ "type": "json_schema" }` 可启用结构化输出，确保模型会匹配你提供的 JSON schema。更多信息请参阅[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)。

默认格式为 `{ "type": "text" }`，不含任何附加选项。

**不建议用于 gpt-4o 及更新的模型：**

设置为 `{ "type": "json_object" }` 会启用较旧的 JSON 模式，确保模型生成的消息是有效的 JSON。对于支持 `json_schema` 的模型，推荐使用 `json_schema`。
</ResponseField>

<ResponseField name="text.verbosity" type="string">
  约束模型响应的详细程度。可为 `low`、`medium` 或 `high` 之一。
</ResponseField>

<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  模型在生成响应时应如何选择要使用的工具（一个或多个）。有关如何指定模型可调用的工具，请参阅 `tools` 参数。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并
  假定它是包含模型生成内容的 `assistant` 消息，
  你可以考虑使用 SDK 中支持的 `output_text` 属性。
</ResponseField>

<ResponseField name="output_text" type="string">
  仅 SDK 提供的便捷属性，包含来自 `output` 数组中所有 `output_text` 项（如果存在）的聚合文本输出。
...
<ResponseField name="top_logprobs" type="integer">
  在每个 token 位置返回的最可能 token 的最大数量，每个 token 都带有对应的对数概率。
</ResponseField>
...
<ResponseField name="user" type="string">
  最终用户的弃用标识符。已替换为 `safety_identifier` 和 `prompt_cache_key`。
</ResponseField>
