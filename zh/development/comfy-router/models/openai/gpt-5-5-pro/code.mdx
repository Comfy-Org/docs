...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/gpt-5.5-pro/requests`。运行一经受理，Router 就会返回 `201` 和 `request_id`；结果就绪后即可收集，可由本进程或其他进程完成。状态、取消与收集的细节见[队列投递](/zh/development/comfy-router/queue)。
