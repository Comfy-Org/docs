---
title: "将 o3 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/o3：端点、请求结构以及 Router 返回的响应。"
sidebarTitle: "o3"
...
{/* 已生成的文件。由 `pnpm code-pages:gen` 从 router-schemas/openai/o3.json 生成。 */}
...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原生 HTTP 发起的相同调用。
...
# SDK 会自动创建幂等键（idempotency key），并在自动重试时复用它。
...
// 从环境中读取 COMFY_API_KEY。
// SDK 会为每次调用生成幂等键（idempotency key），并在自动重试时复用它。
...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/o3/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的相关说明见 [队列投递](/zh/development/comfy-router/queue)。
...
# 3. 收集。仍在运行时返回 202 与状态响应体；完成后返回 200 与模型的原生输出。
...
<ParamField body="instructions" type="string">
  在模型的上下文首位插入一条 system（或 developer）消息。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  一次响应所生成 token 数量的上限，包括可见的输出 token 和推理 token。在推理 id 上，该上限与隐藏的推理 token 共享，因此较小的值可能在产生任何可见文本之前就耗尽整个预算。这正是推理冒烟用例发送 1024、而聊天用例发送 16 的原因。
...
<ParamField body="reasoning" type="object">
  仅推理层级。推理模型的配置，例如 `{"effort": "medium"}`。原样转发；接受的键请参见 OpenAI 的推理指南。聊天层级的 id 会忽略它。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明该字段只是为了让发送它的调用方不被拒绝，但它在当前接口上是无效的：Router 会在分发之前将其固定为 `false`，因为它捕获的是提供商的响应，而不是转发 `text/event-stream`，而 openAiResponsesProxy 的 ModifyResponse 无法解码该流，因此流式生成会由 OpenAI 计费，却没有任何一方进行计量。如果需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅聊天层级：o 系列推理 id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 处会拒绝该参数。Router 不会替它们拒绝，原因请见本组件关于两个层级共用同一 schema 的说明，因此发送该参数的推理调用会由 OpenAI 自身的错误来回应。
...
<ParamField body="tools" type="object[]">
  模型可以调用的工具定义。Router 不会收窄工具分类；接受的形状请参见 OpenAI 的 Responses API 参考。
</ParamField>

<ParamField body="top_p" type="number">
  核采样截止值。仅聊天层级，适用条件与 `temperature` 相同。

  范围：`0` 到 `1`
</ParamField>

<ParamField body="truncation" type="string">
  当上下文超出模型的窗口时采用的截断策略。与上面三个词汇表不同，这里的枚举确实会被强制执行，因为这两个值就是 OpenAI 文档所记录的完整集合，且至今没有扩充。显式的 `null` 仍然被接受，适用条件与上面的各字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  token 用量封装。本契约中包含它，是因为 v1 操作在请求体上声明了它；OpenAI 会在响应中填充它，因此调用方没有理由发送它。
</ParamField>

根据 Router 在 `GET /v2/models/openai/o3/openapi.json` 提供的 schema 生成，这也是它在请求到达提供商之前用于校验调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将一条 system（或 developer）消息插入到模型上下文的第一个位置。

当与 `previous_response_id` 一起使用时，上一个响应的指令不会延续到下一个响应。这样可以轻松地在新的响应中替换 system（或 developer）消息。
</ResponseField>

<ResponseField name="max_output_tokens" type="integer">
  一次响应可生成 token 数量的上限，包括可见的输出 token 和[推理 token](https://platform.openai.com/docs/guides/reasoning)。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型响应的截断策略。
- `auto`：如果本次响应以及此前响应的上下文超过模型的上下文窗口大小，模型将通过丢弃对话中间位置的输入项来截断响应，使其适应上下文窗口。
...
限制[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理投入程度。
当前支持的值为 `low`、`medium` 和 `high`。降低推理投入可以带来更快的响应，并在响应中减少用于推理的 token 数量。
...
模型所执行推理的摘要。这对于调试和理解模型的推理过程非常有用。
可取值为 `auto`、`concise` 或 `detailed`。
...
配置 `{ "type": "json_schema" }` 可启用结构化输出（Structured Outputs），
确保模型匹配你提供的 JSON schema。请在
[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)中了解更多。

默认格式为 `{ "type": "text" }`，无任何附加选项。

**不建议用于 gpt-4o 及更新版本的模型：**

设置为 `{ "type": "json_object" }` 会启用较旧的 JSON 模式，它
确保模型生成的消息是有效的 JSON。对于支持 `json_schema` 的模型，
优先使用 `json_schema`。
</ResponseField>

<ResponseField name="text.verbosity" type="string">
  限制模型响应的详细程度。可取 `low`、`medium` 或 `high`。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并假定它是包含
  模型生成内容的 `assistant` 消息，你可以考虑在 SDK 支持的情况下
  使用 `output_text` 属性。
</ResponseField>
...
<ResponseField name="user" type="string">
  已弃用的最终用户标识符。已被 `safety_identifier` 和 `prompt_cache_key` 取代。
</ResponseField>
