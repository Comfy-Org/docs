---
title: "将 GPT 5 与 Comfy Router 搭配使用"
description: "通过 Comfy Router 调用 openai/gpt-5：端点、请求结构以及 Router 返回的响应。"
sidebarTitle: "GPT 5"
...
`openai/gpt-5` 的 API 参考文档，由 Comfy Router 从 OpenAI 提供。
...
**端点：** `POST https://api.comfy.org/v2/models/openai/gpt-5`
...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/openai/gpt-5/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消与收集的细节。
...
// 与 models.run() 返回的结果相同。失败或已取消的请求会在此处被 reject。
...
# 3. 收集。返回 200 时给出模型的原始输出，仍在运行时返回 202 及状态正文。
...
<ParamField body="instructions" type="string">
  插入一条 system（或 developer）消息，作为模型上下文中的第一项。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  一次响应所生成 token 数量的上限，包含可见的输出 token 与推理 token。在 reasoning 类 id 上，这个上限会与隐藏的推理 token 共享，因此一个很小的值可能会在任何可见文本出现之前就耗尽整个预算。这也是为什么 reasoning 冒烟用例发送 1024，而 chat 用例只发送 16。

  范围：`1` 到 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上，该字段是可选的，Router 会用 `{model}` 路径段填充它；显式的 `null` 也会以同样方式被替换。发送与路径不一致的值会被拒绝。
</ParamField>
...
<ParamField body="reasoning" type="object">
  仅限 REASONING 层级。推理模型的配置，例如 `{"effort": "medium"}`。原样转发；可接受的键请参阅 OpenAI 的推理指南。chat 层级的 id 会忽略它。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明该字段是为了让发送它的调用方不被拒绝，但在此接口上是无效的：Router 会在分发之前将它固定为 `false`，因为这里捕获的是提供商响应，而不是转发 `text/event-stream`，而 openAiResponsesProxy 的 ModifyResponse 无法解码它，因此流式生成会被 OpenAI 计费，却不会有任何一方进行计量。如果你需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限 CHAT 层级：o 系列 reasoning id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 侧会拒绝该参数。Router 不会替它们拒绝，原因参见本组件中关于两个层级为何共用一份 schema 的说明。因此，发送该参数的 reasoning 调用会由 OpenAI 自身的错误来回应。
...
<ParamField body="tools" type="object[]">
  模型可以调用的工具定义。Router 不会收窄工具的类别范围；可接受的形状请参阅 OpenAI 的 Responses API 参考文档。
</ParamField>

<ParamField body="top_p" type="number">
  核采样截断值。仅限 CHAT 层级，适用条件与 `temperature` 相同。

  范围：`0` 到 `1`
</ParamField>

<ParamField body="truncation" type="string">
  当上下文超出模型窗口时的截断策略。与上述三组词汇表不同，这里的枚举值确实会被强制执行，因为这两个值就是 OpenAI 文档所记载的完整集合，且至今没有扩充。显式的 `null` 仍然会被接受，适用条件与它上方的字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  Token 用量信封。该字段之所以出现在本契约中，是因为 v1 操作在请求体上声明了它；OpenAI 是在响应中填充它，因此调用方没有理由发送它。
</ParamField>

根据 Router 在 `GET /v2/models/openai/gpt-5/openapi.json` 所提供的 schema 生成，该文档也正是它在请求到达提供商之前用于校验调用的同一份文档。
### 输出

<ResponseField name="instructions" type="string">
  将系统（或开发者）消息作为模型上下文中的第一项插入。

当与 `previous_response_id` 一起使用时，上一条响应中的指令不会延续到下一条响应。这使得在新的响应中替换系统（或开发者）消息变得简单。
</ResponseField>

<ResponseField name="max_output_tokens" type="integer">
  响应可生成 token 数量的上限，包括可见的输出 token 和[推理 token](https://platform.openai.com/docs/guides/reasoning)。
</ResponseField>
...
<ResponseField name="truncation" type="string" default="&quot;disabled&quot;">
  用于模型响应的截断策略。
- `auto`：如果当前响应和之前响应的上下文超过模型上下文窗口大小，模型将通过丢弃对话中间的输入项来截断响应以适应上下文窗口。
...
<ResponseField name="previous_response_id" type="string">
  模型上一条响应的唯一 ID。使用此参数创建多轮对话。了解更多关于[对话状态](https://platform.openai.com/docs/guides/conversation-state)的内容。
</ResponseField>
...
<ResponseField name="reasoning.context" type="string">
  控制后续轮次中哪些推理项会被回传给模型，例如 `auto`、`current_turn` 或 `all_turns`。
</ResponseField>

<ResponseField name="reasoning.effort" type="string" default="&quot;medium&quot;">
  **仅 o 系列模型**

约束[推理模型](https://platform.openai.com/docs/guides/reasoning)的推理努力程度。目前支持的值有 `low`、`medium` 和 `high`。降低推理努力程度可以使响应更快，并减少响应中用于推理的 token 数量。
...
模型所执行推理的摘要。这对于调试和理解模型的推理过程非常有用。取值为 `auto`、`concise` 或 `detailed` 之一。
...
配置 `{ "type": "json_schema" }` 可启用结构化输出，确保模型匹配你提供的 JSON schema。在[结构化输出指南](https://platform.openai.com/docs/guides/structured-outputs)中了解更多。

默认格式为 `{ "type": "text" }`，不带任何附加选项。

**不推荐用于 gpt-4o 及更新模型：**

设置为 `{ "type": "json_object" }` 可启用较旧的 JSON 模式，确保模型生成的消息是有效的 JSON。对于支持它的模型，优先使用 `json_schema`。
</ResponseField>
...
<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  模型在生成响应时应如何选择使用哪个（或哪些）工具。参见 `tools` 参数以了解如何指定模型可以调用的工具。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并假定它是包含模型生成
  内容的 `assistant` 消息，不如考虑使用 SDK 中支持的
  `output_text` 属性。
</ResponseField>

<ResponseField name="output_text" type="string">
  仅 SDK 提供的便捷属性，包含 `output` 数组中所有 `output_text`
项聚合后的文本输出（如果存在）。Python 和 JavaScript SDK 均支持。
</ResponseField>
...
<ResponseField name="usage.input_tokens_details.cached_tokens" type="integer" required>
  从缓存中检索到的 token 数量。
[提示词缓存的更多说明](https://platform.openai.com/docs/guides/prompt-caching)。
</ResponseField>
