---
title: "将 GPT 6 Sol 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 openai/gpt-6-sol：端点、请求结构以及 Router 返回的响应。"
...
translationBlockHashes:
  ...
  "Examples": 67e6c8f1
---
{/* 已生成的文件。由 `pnpm code-pages:gen` 从 router-schemas/openai/gpt-6-sol.json 生成。 */}

import RouterCodeFooter from "/snippets/zh/comfy-router/model-code-footer.mdx";

`openai/gpt-6-sol` 的 API 参考文档，由 Comfy Router 从 OpenAI 提供。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 片段使用 Comfy SDK（`pip install comfy-sdk` 和 `npm install @comfyorg/sdk`）；cURL 片段是通过原始 HTTP 发出的同一个调用。

**模型 ID：** `openai/gpt-6-sol`

**端点：** `POST https://api.comfy.org/v2/models/openai/gpt-6-sol`
...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/openai/gpt-6-sol/requests`。只要运行被接纳，Router 就会返回 `201` 和一个 `request_id`；结果准备就绪后即可收集，无论来自当前进程还是另一个进程。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消和收集的完整流程。
...
<ParamField body="input" type="string | object[]" required>
  传递给模型的文本、图像或文件输入，用于生成响应。这是本契约中 Router 无法提供的唯一字段，也是下方 `required` 中唯一的条目。
</ParamField>

<ParamField body="instructions" type="string">
  将一条系统（或开发者）消息作为模型上下文中的第一项插入。
</ParamField>

<ParamField body="max_output_tokens" type="integer">
  一次响应所生成 token 数量的上限，包含可见的输出 token 和推理 token。在推理类 id 上，该上限与隐藏的推理 token 共享，因此一个较小的值可能在产生任何可见文本之前就耗尽整个预算。这正是推理冒烟测试用例发送 1024、而聊天用例发送 16 的原因。

  范围：`1` 到 `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 模型标识符。在 Comfy Router 上该字段为可选，Router 会用 `{model}` 路径段填充它；显式传入的 `null` 也会以同样方式被替换。发送与路径不一致的值会被拒绝。
</ParamField>
...
<ParamField body="reasoning" type="object">
  仅限推理层级。推理模型的配置，例如 `{"effort": "medium"}`。原样转发；可接受的键请参见 OpenAI 的推理指南。聊天层级的 id 会忽略它。
</ParamField>
...
<ParamField body="stream" type="boolean">
  声明该字段只是为了让发送它的调用方不被拒绝，但在此接口上它是无效的：Router 会在派发之前将其固定为 `false`，因为它捕获的是提供商响应，而不是转发 `text/event-stream`；而 openAiResponsesProxy 的 ModifyResponse 无法解码 `text/event-stream`，所以流式生成会由 OpenAI 计费，却没有任何一方进行计量。如果你需要流式传输，请使用 `POST /proxy/openai/v1/responses`。
</ParamField>

<ParamField body="temperature" type="number">
  采样温度。仅限聊天层级：o 系列推理 id（`o1`、`o1-pro`、`o3`、`o4-mini`）在 OpenAI 处会拒绝该参数。Router 不会替它们拒绝，参见本组件关于两个层级为何共用同一架构的说明，因此发送该参数的推理调用会由 OpenAI 自身的错误作答。
...
<ParamField body="tools" type="object[]">
  模型可调用的工具定义。Router 不会收窄工具的分类体系；可接受的形状请参见 OpenAI 的 Responses API 参考文档。
</ParamField>

<ParamField body="top_p" type="number">
  核采样截断值。仅限聊天层级，适用条件与 `temperature` 相同。

  范围：`0` 到 `1`
</ParamField>

<ParamField body="truncation" type="string">
  当上下文超出模型窗口时所采用的截断策略。与上面三个词表不同，这里的枚举确实会被强制校验，因为这两个值是 OpenAI 文档记载的完整集合，而且一直没有增加。显式传入的 `null` 仍然会被接受，条件与它上面的字段相同。

  可能的值：`auto`、`disabled`
</ParamField>

<ParamField body="usage" type="object">
  Token 用量封装。本契约中出现该字段，是因为 v1 操作在请求体上声明了它；OpenAI 在响应中填充它，因此调用方没有理由发送它。
</ParamField>

本文档由 Router 在 `GET /v2/models/openai/gpt-6-sol/openapi.json` 提供的架构生成，这也是它在请求到达提供商之前用来校验调用的同一份文档。

### 输出

<ResponseField name="instructions" type="string">
  将系统（或开发者）消息作为第一个项目插入模型的上下文中。

当与 `previous_response_id` 一起使用时，来自上一个响应的指令不会
延续到下一个响应。这使得在新响应中替换系统（或开发者）消息变得简单。
</ResponseField>

<ResponseField name="max_output_tokens" type="integer">
  一个响应可以生成的 token 数量上限，包括可见的输出 token 和 [推理 token](https://platform.openai.com/docs/guides/reasoning)。
</ResponseField>
...
限制[推理模型](https://platform.openai.com/docs/guides/reasoning) 的推理工作量。
目前支持的值有 `low`、`medium` 和 `high`。减少
推理工作量可以加快响应速度，并减少响应中
用于推理的 token 数量。
...
模型所执行推理的摘要。这对于调试和
理解模型的推理过程非常有用。
可以是 `auto`、`concise` 或 `detailed` 之一。
...
<ResponseField name="frequency_penalty" type="number">
  根据 token 在目前文本中已有的出现频率对新 token 进行惩罚。
</ResponseField>
...
- 与其访问 `output` 数组中的第一项并假定它是包含模型生成内容的 `assistant` 消息，不如考虑在 SDK 支持的情况下使用 `output_text` 属性。
</ResponseField>

<ResponseField name="output_text" type="string">
  仅 SDK 提供的便捷属性，包含 `output` 数组中所有 `output_text` 项聚合后的文本输出（如果存在）。
Python 和 JavaScript SDK 支持。
</ResponseField>
...
<ResponseField name="presence_penalty" type="number">
  根据 token 是否已出现在目前文本中对新 token 进行惩罚。
</ResponseField>

<ResponseField name="prompt_cache_key" type="string">
  OpenAI 用它缓存相似请求的响应，以优化缓存命中率。取代 `user` 字段。
</ResponseField>
...
<ResponseField name="usage.input_tokens_details.cached_tokens" type="integer" required>
  从缓存中检索到的 token 数量。
[关于 prompt caching 的更多信息](https://platform.openai.com/docs/guides/prompt-caching)。
</ResponseField>
