...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/tencent/hunyuan-3d-uv/requests`。运行一旦被受理，Router 就会返回 `201` 和 `request_id`；结果就绪后即可收集，可以从本进程或其他进程收集。状态、取消与收集的细节见[队列投递](/zh/development/comfy-router/queue)。
