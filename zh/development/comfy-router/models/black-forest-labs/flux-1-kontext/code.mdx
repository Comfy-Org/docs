...
description: "通过 Comfy Router 调用 FLUX.1 Kontext Pro 和 Kontext Max 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和结果形状"
...
**模型 ID：** `bfl/flux-kontext-pro`

**端点：** `POST https://api.comfy.org/v2/models/bfl/flux-kontext-pro`
...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/bfl/flux-kontext-pro/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [队列投递](/zh/development/comfy-router/queue)。
...
**模型 ID：** `bfl/flux-kontext-max`

**端点：** `POST https://api.comfy.org/v2/models/bfl/flux-kontext-max`
...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/bfl/flux-kontext-max/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [队列投递](/zh/development/comfy-router/queue)。
