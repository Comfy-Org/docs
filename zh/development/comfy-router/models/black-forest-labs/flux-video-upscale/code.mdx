...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/bfl/video-upscale-v1/requests`。运行一旦被受理，Router 就会立即返回 `201` 以及一个 `request_id`，结果就绪后即可收集，无论是从当前进程还是另一个进程。[队列投递](/zh/development/comfy-router/queue)介绍了状态查询、取消和结果收集的完整流程。
...
# 3. 收集。返回 200 时携带模型的原始输出；仍在运行时则返回 202 及状态响应体。
