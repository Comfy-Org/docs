...
```
  ...
  <Tab title="入队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/vertexai/gemini-3.1-flash-lite-image/requests`。一旦运行被接纳，Router 会立即返回 `201` 和 `request_id`，结果准备就绪后即可从当前进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态查询、取消和结果收集的完整流程。
