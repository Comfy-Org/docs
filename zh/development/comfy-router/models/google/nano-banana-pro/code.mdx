...
```
  ...
  <Tab title="排队并稍后收集">
将同样的请求体发送到 `POST https://api.comfy.org/v2/models/vertexai/gemini-3-pro-image/requests`。运行一旦被受理，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从当前进程或另一个进程中收集它。[队列投递](/zh/development/comfy-router/queue) 会逐步介绍状态查询、取消和收集。
...
本内容生成自 Router 在 `GET /v2/models/vertexai/gemini-3-pro-image/openapi.json` 提供的 schema，该文档也是请求到达提供商之前用于校验调用的同一份文档。
