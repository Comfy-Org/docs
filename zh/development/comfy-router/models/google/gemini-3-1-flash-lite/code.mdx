...
<ParamField body="generationConfig.temperature" type="number" default="1">温度用于在响应生成过程中进行采样，采样在应用 topP 和 topK 时发生。温度控制 token 选择的随机程度。较低的温度适合需要较少开放性、较少创造性响应的提示词，而较高的温度则可能带来更多样化或更具创造性的结果。温度为 0 表示始终选择概率最高的 token。在这种情况下，给定提示词对应的响应基本上是确定性的，但仍可能出现少量变化。如果模型返回的响应过于笼统、过于简短，或者模型给出了回退响应，请尝试提高温度
...
由 Router 在 `GET /v2/models/vertexai/gemini-3.1-flash-lite/openapi.json` 提供的 schema 生成，这正是请求到达提供商之前用于校验调用的同一份文档。### 输出
