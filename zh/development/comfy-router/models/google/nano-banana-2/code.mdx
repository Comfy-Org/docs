...
```
  ...
  <Tab title="加入队列并稍后收集">
同样的请求体，发送到 `POST https://api.comfy.org/v2/models/vertexai/gemini-3.1-flash-image/requests`。任务一旦被接纳，Router 会立即返回 `201` 以及一个 `request_id`；结果准备就绪后即可收集，可以在本进程中收集，也可以在另一个进程中收集。[队列交付](/zh/development/comfy-router/queue) 介绍了状态查询、取消与结果收集的完整流程。
