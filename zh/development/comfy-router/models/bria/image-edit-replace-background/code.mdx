...
```
  ...
  <Tab title="入队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/bria/image-edit-replace-background/requests`。运行一经接纳，Router 便会立即返回 `201` 和 `request_id`，结果准备就绪后即可从本进程或其他进程收集。[队列式交付](/zh/development/comfy-router/queue) 会逐一介绍状态、取消和收集。
...
# 3. 收集。返回 200 及模型的原生输出；若仍在运行则返回 202 及状态正文。
