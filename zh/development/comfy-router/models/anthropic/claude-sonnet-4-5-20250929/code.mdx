...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/anthropic/claude-sonnet-4-5-20250929/requests`。运行一经受理，Router 就会立即返回 `201` 和一个 `request_id`；结果就绪后，可从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 详细介绍了状态、取消和收集。
...
with Comfy() as client:
    ...
    # 轮询直到请求完成，按服务器给出的 Retry-After 进行等待。
...
# 3. 收集。返回 200 和模型的原生输出；仍在运行时返回 202 和状态体。
