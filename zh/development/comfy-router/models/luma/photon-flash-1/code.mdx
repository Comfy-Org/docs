...
```
  ...
  <Tab title="排队后稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/luma/photon-flash-1/requests`。任务一旦被接纳，Router 就会返回 `201` 和 `request_id`；结果就绪后即可收集，可以由本进程收集，也可以由另一个进程收集。[排队投递](/zh/development/comfy-router/queue) 详细介绍了状态、取消和结果收集。
