...
```
  ...
  <Tab title="入队并稍后收集">
同样的请求体，发送到 `POST https://api.comfy.org/v2/models/kling/kling-v3-omni/requests`。运行一经受理，Router 就会返回 `201` 和 `request_id`；结果就绪后即可收集，可以从本进程收集，也可以从另一个进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消与收集。
