---
title: "通过 Comfy Router 使用 Kling V3 Omni"
description: "通过 Comfy Router 调用 kling/kling-v3-omni：端点、请求结构以及 Router 返回的响应。"
...
```
  ...
  <Tab title="入队并稍后收集">
同样的请求体，发送到 `POST https://api.comfy.org/v2/models/kling/kling-v3-omni/requests`。运行一经受理，Router 就会返回 `201` 和 `request_id`；结果就绪后，可从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消与收集。
...
# 2. 轮询直到状态为 COMPLETED，每个响应都会给出 Retry-After 秒数，需按此等待。
...
<ParamField body="duration" type="string" default="&quot;5&quot;">
  视频时长，单位为秒。使用视频编辑功能时（refer_type: base），输出时长与输入视频相同，此参数无效。
...
<ParamField body="image_list[].type" type="string">
  该图像是首帧还是结束帧。first_frame 为首帧，end_frame 为结束帧。目前不支持仅使用结束帧。
...
<ParamField body="prompt" type="string">
  文本提示词，可包含正面和负面描述。不得超过 2,500 个字符。可通过 \<\<\<>>> 格式指定元素、图像或视频，例如 \<\<element_1>>、\<\<\<image_1>>>、\<\<\<video_1>>>。
</ParamField>
