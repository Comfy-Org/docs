---
title: "使用 Kling V2 Master 与 Comfy Router"
description: "通过 Comfy Router 调用 kling/kling-v2-master：端点、请求形状以及 Router 返回的响应。"
...
`kling/kling-v2-master` 的 API 参考，由 Comfy Router 提供，来自 Kling。
...
**模型 ID：** `kling/kling-v2-master`

**端点：** `POST https://api.comfy.org/v2/models/kling/kling-v2-master`
...
<ParamField body="cfg_scale" type="number" default="0.5">
  视频生成的灵活性。值越高，模型的灵活度越低，与用户提示词的相关性越强。
...
<ParamField body="mode" type="string" default="&quot;std&quot;">
  视频生成模式。std：标准模式，具有成本效益。pro：专业模式，生成时长更长但质量更高的输出。

  可能的值：`std`、`pro`
</ParamField>

<ParamField body="model_name" type="string">
  模型名。使用 Comfy Router 时省略或发送 null；模型由请求路径选择。提供的名称必须与该路径匹配。
</ParamField>
...
<ParamField body="multi_prompt[].duration" type="string">
  此分镜的时长（秒）。不得超过任务总时长，且不得小于 1。所有分镜时长之和等于任务总时长。
</ParamField>
...
<ParamField body="multi_prompt[].prompt" type="string">
  此分镜的提示词。最大长度 512 个字符。
</ParamField>

<ParamField body="multi_shot" type="boolean" default="false">
  是否生成多镜头视频。当为 true 时，prompt 参数无效。当为 false 时，shot_type 和 multi_prompt 参数无效。
</ParamField>
...
<ParamField body="prompt" type="string">
  正向文本提示词。使用 \<\<\<voice_1>>> 指定与 voice_list 参数顺序匹配的声音。一个任务最多可引用 2 个音调。指定音调时，sound 参数值必须为 on。
</ParamField>
...
由 Router 在 `GET /v2/models/kling/kling-v2-master/openapi.json` 提供的模式生成，也是在请求到达提供商之前用于验证调用的同一份文档。### 输出
