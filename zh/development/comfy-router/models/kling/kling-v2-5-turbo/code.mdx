---
title: "将 Kling V2.5 Turbo 与 Comfy Router 搭配使用"
description: "通过 Comfy Router 调用 kling/kling-v2-5-turbo：端点、请求结构以及 Router 返回的响应。"
...
`kling/kling-v2-5-turbo` 的 API 参考，由 Comfy Router 提供，模型来自 Kling。

## 快速开始

在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原生 HTTP 执行的同一调用。

**模型 ID：** `kling/kling-v2-5-turbo`

**端点：** `POST https://api.comfy.org/v2/models/kling/kling-v2-5-turbo`
...
```
  ...
  <Tab title="排队并稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/kling/kling-v2-5-turbo/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。状态、取消与收集的细节见 [队列投递](/zh/development/comfy-router/queue)。
...
with Comfy() as client:
    ...
    print("request_id:", handle.request_id)  # 连同模型 ID，就是另一个进程收集结果所需的全部信息
...
console.log("requestId:", handle.requestId); // 连同模型 ID，就是另一个进程收集结果所需的全部信息
...
print("requestId:", handle.requestId)  // 连同模型 ID，就是另一个进程收集结果所需的全部信息
...
# 2. 轮询直到状态为 COMPLETED，每次响应指定多少秒的 Retry-After 就等待多少秒。
...
<ParamField body="cfg_scale" type="number" default="0.5">
  视频生成的灵活性。该值越高，模型的灵活度越低，与用户提示词的相关性越强。
...
<ParamField body="model_name" type="string">
  模型名。使用 Comfy Router 时可省略或发送 null；模型由请求路径选择。如提供名称，则必须与该路径一致。
</ParamField>
...
<ParamField body="negative_prompt" type="string">
  负面文本提示词。推荐直接在正面提示词中通过否定语句来补充负面提示信息。
</ParamField>

<ParamField body="prompt" type="string">
  正面文本提示词。使用 \<\<\<voice_1>>> 指定与 voice_list 参数顺序对应的声音。单个任务最多可引用 2 个音色。指定音色时，sound 参数值必须为 on。
</ParamField>
...
本页面内容根据 Router 在 `GET /v2/models/kling/kling-v2-5-turbo/openapi.json` 提供的 schema 生成，这也是请求到达提供商之前用于校验调用的同一份文档。
### 输出
