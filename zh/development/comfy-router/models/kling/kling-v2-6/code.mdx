...
```
  ...
  <Tab title="加入队列并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/kling/kling-v2-6/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可以从本进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue) 介绍了状态、取消与收集的完整流程。
...
# 3. 收集。返回 200 时为模型的原生输出，仍在运行时返回 202 及状态响应体。
