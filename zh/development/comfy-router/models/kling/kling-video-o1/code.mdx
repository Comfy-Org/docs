...
```
  ...
  <Tab title="排队提交，稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/kling/kling-video-o1/requests`。运行被受理后，Router 会立即返回 `201` 和 `request_id`；结果就绪后，可从本进程或其他进程收集。状态、取消与收集的流程见 [队列投递](/zh/development/comfy-router/queue)。
...
# 2. 轮询直到 status 为 COMPLETED，每次响应都按其中给出的 Retry-After 秒数等待。
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/kling/kling-video-o1/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 收集。仍在运行时返回 202 和状态体，完成时返回 200 和模型的原始输出。
