...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python 和 TypeScript 片段使用 Comfy SDK（`pip install comfy-sdk` 和 `npm install @comfyorg/sdk`）；cURL 片段则是通过原始 HTTP 进行的相同调用。
...
```
  ...
  <Tab title="加入队列并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/kling/kling-v3/requests`。运行一经受理，Router 就会立即返回 `201` 和 `request_id`；结果就绪后即可收集，可以从本进程或其他进程收集。状态、取消与收集的细节见[队列投递](/zh/development/comfy-router/queue)。
