...
```
  ...
  <Tab title="加入队列，稍后收集">
将相同的请求体发送到 `POST https://api.comfy.org/v2/models/kling/kling-image-o1/requests`。运行一经受理，Router 就会立即返回 `201` 和 `request_id`；结果就绪后，可从本进程或其他进程进行收集。状态、取消与收集的完整流程见 [队列投递](/zh/development/comfy-router/queue)。
...
# 3. 收集。仍在运行时返回 202 及状态响应体，完成时返回 200 及模型的原始输出。
