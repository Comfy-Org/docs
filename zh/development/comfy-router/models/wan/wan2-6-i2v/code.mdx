...
- wan3.0-video：first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  在同一请求中互斥。数组顺序定义了提示词中资产的参考顺序（图像 1、视频 1、音频 1……）。
...
（wan3.0-video 最多可支持 20,000 个字符；超出限制的内容会被截断）。
对于使用多个参考视频的 wan2.6-r2v，请按照参考视频的顺序使用 'character1'、'character2' 等来指代主体。
...
- 单个文件大小：最大 30MB
- 不能与 audio_url 一起使用
参考时长：单个视频最大 5 秒，两个视频各最大 2.5 秒，三个视频按比例减少。
计费：基于实际使用的参考时长。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。本组件不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时则必须提供，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频总时长
  + 输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型选择合适
  的时长
...
由 Router 在 `GET /v2/models/wan/wan2.6-i2v/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前 Router 用于校验调用的同一份文档。
### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应封装的根层级而非 `output` 之下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应封装的根层级而非 `output` 之下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
