...
```
  ...
  <Tab title="排队并稍后收集">
同一个请求体，发送到 `POST https://api.comfy.org/v2/models/wan/wan2.7-i2v/requests`。运行一经接纳，Router 就会返回 `201` 以及一个 `request_id`，结果就绪后即可收集，无论是在当前进程还是其他进程中。[队列投递](/zh/development/comfy-router/queue) 会详细说明状态、取消与收集的流程。
...
并不在这里传入首帧：它们把首帧作为类型为 `first_frame` 的 `media` 元素传入
（见下方的 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，
...
<ParamField body="input.media" type="object[]">
  适用于 wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。用于指定视频生成的参考
...
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，可以是公共网址或 data: 网址。reference_video 不是
  该操作的输入类型。
- happyhorse-1.x-video-edit：video
上述每个资产的 "最大 20MB" 数值是合作伙伴对其最终拿到的图片所设的上限，
当资产是公共网址时按字面适用，因为这些字节从不经过 Comfy。内联的 data: 网址
确实会经过 Comfy，并且会先遇到更低的传输上限：Comfy Router 的 POST 请求体
总计上限为 20 MiB，超过即返回 413，而 base64 会使载荷膨胀约 4/3，因此
单个超过大约 15 MB 的内联资产，在任何合作伙伴规则被触及时就已经先被拒绝；
当请求体中携带多个此类资产时触发得更早，因为 20 MiB 的 Router 上限约束的是
整个请求，而不是每个元素。任何接近这些上限的内容都请以公共网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公共 HTTP/HTTPS 网址、OSS 临时网址，或者（在该模型于上文 `media` 描述中如此说明时，例如 happyhorse-1.x 的 i2v 和 r2v 写法）内联的 `data:{MIME_type};base64,...` 网址。有关各模型的大小和像素下限，以及在任何合作伙伴规则生效之前约束内联载荷的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
主体。示例："Character1 在路边唱歌，Character2 在旁跳舞"
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等，按 media 数组中对应的顺序
引用媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 则必须提供它，可接受写法的枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频总时长加
  输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型选择合适的时长
...
本代理会拒绝既不提供 resolution 也不提供 size 的视频生成请求，
因为分辨率档位决定计费费率。
...
根据 Router 在 `GET /v2/models/wan/wan2.7-i2v/openapi.json` 提供的 schema 生成，该文档也是它在请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应包的 ROOT 层级，而不是 `output` 之下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应包的 ROOT 层级，而不是 `output` 之下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
