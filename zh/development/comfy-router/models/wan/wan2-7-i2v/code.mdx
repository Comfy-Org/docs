...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方可以省略。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起的 v1 调用必须提供该值，其可接受拼写的枚举定义在该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
根据 Router 在 `GET /v2/models/wan/wan2.7-i2v/openapi.json` 所提供的 schema 生成，该文档与请求到达提供商之前 Router 用于校验调用的文档是同一份。
### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误代码，在响应信封的根层级而非 `output` 下报告（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，在响应信封的根层级而非 `output` 下报告（请求成功时不返回）。在回退到 `output.message` 之前请先读取此处。
</ResponseField>
