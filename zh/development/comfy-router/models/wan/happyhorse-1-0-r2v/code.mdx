...
<ParamField body="input.img_url" type="string">
  首帧图像的网址或 Base64 编码数据。I2V 模型必填。图像格式：JPEG、JPG、PNG、BMP、WEBP。分辨率：360-2000 像素。文件大小：最大 10MB。
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7 和 wan3.0 模型的媒体资产列表。用于指定视频生成的参考素材（图像、音频、视频）。
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL（公开 HTTP/HTTPS 网址或 OSS 临时网址）
</ParamField>
