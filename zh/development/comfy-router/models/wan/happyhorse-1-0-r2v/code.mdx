...
<ParamField body="input.img_url" type="string">
  首帧图像的网址或 Base64 编码数据。仅 wan2.5-i2v-preview 与 wan2.6-i2v 模型需要该参数。happyhorse-1.x 的 i2v 写法不在此处传入首帧：它们把首帧作为 `media` 中 type 为 `first_frame` 的元素传入（见下方 `media` 说明），一个在 wan2.6-i2v 上能成功的 `img_url` 请求体，会被提供方在 happyhorse-1.0-i2v 与 happyhorse-1.1-i2v 上拒绝。图像格式：JPEG、JPG、PNG、BMP、WEBP。分辨率：360-2000 像素。文件大小：最大 10MB。
</ParamField>

<ParamField body="input.media" type="object[]">
  用于 wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。用于指定视频生成的参考素材（图像、音频、视频）。
...
对于包含多个参考视频的 wan2.6-r2v，请按参考视频的顺序使用 'character1'、'character2' 等来指代主体。
示例：“Character1 在路边唱歌，Character2 在旁边跳舞”
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等，按 media 数组中的
对应顺序指代媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不对其进行约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时则必须提供，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频总时长
  + 输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型
  选择合适的时长
...
该内容由 Router 在 `GET /v2/models/wan/happyhorse-1.0-r2v/openapi.json` 提供的 schema 生成，也是它在请求到达提供商之前用于校验调用的同一份文档。
### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在响应信封的根层级而不是 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应信封的根层级而不是 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此内容。
</ResponseField>
