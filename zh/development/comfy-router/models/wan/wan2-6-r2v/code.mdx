...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys)中创建一个密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原生 HTTP 发起的同一次调用。
...
```
  ...
  <Tab title="入队并稍后收集">
同一请求体，发送至 `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests`。只要运行被接受，Router 就会返回 `201` 和一个 `request_id`，结果就绪后即可收集，无论是从当前进程还是其他进程。 [队列投递](/zh/development/comfy-router/queue) 介绍了状态查询、取消和结果收集。
...
i2v 拼写不在此处传入首帧：它们将首帧作为类型为 `first_frame` 的
`media` 元素传入（见下方 `media`），而在 wan2.6-i2v 上能成功的
...
- wan3.0-video：first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  在同一请求中互斥。数组顺序定义了资产在提示词中的引用顺序
  （Image 1、Video 1、Audio 1……）。
- happyhorse-1.x-i2v：仅 first_frame，且恰好 1 个。至少 300x300 像素，
  JPEG/JPG/PNG/WEBP，最大 20MB，可以是公共 URL 或 data:\{MIME_type};base64,... URL。
  这些拼写不接收 img_url；首帧放在这里。
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，可以是公共 URL 或 data: URL。reference_video 
  不是此操作的输入类型。
- happyhorse-1.x-video-edit：video
上文中每个资产“最大 20MB”的数值是合作伙伴对其最终所得到图像的上限，当资产是
公共 URL 时按此数值执行，因为这些字节从不经过 Comfy。内联的 data: URL 则会
经过 Comfy，会先遇到更低的传输上限：Comfy Router 的 POST 请求体总计上限为
20 MiB，超过即返回 413；而 base64 会使负载膨胀约 4/3，因此单个内联资产只要
超过大约 15 MB，就会在触及此处任何合作伙伴规则之前被拒绝。若请求体携带多个
此类资产，则被拒绝得更早，因为 20 MiB 的 Router 上限约束的是整个请求，
而不是每个元素。接近这些上限的内容请以公共 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公共 HTTP/HTTPS URL、OSS 临时 URL，或者（当上方 `media` 描述中该模型的条目如此说明时，例如 happyhorse-1.x i2v 和 r2v 拼写）内联的 `data:{MIME_type};base64,...` URL。有关各模型的大小和像素下限，以及在任何合作伙伴规则生效之前约束内联负载的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，使用 'character1'、'character2' 等按参考视频
...
<ParamField body="input.reference_video_urls" type="string[]">
  仅 wan2.6-r2v 模型的参考视频 URL。由 1-3 个视频 URL 组成的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频长度：2-30 秒
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起的 v1 调用则必须提供它，可接受的拼写枚举位于该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
对于 wan3.0-video，自适应（默认值）会根据输入媒体的比例和意图自动推荐
合适的宽高比。
...
由 Router 在 `GET /v2/models/wan/wan2.6-r2v/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在信封的 ROOT 下，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在信封的 ROOT 下，而不是在 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此信息。
</ResponseField>
