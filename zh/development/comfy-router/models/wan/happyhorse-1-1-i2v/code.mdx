...
<ParamField body="input.media" type="object[]">
  适用于 wan2.7 和 wan3.0 模型的媒体资产列表。用于指定视频生成的参考素材（图像、音频、视频）。
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL（公网 HTTP/HTTPS URL 或 OSS 临时 URL）
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，请按照参考视频的顺序使用 'character1'、'character2' 等来指代主体。
示例："Character1 sings on the roadside, Character2 dances beside it"
对于 wan3.0-video 参考模式，请使用“图像 1”“视频 1”“音频 1”等来指代媒体数组中
对应顺序的媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。该组件不作约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供它，可接受拼写的枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入
  视频总时长 + 输出视频时长不得超过 30 秒；-1 启用智能时长模式，
  由模型选择合适的时长
...
生成自 Router 在 `GET /v2/models/wan/happyhorse-1.1-i2v/openapi.json` 提供的 schema,该文档也是请求到达提供商之前 Router 用于校验调用的同一文档。### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应封装的 ROOT 层级而非 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应封装的 ROOT 层级而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
