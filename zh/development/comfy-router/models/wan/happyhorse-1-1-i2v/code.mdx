...
# 3. 收集。返回 200 即为模型的原始输出，仍在运行时返回 202 和状态响应体。
...
<ParamField body="input.img_url" type="string">
  首帧图像网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型必填。happyhorse-1.x
的 i2v 写法不在这里接收首帧：它们将首帧作为类型为
`first_frame` 的 `media` 元素传入（见下方 `media`），
而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，在
...
<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。用于指定视频生成的参考
素材（图像、音频、视频）。每个元素包含 type 和 url
字段。
支持的 type 取值因模型而异：
...
- wan3.0-video：first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  互斥。数组顺序决定了提示词中各资产的引用顺序（Image 1、Video 1、Audio 1，……）。
- happyhorse-1.x-i2v：仅 first_frame，且必须正好 1 个。至少 300x300 像素，
  JPEG/JPG/PNG/WEBP，最大 20MB，可为公开 URL 或 data:\{MIME_type};base64,... URL。
  这些写法不使用 img_url；首帧放在这里。
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，可为公开 URL 或 data: URL。reference_video 不是该操作的
  输入类型。
- happyhorse-1.x-video-edit：video
上面每个资产的“最大 20MB”数字是合作伙伴对其最终取得的图像所设的上限，当资产为公开 URL
时它们按字面适用，因为这些字节从不经过 Comfy。内联的 data: URL 确实会经过
Comfy，同时也受到传输上限的约束：Comfy Router 的 POST 请求体
总量上限为 100 MiB，超出即返回 413，而 base64 会使载荷膨胀约 4/3，
因此单个内联资产超过约 75 MB 就会在触达这里的任何合作伙伴规则之前被拒绝。
在该体积下，单个资产即使达到合作伙伴自身 20MB 的上限也能以内联方式轻松容纳，
所以对单个资产而言起约束作用的是合作伙伴规则；传输上限则会在多个资产时生效，
因为 100 MiB 的 Router 上限约束的是整个请求，而不是每个元素。接近这些上限的
任何内容都请以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开的 HTTP/HTTPS URL、OSS 临时 URL，或者（在模型于上方 `media` 描述中明确说明支持时，happyhorse-1.x 的 i2v 和 r2v 写法即是如此）内联的 `data:{MIME_type};base64,...` URL。有关各模型的体积与像素下限，以及约束内联载荷总量的 100 MiB Router 请求体上限，请参见该描述。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，使用 'character1'、'character2' 等按参考视频顺序指代
主体。示例："Character1 在路边唱歌，Character2 在旁边跳舞"
对于 wan3.0-video 的参考模式，使用 'Image 1'、'Video 1'、'Audio 1' 等按 media 数组中的
对应顺序指代媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。在本组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充该值，因此 Router 调用方会省略它。直接 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供该值，而可接受写法的枚举定义在该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频
  时长与输出视频时长之和不得超过 30 秒；-1 启用智能时长模式，由模型选择合适的
  时长
...
如果视频生成请求既未提供分辨率也未提供尺寸，此代理会拒绝该请求，
因为分辨率档位决定了计费费率。
...
对于 wan2.5 T2V：480P（480*832、832*480、624*624）、720P、1080P 各尺寸
...
本文档根据 Router 在 `GET /v2/models/wan/happyhorse-1.1-i2v/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在信封的根层级而非 `output` 之下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在信封的根层级而非 `output` 之下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此项。
</ResponseField>
