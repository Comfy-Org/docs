...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-i2v/requests`。请求一经接纳，Router 立即以 `201` 返回一个 `request_id`，待结果就绪后，即可由当前进程或其他进程收集。[排队交付](/zh/development/comfy-router/queue) 详细介绍了状态查询、取消与结果收集。
