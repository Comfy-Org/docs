...
**接口端点:** `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime`
...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体,发送到 `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime/requests`。运行一经受理,Router 就会以 `201` 返回 `request_id`,结果就绪后即可从当前进程或其他进程收集。[队列化交付](/zh/development/comfy-router/queue)介绍了状态、取消和收集的完整流程。
...
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要。happyhorse-1.x 的 i2v 拼写并不在此处接收首帧：它们将首帧作为类型为 `first_frame` 的 `media` 元素传入（见下方 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上会被提供商拒绝。
...
上面每个资产“最大 20MB”的数字是合作伙伴对其最终得到的图像所设的上限，
当资产是公开 URL 时按字面适用，因为这些字节从不经过 Comfy。内联
data: URL 确实会经过 Comfy，并会先遇到更低的传输上限：Comfy Router 的 POST 请求体
总计上限为 20 MiB，超过即返回 413，而 base64 会将负载膨胀约 4/3，
因此单个大于约 15 MB 的内联资产会在触达这里任何合作伙伴规则之前就被拒绝：
当请求体中包含多个此类资产时会更早被拒绝，因为 20 MiB 的 Router 上限约束的是
整个请求，而不是每个元素。接近这些上限的内容请以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开 HTTP/HTTPS URL、OSS 临时 URL，或在上述 `media` 描述中该模型条目允许时（如 happyhorse-1.x 的 i2v 和 r2v 拼写那样），使用内联 `data:{MIME_type};base64,...` URL。有关各模型的大小与像素下限，以及在任何合作伙伴规则适用之前约束内联负载的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>

<ParamField body="input.negative_prompt" type="string">
  反向提示词用于描述你不想在视频画面中看到的内容
</ParamField>

<ParamField body="input.prompt" type="string">
  文本提示词。支持中文和英文，长度不超过 800 个字符
（wan3.0-video 最多 20,000 个字符；超出限制的内容会被截断）。
对于带有多个参考视频的 wan2.6-r2v，按参考视频的顺序使用 'character1'、'character2' 等来指代主体。
示例："Character1 在路边唱歌，Character2 在旁边跳舞"
对于 wan3.0-video 参考模式，使用 'Image 1'、'Video 1'、'Audio 1' 等来按媒体数组中
相应的顺序指代媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。在此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略该字段。直连 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 则必须提供它，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频时长加上输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型选择合适的时长
...
由 Router 在 `GET /v2/models/wan/wan3.0-video-prime/openapi.json` 提供的 schema 生成，这也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在 envelope 的根层级，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在 envelope 的根层级，而不是在 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
