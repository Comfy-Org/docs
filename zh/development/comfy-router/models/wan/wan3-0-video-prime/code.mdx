...
- wan3.0-video：first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  与 file 同时使用）。在同一请求中，reference_*/file/link 类型与 first_frame/last_frame 类型
  互斥。数组顺序决定资产在提示词中的引用顺序
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL（公共 HTTP/HTTPS 网址或 OSS 临时网址）
</ParamField>
...
<ParamField body="input.reference_video_urls" type="string[]">
  仅限 wan2.6-r2v 模型的参考视频网址。由 1-3 个视频网址组成的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频长度：2-30 秒
- 单个文件大小：最大 30MB
- 不能与 audio_url 同时使用
参考时长：单个视频最大 5 秒，两个视频各最大 2.5 秒，三个视频按比例递减。
计费：按实际使用的参考时长计算。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略。直接通过 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供它，可接受的拼写枚举定义在该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
对于 wan3.0-video，adaptive（默认值）会根据输入媒体资产的比例和意图
自动推荐合适的画面比例。
...
根据 Router 在 `GET /v2/models/wan/wan3.0-video-prime/openapi.json` 提供的 schema 生成，这也是它在请求到达提供商之前用于校验调用的同一份文档。### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误码，在响应信封（envelope）的根级返回，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，在响应信封（envelope）的根级返回，而不是在 `output` 下（请求成功时不返回）。在回退读取 `output.message` 之前，请先查看此项。
</ResponseField>
