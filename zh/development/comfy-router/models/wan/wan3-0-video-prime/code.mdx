---
title: "将 Wan 3.0 Video Prime 与 Comfy Router 配合使用"
description: "通过 Comfy Router 以 HTTP 方式调用 wan/wan3.0-video-prime 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和结果形状"
...
Wan 3.0 Video Prime 的 API 参考。Wan 3.0 Video Prime 是 Wan 3.0 Video 中质量更高的层级，并已针对相同的 Router 提示词和媒体 schema 完成验证。
...
**模型 ID：** `wan/wan3.0-video-prime`

**端点：** `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime`
...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体,发送到 `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime/requests`。运行一经受理,Router 就会以 `201` 返回 `request_id`,结果就绪后即可从当前进程或其他进程收集。[队列投递](/zh/development/comfy-router/queue)介绍了状态、取消和收集的完整流程。
...
i2v 拼写方式不在此处接收首帧：它们将首帧作为类型为 `first_frame` 的 `media`
元素传入（见下文 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，
...
- happyhorse-1.x-i2v: 仅 first_frame，且必须正好 1 个。至少 300x300 像素，
  ...
  这些拼写方式不接受 img_url；首帧放在这里。
- happyhorse-1.x-r2v: 仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大值 20MB，公开网址或 data: 网址。reference_video 不是此操作的
  输入类型。
- happyhorse-1.x-video-edit: video
上文每个资产的“max 20MB”数值是合作伙伴对其最终获得的图像所设的上限，当资产是
公开网址时按此数值执行，因为这些字节从不经过 Comfy。内联的 data: URL 则会
经过 Comfy，并且同样会遇到传输上限：Comfy Router 的 POST 请求体总计上限为
100 MiB，超过则返回 413，而 base64 会使负载膨胀约 4/3，因此单个内联资产若超过
约 75 MB，会在触及这里的任何合作伙伴规则之前就被拒绝。在该大小下，单个资产
即使达到合作伙伴自身的 20MB 上限也能轻松以内联方式容纳，所以对于单个资产
起作用的是合作伙伴规则；而传输上限则在多个资产时起作用，因为 100 MiB 的 Router
上限约束的是整个请求，而不是每个元素。凡接近这些上限的内容，请以公开网址方式发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公开的 HTTP/HTTPS 网址、OSS 临时网址，或者（当上文 `media` 描述中该模型的条目如此说明时，例如 happyhorse-1.x 的 i2v 和 r2v 拼写方式）内联的 `data:{MIME_type};base64,...` 网址。有关各模型的大小与像素下限，以及约束内联负载总量的 100 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
主体。示例："Character1 在路边唱歌，Character2 在它旁边跳舞"
...
- 单个文件大小：最大值 30MB
- 不能与 audio_url 同时使用
参考时长：单个视频最多 5 秒，两个视频每个最多 2.5 秒，三个视频则按比例更少。
计费：按实际使用的参考时长计费。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时则必须提供它，可接受拼写的枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">wan2.7-videoedit 模型的视频音频设置。
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频总时长
  与输出视频时长之和不得超过 30 秒；-1 启用智能时长模式，由模型选择合适的时长
...
由 Router 在 `GET /v2/models/wan/wan3.0-video-prime/openapi.json` 提供的 schema 生成，该文档与请求到达提供商之前用于校验调用的文档相同。
...
<ResponseField name="code" type="string">
  失败请求的错误码，在信封的根层级报告，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>
