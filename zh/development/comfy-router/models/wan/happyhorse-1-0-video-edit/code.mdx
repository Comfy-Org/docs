...
```
  ...
  <Tab title="加入队列并稍后收集">
同一请求体，发送到 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-video-edit/requests`。运行一经接纳，Router 便会返回 `201` 与 `request_id`，结果就绪后即可从当前进程或另一个进程收集。[排队交付](/zh/development/comfy-router/queue) 一节介绍了状态查询、取消和结果收集的完整流程。
...
<ParamField body="input.img_url" type="string">
  首帧图片网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要此项。
happyhorse-1.x 的 i2v 拼写不在此处传入首帧：它们将其作为 `media` 中 type 为 `first_frame` 的元素传入（见下文 `media`），
而在 wan2.6-i2v 上可以成功的 `img_url` 请求体，在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上会被提供商拒绝。
图像格式：JPEG、JPG、PNG、BMP、WEBP。分辨率：360-2000 像素。
文件大小：最大 10MB。
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。用于为视频生成指定参考
...
- happyhorse-1.x-i2v：仅支持 first_frame，且必须恰好 1 个。至少 300x300 像素，
  ...
  这些拼写不接受 img_url；首帧从这里传入。
- happyhorse-1.x-r2v：仅支持 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，公开 URL 或 data: URL。reference_video 不是该操作的输入
  类型。
- happyhorse-1.x-video-edit：video
上面每个资产“最大 20MB”的数值是合作伙伴对其最终获得的图像所设的上限，
当资产是公开 URL 时，这些数值完全按字面适用，因为
这些字节从不经过 Comfy 传输。内联的 data: URL 确实会经过
Comfy 传输，并会先触达更低的传输上限：Comfy Router 的 POST 请求体
总上限为 20 MiB，超出时返回 413，而 base64 会使负载膨胀
约 4/3，因此单个内联资产只要超过大约 15 MB，就会在触及此处任何
合作伙伴规则之前被拒绝。若请求体中包含多个此类资产，被拒绝得还会更早，因为 20 MiB 的 Router 上限
约束的是整个请求，而不是每个元素。凡是接近这些上限的内容，都请以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开的 HTTP/HTTPS URL、OSS 临时 URL，或者在上述 `media` 描述中该模型的条目允许时（happyhorse-1.x 的 i2v 和 r2v 拼写即如此），使用内联的 `data:{MIME_type};base64,...` URL。有关按模型区分的大小与像素下限，以及在任何合作伙伴规则生效之前约束内联负载的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于 wan3.0-video 的参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等来按
media 数组中对应的顺序指代媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时则必须提供，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频总时长 + 输出视频总时长不得超过 30 秒；-1 表示启用智能时长模式，由模型自行选择合适的时长
...
对于 wan2.7 模型，若未提供该参数，则根据分辨率档位确定默认值。
对于 wan3.0-video，adaptive（默认值）会根据输入媒体的宽高比例和意图自动推荐合适的宽高比。
...
该代理会拒绝既未提供分辨率也未提供尺寸的视频生成请求，因为分辨率档位决定计费标准。
...
根据 Router 在 `GET /v2/models/wan/happyhorse-1.0-video-edit/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在响应封装的 ROOT 层级，而不是 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应封装的 ROOT 层级，而不是 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
