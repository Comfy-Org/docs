...
在 [你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys) 中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原始 HTTP 执行同一调用。
...
（见下方 `media`），而在 wan2.6-i2v 上能够成功的 `img_url` 请求体，会被
提供商在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上拒绝。
...
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，公开网址或 data: 网址。reference_video 不是此操作的
  输入类型。
- happyhorse-1.x-video-edit：video
上述每个资产的“max 20MB”数值是合作伙伴对其最终获得的图像的上限，
当资产是公开网址时按字面适用，因为这些字节从不经过 Comfy。内联的
data: 网址确实会经过 Comfy，并且会先触及更低的传输上限：Comfy Router POST
请求体总计上限为 20 MiB，超过即返回 413，而 base64 会使负载膨胀约 4/3，
因此单个大于约 15 MB 的内联资产会在触及此处任何合作伙伴规则之前
就被拒绝；当请求体携带多个此类资产时还会更早被拒绝，因为 20 MiB 的
Router 上限约束的是整个请求，而不是每个元素。接近这些上限的内容
请以公开网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公开 HTTP/HTTPS 网址、OSS 临时网址，或者，在上方 `media` 描述中该模型的条目如此说明时（如 happyhorse-1.x 的 i2v 和 r2v 拼写那样），内联的 `data:{MIME_type};base64,...` 网址。有关各模型的大小和像素下限，以及在任何合作伙伴规则适用之前约束内联负载的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，请使用 'character1'、'character2' 等按参考视频的顺序指代
...
- 单个文件大小：最大 30MB
- 不能与 audio_url 同时使用
参考时长：单个视频最大 5s，两个视频各自最大 2.5s，三个视频按比例更少。
计费：基于实际使用的参考时长。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不作约束：Comfy Router 从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时必须提供它，可接受拼写的枚举定义在该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频时长与输出视频时长之和不得超过
  30 秒；-1 启用智能时长模式，由模型自行选择合适的时长
...
由 Router 在 `GET /v2/models/wan/happyhorse-1.0-t2v/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误码，在信封的根层级返回，而非 `output` 之下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，在信封的根层级返回，而非 `output` 之下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此项。
</ResponseField>
