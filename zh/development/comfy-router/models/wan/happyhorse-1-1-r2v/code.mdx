...
<ParamField body="input.audio_url" type="string">
  音频文件下载 URL。支持格式：mp3 和 wav。不能与 reference_video_urls 一起使用。
</ParamField>

<ParamField body="input.img_url" type="string">
  首帧图像 URL 或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型必填。happyhorse-1.x i2v
的拼写形式不在此处接收首帧：它们将首帧作为类型为 `first_frame` 的 `media`
元素接收（见下文 `media`），而在 wan2.6-i2v 上能够成功的 `img_url`
请求体，会被提供商在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上拒绝。
...
上面每个资产的 “max 20MB” 数字是合作伙伴对其最终得到的图像的上限，当资产是
公开 URL 时按原文适用，因为这些字节从不经过 Comfy 传输。内联 data: URL 确实
会经过 Comfy 传输，并会先触及更低的传输上限：Comfy Router 的 POST 请求体
总计上限为 20 MiB，超过则返回 413，而 base64 会使负载膨胀约 4/3，因此单个
内联资产超过约 15 MB 就会在这里的任何合作伙伴规则被触及之前被拒绝；
当请求体携带多个此类资产时甚至会更早，因为 20 MiB 的 Router 上限约束的是
整个请求，而不是每个元素。任何接近这些上限的内容，请以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开 HTTP/HTTPS URL、OSS 临时 URL，或者（当上述 `media` 描述中该模型的条目说明允许时，正如 happyhorse-1.x i2v 和 r2v 拼写形式那样）内联的 `data:{MIME_type};base64,...` URL。有关各模型的大小和像素下限，以及在任何合作伙伴规则适用之前约束内联负载的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带有多个参考视频的 wan2.6-r2v，请使用 'character1'、'character2' 等按参考
视频的顺序指代主体。示例：“Character1 sings on the roadside, Character2 dances beside it”
...
<ParamField body="input.reference_video_urls" type="string[]">
  仅适用于 wan2.6-r2v 模型的参考视频 URL。1-3 个视频 URL 的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频时长：2-30 秒
...
<ParamField body="model" type="string">
  要调用的模型 ID。在此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时必须提供它，而可接受的拼写枚举位于该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
对于 wan2.7 模型，若未提供该参数，则根据分辨率档位使用默认值。
...
对于 wan2.5 T2V：480P（480*832、832*480、624*624）、720P、1080P 规格
...
根据 Router 在 `GET /v2/models/wan/happyhorse-1.1-r2v/openapi.json` 提供的 schema 生成，该文档也是它在请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应包络的 ROOT 层级，而非 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应包络的 ROOT 层级，而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前，请先查看此项。
</ResponseField>
