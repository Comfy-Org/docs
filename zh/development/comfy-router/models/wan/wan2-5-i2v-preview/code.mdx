...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送至 `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview/requests`。Router 在运行被接纳后立即返回 `201` 和一个 `request_id`，结果就绪后再进行收集，可以由本进程或另一个进程完成。[排队投递](/zh/development/comfy-router/queue) 详细介绍了状态、取消和收集。
