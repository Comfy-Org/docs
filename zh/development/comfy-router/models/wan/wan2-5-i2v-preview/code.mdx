...
- wan3.0-video：first_frame（最大 1 个）、last_frame（最大 1 个）、reference_image（最大 10 个）、
  reference_video（最大 5 段，总时长 \<= 15s）、reference_audio（最大 5 段，
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填入它，因此 Router 调用方可省略该字段。直接以 v1 方式调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供该字段，可接受拼写的枚举定义在该操作自有的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
对于 wan2.7 模型，如果未提供，则根据分辨率档位使用默认值。
对于 wan3.0-video，adaptive（默认）会根据输入媒体的
比例和意图自动推荐合适的比例。
...
由 Router 在 `GET /v2/models/wan/wan2.5-i2v-preview/openapi.json` 提供的 schema 生成，这也是请求到达提供商之前用于校验调用的同一份文档。
### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误代码，返回在响应封套的根层级，而不是 `output` 之下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，返回在响应封套的根层级，而不是 `output` 之下（请求成功时不返回）。在改用 `output.message` 之前请先阅读此项。
</ResponseField>
