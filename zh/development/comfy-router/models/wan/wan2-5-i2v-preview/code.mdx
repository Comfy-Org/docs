...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送至 `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview/requests`。Router 在运行被接纳后立即返回 `201` 和一个 `request_id`，结果就绪后再进行收集，可以由本进程或另一个进程完成。[队列投递](/zh/development/comfy-router/queue) 详细介绍了状态、取消和收集。
...
<ParamField body="input.img_url" type="string">
  首帧图像网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要此项。happyhorse-1.x i2v 系列写法
不在这里传入首帧：它们把首帧作为类型为 `first_frame` 的 `media` 元素传入
（见下文 `media`），而在 wan2.6-i2v 上能成功的 `img_url` 请求体，在
...
<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。用于指定视频生成的参考素材
（图像、音频、视频）。每个元素包含一个 type 字段和一个 url 字段。
支持的 type 取值因模型而异：
...
- wan3.0-video：first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  与 file 同时使用）。reference_*/file/link 类型与 first_frame/last_frame 类型在
  同一请求中互斥。数组顺序定义了提示词中资产的参考顺序（Image 1、Video 1、Audio 1……）。
...
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少 400 像素，最大 20MB，
  可以是公开网址或 data: 网址。reference_video 不是此操作的输入类型。
- happyhorse-1.x-video-edit：video
上述每项资产的“最大 20MB”数值是合作伙伴对其最终拿到的图像设定的上限；当资产是
公开网址时，这些数值按字面生效，因为这些字节永远不会经过 Comfy 传输。内联的
data: 网址确实会经过 Comfy 传输，并会先触及更低的传输上限：Comfy Router 的 POST
请求体总大小上限为 20 MiB，超过则返回 413；而 base64 会使载荷膨胀约 4/3，因此单个
内联资产只要超过大约 15 MB，就会在触及这里任何合作伙伴规则之前被拒绝；当请求体中
包含多个内联资产时会更快触发，因为 20 MiB 的 Router 上限约束的是整个请求，而不是
每个元素。凡接近这些上限的内容，请以公开网址方式发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公开 HTTP/HTTPS 网址、OSS 临时网址，或者在上述 `media` 描述中该模型的条目允许时（如 happyhorse-1.x i2v 和 r2v 写法所示），使用内联的 `data:{MIME_type};base64,...` 网址。有关各模型的大小与像素下限，以及在任何合作伙伴规则生效之前约束内联载荷的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带有多个参考视频的 wan2.6-r2v，请使用 'character1'、'character2' 等按参考视频的
顺序指代主体。示例：“Character1 在路边唱歌，Character2 在旁边跳舞”
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等按 media 数组中
对应的顺序指代媒体资产。
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  仅 wan2.6-r2v 模型的参考视频网址。包含 1-3 个视频网址的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频时长：2-30 秒
- 单个文件大小：最大 30MB
- 不能与 audio_url 同时使用
参考时长：单视频最大 5s，两个视频每个最大 2.5s，三个视频相应更短。
计费：按实际使用的参考时长计费。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不作约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方可以省略。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起 v1 调用时则必须提供，可接受写法的枚举定义在该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 区间内的整数；有视频输入时，输入视频
  总时长与输出视频时长之和不得超过 30 秒；-1 启用智能时长模式，由模型
  自动选择合适的时长
...
对于 wan3.0-video，自适应（默认）会根据输入媒体的比例和意图自动推荐
合适的宽高比。
...
根据 Router 在 `GET /v2/models/wan/wan2.5-i2v-preview/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="usage.duration" type="number">
  已生成视频的时长，以秒为单位（I2V 和 wan3.0-video 任务）
</ResponseField>
...
<ResponseField name="message" type="string">
  失败请求的详细信息，报告在响应信封的根层级（ROOT），而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先查看此项。
</ResponseField>
