...
<ParamField body="input.media" type="object[]">
  用于 wan2.7、wan3.0 和 happyhorse-1.x 模型的媒体资产列表。用于为视频生成指定参考素材（图像、音频、视频）。每个元素包含 type 和 url 两个字段。
支持的 type 值因模型而异：
...
对于包含多个参考视频的 wan2.6-r2v，请按照参考视频的顺序使用 'character1'、'character2' 等来
...
<ParamField body="input.reference_video_urls" type="string[]">
  仅用于 wan2.6-r2v 模型的参考视频 URL。1-3 个视频 URL 组成的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频长度：2-30 秒
- 单个文件大小：最大 30MB
- 不能与 audio_url 一起使用
参考时长：单个视频最大 5s，两个视频各最大 2.5s，三个视频则按比例更短。
计费：基于实际使用的参考时长。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不强制要求：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径片段中填充它，因此 Router 调用方可以省略。直接以 v1 方式调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时则必须提供，可接受的拼写枚举定义在该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频
  总时长 + 输出视频时长不得超过 30 秒；-1 会启用智能时长模式，
  由模型选择合适的时长
...
对于 wan3.0-video，adaptive（默认）会根据输入媒体的比例和意图自动推荐合适的
宽高比。
...
由 Router 在 `GET /v2/models/wan/wan2.7-t2v/openapi.json` 提供的 schema 生成，该文档与它在请求到达提供商之前用于校验调用的文档相同。
### 输出
...
<ResponseField name="code" type="string">
  请求失败时的错误码，报告在信封的根层级，而不是在 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  请求失败的详细信息，报告在信封的根层级，而不是在 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
