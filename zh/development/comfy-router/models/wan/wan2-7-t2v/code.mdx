---
title: "使用 Comfy Router 调用 Wan 2.7 T2V"
description: "通过 Comfy Router 以 HTTP 方式调用 wan/wan2.7-t2v 的 Python、TypeScript 和 cURL 代码片段，以及请求字段和结果结构"
...
Wan 2.7 T2V 的 API 参考。Wan 2.7 文生视频仅凭一段提示词即可生成短视频，支持 720P 或 1080P，时长 2 到 15 秒。
...
**模型 ID：** `wan/wan2.7-t2v`

**端点：** `POST https://api.comfy.org/v2/models/wan/wan2.7-t2v`
...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/wan/wan2.7-t2v/requests`。一旦运行被接纳，Router 便会返回 `201` 和 `request_id`，运行就绪后即可收集结果，可以来自当前进程，也可以来自另一个进程。[队列投递](/zh/development/comfy-router/queue)完整介绍了状态、取消与收集的流程。
...
print("requestId:", handle.requestId)  // 连同模型 ID，这就是另一个进程所需的全部信息
...
# 2. 轮询直到状态为 COMPLETED，等待每个响应指定的 Retry-After 秒数。
...
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要此参数。happyhorse-1.x 的
i2v 写法不在此处传入首帧：它们将首帧作为类型为 `first_frame` 的 `media`
...
上文按资产给出的“最大 20MB”数值，是合作伙伴对其最终得到的图像所设的
上限；当资产为公开 URL 时，这些限制按字面生效，因为那些字节
不会经过 Comfy 传输。内联 data: URL 确实会经过
Comfy 传输，因此同样会触及传输上限：Comfy Router 的 POST 请求体
总计上限为 100 MiB，超过即返回 413；而 base64 会使载荷膨胀
约 4/3，因此单个内联资产只要超过大约 75 MB，就会在
触达此处任何合作伙伴规则之前被拒绝。在该规模下，单个
资产即便达到合作伙伴自身的 20MB 上限，也能轻松内联容纳，因此对单个资产来说
起作用的是合作伙伴规则；而对多个资产来说起作用的是传输上限，因为
100 MiB 的 Router 上限约束的是整个请求，而非每个元素。任何
接近这些上限的内容，都应以公开 URL 方式发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：可以是公开的 HTTP/HTTPS URL、OSS 临时 URL，或者（在上文 `media` 描述中该模型对应的条目允许时，例如 happyhorse-1.x i2v 和 r2v 写法）使用内联的 `data:{MIME_type};base64,...` URL。有关各模型的大小与像素下限，以及约束内联载荷总量的 100 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等，按 media 数组中的对应顺序
指代媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。在此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方会省略该参数。直接对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起的 v1 调用则必须提供该参数，可接受的拼写枚举位于该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频
  时长与输出视频时长之和不得超过 30 秒；-1 表示启用智能时长模式，
  由模型自行选择合适的时长
...
由于分辨率档位决定计费标准，该代理会拒绝既未提供 resolution
也未提供 size 的视频生成请求。
...
<ParamField body="parameters.size" type="string">
  视频分辨率，格式为 宽度*高度。不同模型支持的分辨率有所不同：
...
由 Router 在 `GET /v2/models/wan/wan2.7-t2v/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前 Router 用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，报告在响应信封的根层级而非 `output` 下（请求成功时不返回）。
</ResponseField>
