...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/wan/wan2.7-t2v/requests`。一旦运行被接纳，Router 便会返回 `201` 和 `request_id`，运行就绪后即可收集结果，可以来自当前进程，也可以来自另一个进程。[排队投递](/zh/development/comfy-router/queue)完整介绍了状态、取消与收集的流程。
...
<ParamField body="input.img_url" type="string">
  首帧图片网址或 Base64 编码数据。
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要。happyhorse-1.x 的 i2v
写法不在此处传入首帧：它们将首帧作为类型为 `first_frame` 的 `media`
元素传入（见下方 `media`），而在 wan2.6-i2v 上能成功的 `img_url`
...
上述每个资产的“最大 20MB”数值是合作伙伴对其最终拿到的图像所设的上限，
当资产是公开 URL 时按原文适用，因为这些字节从不经过 Comfy。
而内联 data: URL 会经过 Comfy，并会先触达更低的传输上限：
Comfy Router 的 POST 请求体总计上限为 20 MiB，超过即返回 413，而 base64 会让
载荷膨胀约 4/3，因此单个内联资产若超过约 15 MB，就会在触达此处任何合作伙伴
规则之前被拒绝；若请求体携带多个此类资产，则会更早被拒绝，因为 20 MiB 的
Router 上限约束的是整个请求，而不是每个元素。接近这些上限的内容请
以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开 HTTP/HTTPS URL、OSS 临时 URL，或（当上方 `media` 描述中该模型的条目如此说明时，如 happyhorse-1.x 的 i2v 和 r2v 写法）内联的 `data:{MIME_type};base64,...` URL。有关各模型的大小与像素下限，以及在任何合作伙伴规则生效之前约束内联载荷的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带多个参考视频的 wan2.6-r2v，按参考视频的顺序使用 'character1'、'character2' 等
指代主体。示例："Character1 在路边唱歌，Character2 在旁边跳舞"
对于 wan3.0-video 参考模式，按 media 数组中的相应顺序使用 'Image 1'、'Video 1'、
'Audio 1' 等指代媒体资产。
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  仅 wan2.6-r2v 模型使用的参考视频 URL。1-3 个视频 URL 组成的数组。
输入限制：
- 格式：mp4、mov
- 数量：1-3 个视频
- 单个视频时长：2-30 秒
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。对 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 的直接 v1 调用则必须提供它，受支持写法的枚举位于该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
对于 wan3.0-video，adaptive（默认值）会根据输入媒体的宽高比例和
意图自动推荐合适的宽高比。
...
由 Router 在 `GET /v2/models/wan/wan2.7-t2v/openapi.json` 提供的 schema 生成，该文档也是它在请求到达提供商之前校验调用时所使用的同一份文档。
...
<ResponseField name="usage.ratio" type="string">
  已生成视频的宽高比例，例如 16:9（wan3.0-video 任务）
</ResponseField>
...
<ResponseField name="code" type="string">
  失败请求的错误代码，在信封的根层级（ROOT）而非 `output` 下报告（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，在信封的根层级（ROOT）而非 `output` 下报告（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
