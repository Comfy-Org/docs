...
在[您的 Comfy 工作区](https://platform.comfy.org/profile/api-keys)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原始 HTTP 进行的相同调用。
...
i2v 拼写形式不在此处传入首帧：它们将首帧作为类型为 `first_frame`
的 `media` 元素传入（见下方 `media`），并且在 wan2.6-i2v 上成功
的 `img_url` 请求体，会被提供商在 happyhorse-1.0-i2v 和
happyhorse-1.1-i2v 上拒绝。
...
- wan3.0-video：first_frame（最多 1 个）、last_frame（最多 1 个）、reference_image（最多 10 个）、
  ...
  （Image 1、Video 1、Audio 1 等）。
- happyhorse-1.x-i2v：仅 first_frame，且恰好 1 个。至少 300x300 像素，
  JPEG/JPG/PNG/WEBP，最大 20MB，公共网址或 data:\{MIME_type};base64,... 网址。
  这些拼写形式不接受 img_url；首帧放在这里。
- happyhorse-1.x-r2v：仅 reference_image，1 到 9 个。最短边至少
  400 像素，最大 20MB，公共网址或 data: 网址。reference_video 不是
  此操作的输入类型。
- happyhorse-1.x-video-edit：video
上述每项资产“最大 20MB”的数值是合作伙伴对其最终得到的图像所设的上限，
当资产是公共网址时按字面生效，因为这些字节从不经过 Comfy。内联的
data: 网址会经过 Comfy，并会先遇到更低的传输上限：Comfy Router 的 POST 请求体
总上限为 20 MiB，超过即返回 413，而 base64 会使载荷膨胀约 4/3，因此
单个内联资产超过大约 15 MB 就会在此处任何合作伙伴规则生效之前被拒绝；
当请求体携带多个内联资产时会更早触发，因为 20 MiB 的 Router 上限约束的是
整个请求，而不是每个元素。凡是接近这些上限的内容，请以公共网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公共 HTTP/HTTPS 网址、OSS 临时网址，或者（在上方 `media` 描述中该模型的条目明确支持时，正如 happyhorse-1.x 的 i2v 和 r2v 拼写形式那样）内联的 `data:{MIME_type};base64,...` 网址。有关按模型区分的大小和像素下限，以及在任何合作伙伴规则生效之前约束内联载荷的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等来按 media 数组中的
相应顺序指代媒体资产。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不做约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段中填充它，因此 Router 调用方可以省略。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起的 v1 调用则必须提供它，可接受拼写的枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 区间内的整数；有视频输入时，
  输入视频总时长 + 输出视频时长不得超过 30 秒；-1 启用智能时长模式，
  此时由模型自行选择合适的时长
...
wan2.5 T2V：480P（480*832、832*480、624*624）、720P、1080P 各档尺寸
...
根据 Router 在 `GET /v2/models/wan/wan2.5-t2v-preview/openapi.json` 提供的 schema 生成，这也是请求到达提供商之前，Router 用于校验调用时所依据的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误代码，位于响应封装的 ROOT 层级，而不是 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，位于响应封装的 ROOT 层级，而不是 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此字段。
</ResponseField>
