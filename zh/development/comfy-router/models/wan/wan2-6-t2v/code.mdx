...
happyhorse-1.x 的 i2v 拼写不在此处传入首帧：它们将首帧作为类型为 `first_frame` 的 `media` 元素传入（见下方 `media`），
而在 wan2.6-i2v 上能够成功执行的 `img_url` 请求体，会被提供商在 happyhorse-1.0-i2v 和 happyhorse-1.1-i2v 上拒绝。
...
上述每个资产的“最大 20MB”数值是合作伙伴对其最终得到的图像设的上限，
当资产是公开网址时按字面适用，因为这些字节从不经过 Comfy。内联的 data: 网址
确实会经过 Comfy，并会先遇到更低的传输上限：Comfy Router POST 请求体
总计上限为 20 MiB，超过即返回 413，而 base64 会使载荷膨胀约 4/3，
因此单个超过约 15 MB 的内联资产会在触达此处任何合作伙伴规则之前就被拒绝，
当请求体中包含多个此类资产时会更早被拒绝，因为 20 MiB 的 Router 上限约束的是整个
请求，而不是每个元素。凡是接近这些上限的内容，请以公开网址发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的网址：公开的 HTTP/HTTPS 网址、OSS 临时网址，或在模型允许时（即上方 `media` 描述中该模型的条目写明允许，happyhorse-1.x 的 i2v 和 r2v 拼写就是如此）使用内联的 `data:{MIME_type};base64,...` 网址。有关各模型的大小与像素下限，以及在任何合作伙伴规则适用之前约束内联载荷的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
对于带有多个参考视频的 wan2.6-r2v，请使用 'character1'、'character2' 等按参考视频
的顺序指代主体。示例：“Character1 在路边唱歌，Character2 在旁边跳舞”
对于 wan3.0-video 参考模式，请使用 'Image 1'、'Video 1'、'Audio 1' 等指代
media 数组中对应顺序的媒体资产。
</ParamField>
...
- 单个文件大小：最大 30MB
- 不能与 audio_url 一起使用
参考时长：单个视频最大 5s，两个视频各最大 2.5s，三个视频则相应更短。
计费：基于实际使用的参考时长。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不受约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接向 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 发起的 v1 调用则必须提供它，而可接受拼写的枚举定义在该操作自己的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频总时长与输出视频时长之和不得超过 30 秒；-1 启用智能时长模式，由模型自行选择合适的时长
...
对于 wan3.0-video，adaptive（默认值）会根据输入媒体的比例和意图自动推荐合适的画面比例。
...
根据 Router 在 `GET /v2/models/wan/wan2.6-t2v/openapi.json` 上提供的 schema 生成，这也正是它在请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="message" type="string">
  失败请求的详细信息，报告在信封的根层级而非 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先读取此字段。
</ResponseField>
