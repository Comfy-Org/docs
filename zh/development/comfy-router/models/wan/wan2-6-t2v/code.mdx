...
- wan3.0-video：first_frame（最多 1）、last_frame（最多 1）、reference_image（最多 10）、
  reference_video（最多 5 个片段，总时长 \<= 15s）、reference_audio（最多 5 个片段，
  总时长 \<= 15s）、file（最多 1，不能与 link 同时使用）、link（最多 1，不能
  与 file 同时使用）。在同一请求中，reference_*/file/link 类型与 first_frame/last_frame 类型
  互斥。数组的顺序决定了提示词中资产的引用顺序（Image 1、Video 1、Audio 1……）。
...
例如："Character1 sings on the roadside, Character2 dances beside it"
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件不对其进行约束：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方会省略它。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时必须提供它，可接受的拼写枚举位于该操作自身的组件 `WanVideoGenerationRequest` 中。
</ParamField>
...
- wan3.0-video：无视频输入时为 [2, 30] 范围内的整数；有视频输入时，输入视频
  总时长 + 输出视频时长不得超过 30 秒；-1 启用智能时长模式，由模型选择合适的时长
...
对于 wan2.7 模型，如果未提供，则根据分辨率档位给出默认值。
...
由 Router 在 `GET /v2/models/wan/wan2.6-t2v/openapi.json` 提供的 schema 生成，Router 在请求到达提供商之前，会用同一份文档校验调用。
### 输出
...
<ResponseField name="code" type="string">
  失败请求的错误代码，位于响应包的根层级而不是 `output` 下（请求成功时不返回）。
</ResponseField>

<ResponseField name="message" type="string">
  失败请求的详细信息，位于响应包的根层级而不是 `output` 下（请求成功时不返回）。在回退到 `output.message` 之前请先阅读此项。
</ResponseField>
