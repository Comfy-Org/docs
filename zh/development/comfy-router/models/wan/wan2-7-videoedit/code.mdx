...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 代码片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 代码片段则是通过原生 HTTP 发起的同一个调用。
...
仅 wan2.5-i2v-preview 和 wan2.6-i2v 模型需要。happyhorse-1.x
的 i2v 写法不在这里接收首帧：它们把首帧作为类型为 `first_frame` 的 `media`
元素接收（见下方的 `media`），而在 wan2.6-i2v 上能成功的 `img_url`
...
素材（图像、音频、视频）。每个元素包含一个 type 字段和一个 url 字段。
支持的 type 取值因模型而异：
...
上面每个资产“最大 20MB”的数值是合作伙伴对其最终得到的图像所设的上限，当资产是
公开 URL 时按字面适用，因为这些字节从不经过 Comfy。内联的 data: URL 确实会经过
Comfy，并且会先遇到更低的传输上限：Comfy Router 的 POST 请求体总计上限为 20 MiB，
超过则返回 413，而 base64 会使载荷膨胀约 4/3，因此单个内联资产若超过大约 15 MB，
在触及此处任何合作伙伴规则之前就会被拒绝；当请求体携带多个此类资产时会更早触发，
因为 20 MiB 的 Router 上限约束的是整个请求，而不是每个元素。接近这些上限的内容请
以公开 URL 发送。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  媒体文件的 URL：公开 HTTP/HTTPS URL、OSS 临时 URL，或（当上文中 `media` 描述里对应模型的条目如此说明时，正如 happyhorse-1.x 的 i2v 和 r2v 写法那样）内联的 `data:{MIME_type};base64,...` URL。关于各模型的大小与像素下限，以及在任何合作伙伴规则生效之前约束内联载荷的 20 MiB Router 请求体上限，请参阅该描述。
</ParamField>
...
<ParamField body="model" type="string">
  要调用的模型 ID。此组件上不做限制：Comfy Router 会从 `POST /v2/models/wan/{model}` 的 `{model}` 路径段填充它，因此 Router 调用方可以省略。直接以 v1 调用 `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` 时则必须提供它，可接受写法的枚举位于该操作自身的组件 `WanVideoGenerationRequest` 上。
</ParamField>
...
由 Router 在 `GET /v2/models/wan/wan2.7-videoedit/openapi.json` 提供的 schema 生成，这也是它在请求到达提供商之前用于校验调用的同一份文档。
...
<ResponseField name="code" type="string">
  失败请求的错误码，报告在响应封装的 ROOT 层级，而不是 `output` 下（请求成功时不返回）。
</ResponseField>
