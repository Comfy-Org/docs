...
```
  ...
  <Tab title="排队并稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/veo/veo-2.0-generate-001/requests`。运行一经接纳，Router 就会立即返回 `201` 与 `request_id`；结果准备就绪后，即可从当前进程或另一个进程收集。[队列投递](/zh/development/comfy-router/queue)会详细介绍状态、取消和收集的流程。
