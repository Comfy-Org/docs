...
```
  ...
  <Tab title="先排队，稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/veo/veo-3.0-fast-generate-001/requests`。只要运行被受理，Router 就会立即以 `201` 和 `request_id` 应答；结果准备就绪后即可收集，无论收集方是当前进程还是其他进程。[队列投递](/zh/development/comfy-router/queue) 详细讲解了状态查询、取消与结果收集。
...
```bash cURL
# 1. 提交。Router 以 201 应答，并返回 request_id、status_url、response_url 和 cancel_url。
...
# 3. 收集结果。返回 200 时附带模型的原始输出；仍在运行时返回 202 和状态响应体。
