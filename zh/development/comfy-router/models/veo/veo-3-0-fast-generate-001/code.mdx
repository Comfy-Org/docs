---
title: "将 Veo 3.0 Fast Generate 001 与 Comfy Router 配合使用"
description: "通过 Comfy Router 调用 veo/veo-3.0-fast-generate-001：端点、请求结构以及 Router 返回的响应。"
...
{/* 生成文件。由 `pnpm code-pages:gen` 从 router-schemas/veo/veo-3.0-fast-generate-001.json 生成。 */}

import RouterCodeFooter from "/snippets/zh/comfy-router/model-code-footer.mdx";

`veo/veo-3.0-fast-generate-001` 的 API 参考，该模型由 Comfy Router 提供，来自 Veo。
...
**模型 ID：** `veo/veo-3.0-fast-generate-001`

**端点：** `POST https://api.comfy.org/v2/models/veo/veo-3.0-fast-generate-001`
...
```
  ...
  <Tab title="先排队，稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/veo/veo-3.0-fast-generate-001/requests`。只要运行被受理，Router 就会以 `201` 和 `request_id` 立即应答；结果准备就绪后即可收集，无论收集方是当前进程还是其他进程。[队列投递](/zh/development/comfy-router/queue) 详细讲解了状态查询、取消与结果收集。
...
<ParamField body="instances[].lastFrame" type="object">
  可选的尾帧图像。与 image 配合使用，可在首帧和尾帧之间生成视频。受 Veo 3.0+ 模型支持。
</ParamField>
...
<ParamField body="instances[].referenceImages" type="object[]">
  用于引导视频生成的可选参考图像。最多支持 3 张素材图像或 1 张风格图像。受 Veo 3.1 模型支持（预览）。
</ParamField>
...
<ParamField body="parameters.seed" type="integer">
  用于确定性输出的随机种子。如果 sampleCount > 1，每个视频使用不同的种子。

  格式：`uint32`
</ParamField>

<ParamField body="parameters.storageUri" type="string">
  用于保存已生成视频的云端存储 URI (gs://)
</ParamField>
...
本文档根据 Router 在 `GET /v2/models/veo/veo-3.0-fast-generate-001/openapi.json` 提供的 schema 生成，该文档也是请求到达提供商之前 Router 用于校验调用的同一份文档。
### 输出
