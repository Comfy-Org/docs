...
```
  ...
  <Tab title="先排队，稍后收集">
相同的请求体，发送到 `POST https://api.comfy.org/v2/models/veo/veo-3.0-fast-generate-001/requests`。只要运行被受理，Router 就会以 `201` 和 `request_id` 立即应答；结果准备就绪后即可收集，无论收集方是当前进程还是其他进程。[排队交付](/zh/development/comfy-router/queue) 详细讲解了状态查询、取消与结果收集。
