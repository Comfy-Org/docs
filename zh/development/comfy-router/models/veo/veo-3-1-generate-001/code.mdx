...
在[你的 Comfy 工作区](https://platform.comfy.org/profile/api-keys?onboarding=router)中创建密钥，并将其导出为 `COMFY_API_KEY`。Python、TypeScript 和 Swift 片段使用 Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`，以及 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 包）；cURL 片段则是通过原始 HTTP 发起同样的调用。
...
# 3. 收集。返回 200 表示模型的原始输出，仍在运行时返回 202 和状态正文。
...
<ParamField body="parameters.generateAudio" type="boolean">
  是否随视频一起生成音频。默认为 true。由 Veo 3.0+ 模型支持。
</ParamField>
