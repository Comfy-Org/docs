...
Comfy API 密钥也可以作为 Bearer token 发送。当同时提供两个凭证请求头时，以 `X-API-Key` 为准。有关 API 密钥与 JWT 的区别，请参阅[身份验证请求头](/zh/development/comfy-router/headers#请求标头)；有关访问要求，请参阅[快速入门](/zh/development/comfy-router/quickstart)。
...
<ResponseField name="200" type="RouterModelDetail">
  ...
  正文：[`RouterModelDetail`](#routermodeldetail) -- 请求头：`X-Comfy-Request-Id`
</ResponseField>

<ResponseField name="401" type="RouterErrorResponse">
  凭据缺失或无效。

  正文：[`RouterErrorResponse`](#routererrorresponse) -- 请求头：`X-Comfy-Error-Type`、`X-Comfy-Request-Id`
</ResponseField>
...
<ParamField path="provider" type="RouterProviderSegment" required>
  ...
  类型：[`RouterProviderSegment`](#routerprovidersegment) -- 字母数字短标识，例如 `anthropic`，最多 64 个字符
</ParamField>

<ParamField path="model" type="RouterModelSegment" required>
  规范 `{provider}/{model}` 模型 ID 中的模型部分。

  类型：[`RouterModelSegment`](#routermodelsegment) -- 字母数字短标识，例如 `claude-opus-4-6`，最多 128 个字符
</ParamField>

<ParamField header="Idempotency-Key" type="string">
  调用方生成的密钥，使重试同一次逻辑调用变得安全。

  1–255 个字符
</ParamField>

<ParamField query="model_provider" type="string">
  选择另一个提供商来提供此模型，而不是其当前的默认提供商。
</ParamField>
...
<ParamField query="fallback_provider" type="string">
  控制当首次尝试因可归因于 Router 自身一侧或所尝试的特定提供商的原因而失败时，Router 是否针对该模型的其他已注册提供商重试此调用；绝不会因可归因于请求本身的原因而重试（未重试的失败会一如既往地被拒绝）。
</ParamField>
...
合作伙伴模型的原生 JSON 输入。如果没有 `model_provider`，请求会在模型的默认提供商上执行原生分发，此时该正文就是模型自身的原生 schema，原样转发（`strict_mode` 在此毫无意义，不会产生任何改变）。如果 `model_provider` 选择了另一个提供商且 `strict_mode=false`（默认值），则在发送前会将正文转换为该提供商真实的 schema：任何无法精确表达的原生字段都会被丢弃，并通过响应的 `X-Comfy-Router-Dropped-Params` 响应头公开披露，绝不会静默丢弃。如果 `model_provider` 选择了另一个提供商且 `strict_mode=true`，则不会执行任何转换：正文必须已经是该另一个提供商自身的真实 schema，而不是此模型的原生 schema（参见 `strict_mode`），并会被原样转发。正文自身的字段还会决定 Router 在支持多种操作的模型上执行哪种操作：对于可编辑的图像模型，包含输入图像会将其从文生图切换为图生图（编辑）操作；对于 Seedance 视频模型，首帧图像会选择图生视频，而参考图像或片段则选择参考生视频。每个带条件的操作都按其自身费率计费，而不是按基础的文生图或文生视频费率计费。

**响应**

<ResponseField name="200" type="RouterModelOutput">
  OK - 在没有 `model_provider` 时，或使用 `model_provider` 且 `strict_mode=false`（默认值，在可能的情况下会转换回此模型的原生契约；若转换失败则回退到另一个提供商自身的原始响应，并记录日志，绝不静默）时，其形状为此模型自身的原生输出；当 `strict_mode=true` 时，则是另一个提供商的响应原样返回。

  正文：[`RouterModelOutput`](#routermodeloutput) -- 响应头：`X-Comfy-Request-Id`、`X-Content-Type-Options`、`X-Comfy-Router-Fallback-Provider`、`X-Comfy-Router-Dropped-Params`、`X-Comfy-Credits-Used`、`Idempotent-Replayed`、`X-Committed-Spend-Limit`、`X-Committed-Spend-Current`、`X-Committed-Spend-Remaining`
</ResponseField>

<ResponseField name="400" type="RouterErrorResponse">
  请求无效。请检查错误类型和请求正文。

  正文：[`RouterErrorResponse`](#routererrorresponse) -- 响应头：`X-Comfy-Error-Type`、`X-Comfy-Request-Id`、`X-Comfy-Upstream-Status`、`X-Comfy-Upstream-Detail`、`Idempotent-Replayed`
</ResponseField>
...
<ResponseField name="409" type="RouterErrorResponse">
  检查 `X-Comfy-Error-Type`：`concurrency_limit_exceeded` 表示原始调用仍在运行，因此请等待 `Retry-After` 并复用同一个密钥；`invalid_input` 则需要使用新密钥。

  正文：[`RouterErrorResponse`](#routererrorresponse) -- 响应头：`X-Comfy-Error-Type`、`X-Comfy-Request-Id`、`Retry-After`（当出现 `concurrency_limit_exceeded` 时）
</ResponseField>
...
<ResponseField name="422" type="RouterValidationErrorResponse">
  ...
  正文：[`RouterValidationErrorResponse`](#routervalidationerrorresponse) -- 响应头：`X-Comfy-Error-Type`、`X-Comfy-Request-Id`、`Idempotent-Replayed`
</ResponseField>
...
<ResponseField name="502" type="RouterErrorResponse">
  提供商自身的响应无法转换为结果（`provider_error`）。
...
<ResponseField name="504" type="RouterErrorResponse">
  请求超过了截止时间。重试之前请先检查错误类型。

  响应体：[`RouterErrorResponse`](#routererrorresponse) -- 响应头：`X-Comfy-Error-Type`、`X-Comfy-Request-Id`、`X-Comfy-Upstream-Status`、`Retry-After`
</ResponseField>

### `GET /v2/models/{provider}/{model}/openapi.json`

**以 OpenAPI 文档形式读取一个合作伙伴模型的输入和输出 schema。**
...
<ParamField path="provider" type="RouterProviderSegment" required>
  规范 `{provider}/{model}` 模型 ID 中表示提供商的部分。

  类型：[`RouterProviderSegment`](#routerprovidersegment) -- 字母数字 slug，例如 `anthropic`，最多 64 个字符
</ParamField>

<ParamField path="model" type="RouterModelSegment" required>
  规范 `{provider}/{model}` 模型 ID 中表示模型的部分。
...
<ResponseField name="200" type="RouterModelInputSchemaDocument">
  OK - 该模型的输入和输出 schema，作为独立的 OpenAPI 文档。
...
<ResponseField name="304" type="no body">
  未修改 - 自调用方在 `If-None-Match` 中发送的 `ETag` 以来，文档未发生更改。
...
**将合作伙伴模型的一次运行提交到队列并立即返回。**

Comfy Router 的队列投递模式。请求体与该模型的 `POST /v2/models/{provider}/{model}` 所接受的合作伙伴原生 JSON 输入完全相同（一种请求体形状、一套按模型定义的 schema、两种投递模式），但此路由不会为了等待结果而保持连接。它会接收该次运行，返回 `201` 和一个句柄，调用方随后可通过下文三个读取接口获取结果。

**参数**

<ParamField path="provider" type="RouterProviderSegment" required>
  规范 `{provider}/{model}` 模型 ID 中的小写提供商片段：即所运行模型所属的合作伙伴。

  类型：[`RouterProviderSegment`](#routerprovidersegment)：字母数字 slug，例如 `anthropic`，最多 64 个字符
</ParamField>

<ParamField path="model" type="RouterModelSegment" required>
  规范 `{provider}/{model}` 模型 ID 中的小写模型片段：即在该提供商下要运行的模型。

  类型：[`RouterModelSegment`](#routermodelsegment)：字母数字 slug，例如 `claude-opus-4-6`，最多 128 个字符
</ParamField>

<ParamField query="model_provider" type="string">
  选择由另一个提供商来服务此模型，而不是使用其当前的默认提供商。
</ParamField>

<ParamField query="strict_mode" type="boolean">
  仅在配合 `model_provider` 使用时才有意义。
...
合作伙伴模型的原生 JSON 输入，与此模型的同步路由所接受的请求体完全相同；它同样决定运行哪种操作，也以相同方式计费：在支持多种操作的模型上，由请求体自身的字段选择 Router 运行哪种操作（输入图像会把可编辑图像模型切换为图生图；Seedance 的首帧图像会选择图生视频，参考图像或视频片段则选择参考生视频），并且每个带条件的操作都按其自身费率计费，而不是基础文生图或文生视频费率。分发时适用相同的提供商选择约定：未提供 `model_provider`，或提供了 `model_provider` 且 `strict_mode=false`（默认值）时，请求体就是该模型的原生文档，并会在运行被接收之前依据模型自身的输入 schema 进行验证，因此模型本会拒绝的请求体在这里会返回 `422`，而不会变成一个几分钟后才失败的排队请求；非严格模式下的替代提供商请求体在分发时还会被转换为该提供商的真实 schema。使用 `strict_mode=true` 时，请求体必须已经是替代提供商自己的 schema，并会原样转发：原生 schema 验证会被跳过，与同步路由完全一致（参见 `model_provider` 和 `strict_mode`）。
...
<ResponseField name="402" type="RouterErrorResponse">
  Router 请求级失败：请求从未到达模型，或因模型本身未报告的原因而失败。
...
<ResponseField name="409" type="RouterErrorResponse">
  请检查 `X-Comfy-Error-Type`：`concurrency_limit_exceeded` 表示原始调用仍在运行，因此请等待 `Retry-After` 并复用同一个键；`invalid_input` 则需要使用新键。
...
收集端点。对于已成功完成的请求，它会返回合作伙伴模型自身的原生输出，与同步路由针对相同模型和相同输入在 `200` 中返回的内容逐字节一致，因此两种投递模式产生同一种结果形状，调用方无需第二个解析器即可在两者之间切换。
...
<ParamField path="request_id" type="RouterQueueRequestId" required>
  要查询的排队请求，即提交时在响应体中返回的 `request_id`。
...
<ResponseField name="200" type="RouterModelOutput">
  OK：对于产生了输出的请求（即一个已成功完成的请求，或一个同时带有已记录费用和已存储结果的终端请求），返回合作伙伴模型的原生输出，在合作伙伴自身的媒体类型下原样返回，与同步路由的 `200` 返回方式完全一致。
...
<ResponseField name="409" type="RouterErrorResponse">
  请求处于与该操作冲突的状态。请检查错误类型。

  响应体：[`RouterErrorResponse`](#routererrorresponse) -- 响应头：`X-Comfy-Error-Type`, `X-Comfy-Request-Id`
</ResponseField>

<ResponseField name="504" type="RouterErrorResponse">
  请求超过了截止时间。重试之前请检查错误类型。
...
请求 Comfy 停止一个尚未完成的请求。这只是一个请求，而不是保证，`202` 正说明了这一点：`CANCELLATION_REQUESTED` 表示该请求已被接受，并不表示任务已经停止。已经在合作伙伴侧开始传输的生成任务仍可能照常完成；而合作伙伴完成的生成任务都会被计费，无论是否有人取走结果。因此，需要知道实际发生了什么情况的调用方，应在之后读取状态端点：在那里，已经生效的取消会显示为 `COMPLETED`，并像其他所有终端结果一样携带 `error_type`。
...
<ParamField path="model" type="RouterModelSegment" required>
  规范化 `{provider}/{model}` 模型 ID 中的小写模型段：即在该提供商下要运行的模型。

  类型：[`RouterModelSegment`](#routermodelsegment) -- 字母数字 slug，例如 `claude-opus-4-6`，最多 128 个字符
</ParamField>

<ParamField path="request_id" type="RouterQueueRequestId" required>
  要处理的已排队请求：提交时在响应体中返回的 `request_id`。
...
<ResponseField name="202" type="RouterQueueCancelResponse">
  ...
  响应体：[`RouterQueueCancelResponse`](#routerqueuecancelresponse) -- 请求头：`X-Comfy-Request-Id`
</ResponseField>
...
这是轮询端点。它只返回请求的当前状态，从不返回结果，因此客户端可以在长时间生成过程中持续观察，而不必在每次轮询时传输输出。当此端点返回 `COMPLETED` 时，再从下方的读取接口一次性收集结果。
...
<ParamField path="request_id" type="RouterQueueRequestId" required>
  要定位的排队请求 - 即提交时在响应体中返回的 `request_id`。
...
此处的描述较为简短。模型选择、验证、重试和计费请参阅[使用 Comfy Router API](/zh/development/comfy-router/api)，响应头行为请参阅[响应头](/zh/development/comfy-router/headers)。
...
| `queue_backlog_full` | 调用方已有过多等待运行的排队请求，因此此次提交被拒绝。 |
...
<ResponseField name="error_type" type="RouterErrorType" required>
  Router 失败的粗粒度、机器可读分类，同时会镜像到 `X-Comfy-Error-Type` 响应头中，以便调用方无需解析响应体即可分支处理。该集合封闭为十九个值：六个请求级分类 `invalid_input`、`content_policy_violation`、`provider_error`、`provider_timeout`、`insufficient_credits` 和 `model_not_found`，以及传输级分类 `unauthorized`、`forbidden`、`concurrency_limit_exceeded`、`client_disconnected`、`internal_error`、`deadline_exceeded`、`not_enabled`、`service_unavailable`、`rate_limited`、`cancelled`、`queue_timeout`、`request_not_found` 和 `queue_backlog_full`。这里的“封闭”描述的是当前文档所记录的集合，而不是一个永远成立的边界：该集合预计还会增长，这正是它刻意使用普通字符串而非 `enum` 的原因，因此客户端必须把无法识别的值当作 `internal_error`，而不是对上面的列表做穷举分支，从而在下一次新增时出错。

  类型：[`RouterErrorType`](#routererrortype)
</ResponseField>

<ResponseField name="upstream_detail" type="string">
  模型提供商给出的拒绝请求的有界、已脱敏的原因，仅当 `error_type` 为 `invalid_input` 且 `X-Comfy-Upstream-Status` 为提供商的 `4xx` 时才会出现，也就是说，提供商认为该请求格式有误并说明了原因。在其他任何失败情形下都不存在，包括提供商 `5xx`、传输失败、内容策略拒绝以及 Router 针对自身提出的任何拒绝。它会镜像 `X-Comfy-Upstream-Detail` 响应头。
</ResponseField>
...
<ResponseField name="charges_on_policy_rejection" type="RouterChargesOnPolicyRejection" required>
  模型因内容政策原因而拒绝的调用，是否仍会向调用方收费。各提供商的做法不同，这种差异在调用时不可见，而用户即使同时看到错误和同一调用被收费，也无从事先得知。因此这里在调用之前按模型明确说明，而不是留给各提供商的“民间说法”。
...
<ResponseField name="id" type="RouterModelId" required>
  规范的 Comfy Router 模型 ID，格式为 `{provider}/{model}`，正是在 `POST /v2/models/{provider}/{model}` 上寻址该模型所用的值，因此调用方可以直接将它插值进该路径，而无需从其他任何来源重新推导。其 `pattern` 由 `RouterProviderSegment` 与 `RouterModelSegment` 以单个 `/` 连接而成，`maxLength` 为两者长度之和再加上该分隔符。
...
<ResponseField name="provider" type="RouterProviderSegment" required>
  规范 `{provider}/{model}` 模型 ID 中的小写 `provider` 段，即被寻址模型所属的合作伙伴。调用路由的 `provider` 路径参数与目录条目的 `provider` 字段都引用这同一个 schema，这正是让已列出的 ID 与可接受的 ID 不会彼此偏移的原因。

  类型：[`RouterProviderSegment`](#routerprovidersegment) -- 字母数字 slug，例如 `anthropic`，最多 64 个字符
</ResponseField>

<ResponseField name="model" type="RouterModelSegment" required>
  规范 `{provider}/{model}` 模型 ID 中的小写 `model` 段，即在该提供商下要运行的模型。调用路由的 `model` 路径参数与目录条目的 `model` 字段共享它，原因与 `RouterProviderSegment` 相同，同样是为了避免偏移。

  类型：[`RouterModelSegment`](#routermodelsegment) -- 字母数字 slug，例如 `claude-opus-4-6`，最多 128 个字符
</ResponseField>

<ResponseField name="billing" type="RouterModelBilling" required>
  调用方在调用之前需要了解的各模型计费信息，而非价格。使用量与费用数字绝不会出现在这里。
...
<ResponseField name="has_more" type="boolean" required>
  本页之后是否还存在另一页。只要该值为 true，就继续遍历；不要因为 `data` 较短或为空，就推断目录已经结束。
</ResponseField>

<ResponseField name="next_cursor" type="RouterPageCursor">
  指向 Router 列表的不透明游标。它由服务器生成，并且只能原样往返传递：它不是偏移量，不是模型 ID，没有顺序，也不会在目录重建之间保持稳定，因此解析它、对它做自增，或在其所属的那次遍历之外持久化它，都超出了约定范围。之所以使用游标而不是偏移量，是因为目录是一个不断变化的列表：当遍历过程中有条目被添加或删除时，基于偏移量的遍历会静默跳过或重复条目，而调用方无法察觉这种情况的发生。

  类型：[`RouterPageCursor`](#routerpagecursor)，即作为 `next_cursor` 返回的不透明游标，1–512 个字符
</ResponseField>

<ResponseField name="limit" type="integer" required>
  实际返回的页大小。请求的 `limit` 若超过上限，会被钳制到上限，而不是被拒绝，因此该值可能小于请求时的值。请用这个数字进行分页，而不是用你发送的那个数字，否则你会误以为收到了从未返回的行。
...
对取消请求的应答，覆盖两种描述此路由已处理请求的状态：`202` 和 `400`。两者共用一个响应体形状，而非成功信封加错误信封，因为二者表达的是同一个陈述，也就是取消操作发现了什么；而一个必须按状态码解析不同类型的客户端，从这种拆分中得不到任何好处。

**字段**

<ResponseField name="request_id" type="RouterQueueRequestId" required>
  单个排队中的 Router 请求的标识符，即调用方用于轮询、取消和收集结果的句柄。

  类型：[`RouterQueueRequestId`](#routerqueuerequestid)，`pattern: ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`、uuid、最多 36 个字符
</ResponseField>

<ResponseField name="status" type="RouterQueueCancelStatus" required>
  取消请求所发现的内容，对应两种描述此路由实际已处理的请求的结果。两者都由 HTTP 状态码体现，因此客户端可以基于任一者进行分支。
...
取消请求得到的结果，针对的是描述此路由实际已解析请求的两种结果。两者都由 HTTP 状态码体现，因此客户端可以基于其中任意一个进行分支判断。
...
<ResponseField name="request_id" type="RouterQueueRequestId" required>
  标识一个排队中的 Router 请求，也就是调用方用于轮询、取消并获取结果的句柄。

  类型：[`RouterQueueRequestId`](#routerqueuerequestid) -- `pattern: ^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`、uuid、最多 36 个字符
</ResponseField>

<ResponseField name="status" type="RouterQueueStatus" required>
  一个排队中的 Router 请求所处的状态。它恰好有三个取值，而且与 `RouterErrorType` 不同，它确实是一个封闭的 `enum`，因为这两个 schema 是有意朝相反方向封闭的。`RouterErrorType` 用于对失败进行分类，其取值集合预期会增长，因此一个会硬性拒绝无法识别分类桶的生成客户端，恰恰会在已经出错的时候失败得最严重。而这个是生命周期，日后为其新增第四个状态，对所有针对它编写的轮询循环来说都是破坏性变更，无论它是否被声明为 enum 都是如此；所以就把它声明为 enum，并把这一约束写在客户端能看到的地方。
...
一次运行被准入队列时返回的句柄：包含该请求的身份与状态，并组合了用于指向其生命周期其余部分的三个 URL。
...
三个 URL，用于寻址某个已排队请求生命周期的其余阶段。每个携带活动句柄的响应都会返回这三个 URL，因此客户端永远不需要自行拼接队列 URL。
...
<ResponseField name="input" type="RouterValidationErrorInput">
  出错的输入值，原样回显，让调用方无需从 `loc` 重新推导就能看到被拒绝的内容。可为任意 JSON 类型，包括字符串、数字、布尔、数组、对象或 null，因此该 schema 刻意不做类型约束，而不是收窄为对象。当提供商不回显输入时此项缺省。
