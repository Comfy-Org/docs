...
<Steps>
  ...
  <Step title="生成一张图像">
    ...
    // Node.js 22+
    // 安装：npm install @comfyorg/sdk@^0.3.0 --save-dev tsx
  ...
  <Step title="读取并保存结果">
    ...
    请及时下载。Router 可以将 BFL 资产转存到 Comfy 存储上，但网址会过期，重放并不会为其续期。转存失败可能会留下有效期更短的提供商网址。参见[结果资产](/zh/development/comfy-router/reference#结果资产)。
  </Step>
</Steps>
