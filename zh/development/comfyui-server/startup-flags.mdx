...
| `--models-directory` `PATH` | ComfyUI 根目录/models | 模型目录。覆盖 `--base-directory` 中的 models 文件夹。 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--cuda-device` `DEVICE_ID` | — | 使用的 CUDA 设备 ID，逗号分隔（如 `0` 或 `0,1`），或使用 `all` 保留所有当前可见设备。其他设备不可见。 |
...
<Note>
  在 Windows 上，ComfyUI 默认仅使用 GPU 0。要恢复使用所有 GPU，请传入 `--cuda-device all`。
</Note>
...
每个 FP8 参数后缀对应业界 FP8 规范（ONNX / OCP）中的一种 8 位浮点布局：
- **e4m3fn**：4 位指数、3 位尾数。在 NVIDIA Ada Lovelace 及更新 GPU 上，为模型权重提供精度与动态范围的良好平衡。
- **e5m2**：5 位指数、2 位尾数。动态范围比 e4m3fn 更宽，但精度较低。适用于权重幅度变化较大的场景。
- **e8m0fnu**：8 位指数、0 位尾数。动态范围最大，无小数精度。通常用作块级缩放因子（仅能表示 2 的幂）。
...
有关各 FP8 格式后缀的说明，请参见 [UNET 精度](#unet-精度) 小节。
...
| `--disable-triton-backend` | 关闭 | 强制禁用 comfy-kitchen 的 Triton 后端，覆盖 `--enable-triton-backend`。 |
...
<Note>
  缓存模式参数互斥。`--cache-ram`、`--cache-classic`、`--high-ram`、`--cache-lru`、`--cache-none` 只能选其一。
</Note>

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--cache-ram` `[GB] [GB]` | 启用（默认模式） | RAM 压力缓存。第一个值：活跃缓存阈值（GB）；可选第二个值：非活跃缓存/固定阈值（GB）。无值时：活跃 = 系统 RAM 的 10%（最小 2 GB，最大 10 GB）；非活跃 = 系统 RAM 的 100%（最大 128 GB）。最多接受两个值。 |
| `--cache-classic` | — | 使用旧版激进缓存策略。 |
| `--high-ram` | 关闭 | 在内存充足、或更倾向使用页面文件而非重新加载模型的系统上可略微提升性能。同时启用 classic 缓存。 |
| `--cache-lru` `N` | `0`（关闭） | LRU 缓存，最多缓存 N 个节点结果。无硬性上限；受可用 RAM/VRAM 限制。可能占用更多 RAM/VRAM。 |
| `--cache-none` | — | 降低 RAM/VRAM 占用；每次运行重新执行所有节点。 |

<Note>
  ComfyUI 会在同一会话中复用已缓存的节点结果，这正是重复运行更快的原因。如果你希望每次运行都从干净的缓存开始，请使用 `--cache-none`，例如在调试工作流，或需要排除运行之间的缓存复用时。代价是每个节点都会重新执行，生成时间会更长。它是启动参数，需要重启 ComfyUI 才会生效。
</Note>
...
| `--use-ck-attention` | 使用 Comfy Kitchen 注意力。需要硬件支持其 INT8 内核。 |
...
| `--vram-headroom` `GB` | `0` | 动态 VRAM 在默认预留之上额外保持空闲的 VRAM 量（GB）。ComfyUI 会尽量让这部分 VRAM 完全空闲不使用，其他应用占用的 VRAM 也计入其中。 |
| `--disable-nvml-pressure` | 关闭 | 动态 VRAM 的内存压力改用 CUDA 报告，而非 NVML。 |
...
| `--disable-fast-disk` | 关闭 | 禁用磁盘-backed 动态加载与卸载，改用未固定 RAM。覆盖 `--fast-disk`。 |
| `--disable-cuda-graphs` | 关闭 | 禁用 CUDA graphs。 |
| `--disable-comfy-compiler` | 关闭 | 禁用 Comfy 模型编译器，包括其 CUDA graph 子功能。 |
| `--assert-graph-breaks` | 关闭 | 遇到 Comfy 模型编译器 graph break 时直接失败。 |
...
| `--debug-hang` | 关闭 | 在按下 Ctrl-C 时输出堆栈跟踪转储，用于排查卡死问题。 |
...
| `--enable-manager-legacy-ui` | 使用旧版 Manager UI。会自动启用 `--enable-manager`。与 `--disable-manager-ui` 互斥。 |
...
<Note>
  `--disable-api-nodes` 还会改变浏览器可以加载的内容。该参数会附加一条 Content-Security-Policy，把页面限制在 ComfyUI 自身提供的资源上，因此来自其他源的图片、字体、媒体、框架和网络请求都会被拦截，API 节点（合作伙伴节点）也不会被注册。重启后原本能正常加载的预览或缩略图消失时，先检查启动参数里有没有这个参数，再去排查节点本身。
</Note>
...
| `--enable-asset-hashing` | 关闭 | 扫描资产时计算 blake3 内容哈希。可用于后续的资产可移植功能（去重、跨机器模型解析），代价是启动时间以及大型模型目录下逐个输出的哈希开销。默认关闭。 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--verbose` `[LEVEL] [FILE]` | `INFO` | 日志级别。可选：`DEBUG`、`DETAIL`、`INFO`、`WARNING`、`ERROR`、`CRITICAL`。单独使用 `--verbose` 设为 `DEBUG`。在级别后加上文件路径可同时把该级别日志写入文件；该参数可重复指定，以同时配置控制台与文件输出。 |
