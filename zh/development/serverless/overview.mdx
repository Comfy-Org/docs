...
Serverless API 为 ComfyUI 工作流提供托管 URL 和按需 GPU 算力。**Build** 是 ComfyUI 环境的版本化定义：其中包含从本地安装中捕获的模型、自定义节点和设置。Build and Deploy CLI 会将 Build 定义保存在你的项目中，基于该定义创建发布版本（release），并在发布版本准备好对外提供服务时进行部署。

<CardGroup cols={2}>
  <Card title="1. 构建（Build）" icon="box">
    在本地根据你的 ComfyUI 安装创建 Build 定义。
  </Card>
...
`comfy-build.yaml` 是 Build 的本地权威来源。它存储 Build 定义和最近已知的远程状态，因此 CLI 可以自动选择正确的 Build，并在本地更改覆盖较新的远程定义之前发出警告。

将此文件与项目一同保留。它描述 Build；它本身不包含模型字节。
