...
| **您自己的 ComfyUI** | `http://127.0.0.1:8189`（本地代理） | 不使用 |
...
然后将 SDK 指向 `http://127.0.0.1:8189`，并且不要传递任何 API 密钥。默认情况下，代理仅绑定到回环地址。如果您还想将模型文件上传到您的安装中，请使用 `--comfyui-base-dir /path/to/ComfyUI` 运行它。
