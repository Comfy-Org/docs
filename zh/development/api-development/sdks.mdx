...
| `handle.events()`（TypeScript）/ `handle.iter_events()`（Python） | [Comfy Router 队列](/zh/development/comfy-router/queue) | 针对一个 Router 请求的队列观测：状态和队列位置。它会轮询状态路由；没有流，也没有进度或预览事件。 |
| `comfy.models.subscribe(...)` | [Comfy Router 队列](/zh/development/comfy-router/queue) | 不是迭代器。一次调用即可提交、轮询并收集，带有 `on_queue_update` / `onQueueUpdate` 回调，最终解析为结果。 |
