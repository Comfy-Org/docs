...
translationSourceHash: 8ff4dd2b
...
translationBlockHashes:
  ...
  "Advanced workflows with the native nodes": a084a93e
...
三个示例模板只是冰山一角。原生 MiniMax H3 节点还支持任意帧锚定引导、提示词嵌入（embedding）和潜在空间噪声蒙版。请将 ComfyUI 更新到最新版本以使用这些工作流。

### 在任意帧锚定引导

示例模板只能在视频的首帧和尾帧锚定关键帧。`MiniMaxH3AddGuide` 节点取消了这一限制：它可以在连续时间轴上的任意帧锚定引导。

将 MiniMax H3 节点的 condition 和 latent 输出连接到 `MiniMaxH3AddGuide`，然后至少提供一个引导输入：
...
MiniMax H3 支持视频和音频潜在空间的噪声蒙版。像往常一样将蒙版连接到采样器的 `denoise_mask` 输入：蒙版区域会被重新生成，其余部分保持不变。视频蒙版会对齐到模型的 2x2 潜在空间补丁网格，音频蒙版对齐到完整的潜在空间帧。可用于局部重绘、物体移除，或在保持现有内容稳定的同时扩展片段。
