...
模板默认使用一个较快的预览尺寸。要获得全质量输出，请在 16:9 下将百万像素提高到约 `1.0`，大约得到 1376x768：即 H3 的原生画布（短边 768px）。
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA（可选）：minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors">
    仅在使用可选的 Lightning LoRA 加速模式时需要。放入 <code>ComfyUI/models/loras/</code>
  </Card>
</CardGroup>
...
3. **分辨率**：H3 的原生画布短边为 768px，分辨率取整到 32 的倍数。
...
6. **加速模式（可选）**：工作流包含一个 Lightning LoRA（`minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16`），可用于加速生成。在 MiniMax H3 节点上启用 `turbo_mode`，将以 8 步而不是 20 步生成；画质会略有下降。
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA（可选）：minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    仅在使用可选的 Lightning LoRA 加速模式时需要。放置于 <code>ComfyUI/models/loras/</code>
  </Card>
</CardGroup>
...
6. **加速模式（可选）**：勾选工作流中的 **Lightning LoRA** 复选框，即可用 4 步 turbo LoRA（`minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16`）运行 `ref2va` 模型，而非完整的 20 步。速度大幅提升，画质仅有小幅折损。
