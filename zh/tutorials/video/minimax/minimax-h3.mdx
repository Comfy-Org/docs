...
translationSourceHash: 649116d0
...
translationBlockHashes:
  ...
  "Advanced workflows with the native nodes": 7e452463
...
除示例模板外，原生 MiniMax H3 节点还支持任意帧锚定引导、提示词嵌入（embedding）和潜在空间噪声蒙版。这些能力来自近期合并的 ComfyUI 更新：[任意帧引导锚定](https://github.com/Comfy-Org/ComfyUI/pull/15439)、[提示词嵌入](https://github.com/Comfy-Org/ComfyUI/pull/15697) 和 [潜在噪声蒙版](https://github.com/Comfy-Org/ComfyUI/pull/15375)。请将 ComfyUI 更新到最新版本以使用它们。

### 在任意帧锚定引导

示例模板只能在视频的首帧和尾帧锚定关键帧。`MiniMaxH3AddGuide` 节点取消了这一限制：它可以在连续时间轴上的任意帧锚定引导。
...
MiniMax H3 支持 ComfyUI 标准的 `embedding:` 提示词语法。将嵌入文件放入 `ComfyUI/models/embeddings/`，并在提示词中按名称引用，例如 `embedding:my_embedding`。嵌入会像其他 ComfyUI 模型一样被加载并混入文本 condition。

### 使用潜在空间噪声蒙版进行局部重绘与扩展

MiniMax H3 支持视频和音频潜在空间的噪声蒙版。像往常一样将蒙版连接到采样器的 `denoise_mask` 输入：`0` 值保留对应的潜在区域，`1` 值重新生成该区域。视频蒙版会对齐到模型的 2x2 潜在空间补丁网格，音频蒙版对齐到完整的潜在空间帧。可用于局部重绘、物体移除，或在保持现有内容稳定的同时扩展片段。
