...
sidebarTitle: "概述"
translationSourceHash: c0b3e0e2
...
translationBlockHashes:
  ...
  "Speeding up generation with Sage Attention": d5dcd984
---
...
要切换后端，请在工作流中添加内置的 **Model Attention Backend** 节点（分类 `model/patch`），将其连接在 `UNETLoader` 和 `BasicGuider` 节点之间，并将后端设置为 `comfy kitchen attention`。也可以使用 `--use-ck-attention` 启动参数启动 ComfyUI。该后端支持 Nvidia 和 AMD GPU，随 ComfyUI 自带，无需额外安装。将受影响的块改为以 bfloat16 注意力运行同样能修复伪影，但渲染时间会更长。
