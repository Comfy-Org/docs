...
ComfyUI 原生支持 MiniMax H3。模板库目前提供五个示例工作流：

- **文生视频**（T2V）：根据文本提示生成视频
- **图生视频**（I2V）：根据输入图像生成视频，可选择首帧/末帧控制
- **参考生视频**（R2V）：根据参考图像、视频和音频生成视频，锁定角色、风格、动作、相机运动或声音
- **多帧参考**：使用串联的 Add Guide 节点，将参考帧锚定在输出时间线上的特定位置
- **Fun ControlNet Union**：使用 Canny、Depth、HED、MLSD 或 Pose 控制视频驱动 H3，或使用遮罩进行视频修补

这五个是示例模板，并非模型能力的完整清单。通过原生 MiniMax H3 节点可以覆盖更多生成模式：`MiniMaxH3ImageToVideo` 节点支持首帧/末帧图生视频（fl2va），`MiniMaxH3ReferenceToVideo` 节点支持图像、视频和音频参考驱动的生成（ref2va）。你也可以用这些节点搭建其他工作流。
...
使用控制视频（而非仅靠文本）控制 MiniMax H3 视频的运动。Fun ControlNet Union 补丁（[alibaba-pai/MiniMax-H3-Fun-Controlnet-Union](https://huggingface.co/alibaba-pai/MiniMax-H3-Fun-Controlnet-Union)，打包于 [Comfy-Org/MiniMax-H3](https://huggingface.co/Comfy-Org/MiniMax-H3)）是单个检查点，可将 H3 条件化到 Canny、Depth、HED、MLSD 或 Pose 控制视频上，还支持带遮罩的视频修补。它同时兼容 `fl2va` 和 `ref2va` transformer 文件。
...
此模板需要 ComfyUI 0.35.0 或更高版本。示例使用 Pose 控制视频：内置的 SDPose 子图会从输入视频中提取姿态骨架，使输出重现舞者的动作。你也可以将任意预处理后的控制视频直接连接到 `Apply MiniMax H3 Fun ControlNet` 节点的 `control_video` 输入。
...
<CardGroup cols={2}>
  ...
  <Card title="ControlNet Patch: minimax_h3_fun_controlnet_union_pruned_int8_convrot" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/model_patches/minimax_h3_fun_controlnet_union_pruned_int8_convrot.safetensors">
    放置在 <code>ComfyUI/models/model_patches/</code>
  </Card>
  <Card title="Text Encoder: qwen3vl_32b_minimax_h3_nvfp4_awq" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/text_encoders/qwen3vl_32b_minimax_h3_nvfp4_awq.safetensors">
    放置在 <code>ComfyUI/models/text_encoders/</code>
  </Card>
  ...
  <Card title="LoRA: minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    用于可选的 Lightning LoRA 极速模式。放置在 <code>ComfyUI/models/loras/</code>
  </Card>
  <Card title="Checkpoint: sdpose_wholebody_fp16" icon="download" href="https://huggingface.co/Comfy-Org/SDPose/blob/main/checkpoints/sdpose_wholebody_fp16.safetensors">
    内置 SDPose 子图使用的姿态提取器。放置在 <code>ComfyUI/models/checkpoints/</code>
  </Card>
  <Card title="Detector: rt_detr_v4-x-hgnet_fp16" icon="download" href="https://huggingface.co/Comfy-Org/SDPose/blob/main/diffusion_models/rt_detr_v4-x-hgnet_fp16.safetensors">
    内置 SDPose 子图使用的人物检测器。放置在 <code>ComfyUI/models/diffusion_models/</code>
  </Card>
</CardGroup>
...
1. **视频长度**：目标视频时长为 5 到 15 秒，按 24fps 对齐到 17n+5 帧网格（124 帧 = 5 秒）。控制视频比目标长时会被裁剪到前几帧；更短时则保持最后一帧。要让输出精确匹配控制视频长度，可以将示例中计算好的 `batch_size` 输出连接到 `MiniMax H3 Reference to Video` 节点的长度输入
2. **引导与强度**：保持 `guidance_scale` 为 1.0。仅当输出偏离控制时才将补丁 `strength` 提高到 1 以上
3. **视频修补**：将遮罩连接到 `mask` 输入：标记为 `1` 的区域在可选的 `source_video` 输入控制下重新生成，其余部分保持固定
4. **极速模式（可选）**：勾选 **Lightning LoRA** 复选框，使用 4 步极速 LoRA 大幅加快生成速度，音频和动作质量略有下降
5. **准备控制视频**：ComfyUI 为每种控制类型内置了预处理节点。边缘请参考 [Detect Edges (Canny) 节点](https://docs.comfy.org/built-in-nodes/Canny)，深度图请参考 [Depth Anything 3 教程](https://docs.comfy.org/tutorials/utility/depth-anything-3)
