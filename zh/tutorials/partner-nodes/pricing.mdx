...
> **Seedance 2.0 token 计算方式：** 输出 tokens = (输入视频时长 + 输出视频时长) × 输出视频长度 × 输出视频宽度 × 输出视频帧率 ÷ 1024。当输入为文本、图像或音频时，输入时长计为 0。Seedance 2.0 输出帧率为 24fps。对于视频转视频输入，存在最低 token 用量限制。**每千 token 积分**根据模型、输出分辨率以及是否使用参考视频而有所不同。`Seedance 2.0 Fast` 和 `Seedance 2.0 Mini` 支持 480P 和 720P；标准 `Seedance 2.0` 还额外支持 1080P 和 4K。480P 使用与 720P 相同的每千 token 积分费率。
