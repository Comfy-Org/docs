...
OpenRouter 合作节点按 [OpenRouter 模型页](https://openrouter.ai/models) 上各模型 slug 的公开单价计费。**积分 = 美元 × 211**（见页首）。提示与补全的美元单价（每 token）与 `comfy_api_nodes/nodes_openrouter.py` 及 OpenRouter API 的 `pricing` 字段一致。最终扣费以聊天补全返回的实际 token 用量为准。

### 大语言模型（`OpenRouterLLMNode`）

下表为节点内可选模型，每百万 token。
...
可选参考图、参考视频以 URL 形式上传；支持视觉的模型按同一输入单价对多模态内容折算 token 费用。
