...
OpenAI GPT-Image-1 是 ComfyUI 合作节点系列中的一员，它允许用户通过 OpenAI 的 **GPT-Image-1** 模型生成图像。这与 ChatGPT 4o 中用于图像生成的模型相同。

**OpenAI GPT-Image-1** 节点通过 OpenAI 的图像生成 API 同步生成图像。它接收文本提示，并返回与描述匹配的图像。GPT-Image-1 是 OpenAI 目前最先进的图像生成模型，能够生成高度精细且逼真的图像。
