...
与图生图工作流类似，你可以输入一段音乐，使用下面的工作流对音乐重新采样并生成音乐。你也可以通过控制 `Ksampler` 的 `denoise` 参数来调整与原始音频的差异程度。

### 1. 下载工作流文件

点击下面的按钮下载对应的工作流文件，拖入 ComfyUI 中即可加载工作流信息。
...
3. （可选）在 `TextEncodeAceStepAudio` 的 `tags` 和 `lyrics` 字段中输入对应的音乐风格和歌词。提供歌词对于音频编辑来说非常重要
4. （可选）修改 `Ksampler` 节点的 `denoise` 参数，以调整采样过程中添加的噪声，从而控制与原始音频的相似度（值越小与原始音频越相似；设置为 `1.00` 则近似于没有音频输入）
5. 点击 `Run` 按钮，或使用快捷键 `Ctrl(cmd) + Enter` 来执行音频生成
6. 工作流完成后，你可以在 `Save Audio` 节点中预览生成的音频。你可以点击播放试听，音频也会被保存到 `ComfyUI/output/audio`（子目录由 `Save Audio` 节点决定）

你还可以实现ACE-Step项目页面中的歌词修改和编辑功能，通过修改原始歌词来改变音频效果。

### 3. 工作流补充说明

1. 在示例工作流中，你可以将 `TextEncodeAceStepAudio` 的 `tags` 从 `male voice` 改为 `female voice`，以生成女声音频。
2. 你还可以修改 `TextEncodeAceStepAudio` 中的 `lyrics`，以改变歌词，从而改变生成的音频。更多详情请参考ACE-Step项目页面中的示例。
