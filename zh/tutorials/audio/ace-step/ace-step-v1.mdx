...
与图生图工作流类似，你可以输入一段音乐，并使用下面的工作流对其进行重新采样并生成音乐。你还可以通过控制 `Ksampler` 中的 `denoise` 参数，来调整与原始音频的差异程度。
 
### 1. 下载工作流文件

点击下面的按钮下载对应的工作流文件，将其拖入 ComfyUI 中即可加载工作流信息。
...
<a className="prose" target='_blank' href="https://github.com/Comfy-Org/example_workflows/raw/refs/heads/main/audio/ace-step/input.mp3" style={{ display: 'inline-block', backgroundColor: '#0078D6', color: '#ffffff', padding: '10px 20px', borderRadius: '8px', borderColor: "transparent", textDecoration: 'none', fontWeight: 'bold'}}>
    <p className="prose" style={{ margin: 0, fontSize: "0.8rem" }}>下载用于输入的示例音频文件</p>
</a>
...
1. 确保 `Load Checkpoints` 节点已加载 `ace_step_v1_3.5b.safetensors` 模型
2. 在 `LoadAudio` 节点中上传所提供的音频文件
3. （可选）在 `TextEncodeAceStepAudio` 的 `tags` 和 `lyrics` 字段中输入对应的音乐风格和歌词。提供歌词对于音频编辑非常重要
4. （可选）修改 `Ksampler` 节点中的 `denoise` 参数，以调整采样过程中添加的噪波，从而控制与原始音频的相似程度（数值越小，与原始音频越相似；设置为 `1.00` 时，大致相当于没有音频输入）
5. 点击 `Run` 按钮，或使用快捷键 `Ctrl(cmd) + Enter` 来执行音频生成
6. 工作流完成之后，你可以在 `Save Audio` 节点中预览已生成的音频。你可以点击播放并进行试听，音频同时也会保存至 `ComfyUI/output/audio`（子目录由 `Save Audio` 节点决定）。

你还可以实现 ACE-Step 项目页面中的歌词修改和编辑功能，通过修改原始歌词来改变音频效果。

### 3. 工作流补充说明

1. 在示例工作流中，你可以将 `TextEncodeAceStepAudio` 中的 `tags` 从 `male voice` 改为 `female voice`，以生成女声。
2. 你还可以修改 `TextEncodeAceStepAudio` 中的 `lyrics`，以更改歌词，从而改变已生成的音频。更多详情请参考 ACE-Step 项目页面上的示例。
