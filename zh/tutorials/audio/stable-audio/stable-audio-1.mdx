...
该工作流使用**标准 ComfyUI 节点** —— 无需自定义节点。它加载 Stable Audio 1.0 检查点，通过 CLIP 文本编码器（t5-base）编码你的提示，使用 K采样器对 Latent 音频进行去噪，并通过模型的 VAE 将其解码为音频。

**使用方法**：
1. **加载检查点** —— `CheckpointLoaderSimple` 节点使用 `stable-audio-open-1.0.safetensors`
