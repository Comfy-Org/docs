...
**Stable Audio 3** ([GitHub](https://github.com/Stability-AI/stable-audio-3)) 是 Stability AI 最新开源的音频生成模型，能够根据文本描述生成高质量立体声音频——包括音乐、音效和乐器声音等。该模型在 ComfyUI 0.22.0 版本中已获得原生支持。

**主要特点**：

- **文本转音频** — 将简短描述转换为立体声音频片段
- **分类感知的重新提示** — 可选使用 Qwen 进行提示词扩展，预设类别包括：音乐、乐器、SFX、单次音效
- **本地推理** — 使用单个检查点即可在本地运行
- **立体声输出** — 生成完整立体声音频
- **开源** — 面向本地使用和二次开发发布
...
**Stable Audio 3 Medium Base** 工作流是简化版本，不包含 Qwen 提示词扩展。它接收完整的文本提示词并直接传递给 Stable Audio 3 模型。当你已经有详细的提示词、希望加快生成速度时，可以使用此版本。

**使用方法**：
1. **文本提示词** — 输入你想要生成的音频的详细描述（模型直接处理提示词，请提供足够细节）
2. **时长** — 设置音频片段长度（秒）
3. **种子** — 控制可重现性
...
Stable Audio 3 需要以下文件。加载工作流时 ComfyUI 会自动尝试下载。你也可以手动下载并放到正确的目录。

**检查点**（放入 `ComfyUI/models/checkpoints/`）：
...
**文本编码器**（放入 `ComfyUI/models/text_encoders/`）：
...
> **注意**：Medium Base 工作流使用的是 `stable_audio_3_medium_base.safetensors` 模型变体。请确保你下载了对应工作流所需的正确检查点。
