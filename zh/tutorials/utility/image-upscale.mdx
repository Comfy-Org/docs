...
### Z-Image-Turbo 2K 放大器（本地）

对于本地扩散模型放大，Z-Image-Turbo 2K 放大器会先用 RealESRGAN 对图像进行预放大，再用 Z-Image-Turbo 以较低的降噪值重新采样，从而在 2K 分辨率下重建细节。

采样器上的 `denoise` 值决定模型可以偏离输入的程度：`0.15` 到 `0.25` 会贴近原图，`0.25` 到 `0.35` 会通过重新诠释补充更多细节，而超过 `0.35` 可能引入伪影，除非你在提示词中使用详细的图像描述而不是简短标签。模板默认的 `denoise` 为 `0.33`。

<CardGroup cols={2}>
  <Card title="在 Comfy Cloud 上运行" icon="cloud" href="https://cloud.comfy.org/?template=utility_z_image_turbo_2k_upscaler.app&utm_source=docs&utm_medium=referral&utm_campaign=image-upscale">
    在 Comfy Cloud 中打开
  </Card>
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_z_image_turbo_2k_upscaler.app.json">
    下载 JSON 或在模板库中搜索 &quot;Image Upscale: Z-Image-Turbo 2K&quot;
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="fighter_jet.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/fighter_jet.png">
    `LoadImage` 节点 77 · `fighter_jet.png`
  </Card>
</CardGroup>
