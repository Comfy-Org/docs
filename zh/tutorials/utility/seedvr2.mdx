...
使用SeedVR2 3B Int8对图像进行缩放，这是一个基于扩散的单步视频恢复模型，能够生成高质量结果并改善时间一致性。
...
<CardGroup cols={2}>
  <Card title="在 Comfy Cloud 中打开" icon="cloud" href="https://cloud.comfy.org/?template=utility_seedvr2_3b_int8_upscale_image&utm_source=docs&utm_medium=referral&utm_campaign=seedvr2">
    在 Comfy Cloud 中打开
  </Card>
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_seedvr2_3b_int8_upscale_image.json">
    下载JSON或在模板库中搜索“SeedVR2 3B Int8: 缩放图像”
  </Card>
</CardGroup>

**输入素材**

将该文件上传到匹配的`LoadImage`节点：
...
1. 将图像放入 `ComfyUI/input/` 文件夹，然后在“加载图像”节点中选择该图像
2. 在 SeedVR2 模型加载器中选择 `seedvr2_3b_int8_convrot.safetensors` 检查点
3. 点击“队列”或使用 `Ctrl(cmd) + Enter` 运行### SeedVR2 7B Int8：缩放图像（`utility_seedvr2_7b_int8_upscale_image`）

使用 SeedVR2 7B Int8 缩放图像，这是一种单步扩散模型，通过对抗训练和自适应窗口注意力提升分辨率。
...
1. 将您的图像放入 `ComfyUI/input/` 文件夹中，然后在 `加载图像` 节点中选择它。
2. 在 SeedVR2 模型加载器中选择 `seedvr2_7b_int8_convrot.safetensors` 检查点。
3. 点击 `队列` 或使用 `Ctrl(cmd) + Enter` 运行。### SeedVR2 3B Int8: 放大视频（`utility_seedvr2_3b_int8_upscale_video`）

使用 SeedVR2 3B Int8 放大并修复视频素材，这是一种一步扩散模型，可在提升分辨率的同时保持帧间时间一致性。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/utility_seedvr2_3b_int8_upscale_video-1.webp" alt="SeedVR2 3B Int8 放大视频工作流预览" />
...
<CardGroup cols={2}>
  ...
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_seedvr2_3b_int8_upscale_video.json">
    下载 JSON，或在模板库中搜索“SeedVR2 3B Int8: 放大视频”
  </Card>
</CardGroup>
...
1. 将你的视频放入 `ComfyUI/input/` 文件夹，并在 `加载视频` 节点中选择该视频
2. 在 SeedVR2 模型加载器中选择 `seedvr2_3b_int8_convrot.safetensors` 检查点
3. 点击 `队列` 或使用 `Ctrl(cmd) + Enter` 运行### 性能

更高的目标分辨率需要更长的处理时间。与FP16相比，INT8变体以更低的VRAM占用提供高效的推理。
