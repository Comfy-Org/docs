...
### SeedVR2 3B Int8: Upscale 视频 {#utility_seedvr2_3b_int8_upscale_video}
...
在量化模型上，注意力后端可能会静默返回黑色或损坏的输出。如果 SeedVR2 INT8 工作流生成的是全黑图像，请按以下顺序尝试：

1. **不使用 FlashAttention 运行：** 如果你是通过 `--use-flash-attention` 或某个自定义节点包启用的，请使用默认注意力后端启动 ComfyUI 并重新运行。在 RTX 3090 上，禁用 FlashAttention 解决了 3B 和 7B INT8 工作流的全黑输出问题。
2. **切换模型：** 使用相同体量的 FP16 或 FP8 文件。如果这些文件渲染正常，那么出问题的就是 INT8 量化路径。
3. **验证文件：** diffusion 模型和 `seedvr2_ema_vae_fp16.safetensors` 都必须完整下载，并按模型安装中所示的方式放置。下载被截断也会以同样的方式失败。
4. **禁用实时预览：** 在某些情况下，只有预览是黑色的，而保存的文件是正确的，因此关闭实时预览可以消除这种不确定性。

如果结果仍然是黑色，请在教程讨论区发帖，附上你的 GPU、ComfyUI 版本、安装方法（桌面版、便携版或手动安装）以及控制台输出，并说明 3B 和 7B 工作流是否以相同方式失败。
