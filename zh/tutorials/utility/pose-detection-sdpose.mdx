...
将 ComfyUI 更新至最新版本，然后进入 `工作流` -> `浏览模板`，在“实用工具”类别下找到 SDPose 工作流。### SDPose：图像多人检测（`utility_sdpose_multi_person`）

上传图片以检测人体姿势。支持在同一场景中检测单人和多人。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/utility_sdpose_multi_person-1.webp" alt="SDPose图像多人检测工作流预览" />
...
<CardGroup cols={2}>
  ...
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_sdpose_multi_person_video.json">
    下载 JSON 或在模板库中搜索 &quot;SDPose: Video Multi-Person Detection&quot;
  </Card>
</CardGroup>
...
</div>### SDPose-OOD: 图像到姿态映射 (`utility_sdpose_ood_image_to_pose`)

上传图片以提取姿态关键点，并使用 SDPose-OOD 模型生成对应的姿态映射。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/utility_sdpose_ood_image_to_pose-1.webp" alt="SDPose-OOD 图像到姿态映射工作流预览" />
...
<CardGroup cols={2}>
  ...
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_sdpose_ood_image_to_pose.json">
    下载 JSON 或在模板库中搜索 "SDPose-OOD: 图像到姿态映射"
  </Card>
</CardGroup>
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  ...
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/utility_sdpose_ood_image_to_pose.png" alt="SDPose-OOD 图像到姿态映射示例输出" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
</div>### SDPose-OOD: 视频到姿态图（`utility_sdpose_ood_video_to_pose_map`）

上传视频以提取姿态关键点并生成姿态图。该工作流支持多人检测，并使用增强型SDPose模型进行精确的整体特征提取。
...
SDPose和RT-DETRv4模型检查点托管在[Comfy-Org SDPose模型仓库](https://huggingface.co/Comfy-Org/SDPose)上。

**checkpoints**（SDPose模型）：
...
- [rt_detr_v4-x-hgnet_fp32.safetensors](https://huggingface.co/Comfy-Org/SDPose/blob/main/diffusion_models/rt_detr_v4-x-hgnet_fp32.safetensors)（完整精度，体积更大）
...
- **输入** — 通过`加载图像`节点加载图像。使用包含一个或多个人的图像（例如：`group_photo.png`）。
- **检测** — `图像到姿态图 (SDPose 多人)` 子图处理图像并输出：
  - **IMAGE** — 叠加在图像上的骨骼可视化
  - **keypoints** — 原始全身关键点数据
  - **bboxes** — 边界框坐标
- **绘制选项** — 配置要绘制的身体部位：
...
- **检测选项**：
  - `resize_type.longer_size` — 在检测前缩放较长边
  - `max_detections` — 最大检测人数
  - `detect_threshold` — 检测置信度阈值
  - `detect_class` — 要检测的对象类别（默认为人）

#### 多人（视频）

与图像工作流相同，但按顺序处理视频帧。使用`加载视频`输入视频文件，使用`保存视频`导出结果。

#### OOD 图像到姿态图

使用 SDPose 模型从图像生成清晰的姿态图，不带边界框可视化。这对于风格迁移很有用，您可以从一张图像中提取骨骼姿态并应用于另一张图像。

#### OOD 视频到姿态图

从视频生成逐帧的姿态图。输出是一个视频文件，其中每帧包含提取的骨骼姿态，适用于后续的动画或 ControlNet 工作流。
