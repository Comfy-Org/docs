...
### Mediapipe：图像人脸检测（`utility_face_detection_mediapipe`）

输入图像，每张人脸最多检测 6 个面部关键点，实现超快速的多脸检测。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/utility_face_detection_mediapipe-1.webp" alt="Mediapipe 图像人脸检测工作流预览" />

<CardGroup cols={2}>
  <Card title="在 Comfy Cloud 上运行" icon="cloud" href="https://cloud.comfy.org/?template=utility_face_detection_mediapipe&utm_source=docs&utm_medium=referral&utm_campaign=mediapipe">
    在 Comfy Cloud 中打开
  </Card>
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_face_detection_mediapipe.json">
    下载 JSON 文件，或在模板库中搜索 &quot;Mediapipe: Image Face Detection&quot;
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="soft_neon_girl.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/soft_neon_girl.png">
    `LoadImage` 节点 5 · `soft_neon_girl.png`
  </Card>
</CardGroup>

<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/soft_neon_girl.png" alt="输入图像" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
</div>
...
MediaPipe Face Detection 模型托管在 [Comfy-Org MediaPipe 模型仓库](https://huggingface.co/Comfy-Org/mediapipe) 上。
...
本工作流使用一个 **子图** 节点来协调人脸检测、可视化和遮罩生成。子图暴露了以下控制参数：
...
| 参数 | 默认值 | 描述 |
|------|:------:|------|
| **model_name** | `mediapipe_face_fp32.safetensors` | `ComfyUI/models/detection/` 目录下的检查点文件。如缺失则下载上述模型 |
| **detector_variant** | `short` | **short** — 针对近距离/大脸优化（约 2 米范围）。**full** — 覆盖更小/更远的人脸（约 5 米），速度较慢。**both** — 同时运行两个检测器，保留每帧检测到更多人脸的结果（约 2 倍开销） |
| **num_faces** | `1` | 每帧最多返回的人脸数。`0` = 不限制（返回所有检测到的人脸）。范围：0–16 |
| **custom_face_oval** | `否` | 在遮罩输出中包含面部轮廓区域 |
| **custom_lips** | `否` | 在遮罩中包含嘴唇（与其他启用的区域合并） |
| **custom_left_eye** | `否` | 在遮罩中包含左眼区域 |
| **custom_right_eye** | `否` | 在遮罩中包含右眼区域 |
| **custom_irises** | `否` | 在遮罩中包含虹膜区域 |
...
| 输出 | 类型 | 描述 |
|------|------|------|
| **face_landmarks** | `FACE_LANDMARKS` | 每帧人脸数据，包含 478 个 2D/3D 关键点、ARKit-52 混合形状、网格拓扑数据——输入到可视化和遮罩节点 |
