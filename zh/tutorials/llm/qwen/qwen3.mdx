...
输入文本提示词，使用 Qwen3-4B-Thinking 模型生成详细、有推理依据的回复。
...
<CardGroup cols={2}>
  ...
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/llm_qwen3_text_gen.json">
    下载 JSON，或在模板库中搜索 &quot;Qwen3.0: Text Generation&quot;
  </Card>
</CardGroup>

本工作流展示了 Qwen 3.0 的核心**文本生成**能力。它接收文本提示词，并利用模型内置的推理能力生成详细、结构化的回复。

**输入**：
- **文本提示**：你的问题、指令或任务描述

**关键控制**：
- **最大长度**：生成的最大 token 数量（默认 256）
- **采样模式**：开启或关闭采样，并调整 temperature、top-k、top-p、重复惩罚和种子
- **思考模式**：在最终答案之前启用逐步推理
- **使用默认模板**：应用模型内置的系统提示词

**输出**：
- **生成的文本**：模型以纯文本字符串形式返回的回复

<Card title="了解 Subgraph" icon="book-open" href="/zh/interface/features/subgraph">
  本工作流使用 Subgraph 节点进行模块化处理。查看 Subgraph 文档，了解如何自定义和扩展工作流。
</Card>
