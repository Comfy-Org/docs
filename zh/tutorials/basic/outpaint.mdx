...
基本上它要求的内容与[局部重绘](/zh/tutorials/basic/inpaint)相似，只不过我们用来**构建遮罩（Mask）的节点不同**
...
在这个工作流中主要是通过 `Pad Image for outpainting` 节点来控制图片的扩展方向和范围，其实这也是一个 [局部重绘(Inpaint)](/zh/tutorials/basic/inpaint) 工作流，只不过我们用来构建遮罩（Mask）的节点不同。
