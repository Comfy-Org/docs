...
如果你仔细比对本篇教程和[文生图](/zh/tutorials/basic/text-to-image)教程，你会发现图生图的流程和文生图的流程非常相似，只是多了个输入的参考图片作为输入条件，也就是在文生图中，我们是让画家（绘图模型）根据我们的提示词生成自由发挥，而在图生图中，我们是让画家（绘图模型）根据我们的参考图片和提示词生成图片。
...
对应原理可以参考[文生图](/zh/tutorials/basic/text-to-image)教程中的原理讲解。
