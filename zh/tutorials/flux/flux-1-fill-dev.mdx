...
## Flux.1 Fill dev 局部重绘工作流

### Flux.1 Inpaint (`flux_fill_inpaint_example`)

使用 Flux.1 Fill 局部重绘填充图像中缺失的部分。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/flux_fill_inpaint_example-1.webp" alt="Flux.1 局部重绘工作流预览" />
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  ...
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/flux_fill_inpaint_example.png" alt="Flux.1 局部重绘输出示例" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
</div>

### 1. 局部重绘工作流与素材

<Note>
对应的图像已经包含透明度通道，因此无需单独绘制遮罩。
如需自行绘制遮罩，请[点击此处](https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/flux/inpaint/flux_fill_inpaint_input_original.png)获取不带遮罩的图像，并参考 [ComfyUI 布局局部重绘示例](/zh/tutorials/basic/inpaint#using-the-mask-editor)中的遮罩编辑器使用说明，学习如何在 `LoadImage` 节点中绘制遮罩。
</Note>

### 2. 运行工作流的步骤

![ComfyUI Flux.1 Fill dev 局部重绘工作流](/images/tutorial/flux/flow_diagram_inpaint.jpg)
...
4. 将文档中提供的输入图像上传到 `Load Image` 节点；如果使用的是不带遮罩的版本，请记得使用遮罩编辑器完成遮罩的绘制。
...
<CardGroup cols={2}>
  <Card title="flux_fill_outpaint_example_input_image.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/flux_fill_outpaint_example_input_image.png">
    `LoadImage` node 17 · `flux_fill_outpaint_example_input_image.png`
  </Card>
</CardGroup>
...
### 1. Outpainting 工作流与资源

### 2. 运行工作流的步骤

![ComfyUI Flux.1 Fill dev Outpainting 工作流](/images/tutorial/flux/flow_diagram_outpaint.jpg)

1. 请确保 `Load Diffusion Model` 节点已加载 `flux1-fill-dev.safetensors`。
2. 请确保 `DualCLIPLoader` 节点已加载以下模型：
   - clip_name1: `t5xxl_fp16.safetensors`
   - clip_name2: `clip_l.safetensors`
3. 请确保 `Load VAE` 节点已加载 `ae.safetensors`。
