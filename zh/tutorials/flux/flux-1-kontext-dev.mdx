...
目前在 ComfyUI 中，你可以使用所有的这些版本，其中 [Pro 及 Max 版本](/zh/tutorials/partner-nodes/black-forest-labs/flux-1-kontext) 可以通过合作节点来进行调用，而 Dev 开源版本请参考本篇指南中的说明。
...
<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/flux_kontext_dev_basic-1.webp" alt="Flux Kontext Dev 工作流预览" />
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/flux_kontext_dev_basic_input_image.jpg" alt="输入图像" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/flux_kontext_dev_basic.png" alt="Flux Kontext Dev 输出示例" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
</div>
...
智能图像编辑，保持角色一致，仅编辑特定部分而不影响其他区域，并保留原始风格。
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/flux_kontext_dev_basic_input_image.jpg" alt="输入图像" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/flux_kontext_dev_basic.png" alt="Flux Kontext Dev 示例输出" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
</div>

此工作流使用 `Load Image(from output)` 节点加载要编辑的图像，方便您对编辑后的图像进行多轮编辑。

### 1. 工作流和输入图像下载

### 2. 逐步完成工作流

![工作流步骤指南](/images/tutorial/flux/flux_1_kontext_dev_basic_step_guide.jpg)
您可以参考图像中的编号来完成工作流运行：

1. 在 `Load Diffusion Model` 节点中，加载 `flux1-dev-kontext_fp8_scaled.safetensors` 模型
...
5. 在 `CLIP Text Encode` 节点中，修改提示词，仅支持英文
6. 点击 `Queue` 按钮，或使用快捷键 `Ctrl(cmd) + Enter` 来运行工作流
