...
1. 确保在 `Load Diffusion Model` 节点中加载了 `flux1-dev.safetensors`
2. 确保在 `LoraLoaderModelOnly` 节点中加载了 `flux1-depth-dev-lora.safetensors`
3. 确保在 `DualCLIPLoader` 节点中加载了以下模型：
   - clip_name1: t5xxl_fp16.safetensors
   - clip_name2: clip_l.safetensors
4. 在 `Load Image` 节点中上传所提供的输入图像
5. 确保在 `Load VAE` 节点中加载了 `ae.safetensors`
...
借助 [ComfyUI-Advanced-ControlNet](https://github.com/Kosinkadink/ComfyUI-Advanced-ControlNet) 或 [ComfyUI ControlNet aux](https://github.com/Fannovel16/comfyui_controlnet_aux) 完成图像预处理
