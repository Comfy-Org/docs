...
Cosmos-Predict2 支持文本到图像（Text2Image）和视频到世界（Video2World）等多种生成方式，广泛应用于工业仿真、自动驾驶、城市规划、科学研究等领域。
...
本篇指南将引导你完成在 ComfyUI 中的**文生图**工作流。

对于视频生成部分，请参考下面的部分：

<Card title="Cosmos Predict2 视频生成" icon="book" href="/zh/tutorials/video/cosmos/cosmos-predict2-video2world">
  使用 Cosmos-Predict2 进行视频生成
</Card>
...
测试 2B 版本时，显示它约使用 16GB 显存。

### 1. 工作流文件

请下载下方图像并拖拽到 ComfyUI 中以加载工作流。该工作流已内置模型下载链接。

![输入图像](https://raw.githubusercontent.com/Comfy-Org/example_workflows/refs/heads/main/image/cosmos/predict2/cosmos_predict2_2B_t2i.png)

### 2. 手动模型安装

如果模型下载未成功，你可以通过本部分手动下载。
...
4. 在 `EmptySD3LatentImage` 中设置图像尺寸
5. 在 `ClipTextEncode` 节点中修改提示词
6. 点击 `Run` 按钮，或者使用快捷键 `Ctrl(cmd) + Enter` 来运行工作流
7. 生成完成后，图像会自动保存到 `ComfyUI/output/` 目录下，你也可以在 `save image` 节点中预览。 */}