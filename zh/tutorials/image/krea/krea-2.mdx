...
**图像风格参考（Krea-2 Turbo）** 子图暴露了以下控制项：

| 控制项 | 描述 |
|---------|-------------|
| **图像输入** | 通过 LoadImage 节点连接参考图像（最多支持 10 张图像） |
| **字符串（用户提示词）** | 描述要生成图像的文本提示词 |
| **prompt_enhance** | 启用/禁用 LLM 驱动的提示词扩展 |
| **LLM_max_token** | 提示词增强的最大令牌长度 |
| **宽度 / 高度** | 输出分辨率（由 ResolutionSelector 控制） |
| **种子** | 用于可重现结果的随机种子 |

### 模型下载

风格参考工作流使用针对风格参考任务优化的 **INT8** 模型变体：

| 模型 | 文件 | 下载 |
|-------|------|----------|
| 扩散模型（INT8） | `krea2_turbo_int8_convrot.safetensors` | [下载](https://huggingface.co/Comfy-Org/Krea-2/blob/main/diffusion_models/krea2_turbo_int8_convrot.safetensors) |
