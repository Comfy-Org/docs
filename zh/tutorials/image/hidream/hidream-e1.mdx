...
### HiDream E1.1 图像编辑 {#hidream_e1_1}

使用 HiDream E1.1 编辑图像。与 HiDream-E1-Full 相比，图像质量和编辑精度更优。
...
下载下面的图片，并将其拖入已加载对应工作流及模型的 ComfyUI 中：
...
4. 在 `Load Image` 节点中加载提供的输入图像或你想要的图像。
5. 在 `Empty Text Encoder(Positive)` 节点中输入 **你想对图像进行的修改**。
6. 在 `Empty Text Encoder(Negative)` 节点中输入 **你不希望出现在图像中的内容**。
...
### HiDream E1 图像编辑 {#hidream_e1_full}
