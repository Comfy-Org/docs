...
### Pixal3D：多视图到3D模型 {#pixal3d-multi-views}

从角色转面图（同一图像上的正面、侧面和背面视图）生成3D模型。多视图模型利用额外视角重建单图管线看不到的几何结构，从而获得更准确的侧面和背面轮廓。

使用模板库中的 `Character Turnaround Sheet Generator` 模板，通过 Nano Banana 2 生成转面图，或者自行准备主体与背景分离的多视图图像。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/3d_pixal3d_multi_views-1.webp" alt="Pixal3D 多视图工作流预览" />

<CardGroup cols={2}>
  <Card title="在Comfy Cloud上运行" icon="cloud" href="https://cloud.comfy.org/?template=3d_pixal3d_multi_views&utm_source=docs&utm_medium=referral&utm_campaign=pixal3d">
    在Comfy Cloud上立即运行此工作流
  </Card>
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/3d_pixal3d_multi_views.json">
    下载JSON或在模板库中搜索 &quot;Pixal3D: Multi Views to 3D&quot;
  </Card>
</CardGroup>
...
<CardGroup cols={1}>
  <Card title="character_turnaround_4view_sheet.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/character_turnaround_4view_sheet.png">
    `LoadImage`节点 364 · `character_turnaround_4view_sheet.png`
  </Card>
</CardGroup>

工作流会使用以下模型。请下载它们并放入对应的 `models/` 子目录中：

<CardGroup cols={2}>
  <Card title="Pixal3D 多视图 UNet" icon="download" href="https://huggingface.co/Comfy-Org/Pixal3D/blob/main/diffusion_models/pixal3d_multiview_int8_convrot.safetensors">
