...
**TripoSplat** 是一款开源模型，能够直接从单张 2D 图像生成 **3D 高斯泼溅（Gaussian splat）** 表示。它由 VAST-AI 开发，并以开源许可证发布。

与需要多视角输入或以网格作为主要输出的传统 3D 重建方法不同，TripoSplat 创建 **高斯泼溅** 表示：这是一种渲染技术，将数千个带颜色的 3D 高斯体分布在空间中来表示场景。这种方法可以实现快速、高质量的渲染，并支持可控的密度和预算。

### TripoSplat：图像转高斯泼溅 {#3d_triposplat_image_to_gaussian_splat}

上传单张 2D 图像，即可生成高质量的 3D 高斯泼溅表示，并以可控的密度和预算进行渲染。
...
<CardGroup cols={2}>
  ...
  <Card title="下载工作流" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/3d_triposplat_image_to_gaussian_splat.json">
    下载 JSON，或在模板库中搜索“TripoSplat：图像转高斯泼溅”
  </Card>
</CardGroup>
