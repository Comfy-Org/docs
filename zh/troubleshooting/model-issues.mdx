...
3. **在 UI 之间共享模型或使用自定义路径：**
   - 参见 [ComfyUI 模型共享或自定义模型文件夹存储位置配置](/zh/installation/comfyui_portable_windows#添加外部模型路径) 以获取详细说明
   - 编辑 `extra_model_paths.yaml` 文件来添加自定义模型目录

### 模型搜索路径配置

如果您的模型位于自定义位置，请参阅详细的 [ComfyUI 模型共享或自定义模型文件夹存储位置配置](/zh/installation/comfyui_portable_windows#添加外部模型路径) 指南，以配置 ComfyUI 找到这些模型。
