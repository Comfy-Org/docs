...
---
<Tip>
   我们收到大量反馈问题，发现绝大多数提交的问题都与自定义节点有关。因此，在提交错误报告之前，请务必阅读[自定义节点故障排查指南](/zh/troubleshooting/custom-node-issues)，以确保问题并非由 ComfyUI 核心问题引起。
</Tip>
...
**快速修复：**
1. **检查系统依赖项** - 确保您的系统符合[最低要求](/zh/installation/system_requirements)
...
有关综合模型故障排查，包括架构不匹配、缺失模型和加载错误，请参见专门的[模型问题](/zh/troubleshooting/model-issues)页面。
...
**Windows AMD 便携版 - 硬编码路径错误：**
AMD 便携版是在 CI 环境 `D:\a\ComfyUI\` 中构建的。如果你解压到不同的驱动器（例如 `C:\ComfyUI_windows_portable\`），像 `offload-arch.exe` 这样的工具可能会失败，出现 `"Fatal error in launcher: Unable to create process"` 或 `access violation` 错误。

解决方案按优先顺序排列：

1. **重新解压便携版** — 删除整个 `ComfyUI_windows_portable_amd` 文件夹。使用 [7-ZIP](https://7-zip.org/) 将 `.7z` 文件重新解压到 **与 CI 构建路径相同的驱动器**。最简单的解决方法是使用 `D:\`（如果可用）。
2. **改用 DirectML** — 如果重新解压不起作用或你没有 D 盘，在便携版环境中安装 torch-directml：
   ```bat
   .\python_embeded\python.exe -m pip install torch-directml
   .\python_embeded\python.exe -s ComfyUI\main.py --directml
   ```
3. **重新下载便携版** — 从 [发布页面](https://github.com/Comfy-Org/ComfyUI/releases) 再次下载最新版本。新的构建可能已修复路径依赖问题。

> **注意：** `torch-directml` 明显比原生 ROCm 构建慢，并且已超过一年未更新。当 AMD ROCm 便携版（默认）可用时，强烈推荐使用它。
