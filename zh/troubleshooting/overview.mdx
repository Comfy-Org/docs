...
在 Intel GPU 系统上，此错误并不表示 CUDA 有问题，也无需重新安装 PyTorch。Intel GPU 使用的是 XPU 后端而不是 CUDA。请勿按照上方针对 NVIDIA CUDA 的重新安装步骤操作，这些步骤不适用于 Intel 硬件。
...
如果 `torch.xpu.is_available()` 返回 `True`，ComfyUI 会自动使用 XPU 后端。如果返回 `False`，请使用下面的选项之一安装支持 XPU 的 PyTorch。
