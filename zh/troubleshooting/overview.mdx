...
3. **验证模型文件** - 查看[模型文档](/zh/basic-concepts/models)了解模型设置
...
- **维护页面**：如果下载失败，请检查[镜像设置](/zh/installation/desktop/usage/settings#高级)
...
**Blackwell GPU（`sm_103`）：生成会在第一个注意力层失败**

在 Blackwell 代 NVIDIA GPU 上，PyTorch 默认的缩放点积注意力（SDPA）可能会选择 cuDNN 后端，但该后端没有适用于此架构的有效执行计划：

```
cuDNN Frontend error: No valid execution plans built
```

使用 FLUX 时，问题会出现在 UNet 的第一个注意力块上，因此不会生成任何图像。

```bash
# 使用绕开 SDPA 和 cuDNN 的手写注意力实现
python main.py --use-split-cross-attention

# Blackwell 还需要 CUDA 12.8 或更新版本的 PyTorch 构建；
# 针对 cu121 编译的构建在这些显卡上根本无法运行
```

<Note>
在启动前通过代码禁用 cuDNN SDPA 后端并不起作用（例如在 `sitecustomize.py` 中调用
`torch.backends.cuda.enable_cudnn_sdp(False)`），因为该标记在 CUDA 初始化后不会被保留。
启动参数才是可靠的解决办法。
</Note>
...
在 Intel GPU 上，此错误表示 ComfyUI 尝试使用 CUDA，因为 XPU 后端不可用。请勿按照上方针对 NVIDIA CUDA 的重新安装步骤操作。Intel GPU 使用的是 XPU，而不是 CUDA。

检查 PyTorch 是否能识别 Intel GPU：

```bash
python -c "import torch; print('XPU available:', hasattr(torch, 'xpu') and torch.xpu.is_available())"
```

如果输出 `True`，ComfyUI 会自动使用 XPU 后端。

如果输出 `False`：

1. 确认你使用的是受支持的 Intel Arc GPU（Arc A 系列、Arc B 系列，或搭载 Arc Graphics 的 Core Ultra 处理器）。
2. 安装或更新 [Intel GPU 驱动程序](https://www.intel.com/content/www/us/en/developer/articles/tool/pytorch-prerequisites-for-intel-gpu.html)。
3. 使用下面的选项之一安装支持 XPU 的 PyTorch。不要安装 CUDA 版本的 wheel。
...
#### 对于桌面应用问题
**提交位置：** [桌面 GitHub Issues](https://github.com/Comfy-Org/Comfy-Desktop/issues)
