...
自定义节点通过 Python 类定义，必须包含以下四项：`CATEGORY`（指定新节点在添加节点菜单中的位置）、`INPUT_TYPES`（类方法，定义节点输入，详见[后文](/zh/custom-nodes/backend/server_overview#input-types)）、`RETURN_TYPES`（定义节点输出）、`FUNCTION`（节点执行时调用的函数名）。
