...
<Info>
本页的节点定义使用旧版 V1 schema（`INPUT_TYPES` / `RETURN_TYPES`），目前仍完全支持。新节点也可以用现代的 V3 schema 编写（`io.ComfyNode` 配合 `define_schema` 和 `comfy_entrypoint` 函数），ComfyUI 自带的 [`custom_nodes/example_node.py.example`](https://github.com/comfyanonymous/ComfyUI/blob/master/custom_nodes/example_node.py.example) 就是这样写的。两者的区别及迁移方法请参阅 [V3 迁移指南](/custom-nodes/v3_migration)。
</Info>
