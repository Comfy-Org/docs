---
title: "节点扩展"
description: "掌握 ComfyUI 节点扩展，在后端自定义节点中返回新的子图。了解 GraphBuilder 要求和缓存技巧。"
...
虽然您可以向子图中的节点传递非文字输入（如 torch 张量），但这可能会抑制子图内部的缓存。当可能时，您应该传递子图对象的链接，而不是节点本身。（您可以在输入的[附加参数](./datatypes#附加参数)中声明一个 `rawLink` 来轻松实现这一点。）

## 相关文档

- [子图（开发者指南）](/zh/custom-nodes/js/subgraphs) — 前端扩展开发指南
