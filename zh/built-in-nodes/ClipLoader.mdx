...
更多最新模型文件加载详情请查阅[folder_paths](https://github.com/Comfy-Org/ComfyUI/blob/master/folder_paths.py)
...
| `类型`       | COMBO[STRING] | 确定要加载的 CLIP 模型类型，随着 ComfyUI 支持的模型数量增加这里的类型也会新增对应的类型，请查看[node.py](https://github.com/Comfy-Org/ComfyUI/blob/master/nodes.py)中源码里关于`CLIPLoader` 类的相关定义|
...
未来随着 ComfyUI 的更新，这个更新搭配可能会新增，详情请参考 [node.py](https://github.com/Comfy-Org/ComfyUI/blob/master/nodes.py)中源码里关于`CLIPLoader` 类的相关定义说明
