...
账号系统是为了支持 `Partner Nodes` 节点而新增的，`Partner Nodes`支持了对闭源模型 API 的调用，这大大扩展了 ComfyUI 的可能性，由于对应的 API 调用需要消耗 Token，所以我们增加了对应的用户系统。
...
<Card title="使用 API Key 进行登录" icon="key" href="/zh/account/login#使用-api-key-进行登录">
