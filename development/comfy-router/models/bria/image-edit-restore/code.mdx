---
title: "Use Image Edit Restore with Comfy Router"
description: "Call bria/image-edit-restore through Comfy Router: endpoint, request shape and the response Router returns."
...
API Reference for `bria/image-edit-restore`, served by Comfy Router from Bria.
...
**Model ID:** `bria/image-edit-restore`

**Endpoint:** `POST https://api.comfy.org/v2/models/bria/image-edit-restore`
...
<ParamField body="image" type="string" required>
  The photo to restore. Supported input types are Base64-encoded string or URL pointing to a publicly accessible image file. Accepted formats JPEG, JPG, PNG, WEBP.
</ParamField>

Generated from the schema Router serves at `GET /v2/models/bria/image-edit-restore/openapi.json`, the same document it validates a call against before the request reaches the provider.
