---
title: "Use Image Edit Add Object By Text with Comfy Router"
description: "Call bria/image-edit-add-object-by-text through Comfy Router: endpoint, request shape and the response Router returns."
...
API Reference for `bria/image-edit-add-object-by-text`, served by Comfy Router from Bria.
...
<ParamField body="instruction" type="string" required>
  A text instruction that describes the object to add and its position (for example, "Place a red vase with flowers on the table").
</ParamField>

Generated from the schema Router serves at `GET /v2/models/bria/image-edit-add-object-by-text/openapi.json`, the same document it validates a call against before the request reaches the provider.
