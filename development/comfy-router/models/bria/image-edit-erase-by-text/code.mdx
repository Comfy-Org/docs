---
title: "Use Image Edit Erase By Text with Comfy Router"
description: "Call bria/image-edit-erase-by-text through Comfy Router: endpoint, request shape and the response Router returns."
...
API Reference for `bria/image-edit-erase-by-text`, served by Comfy Router from Bria.
...
<ParamField body="object_name" type="string" required>
  A text description of the object to remove (for example, "the lamp").
</ParamField>

Generated from the schema Router serves at `GET /v2/models/bria/image-edit-erase-by-text/openapi.json`, the same document it validates a call against before the request reaches the provider.
