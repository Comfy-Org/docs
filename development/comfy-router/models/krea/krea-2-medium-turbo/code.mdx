---
title: "Use Krea 2 Medium Turbo with Comfy Router"
description: "Call krea/krea-2-medium-turbo through Comfy Router: endpoint, request shape and the response Router returns."
sidebarTitle: "Krea 2 Medium Turbo"
---

{/* GENERATED FILE. Generated from router-schemas/krea/krea-2-medium-turbo.json by `pnpm code-pages:gen`. */}

import RouterCodeFooter from "/snippets/comfy-router/model-code-footer.mdx";

Use `krea/krea-2-medium-turbo` with the Comfy Router API. Krea provides the model; Router gives it the shared authentication and request route below.

## Quick start

Create a key at [platform.comfy.org/profile/api-keys](https://platform.comfy.org/profile/api-keys) and export it as `COMFY_API_KEY`. The Python and TypeScript snippets use the Comfy SDKs (`pip install comfy-sdk`, `npm install @comfyorg/sdk`); the cURL snippet is the same call over raw HTTP.

**Model ID:** `krea/krea-2-medium-turbo`

**Endpoint:** `POST https://api.comfy.org/v2/models/krea/krea-2-medium-turbo`

<CodeGroup>
```python Python
import uuid
from comfy_sdk import Comfy

# Reads COMFY_API_KEY from the environment. Store this UUID before starting a
# generation. Use the same value if you retry it.
idempotency_key = str(uuid.uuid4())
with Comfy() as client:
    result = client.models.run(
        "krea/krea-2-medium-turbo",
        {
            "aspect_ratio": "1:1",
            "prompt": "a red circle",
            "resolution": "1K",
        },
        idempotency_key=idempotency_key,
        timeout=660.0,
    )

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// Reads COMFY_API_KEY from the environment.
// The SDK automatically creates an idempotency key and reuses it for automatic retries.
const { data } = await comfy.models.run("krea/krea-2-medium-turbo", {
  aspect_ratio: "1:1",
  prompt: "a red circle",
  resolution: "1K",
});

console.log(data);
```

```bash cURL
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 https://api.comfy.org/v2/models/krea/krea-2-medium-turbo \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"aspect_ratio\": \"1:1\", \"prompt\": \"a red circle\", \"resolution\": \"1K\"}"
```
</CodeGroup>

## Schema

### Input

<ParamField body="aspect_ratio" type="string" required>
  Aspect ratio. One of: 1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16.

  Possible values: `1:1`, `4:3`, `3:2`, `16:9`, `2.35:1`, `4:5`, `2:3`, `9:16`
</ParamField>

<ParamField body="creativity" type="string" default="&quot;medium&quot;">
  Prompt interpretation strength: raw=0, low=10, medium=50, high=100.

  Possible values: `raw`, `low`, `medium`, `high`
</ParamField>

<ParamField body="image_style_references" type="object[]">
  Style references to use for the generation
</ParamField>

<ParamField body="image_style_references[].strength" type="number" required>
  Range: `-2` to `2`

  Format: `double`
</ParamField>

<ParamField body="image_style_references[].url" type="string (uri)">
  Format: `uri`
</ParamField>

<ParamField body="moodboards" type="object[]">
  Moodboards to use for generation. Currently limited to one moodboard.
</ParamField>

<ParamField body="moodboards[].id" type="string (uuid)" required>
  Format: `uuid`
</ParamField>

<ParamField body="moodboards[].strength" type="number" default="0.35">
  Range: `-0.5` to `1.5`

  Format: `double`
</ParamField>

<ParamField body="prompt" type="string" required>
   
</ParamField>

<ParamField body="resolution" type="string" required>
  Resolution scale. One of: 1K.

  Possible values: `1K`
</ParamField>

<ParamField body="seed" type="number">
   
</ParamField>

<ParamField body="styles" type="object[]">
  Styles (typically LoRAs) to use for the generation
</ParamField>

<ParamField body="styles[].id" type="string" required>
   
</ParamField>

<ParamField body="styles[].strength" type="number" required>
  Range: `-2` to `2`

  Format: `double`
</ParamField>

Generated from the schema Router serves at `GET /v2/models/krea/krea-2-medium-turbo/openapi.json`, the same document it validates a call against before the request reaches the provider.

### Output

<ResponseField name="completed_at" type="string (date-time)" required>
  Format: `date-time`
</ResponseField>

<ResponseField name="created_at" type="string (date-time)" required>
  Format: `date-time`
</ResponseField>

<ResponseField name="job_id" type="string (uuid)" required>
  Format: `uuid`
</ResponseField>

<ResponseField name="result" type="object" required>
  The finished generation. Not nullable here, unlike on `KreaJob`: a terminal job whose `result` carries no `urls` is answered as a Comfy Router error, so a `200` always carries it.
</ResponseField>

<ResponseField name="result.style_id" type="string">
  Set by loraTraining jobs rather than by a generation; a `200` for these models carries `urls`.
</ResponseField>

<ResponseField name="result.urls" type="string (uri)[]" required>
  The generated images, as downloadable links. At least one, because a `completed` job with an empty `urls` is `success_without_output` and never reaches this document.
</ResponseField>

<ResponseField name="status" type="string" required>
  Always `completed` on this document: it is the only status `classifyKrea` answers `succeeded` on, and Krea's vocabulary is matched without a case fold.

  Possible values: `completed`
</ResponseField>

## Examples

### Input

```json
{
  "aspect_ratio": "1:1",
  "prompt": "a red circle",
  "resolution": "1K"
}
```

### Output

Illustrative response from the published schema.

```json
{
  "completed_at": "2027-01-01T00:00:37Z",
  "created_at": "2027-01-01T00:00:00Z",
  "job_id": "7f1c2e84-5b90-4a37-8d61-2c0f9ab4e153",
  "result": {
    "urls": [
      "https://example.invalid/krea/krea-2/generated.png"
    ]
  },
  "status": "completed"
}
```


<RouterCodeFooter />
