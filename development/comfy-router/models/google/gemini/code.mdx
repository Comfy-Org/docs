...
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/vertexai/gemini-3.1-pro-preview \
...
# Save this key and reuse it when retrying this request.
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/vertexai/gemini-3.5-flash \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"contents\": [{\"role\":\"user\",\"parts\":[{\"text\":\"Describe a single red maple leaf on a white background in one sentence.\"}]}], \"generationConfig\": {\"temperature\":0.7,\"maxOutputTokens\":256}}"
...
# Save this key and reuse it when retrying this request.
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/vertexai/gemini-2.5-pro \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"contents\": [{\"role\":\"user\",\"parts\":[{\"text\":\"Describe a single red maple leaf on a white background in one sentence.\"}]}], \"generationConfig\": {\"temperature\":0.7,\"maxOutputTokens\":256}}"
...
# Save this key and reuse it when retrying this request.
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/vertexai/gemini-2.5-flash \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"contents\": [{\"role\":\"user\",\"parts\":[{\"text\":\"Describe a single red maple leaf on a white background in one sentence.\"}]}], \"generationConfig\": {\"temperature\":0.7,\"maxOutputTokens\":256}}"
...
Generated from the schema Router serves at `GET /v2/models/vertexai/gemini-3.1-pro-preview/openapi.json`, the same document it validates a call against before the request reaches the provider.
