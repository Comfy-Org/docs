...
**`fileData` can appear on a response that did not ask for it.** The two shapes are per part, not per response: when `uploadImagesToStorage: true` is set, an image whose upload fails is left as `inlineData`, so one response can mix both. Branch on which key is present rather than on what you requested. The one case that cannot happen is the reverse: with the field unset or false, every generated image comes back as `inlineData` and no `fileData` image part is produced. Text parts can also appear, and the image is not guaranteed to be the first part, so select a part by the field you need rather than by index.
...
Feeding a previously generated image straight back in as an `inlineData` part is the way to get a new camera angle on the same subject. Architecture, materials and lighting hold across generations, so a sequence of shots of one scene is a chain of calls rather than one prompt that has to describe everything at once. Take the base64 bytes out of the first response's `inlineData.data`, send them back unchanged as a `user` part, and ask for the change you want:
