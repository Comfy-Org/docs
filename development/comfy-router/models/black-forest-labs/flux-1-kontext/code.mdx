...
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/bfl/flux-kontext-pro \
...
# Save this key and reuse it when retrying this request.
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/bfl/flux-kontext-max \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"prompt\": \"replace the background with a sunlit beach, keep the subject unchanged\", \"input_image\": \"$INPUT_IMAGE\", \"aspect_ratio\": \"1:1\"}"
