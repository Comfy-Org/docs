...
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/bfl/flux-pro-1.1-ultra \
...
# Save this key and reuse it when retrying this request.
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 --dump-header router-headers.txt https://api.comfy.org/v2/models/bfl/flux-pro-1.1 \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
