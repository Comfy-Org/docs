...
Use `openai/gpt-6-astra` with the Comfy Router API. OpenAI provides the model; Router gives it the shared authentication and request route below.
...
curl --max-time 660 https://api.comfy.org/v2/models/openai/gpt-6-astra \
...
Generated from the schema Router serves at `GET /v2/models/openai/gpt-6-astra/openapi.json`, the same document it validates a call against before the request reaches the provider.
