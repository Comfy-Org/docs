...
The video URL expires. Router re-hosts the MP4 and answers a Comfy-signed URL valid for 12 hours, falling back to MiniMax's own shorter-lived link when the re-host fails, and there is no task-query route to refresh either one with. Download promptly rather than storing the link.
