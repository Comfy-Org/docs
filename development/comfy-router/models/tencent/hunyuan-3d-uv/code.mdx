---
title: "Use Hunyuan 3D Uv with Comfy Router"
description: "Call tencent/hunyuan-3d-uv through Comfy Router: endpoint, request shape and the response Router returns."
sidebarTitle: "Hunyuan 3D Uv"
---

{/* GENERATED FILE. Generated from router-schemas/tencent/hunyuan-3d-uv.json by `pnpm code-pages:gen`. */}

import RouterCodeFooter from "/snippets/comfy-router/model-code-footer.mdx";

Use `tencent/hunyuan-3d-uv` with the Comfy Router API. Tencent provides the model; Router gives it the shared authentication and request route below.

## Quick start

Create a key at [platform.comfy.org/profile/api-keys](https://platform.comfy.org/profile/api-keys) and export it as `COMFY_API_KEY`. The Python and TypeScript snippets use the Comfy SDKs (`pip install comfy-sdk`, `npm install @comfyorg/sdk`); the cURL snippet is the same call over raw HTTP.

**Model ID:** `tencent/hunyuan-3d-uv`

**Endpoint:** `POST https://api.comfy.org/v2/models/tencent/hunyuan-3d-uv`

<CodeGroup>
```python Python
import uuid
from comfy_sdk import Comfy

# Reads COMFY_API_KEY from the environment. Store this UUID before starting a
# generation. Use the same value if you retry it.
idempotency_key = str(uuid.uuid4())
with Comfy() as client:
    result = client.models.run(
        "tencent/hunyuan-3d-uv",
        {
            "File": {
                "Type": "GLB",
                "Url": "https://vcg-test-1258344699.cos.ap-guangzhou.myqcloud.com/test/3d/test.glb",
            },
        },
        idempotency_key=idempotency_key,
        timeout=660.0,
    )

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// Reads COMFY_API_KEY from the environment. Store this UUID before starting a
// generation. Use the same value if you retry it.
const idempotencyKey = crypto.randomUUID();
const { data } = await comfy.models.run("tencent/hunyuan-3d-uv", {
  File: {
    Type: "GLB",
    Url: "https://vcg-test-1258344699.cos.ap-guangzhou.myqcloud.com/test/3d/test.glb",
  },
}, { idempotencyKey, timeoutMs: 660_000 });

console.log(data);
```

```bash cURL
ROUTER_REQUEST_KEY=$(uuidgen)
curl --max-time 660 https://api.comfy.org/v2/models/tencent/hunyuan-3d-uv \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $ROUTER_REQUEST_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"File\": {\"Type\":\"GLB\",\"Url\":\"https://vcg-test-1258344699.cos.ap-guangzhou.myqcloud.com/test/3d/test.glb\"}}"
```
</CodeGroup>

## Schema

### Input

<ParamField body="File" type="object">
  3D file input for UV unwrapping
</ParamField>

<ParamField body="File.Type" type="string" required>
  3D file format type

  Possible values: `FBX`, `OBJ`, `GLB`
</ParamField>

<ParamField body="File.Url" type="string (uri)" required>
  URL of the 3D file that needs UV unwrapping

  Format: `uri`
</ParamField>

Generated from the schema Router serves at `GET /v2/models/tencent/hunyuan-3d-uv/openapi.json`, the same document it validates a call against before the request reaches the provider.

### Output

Router does not publish an output schema for this model.

## Examples

### Input

```json
{
  "File": {
    "Type": "GLB",
    "Url": "https://vcg-test-1258344699.cos.ap-guangzhou.myqcloud.com/test/3d/test.glb"
  }
}
```


<RouterCodeFooter />
