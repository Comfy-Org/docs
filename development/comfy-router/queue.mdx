...
**The synchronous route is bounded at 10 minutes.** That bound is the reason this page exists: Router's default deadline is 10 minutes (configurable per deployment), and a synchronous call that reaches it is cut off with `504` / `deadline_exceeded` whether or not the provider is still working. A generation that can run longer than that, or a caller that cannot hold a connection open for ten minutes, has to queue. See [Calls are cut off at a server deadline](/development/comfy-router/limitations#calls-are-cut-off-at-a-server-deadline) for what the deadline does and does not tell you about billing.
