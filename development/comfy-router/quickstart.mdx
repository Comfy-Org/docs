...
Queued delivery is in a gated preview, scoped to the workspace behind your key. A key created in [your Comfy workspace](https://platform.comfy.org/profile/api-keys) carries that workspace; until the workspace is enabled, the submit route answers `403` with `not_enabled`.
