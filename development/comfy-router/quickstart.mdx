...
This example is text to image. To send an image *as input* (image to image, editing, upscaling), see [Image to image with Comfy Router](/development/comfy-router/image-to-image): upload the image as an asset and pass its URL, or inline it as base64, depending on the model's input schema.
