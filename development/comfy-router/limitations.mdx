...
Queued delivery returns a `request_id` and URLs for status, result collection, and cancellation. It is enabled per caller; callers without access receive `403` with `not_enabled`. Cancellation is best effort, and Router does not provide a completion webhook. See [Queued delivery](/development/comfy-router/queue) for examples and the full lifecycle.
