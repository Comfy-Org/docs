...
| Submit a job and collect it later | Queued delivery: `POST /v2/models/{provider}/{model}/requests` returns a `request_id` to poll and collect. No completion webhook. | Use [queued delivery](/development/comfy-router/api#queued-delivery). A partner proxy with submit-and-poll operations exposes the provider's own controls. |
