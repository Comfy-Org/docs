...
<ResponseField name="403" type="RouterErrorResponse">
  The request is not allowed for this caller or model. A model whose partner answers a generation directly as bytes cannot yet be queued and is refused with `not_enabled`; run it on the synchronous route instead.
