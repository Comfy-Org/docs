...
`503` / `service_unavailable` is temporary. Retry with backoff; do not treat it as an empty catalog. Cache catalog reads. SDK run methods call the selected model directly; they do not list models first.
