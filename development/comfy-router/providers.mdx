---
title: "Comfy Router provider coverage"
sidebarTitle: "Provider coverage"
description: "See which providers serve each Comfy Router model, how to select an alternate provider, and which provider-specific model IDs are available."
---

{/* GENERATED FILE. Generated from the Router catalog by `pnpm code-pages:gen`. */}

<div className="router-provider-coverage-marker" />

Comfy Router uses one endpoint for every model: `POST /v2/models/{provider}/{model}`. Comfy serves each model by default. Some models are also available through alternate providers. The model ID, request body, and response format stay the same when you choose one. See [`model_provider`, `strict_mode` and `fallback_provider`](/development/comfy-router/reference#post-v2modelsprovidermodel) in the API reference.

<Note>
To use Comfy, omit `model_provider`. To choose an alternate provider, add `?model_provider=<provider>`. The matrix compares every model with alternate-provider coverage.
</Note>

## Provider coverage

| Provider / model | [Dreamina Seedance 2.0 260128](/development/comfy-router/models/byteplus/dreamina-seedance-2-0-260128/code) | [Dreamina Seedance 2.5 260628](/development/comfy-router/models/byteplus/dreamina-seedance-2-5-260628/code) | [GPT Image 2](/development/comfy-router/models/openai/gpt-image-2/code) | [Kling 3.0 Turbo](/development/comfy-router/models/kling/kling-3-0-turbo/code) | [Kling V3](/development/comfy-router/models/kling/kling-v3/code) | [Nano Banana 2](/development/comfy-router/models/google/nano-banana-2/code) | [Nano Banana Pro](/development/comfy-router/models/google/nano-banana-pro/code) | [Wan 3.0 Video](/development/comfy-router/models/wan/wan3-0-video/code) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **Comfy (default)** | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| **fal** | `fal/fal-seedance-2.0` | `fal/fal-seedance-2.5` | `fal/fal-gpt-image-2` | - | - | `fal/fal-nano-banana-2` | `fal/fal-nano-banana-pro` | - |
| **Higgsfield** | - | - | - | `higgsfield/higgsfield-kling-3-turbo` | `higgsfield/higgsfield-kling-3-std` | - | - | `higgsfield/higgsfield-wan-3` |
| **Runware** | - | - | `runware/runware-gpt-image-2` | - | - | `runware/runware-nano-banana-2` | `runware/runware-nano-banana-pro` | - |
| **WaveSpeed** | - | - | - | - | - | - | `wavespeed/wavespeed-nano-banana-pro` | - |

Model names link to their native Code pages. Alternate-provider cells show the provider's alias model ID. A `-` means that provider does not serve that model.

## Try an alternate provider

These examples send [Nano Banana Pro](/development/comfy-router/models/google/nano-banana-pro/code) through **Runware**. They use the model's native ID and request body; only the provider selection changes.

<CodeGroup>
```python Python
from comfy_sdk import Comfy

# Reads COMFY_API_KEY from the environment.
# The SDK automatically creates an idempotency key and reuses it for automatic retries.
with Comfy() as client:
    result = client.models.run(
        "vertexai/gemini-3-pro-image",
        {
            "contents": [
                {
                    "role": "user",
                    "parts": [
                        {
                            "text": "a single red maple leaf on a plain white background, studio lighting",
                        },
                    ],
                },
            ],
            "generationConfig": {
                "responseModalities": ["IMAGE"],
                "imageConfig": {
                    "aspectRatio": "1:1",
                },
            },
        },
        model_provider="runware",
    )

print("image (base64):", result["candidates"][0]["content"]["parts"][0]["inlineData"]["data"])
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// Reads COMFY_API_KEY from the environment.
// The SDK automatically creates an idempotency key and reuses it for automatic retries.
type Result = { candidates: { content: { parts: { inlineData: { data: string } }[] } }[] };
const result = await comfy.models.run<Result>("vertexai/gemini-3-pro-image", {
  contents: [
    {
      role: "user",
      parts: [
        {
          text: "a single red maple leaf on a plain white background, studio lighting",
        },
      ],
    },
  ],
  generationConfig: {
    responseModalities: ["IMAGE"],
    imageConfig: {
      aspectRatio: "1:1",
    },
  },
}, {
  modelProvider: "runware",
});
if (result.kind !== "json") throw new Error("expected a JSON result");

console.log("image (base64):", result.data.candidates[0].content.parts[0].inlineData.data);
```

```swift Swift
import Foundation
import ComfySwiftSDK

// Reads COMFY_API_KEY from the environment.
// The SDK mints an idempotency key per call and reuses it for automatic retries.
let client = ComfyCloudClient(apiKey: ProcessInfo.processInfo.environment["COMFY_API_KEY"]!)
let result = try await client.models.run(
    "vertexai/gemini-3-pro-image",
    input: [
        "contents": [
            [
                "role": "user",
                "parts": [
                    [
                        "text": "a single red maple leaf on a plain white background, studio lighting",
                    ],
                ],
            ],
        ],
        "generationConfig": [
            "responseModalities": ["IMAGE"],
            "imageConfig": [
                "aspectRatio": "1:1",
            ],
        ],
    ],
    modelProvider: "runware"
)

print("image (base64):", result.output["candidates"][0]["content"]["parts"][0]["inlineData"]["data"].stringValue ?? "")
```

```bash cURL
curl https://api.comfy.org/v2/models/vertexai/gemini-3-pro-image?model_provider=runware \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"contents\": [{\"role\":\"user\",\"parts\":[{\"text\":\"a single red maple leaf on a plain white background, studio lighting\"}]}], \"generationConfig\": {\"responseModalities\":[\"IMAGE\"],\"imageConfig\":{\"aspectRatio\":\"1:1\"}}}"
```
</CodeGroup>

Provider selection is available on the synchronous route. The queued `/requests` route does not accept `model_provider`; see the [queued delivery guide](/development/comfy-router/queue) for queued requests.

## Request compatibility

When `strict_mode=false` (the default), Router translates the native request for the selected provider and translates the response back. If a native field cannot be represented, Router drops it and names it in the `X-Comfy-Router-Dropped-Params` response header. See [`model_provider`, `strict_mode` and `fallback_provider`](/development/comfy-router/reference#post-v2modelsprovidermodel) for the other routing options.

<CardGroup cols={2}>
  <Card title="Model catalog" icon="list" href="/development/comfy-router/models">
    Browse every model in the Comfy Router catalog.
  </Card>
  <Card title="Router API reference" icon="code" href="/development/comfy-router/api">
    Read the routing, request, response, retry, and billing details.
  </Card>
</CardGroup>
