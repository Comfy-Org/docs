...
- **Top-level keys** are numeric node IDs (`"3"`, `"4"`, `"8"`)
- Each node has `inputs` (with links as `[node_id, output_index]`) and `class_type`
- No `x`, `y`, `width` fields — UI-only data is stripped

You can also load an API-format workflow back into the frontend (`File → Load` or drag-and-drop), but node positions will be auto-arranged.
