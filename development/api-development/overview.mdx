...
For a new integration, use the **Comfy SDKs** and the Comfy API v2 they call. It is the one API we version and commit to supporting, and the same code runs against Comfy Cloud and against your own machine.
...
| **Authentication** | `Authorization: Bearer` on Cloud, none self-hosted | `X-API-Key` header (Comfy Cloud account) | None (local) or API key for Partner Nodes |
