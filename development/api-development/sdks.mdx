...
| **Your own ComfyUI** | `http://127.0.0.1:8189` (the local proxy) | Not used |
...
Then point the SDK at `http://127.0.0.1:8189` and pass no API key. The proxy binds to loopback only by default. Run it with `--comfyui-base-dir /path/to/ComfyUI` if you also want to upload model files into your install.
