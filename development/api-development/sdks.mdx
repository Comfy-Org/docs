...
```bash
export COMFY_BASE_URL="https://<deployment>.run.comfy.app"  # serverless deployment
...
A Serverless deployment can run multiple workflows with the models and custom nodes included in its Build. For each job, `get_workflow()` returns a workflow response with `format: "api"` and the executed graph in its `.graph` property.
