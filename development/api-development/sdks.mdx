...
| `handle.events()` (TypeScript) / `handle.iter_events()` (Python) | [Comfy Router queued delivery](/development/comfy-router/api#queued-delivery) | Queue observations for one Router request: status and queue position. It polls the status route; there is no stream and there are no progress or preview events. |
| `comfy.models.subscribe(...)` | [Comfy Router queued delivery](/development/comfy-router/api#queued-delivery) | Not an iterator. Submit, poll and collect in one call, with an `on_queue_update` / `onQueueUpdate` callback, resolving to the result. |
