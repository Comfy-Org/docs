...
A Serverless deployment can run multiple workflows with the models and custom nodes included in its Build. For each job, `get_workflow()` returns the executed graph (`format: "api"`).
