...
<Warning>
  Comfy API deployments are in beta. Sign up for access at [platform.comfy.org](https://platform.comfy.org).
</Warning>
...
<AccordionGroup>
  <Accordion title="What does a Comfy API deployment provide?">
    A Comfy API deployment is a managed, autoscaling endpoint for running API-format workflows. Submit a workflow with `comfy deploy run` or the [Comfy SDKs](/development/api-development/sdks).
  </Accordion>
