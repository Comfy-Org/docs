...
<AccordionGroup>
  <Accordion title="Is this an interactive ComfyUI, or an API endpoint?">
    A Serverless deployment is an API endpoint for the workflow you deployed. Submit an API-format workflow with `comfy deploy run` or the [Comfy SDKs](/development/api-development/sdks); it does not provide an interactive hosted ComfyUI interface.
  </Accordion>

  <Accordion title="What will I pay for a deployment?">
    Builds and releases are stored at no charge until you deploy them. Once a release is deployed, its models are staged on network storage in that region and billed per GB-month until the last deployment using that Build in the region is deleted. Pausing a deployment keeps that storage.

    `--min` sets workers that stay running and are billed per second, even while idle. Workers above `--min`, up to `--max`, are flex workers: they are billed only while running, including startup and a short idle window. With `--min 0`, compute scales to zero while idle, but the next request has a cold start.

    Each running worker has an ephemeral 50 GB container disk as part of compute. For current storage and GPU rates, see [comfy.org/pricing](https://www.comfy.org/pricing) and run `comfy deploy refs compute` for the available region and GPU pairs.
  </Accordion>

  <Accordion title="What should I do if provisioning takes a long time or a GPU is unavailable?">
    Start by checking the deployment state and logs:

    ```bash
    comfy deploy status --watch
    comfy deploy logs --deployment <deployment-id>
    comfy deploy events --deployment <deployment-id>
    ```

    Capacity changes by region and GPU. Run `comfy deploy refs compute`, choose a listed pair, and retry later or choose another pair if capacity is not available. Keep the Build and deployment IDs when asking for help.
  </Accordion>
</AccordionGroup>
