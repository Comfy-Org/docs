...
<AccordionGroup>
  <Accordion title="What does a Serverless deployment provide?">
    A Serverless deployment is a managed, autoscaling endpoint for running API-format workflows. Submit a workflow with `comfy deploy run` or the [Comfy SDKs](/development/api-development/sdks).
  </Accordion>

  <Accordion title="Can one Build run multiple workflows?">
  ...
  <Accordion title="How does a deployment handle multiple requests?">
    A deployment automatically distributes requests across its workers and scales within the configured `--min` and `--max` bounds. Requests that cannot run immediately are queued and evaluated in the order they arrived.
  </Accordion>
  ...
  <Accordion title="What should I do if provisioning takes a long time?">
    ...
    If the selected GPU or region has no capacity, run `comfy deploy refs compute` and choose an available region and GPU pair. Include the Build, release, and deployment IDs when asking for help.
  </Accordion>
