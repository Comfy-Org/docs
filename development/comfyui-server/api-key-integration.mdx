...
<Note>
  **Important:** This page describes the **ComfyUI Account API Key** used for accessing paid API nodes in workflows. This is **NOT** the same as:
  - The **Registry Publishing API Key** (used by developers to publish custom nodes - see [Publishing Nodes](/registry/publishing))
  - The **ComfyUI Server API** (the native REST API for workflow execution - see [Server Communication](/development/comfyui-server/comms_overview))
</Note>
