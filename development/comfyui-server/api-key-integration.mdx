...
Starting from [PR #8041](https://github.com/comfyanonymous/ComfyUI/pull/8041), ComfyUI supports directly using built-in API nodes through API Keys, without requiring a specific frontend interface (you can even run without a frontend).
...
<Note>
  **Important:** This page describes the ComfyUI Account API Key used for accessing paid API nodes in workflows. This is different from:
  - The Registry Publishing API Key (used by developers to publish custom nodes)
  - The ComfyUI Server API (the native REST API for workflow execution)
  
  See [Understanding Different API Keys](/registry/overview#understanding-different-api-keys-in-comfyui) for clarification.
</Note>
