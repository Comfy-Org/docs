...
| `--fast-disk` | auto | Force disk-backed dynamic loading over unpinned RAM. Useful with fast NVMe. Enabled automatically when the model files are on storage detected as fast. |
| `--disable-fast-disk` | disabled | Disable disk-backed dynamic loading. Overrides `--fast-disk` and the automatic detection. |
