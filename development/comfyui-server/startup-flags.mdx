...
**Default behavior on Nvidia GPUs:** Async weight offloading (`--async-offload`) and dynamic VRAM (`--enable-dynamic-vram`) are enabled by default. This means models are automatically offloaded between VRAM and system RAM as needed. On non-Nvidia systems, these features may need to be explicitly enabled.
