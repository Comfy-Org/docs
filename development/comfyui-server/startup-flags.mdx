...
| `--models-directory` `PATH` | ComfyUI root/models | Models directory. Overrides the models folder in `--base-directory`. |
...
| Flag | Default | Description |
|------|---------|-------------|
| `--cuda-device` `DEVICE_ID` | — | CUDA device IDs to use, comma-separated (e.g. `0` or `0,1`), or `all` to keep all currently visible devices. Other devices are hidden. |
...
See the [UNET precision](#unet-precision) section for the explanation of each FP8 format suffix.
...
| `--disable-triton-backend` | disabled | Force-disable the comfy-kitchen Triton backend, overriding `--enable-triton-backend`. |
...
<Note>
  Cache mode flags are mutually exclusive. Only one of `--cache-ram`, `--cache-classic`, `--high-ram`, `--cache-lru`, or `--cache-none` should be used.
</Note>

| Flag | Default | Description |
|------|---------|-------------|
| `--cache-ram` `[GB] [GB]` | enabled (default mode) | RAM pressure caching. First value: active-cache threshold in GB. Optional second value: inactive-cache/pin threshold in GB. When no values given: active = 10% of system RAM (min 2 GB, max 10 GB); inactive = 100% of system RAM (max 128 GB). Accepts at most two values. |
| `--cache-classic` | — | Use the old aggressive caching style. |
| `--high-ram` | disabled | Can improve performance slightly on systems with plenty of RAM or where pagefile use is preferred over reloading models. Also enables classic caching. |
...
<Note>
  ComfyUI reuses cached node results between runs in the same session, which is what makes repeat runs faster. Use `--cache-none` when you would rather every run start from a clean cache, for example while debugging a workflow or ruling out cached reuse between runs. Every node re-executes, so generations take longer. It is a startup flag, so restart ComfyUI for it to take effect.
</Note>
...
| `--use-ck-attention` | Use Comfy Kitchen attention. Requires hardware support for its INT8 kernels. |
...
| `--vram-headroom` `GB` | `0` | Extra VRAM in GB for dynamic VRAM to keep free above the default headroom. ComfyUI tries to keep this much VRAM completely free and unused, even counting VRAM used by other apps. |
| `--disable-nvml-pressure` | disabled | Use CUDA instead of NVML to report dynamic VRAM memory pressure. |
...
| `--disable-fast-disk` | disabled | Disable disk-backed dynamic loading and offload, so unpinned RAM is used instead. Overrides `--fast-disk`. |
| `--disable-cuda-graphs` | disabled | Disable CUDA graphs. |
| `--disable-comfy-compiler` | disabled | Disable the Comfy model compiler, including its CUDA graph subfeature. |
| `--assert-graph-breaks` | disabled | Fail on Comfy model compiler graph breaks. |
...
| `--debug-hang` | disabled | Enable stack trace dumps on Ctrl-C for debugging hangs. |
...
| `--enable-manager-legacy-ui` | Use the legacy Manager UI. Implies `--enable-manager`. Mutually exclusive with `--disable-manager-ui`. |
...
<Note>
  `--disable-api-nodes` also changes what the browser is allowed to load. The flag adds a Content-Security-Policy that keeps the page on resources served by ComfyUI itself, so images, fonts, media, frames, and network requests from other origins are blocked, and API nodes (Partner Nodes) are not registered at all. If a preview or a thumbnail that normally loads goes missing after a restart, check the launch arguments for this flag before troubleshooting the node.
</Note>
...
| `--enable-asset-hashing` | disabled | Compute blake3 content hashes when scanning assets. Enables future asset portability features such as deduplication and cross-machine model resolution, at the cost of startup time and per-output hashing on large model directories. Off by default. |
...
| Flag | Default | Description |
|------|---------|-------------|
| `--verbose` `[LEVEL] [FILE]` | `INFO` | Logging level. Choices: `DEBUG`, `DETAIL`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`. `--verbose` alone sets `DEBUG`. Add a level followed by a file path to write that level to a file as well; the flag can be repeated to configure the console and file outputs together. |
