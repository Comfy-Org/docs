...
- A way to sign in: **OAuth** (recommended where supported) or a **[Comfy API key](/development/api-development/getting-an-api-key)** (`comfyui-…`)
...
**[API key](/development/api-development/getting-an-api-key)** — Still fully supported. Use the one-click installer or manual setup below if your client does not offer OAuth, or if you prefer a key.

<Warning>
  **OAuth rollout.** The MCP server already accepts OAuth on production, but sign-in may not work end-to-end for everyone until the next Comfy Cloud production deployment finishes (the discovery endpoint must catch up). Until OAuth works for you, [use an API key](/development/api-development/getting-an-api-key) — existing API-key setups are unchanged.
</Warning>

If you use an API key and do not have one yet, see [Getting an API Key](/development/api-development/getting-an-api-key):
...
The one-click installer detects your MCP client (Claude Code, Cursor, Amp), asks for your [Comfy API key](/development/api-development/getting-an-api-key), and configures the remote MCP server — no Node.js or other dependencies required.
...
**Authentication**
- **API key or OAuth.** Manual setup uses a [Comfy Cloud API key](/development/api-development/getting-an-api-key) in the `X-API-Key` header. MCP clients that sign in with Comfy OAuth send an `Authorization: Bearer` token instead; both are supported on the hosted server.
