...
Poll for job completion and retrieve outputs.

<Note>
  The API provides two endpoints for checking job status:
  - **`GET /api/jobs/{id}`** - Returns user-friendly status (`pending`, `in_progress`, `completed`, `failed`, `cancelled`) along with outputs. **Recommended for polling.**
  - **`GET /api/job/{id}/status`** - Returns raw internal states (e.g., `success`, `executing`, `queued_limited`). Useful for debugging but not recommended for polling logic.
</Note>
...
async function getJob(promptId: string): Promise<JobResponse> {
  const response = await fetch(`${BASE_URL}/api/jobs/${promptId}`, {
    headers: getHeaders(),
  });
  if (!response.ok) throw new Error(`HTTP ${response.status}`);
  return response.json();
...
async function pollForCompletion(
  ...
  while (Date.now() - startTime < timeout * 1000) {
    const job = await getJob(promptId);

    if (job.status === "completed") {
      return job;
    } else if (["error", "failed", "cancelled"].includes(job.status)) {
...
def get_job(prompt_id: str) -> dict:
    ...
    Returns:
        Job data with fields: id, status, outputs
    """
    response = requests.get(
...
print(f"Job completed! Outputs: {list(result.get('outputs', {}).keys())}")
```
</CodeGroup>
