...
// Use when workflow contains API nodes (e.g., Flux Pro, Ideogram, etc.)
...
def submit_workflow_with_api_nodes(workflow: dict, comfy_api_key: str) -> str:
    """Submit a workflow that uses API nodes (partner nodes).
    
    Args:
        workflow: ComfyUI workflow in API format
        comfy_api_key: Your Comfy API key from platform.comfy.org
...
# Use when workflow contains API nodes
...
<Info>
  Generate a Comfy API key at [platform.comfy.org](https://platform.comfy.org/login). This key is separate from your Cloud API key and is used to authenticate with partner AI services.
</Info>
