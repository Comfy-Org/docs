...
**ComfyUI Portable** is a standalone packaged complete ComfyUI Windows version that has integrated an independent **Python (python_embeded)** required for ComfyUI to run. You only need to extract it to use it. The portable version supports running through **Nvidia GPU**, **AMD GPU** (experimental), or **CPU**.
...
<Note>The AMD portable build ships with its own launch script (for example `run_amd_gpu.bat`) instead of `run_nvidia_gpu.bat`. Refer to `README_VERY_IMPORTANT.txt` in the extracted folder for the exact filename and usage.</Note>

## How to Launch ComfyUI

Double click either `run_nvidia_gpu.bat` (or `run_amd_gpu.bat` for the AMD build) or `run_cpu.bat` depending on your computer's configuration to launch ComfyUI. 
