...
**ComfyUI Portable** is a standalone packaged complete ComfyUI Windows version that has integrated an independent **Python (python_embedded)** required for ComfyUI to run. You only need to extract it to use it. Currently, the portable version supports running through **Nvidia GPU** or **CPU**.
...
├── 📂python_embedded           // Independent Python environment
...
```bat
.\python_embedded\python.exe -s ComfyUI\main.py --listen --windows-standalone-build
pause
```
