...
<Tip>
  For performance tuning, you can enable **FlashAttention**, **SageAttention**, or other attention optimizations by configuring startup flags in **Settings > Server Config > Cross attention method**, or by passing flags like `--use-flash-attention` at launch. See the [startup flags reference](/development/comfyui-server/startup-flags#attention) for all options.
</Tip>
