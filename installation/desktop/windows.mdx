...
- NVIDIA GPU
- AMD GPU (experimental support for RDNA 3/3.5/4 — RX 7000 series, Strix Halo/Ryzen AI Max+ 365, and RX 9000 series)
...
<Steps>
  ...
  ![ComfyUI Installation Steps - GPU Selection](/images/desktop/win-comfyui-desktop-2.jpg)
   ...
   2. **AMD GPU (Experimental):** Experimental support for AMD GPUs (RDNA 3/3.5/4 — RX 7000 series, Strix Halo/Ryzen AI Max+ 365, RX 9000 series). The installer detects your hardware and preselects the appropriate option.
   ...
   4. **Enable CPU Mode:** For developers and special cases only. Don't select this unless you're sure you need it
    ...
    The installer will preselect the appropriate option based on your detected hardware. Unless there are special circumstances, keep the preselected option and click **Next** to proceed
  </Step>
...
ComfyUI Desktop (Windows) supports **NVIDIA GPUs with CUDA** and experimental support for **AMD GPUs** (RDNA 3/3.5/4). You may see this screen if your device is not supported.
- Please switch to a supported device
- Or consider using [ComfyUI Portable](/installation/comfyui_portable_windows) or [manual installation](/installation/manual_install) to use ComfyUI
