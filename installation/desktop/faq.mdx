...
<Accordion title="Does ComfyUI support for loops or iteration?">
ComfyUI does not have a traditional "for loop" node. Instead, iteration and batch processing are achieved through dedicated nodes:

- **Batch processing nodes** — Use nodes like `RepeatImageBatch`, `RebatchImages`, or `CreateList` to process multiple inputs in a single run.
- **Node expansion** — ComfyUI automatically queues multiple jobs when you connect a batch of inputs to a node, effectively iterating over each item.
- **Custom nodes** — Community-created custom nodes may provide looping or iteration functionality for advanced use cases.

For most workflows, connecting a batch of images or latents to sequential processing nodes handles iteration automatically.
</Accordion>
