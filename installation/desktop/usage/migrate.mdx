...
<Info>
This page covers migration from **Desktop Legacy** (the previous single-install desktop app). If you're a first-time user, you can skip this.
</Info>

If you already have Desktop Legacy installed, Comfy Desktop can detect and migrate it, creating a new Standalone installation in the Chooser view.

## What Gets Migrated

Desktop Legacy was a single-install app. Comfy Desktop is a multi-installation manager. When you migrate:
...
The Desktop Legacy installation itself is **not modified** during migration. It remains on your system as a backup.

## What Changes After Migration

Once migration completes, your new Standalone installation works the same way as any Comfy Desktop installation:

- **Startup** — click the play button on the new installation card in the Chooser view to launch ComfyUI. The server starts on the default port (usually 8188).
- **Manage** — use the Manage panel to update, snapshot, and configure the installation.
- **Updates** — Standalone installations can be updated independently via the Manage panel. Unlike Desktop Legacy, you no longer need to download a new build for each update.
...
- **Shared storage** — models, inputs, and outputs are shared across all Standalone installations by default.

## Does My Old Link Still Work?

The old Comfy Desktop (Legacy) URL — such as `http://localhost:17888` — is replaced by the new Standalone installation's URL (usually `http://localhost:8188` or the port you configure in Startup Args).

If you have bookmarks, workflows that reference the old URL, or external integrations pointing to the Legacy port, you'll need to update them to the new installation's URL. Your Legacy installation remains intact as a fallback.
...
When you first open Comfy Desktop, it automatically detects any Desktop Legacy installation on your system and shows a migration prompt.
</Step>
...
- Your server is running on the new URL (default `http://localhost:8188`)
</Step>
</Steps>
...
3. Choose your Desktop Legacy installation and follow the prompts
