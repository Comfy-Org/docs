...
| **Share** | Export the latest snapshot as a file to share with others |
| **Copy Installation** | Duplicate an existing standalone installation with a new name, Python environment, and ComfyUI version |
| **Untrack** | Remove the installation from the Chooser without deleting any files |
| **Delete** | Permanently remove this installation and all its files (model files in shared directories are not affected) |
| **Dismiss Error** | Clear the error state and dismiss the warning indicator on an installation that encountered a problem |
...
Comfy Desktop automatically creates a **snapshot** before each update. Snapshots capture the ComfyUI version, all installed custom nodes, and the Python package list.

> Snapshots only capture ComfyUI version info and custom node metadata — they do not include model files.

For full details on creating, restoring, exporting, and importing snapshots, see the [Snapshots](/installation/desktop/usage/snapshots) page.
