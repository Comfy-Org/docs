...
The screenshot shows the Update tab with version info, a "Check for update" button, and an Update Channel dropdown (Stable — Current is selected).
...
The screenshot shows Startup Arguments with `--enable-manager`, Launch Mode set to "App window", Browser Cache set to "Unique to this install", and one environment variable (`COMFYUI_VER`).
...
The screenshot shows a snapshot timeline with two snapshots. The latest is highlighted (v0.20.1 → v0.24.1) with Export and Delete buttons and package statistics.
...
The screenshot shows two toggle switches (Use Shared Models, Use Shared Input/Output Folders), a Shared Models list with one Primary directory, and the Input/Output directory paths.
...
The screenshot shows a live terminal with ComfyUI backend startup logs, including Python version, hardware detection, and loaded node counts.
...
The screenshot shows the current version, hardware (Apple Silicon MPS), Python and PyTorch versions, installation path, and disk usage.
