...
Every output file saved by a built-in save node carries the workflow that generated it. To recover a workflow, drag the file back onto the canvas, or open it from the `Workflow` menu. ComfyUI rebuilds the graph from the embedded workflow.

Workflows can also be saved as human-readable JSON files. This is convenient for versioning, archiving, and sharing graphs independently of any generated media.

To save files without the embedded workflow, turn on **Disable saving prompt metadata in files** in the ComfyUI settings, or launch ComfyUI with the `--disable-metadata` flag. Files saved this way cannot be used to recover the workflow.

### What gets embedded

The embedded metadata contains two JSON payloads:
