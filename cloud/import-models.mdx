...
2. Click **Add Secret**, then choose the **Provider** the key belongs to (such as Hugging Face or Civitai), give the key a name, and paste the key into **Secret Value**. The provider is required: it tells Comfy Cloud which platform the key is for, so the token is used automatically when you import models from that platform.
...
<Note>
Secrets are encrypted and cannot be viewed again after you save them, so keep a copy of the key. You can save one key per provider. To replace a key, open it and enter a new value.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="What model types can I import (BYOM)?">
