...
icon: "lightbulb"
sidebarTitle: "개요"
translationSourceHash: 2608a85e
...
Intel GPU 시스템에서 이 오류는 CUDA 문제를 의미하지 않으며 PyTorch를 다시 설치할 필요도 없습니다. Intel GPU는 CUDA 대신 XPU 백엔드를 사용합니다. 위의 NVIDIA CUDA 재설치 단계를 따르지 마세요. 해당 단계는 Intel 하드웨어에는 적용되지 않습니다.

설정이 올바르게 작동하는지 확인하려면:
```bash
python -c "import torch; print('XPU available:', torch.xpu.is_available())"
```

`torch.xpu.is_available()`이(가) `True`를 반환하면 ComfyUI가 자동으로 XPU 백엔드를 사용합니다. `False`를 반환하면 아래 옵션 중 하나를 사용하여 XPU 지원 PyTorch를 설치하세요.
