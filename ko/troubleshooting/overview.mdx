...
**증상:** "프롬프트 실행 실패" 대화상자에 "보고서 표시" 버튼이 있고, 워크플로가 중단됨
...
- **유지관리 페이지**: 다운로드가 실패하면 [미러 URL 설정](/ko/installation/desktop/windows#mirror-settings)을 확인하세요.
- **누락된 모델**: 모델은 마이그레이션 시 복사되지 않고 링크만 생성됩니다. 모델 경로를 확인하세요.
</Tab>
<Tab title="macOS">
...
**성능 최적화:**
```bash
# 실험적 메모리 효율적 어텐션 활성화 (PyTorch 2.4 이후 더 이상 필요 없음)
...
**Windows AMD Portable - 하드코딩된 경로 오류:**
AMD 포터블 배포판은 CI 환경에서 `D:\a\ComfyUI\` 경로로 빌드됩니다. 다른 드라이브 (예: `C:\ComfyUI_windows_portable\`)에 압축을 풀면 `offload-arch.exe` 등의 도구가 `"Fatal error in launcher: Unable to create process"` 또는 `access violation` 오류를 발생시킬 수 있습니다.

다음 방법을 선호 순서대로 시도하세요.

1. **포터블 재압축 풀기** — `ComfyUI_windows_portable_amd` 폴더 전체를 삭제합니다. [7-ZIP](https://7-zip.org/)을 사용하여 `.7z` 파일을 **CI 빌드 경로와 동일한 드라이브**에 다시 압축 풉니다. 가장 간단한 방법은 `D:\` 드라이브가 있으면 이를 사용하는 것입니다.
2. **DirectML 사용** — 재압축 풀기가 작동하지 않거나 D 드라이브가 없는 경우, 포터블 환경 내에 torch-directml을 설치합니다:
   ```bat
   .\python_embeded\python.exe -m pip install torch-directml
   .\python_embeded\python.exe -s ComfyUI\main.py --directml
   ```
3. **포터블 재다운로드** — [릴리스 페이지](https://github.com/Comfy-Org/ComfyUI/releases)에서 최신 릴리스를 다시 다운로드합니다. 더 새로운 빌드에서 경로 의존성이 수정되었을 수 있습니다.

> **참고:** `torch-directml`은 기본 ROCm 빌드보다 훨씬 느리며 1년 넘게 업데이트되지 않았습니다. 작동 가능한 경우 AMD ROCm 포터블(기본값)을 적극 권장합니다.
...
2. **기본 문제 해결 시도:**
   ...
   - 모든 커스텀 노드 비활성화 ([커스텀 노드 문제 해결](/ko/troubleshooting/custom-node-issues) 참고)
   - 콘솔/터미널에서 오류 메시지 확인
