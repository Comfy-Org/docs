...
Comfy API v2는 제출 후 폴링하는 영속적인 작업(job)으로 워크플로를 실행합니다. 모델을 직접 호출해야 하는 경우(파트너 모델 하나, 요청 하나, 모델 고유의 네이티브 입력 및 출력)에는 [Comfy Router](/ko/api-reference/comfy-router/quickstart)를 참조하세요. 먼저 [Router 제한 사항](/ko/api-reference/comfy-router/limitations)을 검토하세요. Router는 아직 일반 공개되지 않았습니다.
