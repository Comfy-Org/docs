...
| `503` | [`RouterErrorResponse`](#routererrorresponse) | `X-Comfy-Error-Type`, `X-Comfy-Request-Id` | 라우터 요청 수준의 실패입니다. 요청이 모델에 도달하지 못했거나, 모델 자체가 신고하지 않은 이유로 실패했습니다. 본문은 `RouterErrorResponse`이며, 버킷은 `X-Comfy-Error-Type` 헤더에도 동일하게 포함됩니다. |### `POST /v1/models/{provider}/{model}`

**표준 모델 ID로 파트너 모델을 동기식으로 실행합니다.**

Comfy Router의 표준 진입점으로, 모델 ID로 주소가 지정됩니다. 요청 본문은 파트너 모델 자체의 네이티브 JSON 입력이고, 성공 응답은 해당 모델 자체의 네이티브 JSON 출력입니다. Router는 Comfy 형태의 래퍼를 강제하는 대신 둘 다 변경 없이 그대로 전달하므로, 호출자는 호스트만 변경하여 파트너 API와 Router 사이를 이동할 수 있습니다. 이 경로는 `POST https://fal.run/{id}`를 미러링하는 동기(SYNCHRONOUS) 경로입니다. 응답에는 완료된 결과가 포함됩니다. 대기 중인 대응 엔드포인트인 `/v1/queue/models/{provider}/{model}`이 계획되어 있으며, fal의 `fal.run` / `queue.fal.run` 분할을 단일 호스트로 통합하게 됩니다. 아직 이 계약의 일부는 아닙니다.
...
| `422` | [`RouterValidationErrorResponse`](#routervalidationerrorresponse) | `X-Comfy-Error-Type`, `X-Comfy-Request-Id` | 요청이 모델에 도달했지만 모델이 내용을 거부했습니다. 본문은 fal/FastAPI `detail[]` 형태의 `RouterValidationErrorResponse`이므로, 각 오류 필드는 고유한 `type`과 `ctx`를 유지합니다. `X-Comfy-Error-Type`은 전체 응답에 대한 포괄적인 버킷을 전달합니다. |
| `429` | [`RouterErrorResponse`](#routererrorresponse) | `X-Comfy-Error-Type`, `X-Comfy-Request-Id` | Router 요청 수준의 실패입니다. 요청이 모델에 도달하지 못했거나, 모델 자체가 보고하지 않은 이유로 실패했습니다. 본문은 `RouterErrorResponse`이며, 버킷은 `X-Comfy-Error-Type` 헤더에 동일하게 표시됩니다. |
| `500` | [`RouterErrorResponse`](#routererrorresponse) | `X-Comfy-Error-Type`, `X-Comfy-Request-Id` | Router 요청 수준의 실패입니다. 요청이 모델에 도달하지 못했거나, 모델 자체가 보고하지 않은 이유로 실패했습니다. 본문은 `RouterErrorResponse`이며, 버킷은 `X-Comfy-Error-Type` 헤더에 동일하게 표시됩니다. |
...
| `504` | [`RouterErrorResponse`](#routererrorresponse) | `X-Comfy-Error-Type`, `X-Comfy-Request-Id` | Router 요청 수준의 실패입니다. 요청이 모델에 도달하지 못했거나, 모델 자체가 보고하지 않은 이유로 실패했습니다. 본문은 `RouterErrorResponse`이며, 버킷은 `X-Comfy-Error-Type` 헤더에 동일하게 표시됩니다. |### `GET /v1/models/{provider}/{model}/openapi.json`

**파트너 모델 하나의 입력 스키마를 OpenAPI 문서로 읽습니다.**

단일 Comfy Router 모델의 모델별 입력 스키마를 독립형 OpenAPI 문서로 제공합니다. 이를 통해 호출자(SDK, 코드 생성 도구 또는 에이전트)는 Comfy의 문서를 읽지 않고도 모델의 인자를 발견할 수 있습니다. 이는 fal의 모델별 스키마 엔드포인트를 미러링하며, SDK 퀵스타트가 의존하는 발견 메커니즘입니다.
