...
[내 Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성하고 `COMFY_API_KEY`로 내보내기하세요. Python의 경우 `pip install comfy-sdk`를 실행하세요. TypeScript의 경우 `npm install @comfyorg/sdk`를 실행하세요. Swift의 경우 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) 패키지를 추가하세요. cURL은 원시 HTTP를 사용합니다.
