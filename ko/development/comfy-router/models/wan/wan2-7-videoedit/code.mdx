...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성한 뒤 `COMFY_API_KEY`로 내보내세요. Python과 TypeScript 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`)를 사용하며, cURL 스니펫은 동일한 호출을 순수 HTTP로 수행합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비되는 대로 이 프로세스 또는 다른 프로세스에서 수집됩니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
도달하기도 전에 거부됩니다. 이 크기에서는 파트너 자체의 20MB 상한에
해당하는 단일 에셋이 인라인으로도 충분히 들어가므로, 에셋 하나에 대해서는
파트너 규칙이 구속력을 가집니다. 반면 에셋이 여러 개일 때는 전송 상한이
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">
  Video audio setting for wan2.7-videoedit model. → wan2.7-videoedit 모델의 비디오 오디오 설정입니다.
