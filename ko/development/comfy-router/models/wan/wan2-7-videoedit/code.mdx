---
title: "Comfy Router로 Wan 2.7 Video Edit 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.7-videoedit을 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
sidebarTitle: "Wan 2.7 Video Edit"
translationSourceHash: 1083d3f6
translationFrom: development/comfy-router/models/wan/wan2-7-videoedit/code.mdx
translationBlockHashes:
  "_intro": 47b7d5e3
  "Quick start": 183efcd6
  "Schema": 53171aef
  "Examples": 622d1416
---
...
Wan 2.7 Video Edit의 API 레퍼런스입니다. Wan 2.7 video edit은 텍스트 지시에 따라 기존 클립을 다시 작성하며, 선택적으로 참조 이미지의 안내를 받을 수 있습니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성한 뒤 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 동일한 호출을 순수 HTTP로 수행합니다.

**모델 ID:** `wan/wan2.7-videoedit`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.7-videoedit/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비되는 대로 이 프로세스 또는 다른 프로세스에서 수집됩니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
<ParamField body="input.audio_url" type="string">
  오디오 파일 다운로드 URL. 지원 형식: mp3, wav. reference_video_urls와 함께 사용할 수 없습니다.
</ParamField>
...
`first_frame`인 `media` 요소로 전달하며(아래 `media` 참조),
...
자료(이미지, 오디오, 비디오)를 지정합니다. 각 요소는 type 및 url 필드를 포함합니다.
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  file과 함께 사용할 수 없음). reference_*/file/link 타입과 first_frame/last_frame 타입은
...
도달하기도 전에 거부됩니다. body에 이러한 에셋이 여러 개 있으면 더욱 그렇습니다.
...
(wan3.0-video의 경우 최대 20,000자, 제한을 초과한 내용은 잘립니다).
여러 참조 비디오를 사용하는 wan2.6-r2v의 경우, 참조 비디오 순서대로 대상을 가리킬 때
'character1', 'character2' 등을 사용합니다. 예: "Character1 sings on the roadside, Character2 dances beside it"
wan3.0-video 참조 모드의 경우, media 배열 내 해당 순서의 미디어 에셋을 가리킬 때
'Image 1', 'Video 1', 'Audio 1' 등을 사용합니다.
</ParamField>
...
<ParamField body="model" type="string">
  호출할 모델의 ID. 이 컴포넌트에서는 제약되지 않습니다. Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수, 비디오 입력이 있으면 입력
  비디오 재생 시간 + 출력 비디오 재생 시간의 합이 30초를 초과할 수 없습니다. -1은
...
<ParamField body="parameters.size" type="string">
  비디오 해상도로, 너비*높이 형식입니다. 지원되는 해상도는 모델에 따라 다릅니다:
...
Router가 `GET /v2/models/wan/wan2.7-videoedit/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
