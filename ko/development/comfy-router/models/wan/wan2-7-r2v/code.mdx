---
title: "Comfy Router로 Wan 2.7 R2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.7-r2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드, 결과 형태를 설명합니다"
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.7-r2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [실행 대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64 인코딩 데이터입니다. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 공급자가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000 픽셀. 파일 크기: 최대 10MB.
</ParamField>
...
도달하기 전에 거부됩니다. 그 정도 크기라면 파트너 자체의 20MB 상한에
해당하는 단일 에셋은 인라인으로도 충분히 전송되므로, 에셋 하나에 대해서는
파트너 규칙이 적용됩니다. 전송 상한은 여러 에셋에 걸쳐 적용되는데,
100 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하기
때문입니다. 이 상한에 근접하는 것은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, 예를 들어 happyhorse-1.x의 i2v와 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 인라인 페이로드를 총체적으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
...
이 프록시는 해상도와 size를 모두 제공하지 않은 비디오 생성 요청을 거부합니다.
...
<ParamField body="parameters.shot_type" type="string" default="&quot;single&quot;">
  지능형 멀티 렌즈 제어입니다. prompt_extend가 활성화된 경우에만 적용됩니다.
