...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.7-r2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 살펴봅니다.
...
도달하기 전에 거부됩니다. body에 그러한 에셋이 여러 개 포함되면 더욱
빨라집니다. 20 MiB 상한은 각 요소가 아니라 전체 요청을 제한하기
