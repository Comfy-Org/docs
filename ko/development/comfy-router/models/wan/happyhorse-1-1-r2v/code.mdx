...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64 인코딩 데이터. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 프로바이더가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360~2000픽셀. 파일 크기: 최대 10MB.
</ParamField>
...
<ParamField body="model" type="string">
  호출할 모델의 ID. 이 컴포넌트에서는 NOT constrained입니다. Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수, 비디오 입력이 있으면
  입력 비디오 총 재생 시간 + 출력 비디오 재생 시간이 30초를 초과할 수 없음; -1은
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성이 활성화되었는지 여부. 기본값은 참
</ParamField>

<ParamField body="parameters.ratio" type="string">
  생성되는 비디오의 화면 비율. wan2.7 및 wan3.0 모델 전용.
wan2.7 모델의 경우, 제공되지 않으면 해상도 등급에 따라 기본값이 정해집니다.
wan3.0-video의 경우, adaptive(기본값)는 입력 미디어 비율과 의도에 따라
