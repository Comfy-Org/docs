---
title: "Comfy Router로 HappyHorse 1.1 R2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/happyhorse-1.1-r2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
HappyHorse 1.1 R2V의 API 레퍼런스입니다. HappyHorse 1.1 레퍼런스 기반 비디오 생성은 최대 9장의 레퍼런스 이미지에 담긴 피사체를 새로운 장면 전반에서 일관되게 유지하며, 프롬프트에서는 character1, character2 등으로 지칭합니다.
...
**모델 ID:** `wan/happyhorse-1.1-r2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-r2v`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-r2v/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 되면 이 프로세스에서든 다른 프로세스에서든 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64 인코딩 데이터.
wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는
여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로
전달하며(아래 `media` 참조), wan2.6-i2v에서 성공하는 `img_url` body는
...
<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록. 비디오 생성을 위한 참조 자료(이미지, 오디오, 비디오)를 지정합니다.
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  같은 요청 내에서 상호 배타적입니다. 배열 순서는 프롬프트에서 에셋의 참조 순서
...
도달하기 전에 거부됩니다. body에 이러한 에셋이 여러 개 담기면 거부 시점은
더 앞당겨집니다. 20 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하기
...
<ParamField body="input.negative_prompt" type="string">
  네거티브 프롬프트 단어는 비디오 화면에 나타나지 않기를 원하는 콘텐츠를 설명하는 데 사용됩니다
</ParamField>
...
여러 참조 비디오를 사용하는 wan2.6-r2v의 경우, 참조 비디오 순서에 따라 'character1', 'character2'
등으로 대상을 지칭하세요. 예: "Character1 sings on the roadside, Character2 dances beside it"
wan3.0-video 참조 모드의 경우, media 배열 내 해당 순서에 따라 'Image 1', 'Video 1', 'Audio 1'
등으로 미디어 에셋을 지칭하세요.
</ParamField>

<ParamField body="input.reference_video_urls" type="string[]">
  wan2.6-r2v 모델 전용 참조 비디오 URL. 1~3개의 비디오 URL 배열.
입력 제한 사항:
- 형식: mp4, mov
- 개수: 1~3개 비디오
- 단일 비디오 길이: 2~30초
...
<ParamField body="model" type="string">
  호출할 모델의 ID. 이 컴포넌트에서는 값이 제약되지 않습니다. Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수, 비디오 입력이 있으면
  입력 비디오 재생 시간 + 출력 비디오 재생 시간의 합계가 30초를 초과할 수 없습니다. -1은
...
<ParamField body="parameters.ratio" type="string">
  생성되는 비디오의 화면 비율입니다. wan2.7 및 wan3.0 모델에만 해당합니다.
wan2.7 모델의 경우, 제공되지 않으면 해상도 등급에 따라 기본값이 정해집니다.
wan3.0-video의 경우 adaptive(기본값)는 입력 미디어의 비율과 의도를 기반으로 적절한
화면 비율을 자동으로 추천합니다.
...
Router가 `GET /v2/models/wan/happyhorse-1.1-r2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="code" type="string">
  실패한 요청의 오류 코드로, `output` 아래가 아니라 응답 envelope의 최상위(ROOT)에 보고됩니다 (요청이 성공하면 반환되지 않음).
</ResponseField>

<ResponseField name="message" type="string">
  실패한 요청에 대한 상세 정보로, `output` 아래가 아니라 응답 envelope의 최상위(ROOT)에 보고됩니다 (요청이 성공하면 반환되지 않음). `output.message`로 대체하기 전에 이 값을 먼저 확인하세요.
</ResponseField>
