...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-r2v/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 되면 이 프로세스에서든 다른 프로세스에서든 수집합니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
도달하기 전에 거부됩니다. 그 정도 크기에서는 파트너 자체의 20MB 상한에
해당하는 단일 에셋도 인라인으로 충분히 들어가므로, 에셋 하나에 대해서는
파트너 규칙이 기준이 됩니다. 전송 상한은 여러 에셋에 걸칠 때 기준이 됩니다.
100 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하기
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성 기능의 활성화 여부입니다. 기본값은 true입니다
</ParamField>
...
wan3.0-video의 경우, adaptive(기본값)는 입력 미디어의 비율과 의도에 따라
적절한 비율을 자동으로 추천합니다.
...
이 프록시는 해상도와 크기(size)를 모두 제공하지 않는 비디오 생성 요청을 거부합니다.
