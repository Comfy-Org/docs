---
title: "Comfy Router로 Wan 2.5 T2V Preview 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.5-t2v-preview를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
Wan 2.5 T2V Preview에 대한 API 레퍼런스입니다. Wan 2.5 텍스트 기반 비디오 생성은 프롬프트만으로 짧은 비디오를 만들며, 참조 이미지나 비디오가 필요하지 않습니다.
...
**모델 ID:** `wan/wan2.5-t2v-preview`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 안내합니다.
...
`first_frame`인 `media` 요소로 전달합니다(아래 `media` 참조).
...
자료(이미지, 오디오, 비디오)를 지정합니다. 각 요소는 type 및
url 필드를 포함합니다.
...
위의 에셋별 "최대 20MB" 수치는 파트너가 최종적으로 받게 되는 이미지에 적용하는
상한이며, 에셋이 공개 URL일 때는 그대로 적용됩니다. 해당 바이트가
Comfy를 거치지 않기 때문입니다. 인라인 data: URL은 Comfy를 거치므로
전송 상한에도 부딪힙니다. Comfy Router의 POST body는 총 100 MiB가
상한이며 이를 초과하면 413이 반환됩니다. base64는 페이로드를 약 4/3로
부풀리므로, 약 75 MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기 전에 거부됩니다. 파트너 자체의 20MB 상한에 해당하는 크기의 단일
에셋은 인라인으로도 여유 있게 전송되므로, 에셋 하나에 대해서는 파트너 규칙이
기준이 됩니다. 전송 상한은 여러 에셋에 걸쳐 적용됩니다. 100 MiB Router 상한은
각 요소가 아니라 전체 요청을 제한하기 때문입니다. 이 상한에 가까운 것은
공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우(happyhorse-1.x의 i2v 및 r2v 표기처럼)에는 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기 및 픽셀 하한과, 인라인 페이로드를 총체적으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>

<ParamField body="input.negative_prompt" type="string">
  네거티브 프롬프트 단어로, 비디오 화면에 나타나지 않았으면 하는 콘텐츠를 설명하는 데 사용됩니다
</ParamField>

<ParamField body="input.prompt" type="string">
  텍스트 프롬프트 단어. 중국어와 영어를 지원하며 길이는 800자를 초과할 수 없습니다
(wan3.0-video의 경우 최대 20,000자이며, 한도를 초과하는 내용은 잘립니다).
여러 참조 비디오를 사용하는 wan2.6-r2v의 경우 'character1', 'character2' 등을 사용하여
참조 비디오 순서대로 피사체를 지칭합니다. 예: "Character1이 길가에서 노래하고, Character2가 옆에서 춤을 춥니다"
...
참조 재생 시간: 단일 비디오는 최대 5초, 두 개의 비디오는 각각 최대 2.5초, 세 개의 비디오는 그보다 비례적으로 짧습니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID. 이 컴포넌트에서는 제약되지 않습니다. Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 이 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이 값을 제공해야 하며, 허용되는 표기의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">  wan2.7-videoedit 모델의 비디오 오디오 설정.
...
<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 비율. wan2.7 및 wan3.0 모델 전용입니다.
...
<ParamField body="parameters.seed" type="integer">
  난수 시드. 모델이 생성하는 콘텐츠의 무작위성을 제어하는 데 사용됩니다
...
<ParamField body="parameters.watermark" type="boolean" default="false">
  워터마크 로고 추가 여부. 워터마크는 오른쪽 아래 모서리에 위치합니다
</ParamField>

Router가 `GET /v2/models/wan/wan2.5-t2v-preview/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="message" type="string">
  실패한 요청에 대한 상세 정보로, `output` 아래가 아니라 envelope의 ROOT에 보고됩니다 (요청이 성공하면 반환되지 않음). `output.message`로 대체하기 전에 이 항목을 먼저 확인하세요.
</ResponseField>
...
비디오 URL은 24시간 동안 유효합니다. 보관하려면 비디오를 즉시 다운로드하세요.
