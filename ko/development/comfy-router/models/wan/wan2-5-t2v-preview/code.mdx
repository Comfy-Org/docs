...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.5-t2v-preview/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 안내합니다.
...
도달하기 전에 거부됩니다. body에 이러한 에셋이 여러 개 포함되면 거부는 더욱
빨리 일어납니다. 20 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하기
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우(happyhorse-1.x의 i2v 및 r2v 표기처럼)에는 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기 및 픽셀 하한과, 파트너 규칙이 적용되기 전에 인라인 페이로드를 제한하는 20 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
