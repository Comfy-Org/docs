...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python 및 TypeScript 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`)를 사용하며, cURL 스니펫은 raw HTTP로 동일한 호출을 수행합니다.
...
<ParamField body="input.media" type="object[]">
  wan2.7 및 wan3.0 모델의 미디어 에셋 목록입니다. 비디오 생성에 사용할 참조 자료(이미지, 오디오, 비디오)를 지정합니다. 각 요소는 type 및 url 필드를 포함합니다.
...
Router가 `GET /v2/models/wan/wan2.5-t2v-preview/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.### 출력
