---
title: "Comfy Router로 Wan 3.0 Video 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan3.0-video를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
{/* 생성된 파일입니다. 이 디렉터리의 code.yaml을 편집한 후 `pnpm code-pages:gen`을 실행하세요. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

Wan 3.0 Video API 레퍼런스. Wan 3.0 Video는 프롬프트만으로 생성하거나, 첫 프레임과 마지막 프레임, 레퍼런스 이미지, 비디오, 오디오로 구성된 미디어 목록으로 생성할 수 있으며, 프롬프트에서는 각각 Image 1, Video 1, Audio 1로 지칭됩니다.
...
**모델 ID:** `wan/wan3.0-video`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan3.0-video`
...
# SDK는 자동으로 idempotency 키를 생성하고 자동 재시도를 위해 재사용합니다.
...
// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 idempotency 키를 발급하고 자동 재시도를 위해 재사용합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan3.0-video/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되는 대로 이 프로세스 또는 다른 프로세스에서 수집됩니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
이 모델은 요청에서 다른 공급자를 지정하지 않는 한 Comfy Router가 직접 제공합니다. 아래 공급자들도 같은 엔드포인트, 같은 모델 ID로 이 모델을 제공하며, `model_provider` 쿼리 매개변수로 선택합니다.

- **Comfy** (기본값): `POST https://api.comfy.org/v2/models/wan/wan3.0-video`
- **Higgsfield**, `higgsfield/higgsfield-wan-3`(으)로: `POST https://api.comfy.org/v2/models/wan/wan3.0-video?model_provider=higgsfield`

`strict_mode`의 기본값은 거짓(false)이므로, Router는 이 페이지에 문서화된 네이티브 요청 본문을 해당 공급자 자체의 스키마로 변환하고 응답을 다시 변환합니다. API 레퍼런스의 [`model_provider`, `strict_mode` 및 `fallback_provider`](/ko/development/comfy-router/reference#post-v2modelsprovidermodel)와, 이 방식으로 라우팅되는 모든 모델을 다루는 [제공 공급자](/ko/development/comfy-router/providers)를 참고하세요.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64로 인코딩된 데이터입니다. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 공급자가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000 픽셀. 파일 크기: 최대 10MB.
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델의 미디어 에셋 목록입니다. 비디오 생성을 위한 참조 자료(이미지, 오디오, 비디오)를
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  총 재생 시간 \<= 15초), file (최대 1, link와 함께 사용할 수 없음), link (최대 1, file과
  함께 사용할 수 없음). reference_*/file/link 유형과 first_frame/last_frame 유형은
  동일한 요청 내에서 상호 배타적입니다. 배열 순서는 프롬프트에서 에셋의 참조 순서
  (Image 1, Video 1, Audio 1, ...)를 정의합니다.
...
위의 에셋별 "최대 20MB" 수치는 파트너가 최종적으로 받는 이미지에 적용하는
상한이며, 에셋이 공개 URL일 때는 적힌 그대로 적용됩니다. 해당 바이트가
...
도달하기도 전에 거부됩니다. 그 크기라면 파트너 자체 상한인 20MB의 단일
에셋은 인라인으로 충분히 들어가므로, 에셋 하나에 대해서는 파트너 규칙이
구속력을 가집니다. 여러 에셋에 걸치면 전송 상한이 구속력을 가지는데,
100 MiB Router 상한은 각 요소가 아니라 요청 전체를 제한하기 때문입니다.
이 상한에 가까운 것은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.negative_prompt" type="string">
  역방향 프롬프트 단어는 비디오 화면에 표시되길 원하지 않는 내용을 설명하는 데 사용됩니다
</ParamField>

<ParamField body="input.prompt" type="string">
  텍스트 프롬프트 단어입니다. 중국어와 영어를 지원하며 길이는 800자를 초과할 수 없습니다
(wan3.0-video의 경우 최대 20,000자, 한도를 초과하는 내용은 잘립니다).
...
<ParamField body="input.reference_video_urls" type="string[]">
  wan2.6-r2v 모델 전용 참조 비디오 URL입니다. 1-3개 비디오 URL의 배열입니다.
입력 제한:
- 형식: mp4, mov
- 개수: 비디오 1-3개
- 단일 비디오 길이: 2-30초
...
참조 재생 시간: 단일 비디오 최대 5초, 비디오 두 개는 각각 최대 2.5초, 비디오 세 개는 그보다 비례적으로 짧습니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약되지 않습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 작업 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">wan2.7-videoedit 모델의 비디오 오디오 설정입니다.
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성 사용 여부입니다. 기본값은 true입니다
</ParamField>
...
이 프록시는 해상도도 크기도 제공하지 않는 비디오 생성 요청을 거부합니다.
해상도 등급이 청구 요율을 선택하기 때문입니다.
...
Router가 `GET /v2/models/wan/wan3.0-video/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="usage.input_video_duration" type="number">
  입력 비디오의 재생 시간(초), 비디오 입력이 없을 때는 0.0 (wan3.0-video 작업)
</ResponseField>
