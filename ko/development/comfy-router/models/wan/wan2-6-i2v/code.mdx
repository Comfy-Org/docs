...
파트너 측 상한이며, 에셋이 공개 URL일 때는 해당 바이트가 Comfy를 거치지 않으므로
표기된 그대로 적용됩니다. 인라인 data: URL은 Comfy를 거치므로 전송
상한도 함께 적용됩니다. Comfy Router의 POST body는 총 100 MiB가 상한이며
이를 초과하면 413이 반환되고, base64는 페이로드를 약 4/3로 부풀리므로
약 75 MB를 넘는 단일 인라인 에셋은 여기의 파트너 규칙에 도달하기
전에 거부됩니다. 그 정도 크기라면 파트너 자체 상한인 20MB의 단일 에셋은
인라인으로 충분히 들어가므로, 에셋 하나에 대해서는 파트너 규칙이 구속력을
가집니다. 100 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하므로,
여러 개를 보낼 때는 전송 상한이 구속력을 가집니다.
