---
title: "Comfy Router로 Wan 2.6 I2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.6-i2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태를 설명합니다"
...
Wan 2.6 I2V API 레퍼런스. Wan 2.6 이미지 기반 비디오 생성은 정지된 첫 프레임을 720P 또는 1080P로 최대 15초까지 애니메이션화합니다.
...
**모델 ID:** `wan/wan2.6-i2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan2.6-i2v`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.6-i2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비되는 대로 이 프로세스에서든 다른 프로세스에서든 수집됩니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 설명합니다.
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  동일한 요청 내에서 상호 배타적입니다. 배열 순서는 프롬프트 내 에셋의 참조
...
위의 에셋별 "최대 20MB" 수치는 파트너가 최종적으로 받게 되는 이미지에 대한
상한이며, 에셋이 공개 URL일 때는 해당 바이트가 Comfy를 거치지 않으므로
그대로 적용됩니다. 인라인 data: URL은 Comfy를 거치므로 더 낮은 전송
상한에 먼저 도달합니다. Comfy Router의 POST body는 총 20 MiB가 상한이며
이를 초과하면 413이 반환됩니다. base64는 페이로드를 약 4/3로 부풀리므로,
약 15 MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에 도달하기
전에 거부됩니다. body에 그러한 에셋이 여러 개 실리면 더욱 빨리 거부됩니다.
...
(wan3.0-video의 경우 최대 20,000자이며, 제한을 초과하는 내용은 잘립니다).
...
참조 재생 시간: 단일 비디오 최대 5초, 비디오 2개는 각각 최대 2.5초, 비디오 3개는 그에 비례해 더 짧습니다.
과금: 실제로 사용된 참조 재생 시간을 기준으로 합니다.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이 있으면
  총 입력 비디오 재생 시간과 출력 비디오 재생 시간의 합이 30초를 초과할 수 없습니다.
...
`GET /v2/models/wan/wan2.6-i2v/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="message" type="string">
  실패한 요청에 대한 상세 정보로, `output` 아래가 아니라 envelope의 ROOT에 보고됩니다 (요청이 성공하면 반환되지 않음). `output.message`로 대체하기 전에 이 내용을 확인하세요.
</ResponseField>
