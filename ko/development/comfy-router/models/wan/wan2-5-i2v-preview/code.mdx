...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
부풀리므로, 약 75MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기도 전에 거부됩니다. 그 정도 크기라면 파트너 자체의 20MB 상한에
해당하는 단일 에셋은 인라인으로도 충분히 들어가므로, 에셋 하나에 대해서는
파트너 규칙이 구속력을 가집니다. 전송 상한은 여러 에셋에 걸쳐 구속력을
가집니다. 100 MiB Router 상한은 각 요소가 아니라 요청 전체를 제한하기
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이 있는 경우
...
wan2.7 모델의 경우, 지정하지 않으면 해상도 등급에 따라 기본값이 정해집니다.
