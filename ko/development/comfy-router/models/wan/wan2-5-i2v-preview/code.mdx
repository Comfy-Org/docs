...
description: "Comfy Router를 통해 HTTP로 wan/wan2.5-i2v-preview를 호출하기 위한 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64 인코딩 데이터입니다. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 프로바이더가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000 픽셀. 파일 크기: 최대 10MB.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수, 비디오 입력이 있으면 총
  입력 비디오 재생 시간 + 출력 비디오 재생 시간이 30초를 초과할 수 없으며, -1은
...
wan3.0-video의 경우 adaptive(기본값)가 입력 미디어 비율과 의도에 따라 적합한
화면 비율을 자동으로 추천합니다.
