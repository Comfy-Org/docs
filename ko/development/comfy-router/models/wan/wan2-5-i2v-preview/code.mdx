---
title: "Comfy Router로 Wan 2.5 I2V Preview 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.5-i2v-preview를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
sidebarTitle: "Wan 2.5 I2V Preview"
translationSourceHash: 9fa475e9
translationFrom: development/comfy-router/models/wan/wan2-5-i2v-preview/code.mdx
translationBlockHashes:
  "_intro": 92f3b1d3
  "Quick start": 38ecca40
  "Schema": 7b4875d3
  "Examples": 2b581e5f
---
...
Wan 2.5 I2V Preview의 API 레퍼런스입니다. Wan 2.5 이미지 기반 비디오 생성은 정지된 첫 프레임을 짧은 비디오로 변환하며, 움직임과 카메라 무브는 프롬프트로 설명합니다.
...
**모델 ID:** `wan/wan2.5-i2v-preview`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.5-i2v-preview/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 기반 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
<ParamField body="input.audio_url" type="string">
  오디오 파일 다운로드 URL입니다. 지원 형식: mp3와 wav. reference_video_urls와 함께 사용할 수 없습니다.
</ParamField>

<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64 인코딩 데이터입니다.
wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다.
happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다.
첫 프레임은 type이 `first_frame`인 `media` 요소로 전달하며(아래 `media` 참조),
...
부풀리므로, 약 15MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기도 전에 거부됩니다. body에 그러한 에셋이 여러 개 들어 있으면 더욱
빠릅니다. 20 MiB Router 상한은 각 요소가 아니라 요청 전체를 제한하기
...
<ParamField body="input.prompt" type="string">
  텍스트 프롬프트 단어입니다. 중국어와 영어를 지원하며, 길이는 800자를 초과할 수
없습니다 (wan3.0-video의 경우 최대 20,000자이며, 제한을 초과하는 콘텐츠는 잘립니다).
...
참조 재생 시간: 단일 비디오 최대 5초, 비디오 두 개는 각각 최대 2.5초, 비디오 세 개는 그에 비례하여 더 짧습니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약이 없습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`를 직접 v1 호출할 때는 반드시 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
Router가 `GET /v2/models/wan/wan2.5-i2v-preview/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이 문서는 요청이 공급자에 도달하기 이전에 호출을 검증할 때 사용하는 문서와 동일합니다.
...
<ResponseField name="output.actual_prompt" type="string">
  지능형 재작성 이후의 실제 프롬프트 (비디오 작업의 경우)
</ResponseField>
