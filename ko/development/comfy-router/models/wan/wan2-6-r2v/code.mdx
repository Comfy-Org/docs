---
title: "Comfy Router로 Wan 2.6 R2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.6-r2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
Wan 2.6 R2V API 레퍼런스입니다. Wan 2.6 레퍼런스 기반 비디오 생성은 하나에서 세 개의 레퍼런스 비디오에 등장하는 피사체를 새로운 장면으로 가져오며, 프롬프트에서 character1, character2 등으로 지정합니다.
...
**모델 ID:** `wan/wan2.6-r2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v`
...
```
  ...
  </Tab>  <Tab title="대기열에 제출하고 나중에 수집">
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 되면 이 프로세스에서든 다른 프로세스에서든 수집합니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 단계별로 안내합니다.
...
여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media`
요소로 전달하며(아래 `media` 참조), wan2.6-i2v에서 성공하는 `img_url` body는
...
<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록. 비디오 생성을 위한 참조
...
상한이며 이를 초과하면 413 응답이 반환됩니다. base64는 페이로드를 약 4/3로
부풀리므로, 약 75 MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기도 전에 거부됩니다. 그 정도 크기에서는 파트너 자체의 20MB 상한에
해당하는 단일 에셋이라도 인라인으로 충분히 전송되므로, 에셋 하나에 대해서는
파트너 규칙이 구속력을 갖습니다. 전송 상한은 이러한 에셋이 여러 개일 때
구속력을 갖는데, 100 MiB Router 상한은 각 요소가 아니라 전체 요청을
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, 예를 들어 happyhorse-1.x의 i2v와 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 최소 픽셀 기준, 그리고 인라인 페이로드를 총합으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
...
참조 재생 시간: 단일 비디오 최대 5초, 비디오 두 개인 경우 각각 최대 2.5초, 세 개인 경우 그보다 비례적으로 짧습니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID. 이 컴포넌트에서는 제약이 없습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 작업 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">
  wan2.7-videoedit 모델용 비디오 오디오 설정.
...
Router가 `GET /v2/models/wan/wan2.6-r2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
