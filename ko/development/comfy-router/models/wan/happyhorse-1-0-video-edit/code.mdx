...
# 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에서 재사용합니다.
with Comfy() as client:
...
Comfy를 거치지 않기 때문입니다. 인라인 data: URL은 실제로 Comfy를 거치므로
...
도달하기도 전에 거부됩니다. 그 크기에서는 파트너 자체의 20MB 상한에 해당하는
단일 에셋도 인라인으로 충분히 들어가므로, 에셋 하나에 대해서는 파트너 규칙이
구속력을 가집니다. 전송 상한은 여러 에셋에 걸쳐 구속력을 가지는데,
100 MiB Router 상한이 각 요소가 아니라 요청 전체를 제한하기 때문입니다. 이
상한에 가까운 것은 공개 URL로 전송하세요.
</ParamField>
