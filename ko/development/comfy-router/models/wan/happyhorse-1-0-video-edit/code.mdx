---
title: "Comfy Router로 HappyHorse 1.0 Video Edit 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/happyhorse-1.0-video-edit을 호출하기 위한 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
HappyHorse 1.0 Video Edit API 레퍼런스. HappyHorse 1.0 비디오 편집은 텍스트 지시문에 따라 기존 클립을 다시 작성하며, 선택적으로 참조 이미지를 가이드로 사용할 수 있습니다.
...
**모델 ID:** `wan/happyhorse-1.0-video-edit`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-video-edit`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-video-edit/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 설명합니다.
...
`first_frame` 유형의 `media` 요소로 전달하며(아래 `media` 참조),
wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와
happyhorse-1.1-i2v에서 공급자가 거부합니다.
이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000 픽셀.
파일 크기: 최대 10MB.
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록입니다. 비디오 생성에 사용할 참조
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  동일한 요청 내에서 상호 배타적입니다. 배열 순서가 프롬프트에서 에셋의 참조 순서를
...
위의 에셋별 "최대 20MB" 수치는 최종적으로 받게 되는 이미지에 대한 파트너의
상한이며, 에셋이 공개 URL일 때는 그대로 적용됩니다. 해당 바이트가
Comfy를 거치지 않기 때문입니다. 인라인 data: URL은 Comfy를 거치므로
더 낮은 전송 상한에 먼저 부딪힙니다. Comfy Router의 POST body는 총 20 MiB가
상한이며 이를 초과하면 413으로 응답합니다. 그리고 base64는 페이로드를 약 4/3로
부풀리므로, 약 15 MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기도 전에 거부됩니다. body에 그러한 에셋이 여러 개 실리면 더욱 빨라집니다.
20 MiB Router 상한은 각 요소가 아니라 요청 전체를 제한하기 때문입니다. 이 상한에
가까운 것은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.negative_prompt" type="string">
  역방향 프롬프트는 비디오 화면에 나타나지 않았으면 하는 콘텐츠를 설명하는 데 사용됩니다
</ParamField>

<ParamField body="input.prompt" type="string">
  텍스트 프롬프트입니다. 중국어와 영어를 지원하며 길이는 800자를 초과할 수 없습니다
(wan3.0-video의 경우 최대 20,000자, 제한을 초과하는 내용은 잘립니다).
여러 참조 비디오를 사용하는 wan2.6-r2v의 경우 'character1', 'character2' 등을 사용하여 참조
비디오 순서대로 피사체를 지칭합니다. 예: "Character1 sings on the roadside, Character2 dances beside it"
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약되지 않습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 이 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이 값을 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
해상도 등급이 청구 요율을 결정하기 때문에, 이 프록시는 해상도와
...
<ParamField body="parameters.seed" type="integer">
  난수 시드이며, 모델이 생성하는 콘텐츠의 무작위성을 제어하는 데 사용됩니다
...
<ParamField body="parameters.size" type="string">
  비디오 해상도이며 너비*높이 형식입니다. 지원되는 해상도는 모델에 따라 다릅니다:
...
Router가 `GET /v2/models/wan/happyhorse-1.0-video-edit/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 전에 호출을 검증하는 기준이 되는 것과 동일한 문서입니다.
...
<ResponseField name="message" type="string">
  실패한 요청에 대한 상세 정보로, `output` 아래가 아니라 envelope의 ROOT에 보고됩니다 (요청이 성공하면 반환되지 않음). `output.message`로 대체하기 전에 이 항목을 확인하세요.
</ResponseField>
