---
title: "Comfy Router로 Wan 2.6 T2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan2.6-t2v를 호출하기 위한 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
Wan 2.6 T2V의 API 레퍼런스입니다. Wan 2.6 텍스트 기반 비디오 생성은 프롬프트만으로 720P 또는 1080P 해상도, 최대 15초 길이의 짧은 비디오를 만듭니다.
...
**모델 ID:** `wan/wan2.6-t2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan2.6-t2v`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan2.6-t2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집할 수 있습니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 안내합니다.
...
`first_frame`인 `media` 요소로 전달합니다(아래 `media` 참조). 또한
...
참조 자료(이미지, 오디오, 비디오)를 지정합니다. 각 요소는 type과 url 필드를 포함합니다.
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  함께 사용할 수 없음). reference_*/file/link 타입과 first_frame/last_frame 타입은
...
- happyhorse-1.x-i2v: first_frame만, 정확히 1개. 최소 300x300 픽셀,
  ...
  이 표기들은 img_url을 받지 않습니다. 첫 프레임은 여기에 넣습니다.
...
위의 에셋별 "최대 20MB" 수치는 파트너가 최종적으로 받는 이미지에 대한
...
도달하기도 전에 거부됩니다. body에 이러한 에셋이 여러 개 들어 있으면
더욱 빨리 거부됩니다. 20 MiB Router 상한은 각 요소가 아니라 전체 요청을
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우(예: happyhorse-1.x의 i2v와 r2v 표기) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 파트너 규칙이 적용되기 전에 인라인 페이로드를 제한하는 20 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
...
없습니다 (wan3.0-video의 경우 최대 20,000자, 제한을 초과한 콘텐츠는 잘립니다).
...
media 배열 내 해당 순서의 미디어 에셋을 지칭합니다.
</ParamField>
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약되지 않습니다. Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 이를 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`를 직접 v1 호출하는 경우 반드시 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan2.6-r2v: 5 또는 10초만 (15초 지원 없음)
- wan2.7-i2v, wan2.7-t2v: [2, 15] 범위의 정수
- wan2.7-r2v, wan2.7-videoedit: [2, 10] 범위의 정수
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이고, 비디오 입력이 있으면
...
wan3.0-video의 경우, adaptive(기본값)는 입력 미디어의 비율과 의도에 따라 적절한
비율을 자동으로 추천합니다.
...
Router가 `GET /v2/models/wan/wan2.6-t2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="message" type="string">
  실패한 요청에 대한 상세 정보로, `output` 아래가 아니라 envelope의 ROOT에 보고됩니다 (요청이 성공하면 반환되지 않음). `output.message`로 대체하기 전에 이 값을 확인하세요.
</ResponseField>
