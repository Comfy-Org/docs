...
# 3. 수집합니다. 아직 실행 중이면 상태 본문과 함께 202를, 완료되면 모델의 기본 출력과 함께 200을 반환합니다.
...
해당하는 단일 에셋은 인라인으로 충분히 들어가므로, 에셋 하나에 대해서는
파트너 규칙이 구속력을 가집니다. 반면 전송 상한은 여러 에셋에 걸쳐
구속력을 가집니다. 100 MiB Router 상한은 각 요소가 아니라 전체 요청을
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우(예: happyhorse-1.x의 i2v와 r2v 표기) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 인라인 페이로드를 전체적으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">  비디오 오디오 설정(wan2.7-videoedit 모델용)입니다.
...
이 프록시는 resolution과 size를 모두 제공하지 않는 비디오 생성 요청을
거부합니다. 해상도 등급이 청구 요율을 결정하기 때문입니다.

  가능한 값: `480P`, `720P`, `1080P`
</ParamField>

<ParamField body="parameters.seed" type="integer">
  무작위 숫자 시드로, 모델이 생성하는 콘텐츠의 무작위성을 제어하는 데 사용됩니다
