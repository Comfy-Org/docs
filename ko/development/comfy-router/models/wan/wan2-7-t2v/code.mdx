...
때문입니다. 인라인 data: URL은 Comfy를 거치므로 전송 상한도 적용됩니다. Comfy
Router의 POST body는 총 100 MiB가 상한이며 이를 초과하면 413이 반환되고,
base64는 페이로드를 약 4/3로 부풀리므로, 약 75 MB를 넘는 단일 인라인 에셋은
여기에 설명된 파트너 규칙에 도달하기도 전에 거부됩니다. 그 정도 크기라면 파트너
자체의 20MB 상한에 해당하는 단일 에셋은 인라인으로 충분히 전송되므로, 에셋
하나에 대해서는 파트너 규칙이 구속력을 가집니다. 에셋이 여러 개에 걸치면 전송
상한이 구속력을 가지는데, 100 MiB Router 상한은 각 요소가 아니라 전체 요청을
제한하기 때문입니다. 이 상한에 가까운 것은 공개 URL로 전송하세요.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이 있으면
  입력 비디오 재생 시간과 출력 비디오 재생 시간의 합이 30초를 초과할 수 없습니다. -1은
...
wan2.7 모델의 경우 제공하지 않으면 해상도 등급에 따라 기본값이 정해집니다.
wan3.0-video의 경우 adaptive(기본값)가 입력 미디어의 비율과 의도에 따라
적합한 화면 비율을 자동으로 추천합니다.
...
- wan2.6-i2v: 720P, 1080P만 지원 (480P 지원 없음)
