...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK를 사용하며(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지), cURL 스니펫은 동일한 호출을 raw HTTP로 수행합니다.
...
도달하기도 전에 거부됩니다. body에 그러한 에셋이 여러 개 들어 있으면 거부는
더욱 빨라집니다. 20 MiB Router 상한은 각 요소가 아니라 전체 요청을 제한하기
...
<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 비율입니다. wan2.7 및 wan3.0 모델 전용입니다.
wan2.7 모델의 경우 제공되지 않으면 해상도 등급에 따라 기본값이 정해집니다.
wan3.0-video의 경우 adaptive(기본값)가 입력 미디어의 비율과 의도를 바탕으로
적절한 비율을 자동으로 추천합니다.
