...
동일한 body를 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-i2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비되면 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
도달하기 전에 거부됩니다. body에 그러한 에셋이 여러 개 담기면 거부되는
시점은 더욱 앞당겨집니다. 20 MiB Router 상한은 각 요소가 아니라 전체 요청을
제한하기 때문입니다. 이러한 상한에 근접한 항목은 공개 URL로 전송하세요.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수, 비디오 입력이 있으면
  입력 비디오 재생 시간 + 출력 비디오 재생 시간의 합계가 30초를 초과할 수 없습니다.
...
<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 비율입니다. wan2.7 및 wan3.0 모델에만 해당됩니다.
