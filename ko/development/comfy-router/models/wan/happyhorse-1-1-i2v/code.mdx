---
title: "Comfy Router로 HappyHorse 1.1 I2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/happyhorse-1.1-i2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
HappyHorse 1.1 I2V의 API 레퍼런스입니다. HappyHorse 1.1 이미지 기반 비디오 생성은 정지 상태의 첫 프레임을 애니메이션으로 만들고, 한 번의 패스에서 동기화된 대사, 효과음, 음악을 함께 렌더링합니다.
...
**모델 ID:** `wan/happyhorse-1.1-i2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-i2v`
...
동일한 body를 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-i2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비되면 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
# 3. 수집. 모델의 네이티브 출력과 함께 200, 아직 실행 중이면 상태 body와 함께 202.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64로 인코딩된 데이터입니다. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달하며(아래 `media` 참조), wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 공급자가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000 픽셀. 파일 크기: 최대 10MB.
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델의 미디어 에셋 목록입니다. 비디오 생성에 사용할 참조
자료(이미지, 오디오, 비디오)를 지정합니다. 각 요소는 type과 url 필드를
포함합니다.
...
- wan3.0-video: first_frame(최대 1개), last_frame(최대 1개), reference_image(최대 10개),
  ...
  file과 함께 사용할 수 없음). reference_*/file/link 유형과 first_frame/last_frame
  유형은 동일한 요청 내에서 상호 배타적입니다. 배열 순서는 프롬프트에서
  에셋의 참조 순서(Image 1, Video 1, Audio 1, ...)를 정의합니다.
...
전송 상한도 적용됩니다. Comfy Router의 POST body는 총 100 MiB가
...
해당하는 단일 에셋은 인라인으로도 충분히 전송되므로, 에셋 하나에는 파트너
규칙이 적용됩니다. 전송 상한은 여러 에셋을 함께 보낼 때 제약이 되는데,
100 MiB Router 상한이 각 요소가 아니라 전체 요청을 제한하기 때문입니다.
이러한 상한에 근접한 항목은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, happyhorse-1.x의 i2v 및 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 인라인 페이로드를 총합으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>

<ParamField body="input.negative_prompt" type="string">
  역프롬프트는 비디오 화면에 나타나지 않았으면 하는 콘텐츠를 설명하는 데 사용됩니다
</ParamField>
...
wan3.0-video 참조 모드의 경우 'Image 1', 'Video 1', 'Audio 1' 등을 사용하여
media 배열에서 해당 순서에 있는 미디어 에셋을 지칭합니다.
</ParamField>
...
- 단일 비디오 길이: 2-30초
- 단일 파일 크기: 최대 30MB
- audio_url과 함께 사용할 수 없습니다
참조 재생 시간: 단일 비디오는 최대 5초, 비디오 두 개는 각각 최대 2.5초, 세 개는 그에 비례하여 더 짧습니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약이 없습니다. Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 이 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이 값을 제공해야 하며, 허용되는 표기의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이 있으면
  입력 비디오 재생 시간 + 출력 비디오 재생 시간의 합이 30초를 초과할 수 없습니다.
  -1은 모델이 적절한 재생 시간을 선택하는 스마트 재생 시간 모드를 활성화합니다

  범위: `-1` ~ `30`
</ParamField>

<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성 사용 여부입니다. 기본값은 참입니다
</ParamField>
...
Router가 `GET /v2/models/wan/happyhorse-1.1-i2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
