---
title: "Comfy Router로 HappyHorse 1.1 T2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/happyhorse-1.1-t2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
HappyHorse 1.1 T2V API 레퍼런스입니다. HappyHorse 1.1 텍스트 기반 비디오 생성은 프롬프트만으로 대사, 효과, 음악이 동기화된 짧은 비디오를 만듭니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 동일한 호출을 순수 HTTP로 실행한 것입니다.

**모델 ID:** `wan/happyhorse-1.1-t2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-t2v`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-t2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비가 완료되면 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소 및 수집 방법을 자세히 다룹니다.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64로 인코딩된 데이터입니다. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 공급자가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000픽셀. 파일 크기: 최대 10MB.
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록입니다. 비디오 생성에 사용할 참조 자료(이미지, 오디오, 비디오)를 지정합니다.
...
도달합니다. Comfy Router의 POST body는 총 20 MiB가 상한이며 이를 초과하면
413이 반환됩니다. base64는 페이로드를 약 4/3로 부풀리므로, 약 15 MB를 넘는
단일 인라인 에셋은 여기에 설명된 파트너 규칙에 도달하기 전에 거부됩니다.
body에 이러한 에셋이 여러 개 있으면 더욱 빨리 거부됩니다. 20 MiB Router 상한은
각 요소가 아니라 전체 요청을 제한하기 때문입니다. 이 상한에 가까운
것은 공개 URL로 전송하세요.
</ParamField>
...
(wan3.0-video의 경우 최대 20,000자이며, 한도를 초과하는 콘텐츠는 잘립니다).
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성의 활성화 여부입니다. 기본값은 참입니다
</ParamField>
...
Router가 `GET /v2/models/wan/happyhorse-1.1-t2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
