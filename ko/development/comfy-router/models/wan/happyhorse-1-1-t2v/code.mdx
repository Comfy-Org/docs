...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-t2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비가 완료되면 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
때문입니다. 인라인 data: URL은 Comfy를 거치므로 전송 상한도 함께 적용됩니다.
Comfy Router의 POST body는 총 100 MiB가 상한이며 이를 초과하면
413이 반환되고, base64는 페이로드를 약 4/3로 부풀리므로 약 75 MB를 넘는
단일 인라인 에셋은 여기에 설명된 파트너 규칙에 도달하기 전에 거부됩니다.
그 정도 크기라면 파트너 자체의 20MB 상한에 해당하는 단일 에셋은 인라인으로도
충분히 전송되므로, 에셋 하나에 대해서는 파트너 규칙이 구속력을 갖습니다. 전송
상한은 여러 에셋에 걸쳐 적용되는데, 100 MiB Router 상한은 각 요소가 아니라
전체 요청을 제한하기 때문입니다. 이 상한에 가까운
것은 공개 URL로 전송하세요.
</ParamField>
...
Careful: my job is to translate the whole English fragment. Let me produce it.
...
- wan2.5 모델: 5초 또는 10초
- wan2.6-t2v, wan2.6-i2v: 5초, 10초, 15초
- wan2.6-r2v: 5초 또는 10초만 지원 (15초 지원 없음)
...
wan2.7 모델의 경우, 제공되지 않으면 해상도 등급을 기준으로 기본값이 설정됩니다.
