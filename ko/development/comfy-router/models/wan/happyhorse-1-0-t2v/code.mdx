---
title: "Comfy Router로 HappyHorse 1.0 T2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/happyhorse-1.0-t2v를 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
HappyHorse 1.0 T2V API 레퍼런스입니다. HappyHorse 1.0 텍스트 기반 비디오 생성은 프롬프트 하나만으로 동기화된 오디오가 포함된 짧은 비디오를 만듭니다.
...
**모델 ID:** `wan/happyhorse-1.0-t2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-t2v`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-t2v/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [큐 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 안내합니다.
...
<ParamField body="input.audio_url" type="string">
  오디오 파일 다운로드 URL입니다. 지원 형식: mp3, wav. reference_video_urls와 함께 사용할 수 없습니다.
</ParamField>
...
<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록입니다. 비디오 생성에 사용할 참조 자료(이미지, 오디오, 비디오)를
...
위의 에셋별 "최대 20MB" 수치는 최종적으로 만들어지는 이미지에 대한 파트너의
상한이며, 에셋이 공개 URL일 때는 그대로 적용됩니다. 해당 바이트가 Comfy를
거치지 않기 때문입니다. 인라인 data: URL은 Comfy를 거치므로 전송 상한도
함께 적용됩니다. Comfy Router의 POST body는 총 100 MiB가 상한이며,
이를 초과하면 413으로 응답합니다. base64는 페이로드를 약 4/3로 부풀리므로,
약 75 MB를 넘는 단일 인라인 에셋은 여기의 파트너 규칙에 도달하기
전에 거부됩니다. 이 정도 크기에서는 파트너 자체의 20MB 상한에 달하는 단일
에셋도 인라인으로 여유 있게 들어가므로, 에셋 하나에 대해서는 파트너 규칙이
구속력을 가집니다. 전송 상한은 여러 에셋에 걸칠 때 구속력을 가지는데,
100 MiB Router 상한이 각 요소가 아니라 전체 요청을 제한하기
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, 예를 들어 happyhorse-1.x의 i2v와 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 인라인 페이로드를 전체적으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>

<ParamField body="input.negative_prompt" type="string">
  역방향 프롬프트 단어로, 비디오 화면에 나타나지 않았으면 하는 콘텐츠를 설명하는 데 사용됩니다
</ParamField>

<ParamField body="input.prompt" type="string">
  텍스트 프롬프트 단어입니다. 중국어와 영어를 지원하며 길이는 800자를 초과할 수 없습니다
(wan3.0-video의 경우 최대 20,000자, 제한을 초과한 콘텐츠는 잘립니다).
참조 비디오가 여러 개인 wan2.6-r2v의 경우 'character1', 'character2' 등을 사용하여 참조 비디오
순서대로 피사체를 지칭합니다. 예: "Character1 sings on the roadside, Character2 dances beside it"
wan3.0-video 참조 모드에서는 'Image 1', 'Video 1', 'Audio 1' 등을 사용하여 media 배열 내
...
참조 재생 시간: 단일 비디오 최대 5초, 비디오 2개는 각각 최대 2.5초, 비디오 3개는 그보다 비례하여 짧아집니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약이 없습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이 있으면
...
<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 화면 비율입니다. wan2.7 및 wan3.0 모델에만 해당됩니다.
wan2.7 모델의 경우 값을 제공하지 않으면 해상도 등급을 기준으로 기본값이 정해집니다.
...
Router가 `GET /v2/models/wan/happyhorse-1.0-t2v/openapi.json`에서 제공하는 스키마에서 생성되었습니다. 이 문서는 요청이 공급자에 도달하기 전에 호출을 검증할 때 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="code" type="string">
  실패한 요청의 오류 코드로, `output` 아래가 아니라 envelope의 ROOT에 보고됩니다 (요청이 성공하면 반환되지 않습니다).
</ResponseField>

<ResponseField name="message" type="string">
  실패한 요청에 대한 상세 정보로, `output` 아래가 아니라 envelope의 ROOT에 보고됩니다 (요청이 성공하면 반환되지 않습니다). `output.message`로 대체하기 전에 이 값을 먼저 확인하세요.
</ResponseField>
