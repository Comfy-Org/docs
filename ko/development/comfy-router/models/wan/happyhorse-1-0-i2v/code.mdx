---
title: "Comfy Router로 HappyHorse 1.0 I2V 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/happyhorse-1.0-i2v을 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
{/* GENERATED FILE. 이 디렉터리의 code.yaml을 편집한 뒤 `pnpm code-pages:gen`을 실행하세요. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

HappyHorse 1.0 I2V API 레퍼런스입니다. HappyHorse 1.0 이미지 기반 비디오 생성은 정지된 첫 프레임을 애니메이션화하고 동일한 패스에서 동기화된 오디오를 렌더링합니다.
...
**모델 ID:** `wan/happyhorse-1.0-i2v`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-i2v`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-i2v/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기 중 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 살펴봅니다.
...
# 2. 각 응답이 지정한 Retry-After 초만큼 대기하면서 상태가 COMPLETED가 될 때까지 폴링합니다.
...
wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의
i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이
`first_frame`인 `media` 요소로 전달하며(아래 `media` 참조), wan2.6-i2v에서
성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는
공급자가 거부합니다.
...
<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델용 미디어 에셋 목록입니다. 비디오 생성을 위한 참조
자료(이미지, 오디오, 비디오)를 지정합니다. 각 요소는 type과 url 필드를 포함합니다.
...
- wan3.0-video: first_frame (최대 1), last_frame (최대 1), reference_image (최대 10),
  ...
  함께 사용할 수 없음). reference_*/file/link type과 first_frame/last_frame type은
  동일한 요청 내에서 함께 사용할 수 없습니다. 배열 순서는 프롬프트에서 에셋의 참조 순서를
...
에셋이 공개 URL일 때는 명시된 그대로 적용됩니다. 그 바이트가 Comfy를 거치지
않기 때문입니다. 인라인 data: URL은 Comfy를 거치므로 전송 상한도 함께
적용됩니다. Comfy Router의 POST body는 총 100 MiB가 상한이며 이를 초과하면
413으로 응답합니다. base64는 페이로드를 약 4/3로 부풀리므로, 약 75 MB를 넘는
단일 인라인 에셋은 여기의 파트너 규칙에 도달하기도 전에 거부됩니다. 그 정도
크기에서는 파트너 자체 상한인 20MB의 단일 에셋이 인라인으로도 넉넉히 들어가므로,
에셋 하나에 대해서는 파트너 규칙이 구속력을 가집니다. 전송 상한은 여러 에셋에
걸쳐 구속력을 가집니다. 100 MiB Router 상한은 각 요소가 아니라 전체 요청을
제한하기 때문입니다. 이러한 상한에 가까운 것은 공개 URL로 전송하세요.
</ParamField>
...
(wan3.0-video의 경우 최대 20,000자, 제한을 초과하는 콘텐츠는 잘립니다).
...
참조 재생 시간: 단일 비디오 최대 5초, 비디오 2개는 각각 최대 2.5초, 비디오 3개는 그보다 비례하여 짧습니다.
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약되지 않습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 값을 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 반드시 이를 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan2.6-r2v: 5초 또는 10초만 (15초 지원 없음)
- wan2.7-i2v, wan2.7-t2v: [2, 15] 범위의 정수
- wan2.7-r2v, wan2.7-videoedit: [2, 10] 범위의 정수
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이 있으면 총
...
<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 비율입니다. wan2.7 및 wan3.0 모델에만 해당합니다.
...
- wan2.6-i2v: 720P, 1080P만 (480P 지원 없음)
- wan2.7 모델 (i2v, t2v, r2v, videoedit): 720P, 1080P (기본 1080P)
- wan3.0-video, wan3.0-video-prime: 480P, 720P, 1080P (업스트림 기본 1080P)
이 프록시는 해상도와 size를 모두 제공하지 않는 비디오 생성 요청을 거부합니다.
해상도 등급이 요금 청구 기준을 선택하기 때문입니다.
...
Router가 `GET /v2/models/wan/happyhorse-1.0-i2v/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
