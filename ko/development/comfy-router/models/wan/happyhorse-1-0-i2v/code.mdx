...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-i2v/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 살펴봅니다.
...
- wan3.0-video: 비디오 입력이 없으면 [2, 30] 범위의 정수이며, 비디오 입력이
  있으면 입력 비디오 재생 시간 + 출력 비디오 재생 시간의 합이 30초를 초과할 수
  없습니다. -1은 모델이 적절한 재생 시간을 선택하는 스마트 재생 시간 모드를
  활성화합니다
