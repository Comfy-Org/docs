---
title: "Comfy Router로 Wan 3.0 Video Prime 사용하기"
description: "Comfy Router를 통해 HTTP로 wan/wan3.0-video-prime을 호출하는 Python, TypeScript, cURL 스니펫과 요청 필드 및 결과 형태"
...
Wan 3.0 Video Prime에 대한 API 레퍼런스입니다. Wan 3.0 Video Prime은 Wan 3.0 Video의 더 높은 품질 등급이며, 동일한 Router 프롬프트와 미디어 스키마에 대해 검증되었습니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 원시 HTTP로 동일한 호출을 수행합니다.

**모델 ID:** `wan/wan3.0-video-prime`

**엔드포인트:** `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되는 대로 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [큐 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 안내합니다.
...
<ParamField body="input.img_url" type="string">
  첫 프레임 이미지 URL 또는 Base64로 인코딩된 데이터. wan2.5-i2v-preview 및 wan2.6-i2v 모델에서만 필수입니다. happyhorse-1.x의 i2v 표기는 여기에서 첫 프레임을 받지 않습니다. 첫 프레임은 type이 `first_frame`인 `media` 요소로 전달합니다 (아래 `media` 참조). wan2.6-i2v에서 성공하는 `img_url` body는 happyhorse-1.0-i2v와 happyhorse-1.1-i2v에서는 공급자가 거부합니다. 이미지 형식: JPEG, JPG, PNG, BMP, WEBP. 해상도: 360-2000픽셀. 파일 크기: 최대 10MB.
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7, wan3.0 및 happyhorse-1.x 모델의 미디어 에셋 목록. 비디오 생성을 위한 참조 자료(이미지, 오디오, 비디오)를
지정합니다. 각 요소는 type 및 url 필드를 포함합니다.
...
위의 에셋별 "최대 20MB" 수치는 최종적으로 받는 이미지에 대한 파트너 측
...
도달하기도 전에 거부됩니다. 이 정도 크기라면 파트너 자체의 20MB 상한에
해당하는 단일 에셋은 인라인으로 충분히 전송할 수 있으므로, 에셋 하나에
대해서는 파트너 규칙이 구속력을 가집니다. 100 MiB Router 상한은 각 요소가
아니라 전체 요청을 제한하므로, 여러 에셋을 보낼 때는 전송 상한이
구속력을 가집니다. 이 상한에 가까운 것은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  미디어 파일의 URL입니다. 공개 HTTP/HTTPS URL, OSS 임시 URL, 또는 (위 `media` 설명에서 해당 모델의 항목이 그렇다고 명시한 경우, 예를 들어 happyhorse-1.x의 i2v와 r2v 표기처럼) 인라인 `data:{MIME_type};base64,...` URL입니다. 모델별 크기와 픽셀 하한, 그리고 인라인 페이로드를 총합 기준으로 제한하는 100 MiB Router 요청 body 상한에 대해서는 해당 설명을 참조하세요.
</ParamField>

<ParamField body="input.negative_prompt" type="string">
  비디오 화면에서 보고 싶지 않은 콘텐츠를 설명하는 데 사용되는 반대 프롬프트 단어
</ParamField>
...
여러 참조 비디오가 있는 wan2.6-r2v의 경우 'character1', 'character2' 등을 사용하여 참조
비디오 순서대로 피사체를 참조합니다. 예: "Character1 sings on the roadside, Character2 dances beside it"
wan3.0-video 참조 모드의 경우 'Image 1', 'Video 1', 'Audio 1' 등을 사용하여 media 배열 내
해당 순서의 미디어 에셋을 참조합니다.
</ParamField>
...
<ParamField body="model" type="string">
  호출할 모델의 ID입니다. 이 컴포넌트에서는 제약되지 않습니다: Comfy Router가 `POST /v2/models/wan/{model}`의 `{model}` 경로 세그먼트에서 이를 채우므로 Router 호출자는 이를 생략합니다. `POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis`에 대한 직접 v1 호출은 이를 반드시 제공해야 하며, 허용되는 표기법의 enum은 해당 오퍼레이션 자체의 컴포넌트인 `WanVideoGenerationRequest`에 있습니다.
</ParamField>
...
- wan2.6-r2v: 5초 또는 10초만 지원 (15초 미지원)
...
wan2.7 모델의 경우, 제공되지 않으면 해상도 등급을 기준으로 기본값이 정해집니다.
...
- wan2.6-i2v: 720P, 1080P만 지원 (480P 미지원)
...
<ParamField body="parameters.shot_type" type="string" default="&quot;single&quot;">
  지능형 다중 렌즈 제어입니다. prompt_extend가 활성화된 경우에만 동작합니다.
wan2.6 및 wan2.7-r2v 모델에 적용됩니다.
- single: 단일 샷 비디오 (기본값)
- multi: 다중 샷 비디오

  사용 가능한 값: `multi`, `single`
</ParamField>
...
Router가 `GET /v2/models/wan/wan3.0-video-prime/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
