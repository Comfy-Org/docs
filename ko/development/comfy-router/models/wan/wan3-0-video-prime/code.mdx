...
- wan2.6-r2v: 5 또는 10초만 가능 (15초 미지원)
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능적 재작성을 활성화할지 여부. 기본값은 참
</ParamField>

<ParamField body="parameters.ratio" type="string">
  생성된 비디오의 비율. wan2.7 및 wan3.0 모델 전용.
...
이 프록시는 해상도와 크기 모두 제공하지 않는 비디오 생성 요청을 거부합니다.
...
`GET /v2/models/wan/wan3.0-video-prime/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
### 출력
