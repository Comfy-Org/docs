...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 원시 HTTP로 동일한 호출을 수행합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/wan/wan3.0-video-prime/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되는 대로 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 안내합니다.
...
더 낮은 전송 상한에 먼저 도달합니다. Comfy Router의 POST body는 총 20 MiB가
상한이며 이를 초과하면 413이 반환됩니다. base64는 페이로드를 약 4/3로
부풀리므로, 약 15 MB를 넘는 단일 인라인 에셋은 여기에 설명된 파트너 규칙에
도달하기 전에 거부됩니다. body에 이러한 에셋이 여러 개 있으면 더욱 빨라집니다.
20 MiB는 각 요소가 아니라 전체 요청을 제한하기 때문입니다. 이 상한에 가까운
것은 공개 URL로 전송하세요.
</ParamField>
...
<ParamField body="parameters.prompt_extend" type="boolean" default="true">
  프롬프트 지능형 재작성을 활성화할지 여부입니다. 기본값은 true입니다
</ParamField>
