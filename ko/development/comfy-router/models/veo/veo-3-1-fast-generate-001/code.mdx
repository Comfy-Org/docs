---
title: "Comfy Router로 Veo 3.1 Fast Generate 001 사용하기"
description: "Comfy Router를 통해 veo/veo-3.1-fast-generate-001을 호출합니다: 엔드포인트, 요청 형태, 그리고 Router가 반환하는 응답."
sidebarTitle: "Veo 3.1 Fast Generate 001"
translationSourceHash: 3902ced9
translationFrom: development/comfy-router/models/veo/veo-3-1-fast-generate-001/code.mdx
translationBlockHashes:
  "_intro": 25f9d753
  "Quick start": 25a6a009
  "Schema": 7a21ed84
  "Examples": 88e27039
---

{/* 생성된 파일. `pnpm code-pages:gen`으로 router-schemas/veo/veo-3.1-fast-generate-001.json에서 생성됨. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

Veo가 제공하는 `veo/veo-3.1-fast-generate-001`에 대한 API 레퍼런스이며, Comfy Router를 통해 서비스됩니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys)에서 키를 생성한 뒤 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 동일한 호출을 순수 HTTP로 수행한 것입니다.

**모델 ID:** `veo/veo-3.1-fast-generate-001`

**엔드포인트:** `POST https://api.comfy.org/v2/models/veo/veo-3.1-fast-generate-001`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/veo/veo-3.1-fast-generate-001/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비가 완료되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [큐 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
<ParamField body="instances[].cameraControl" type="string">
  ...
  사용 가능한 값: `fixed`, `pan_left`, `pan_right`, `tilt_up`, `tilt_down`, `truck_left`, `truck_right`, `pedestal_up`, `pedestal_down`, `push_in`, `pull_out`
</ParamField>
...
<ParamField body="instances[].mask.maskMode" type="string">
  ...
  사용 가능한 값: `insert`, `remove`, `remove_static`, `outpaint`
</ParamField>
...
<ParamField body="instances[].referenceImages[].referenceType" type="string" required>
  ...
  사용 가능한 값: `asset`, `style`
</ParamField>

<ParamField body="instances[].video" type="object">
  비디오 확장 또는 편집을 위한 선택적 입력 비디오입니다. image 및 referenceImages와 호환되지 않습니다.
</ParamField>
...
<ParamField body="instances[].video.mimeType" type="string">
  ...
  사용 가능한 값: `video/mov`, `video/mpeg`, `video/mp4`, `video/mpg`, `video/avi`, `video/wmv`, `video/mpegps`, `video/x-flv`
</ParamField>
...
Router가 `GET /v2/models/veo-3.1-fast-generate-001/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력
