...
동일한 본문을 `POST https://api.comfy.org/v2/models/veo/veo-3.1-lite-generate-001/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 완료되면 이 프로세스에서든 다른 프로세스에서든 수집됩니다. [실행 대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 설명합니다.
...
Router가 `GET /v2/models/veo/veo-3.1-lite-generate-001/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력
