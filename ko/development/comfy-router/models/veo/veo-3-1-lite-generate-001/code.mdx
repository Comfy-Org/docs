...
동일한 본문을 `POST https://api.comfy.org/v2/models/veo/veo-3.1-lite-generate-001/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 완료되면 이 프로세스에서든 다른 프로세스에서든 수집됩니다. [큐 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 설명합니다.
