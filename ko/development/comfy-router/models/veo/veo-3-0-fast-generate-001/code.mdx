...
`veo/veo-3.0-fast-generate-001`에 대한 API 레퍼런스입니다. Veo에서 Comfy Router를 통해 제공됩니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/veo/veo-3.0-fast-generate-001/requests`로 전송합니다. 라우터는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집됩니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 안내합니다.
...
```bash cURL
# 1. 제출합니다. 라우터는 request_id, status_url, response_url, cancel_url과 함께 201로 응답합니다.
...
<ParamField body="parameters.seed" type="integer">
  결정론적 출력을 위한 랜덤 시드입니다. sampleCount > 1인 경우 비디오마다 다른 시드가 사용됩니다.
...
Router가 `GET /v2/models/veo-3.0-fast-generate-001/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력
