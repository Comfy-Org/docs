---
title: "Comfy Router로 Veo 3.0 Generate 001 사용하기"
description: "Comfy Router를 통해 veo/veo-3.0-generate-001을 호출합니다: endpoint, 요청 형태, 그리고 Router가 반환하는 응답입니다."
sidebarTitle: "Veo 3.0 Generate 001"
translationSourceHash: ba0d0c93
translationFrom: development/comfy-router/models/veo/veo-3-0-generate-001/code.mdx
translationBlockHashes:
  "_intro": c87da726
  "Quick start": 86fd21d8
  "Schema": e9acd9e6
  "Examples": 88e27039
---


{/* GENERATED FILE. Generated from router-schemas/veo/veo-3.0-generate-001.json by `pnpm code-pages:gen`. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

`veo/veo-3.0-generate-001`에 대한 API 레퍼런스이며, Comfy Router가 Veo에서 제공합니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python과 TypeScript 스니펫은 Comfy SDK를 사용하며(`pip install comfy-sdk`, `npm install @comfyorg/sdk`), cURL 스니펫은 동일한 호출을 raw HTTP로 수행합니다.

**모델 ID:** `veo/veo-3.0-generate-001`

**엔드포인트:** `POST https://api.comfy.org/v2/models/veo/veo-3.0-generate-001`

<Tabs defaultTabIndex={1}>
  <Tab title="결과 기다리기">
<CodeGroup>
```python Python
from comfy_sdk import Comfy

# 환경 변수에서 COMFY_API_KEY를 읽습니다.
# SDK는 멱등성 키를 자동으로 생성하고 자동 재시도에 재사용합니다.
with Comfy() as client:
    result = client.models.run(
        "veo/veo-3.0-generate-001",
        {
            "instances": [
                {
                    "prompt": "a single red maple leaf falling onto still water, slow motion",
                },
            ],
            "parameters": {
                "durationSeconds": 4,
                "generateAudio": False,
                "sampleCount": 1,
            },
        },
    )

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 멱등성 키를 자동으로 생성하고 자동 재시도에 재사용합니다.
const { data } = await comfy.models.run("veo/veo-3.0-generate-001", {
  instances: [
    {
      prompt: "a single red maple leaf falling onto still water, slow motion",
    },
  ],
  parameters: {
    durationSeconds: 4,
    generateAudio: false,
    sampleCount: 1,
  },
});

console.log(data);
```

```bash cURL
curl https://api.comfy.org/v2/models/veo/veo-3.0-generate-001 \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"instances\": [{\"prompt\":\"a single red maple leaf falling onto still water, slow motion\"}], \"parameters\": {\"durationSeconds\":4,\"generateAudio\":false,\"sampleCount\":1}}"
```
</CodeGroup>
  </Tab>
  <Tab title="실행 대기열에 넣고 나중에 수집하기">
동일한 본문을 `POST https://api.comfy.org/v2/models/veo/veo-3.0-generate-001/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `request_id`와 함께 `201`을 응답하고, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전송](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 안내합니다.

<CodeGroup>
```python Python
import asyncio
from comfy_sdk import AsyncComfy

# 환경 변수에서 COMFY_API_KEY를 읽습니다.
# 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
async def main():
    async with AsyncComfy() as client:
        handle = await client.models.submit(
            "veo/veo-3.0-generate-001",
            {
                "instances": [
                    {
                        "prompt": "a single red maple leaf falling onto still water, slow motion",
                    },
                ],
                "parameters": {
                    "durationSeconds": 4,
                    "generateAudio": False,
                    "sampleCount": 1,
                },
            },
        )
        print("request_id:", handle.request_id)  # 모델 ID와 함께라면, 다른 프로세스에 필요한 전부입니다

        # 요청이 완료될 때까지 폴링하며, 서버가 지정한 Retry-After만큼 기다립니다.
        async for update in handle.iter_events():
            print(update.status, update.queue_position)

        # 공급자 자체의 페이로드이며, models.run()이 반환하는 것과 동일한 값입니다.
        # 실패했거나 취소된 요청은 여기에서 타입이 지정된 Router 오류를 발생시킵니다.
        result = await handle.get()

    print(result)

asyncio.run(main())
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
const handle = await comfy.models.submit("veo/veo-3.0-generate-001", {
  instances: [
    {
      prompt: "a single red maple leaf falling onto still water, slow motion",
    },
  ],
  parameters: {
    durationSeconds: 4,
    generateAudio: false,
    sampleCount: 1,
  },
});
console.log("requestId:", handle.requestId); // 모델 ID와 함께라면, 다른 프로세스에 필요한 전부입니다

// 요청이 완료될 때까지 폴링하며, 서버가 지정한 Retry-After만큼 기다립니다.
for await (const update of handle.events()) {
  console.log(update.status, update.queuePosition);
}

// models.run()이 반환하는 것과 동일한 결과입니다. 실패했거나 취소된 요청은 여기에서 거부됩니다.
const result = await handle.get();

console.log(result.data);
```

```bash cURL
# 1. 제출. Router는 request_id, status_url, response_url, cancel_url과 함께 201을 응답합니다.
curl https://api.comfy.org/v2/models/veo/veo-3.0-generate-001/requests \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"instances\": [{\"prompt\":\"a single red maple leaf falling onto still water, slow motion\"}], \"parameters\": {\"durationSeconds\":4,\"generateAudio\":false,\"sampleCount\":1}}"

# 2. 상태가 COMPLETED가 될 때까지 폴링하며, 각 응답이 지정한 Retry-After 초만큼 기다립니다.
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/veo/veo-3.0-generate-001/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 수집. 모델의 네이티브 출력과 함께 200을, 아직 실행 중이면 상태 본문과 함께 202를 반환합니다.
curl https://api.comfy.org/v2/models/veo/veo-3.0-generate-001/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
</CodeGroup>
  </Tab>
</Tabs>

## 스키마

### 입력

<ParamField body="instances" type="object[]">
   
</ParamField>

<ParamField body="instances[].cameraControl" type="string">
  카메라 모션 유형입니다. 이미지를 제공해야 합니다.

  가능한 값: `fixed`, `pan_left`, `pan_right`, `tilt_up`, `tilt_down`, `truck_left`, `truck_right`, `pedestal_up`, `pedestal_down`, `push_in`, `pull_out`
</ParamField>

<ParamField body="instances[].image" type="object">
  비디오 생성을 유도하는 선택적 첫 프레임 이미지
</ParamField>

<ParamField body="instances[].image.bytesBase64Encoded" type="string (byte)">
  Base64로 인코딩된 이미지 데이터

  형식: `byte`
</ParamField>

<ParamField body="instances[].image.gcsUri" type="string">
  이미지의 Cloud Storage URI
</ParamField>

<ParamField body="instances[].image.mimeType" type="string">
  이미지의 MIME 유형 (image/jpeg 또는 image/png)

  가능한 값: `image/jpeg`, `image/png`
</ParamField>

<ParamField body="instances[].lastFrame" type="object">
  선택적 마지막 프레임 이미지입니다. image와 함께 사용하여 첫 프레임과 마지막 프레임 사이의 비디오를 생성합니다. Veo 3.0+ 모델에서 지원됩니다.
</ParamField>

<ParamField body="instances[].lastFrame.bytesBase64Encoded" type="string (byte)">
  Base64로 인코딩된 이미지 데이터

  형식: `byte`
</ParamField>

<ParamField body="instances[].lastFrame.gcsUri" type="string">
  이미지의 Cloud Storage URI
</ParamField>

<ParamField body="instances[].lastFrame.mimeType" type="string">
  이미지의 MIME 유형 (image/jpeg 또는 image/png)

  가능한 값: `image/jpeg`, `image/png`
</ParamField>

<ParamField body="instances[].mask" type="object">
  비디오 편집을 위한 선택적 마스크입니다. 입력 비디오에 적용됩니다.
</ParamField>

<ParamField body="instances[].mask.bytesBase64Encoded" type="string (byte)">
  Base64로 인코딩된 마스크 바이트

  형식: `byte`
</ParamField>

<ParamField body="instances[].mask.gcsUri" type="string">
  마스크 파일의 Cloud Storage URI
</ParamField>

<ParamField body="instances[].mask.maskMode" type="string">
  마스크가 적용되는 방식

  가능한 값: `insert`, `remove`, `remove_static`, `outpaint`
</ParamField>

<ParamField body="instances[].mask.mimeType" type="string">
  마스크의 MIME 유형 (image/png, image/jpeg, image/webp 또는 비디오 형식)
</ParamField>

<ParamField body="instances[].prompt" type="string" required>
  생성할 비디오에 대한 텍스트 설명
</ParamField>

<ParamField body="instances[].referenceImages" type="object[]">
  비디오 생성을 유도하는 선택적 참조 이미지입니다. 최대 3개의 에셋 이미지 또는 1개의 스타일 이미지를 지원합니다. Veo 3.1 모델(미리보기)에서 지원됩니다.
</ParamField>

<ParamField body="instances[].referenceImages[].image" type="object" required>
   
</ParamField>

<ParamField body="instances[].referenceImages[].image.bytesBase64Encoded" type="string (byte)">
  Base64로 인코딩된 이미지 데이터

  형식: `byte`
</ParamField>

<ParamField body="instances[].referenceImages[].image.gcsUri" type="string">
  이미지의 Cloud Storage URI
</ParamField>

<ParamField body="instances[].referenceImages[].image.mimeType" type="string">
  이미지의 MIME 유형 (image/jpeg 또는 image/png)

  가능한 값: `image/jpeg`, `image/png`
</ParamField>

<ParamField body="instances[].referenceImages[].referenceId" type="string">
  참조 이미지의 선택적 식별자
</ParamField>

<ParamField body="instances[].referenceImages[].referenceType" type="string" required>
  참조 이미지의 유형

  가능한 값: `asset`, `style`
</ParamField>

<ParamField body="instances[].video" type="object">
  비디오 확장 또는 편집을 위한 선택적 입력 비디오입니다. image 및 referenceImages와 함께 사용할 수 없습니다.
</ParamField>

<ParamField body="instances[].video.bytesBase64Encoded" type="string (byte)">
  Base64로 인코딩된 비디오 바이트

  형식: `byte`
</ParamField>

<ParamField body="instances[].video.gcsUri" type="string">
  입력 비디오의 Cloud Storage URI
</ParamField>

<ParamField body="instances[].video.mimeType" type="string">
  비디오의 MIME 유형

  가능한 값: `video/mov`, `video/mpeg`, `video/mp4`, `video/mpg`, `video/avi`, `video/wmv`, `video/mpegps`, `video/x-flv`
</ParamField>

<ParamField body="parameters" type="object">
   
</ParamField>

<ParamField body="parameters.aspectRatio" type="string">
  생성된 비디오의 화면 비율입니다. 기본값: 16:9

  가능한 값: `16:9`, `9:16`
</ParamField>

<ParamField body="parameters.compressionQuality" type="string">
  비디오 압축 품질입니다. 기본값: optimized

  가능한 값: `optimized`, `lossless`
</ParamField>

<ParamField body="parameters.durationSeconds" type="number">
  생성된 비디오의 목표 재생 시간(초)입니다. Veo 2: 5-8. Veo 3/3.1: 4, 6 또는 8. 기본값: 8
</ParamField>

<ParamField body="parameters.enhancePrompt" type="boolean">
  더 높은 품질을 위해 프롬프트를 자동으로 개선합니다. 기본값은 참입니다.
</ParamField>

<ParamField body="parameters.fps" type="integer">
  생성된 비디오의 프레임 레이트(초당 프레임 수)
</ParamField>

<ParamField body="parameters.generateAudio" type="boolean">
  비디오와 함께 오디오를 생성할지 여부입니다. 기본값은 참입니다. Veo 3.0+ 모델에서 지원됩니다.
</ParamField>

<ParamField body="parameters.negativePrompt" type="string">
  생성된 비디오에서 피해야 할 내용을 설명하는 텍스트
</ParamField>

<ParamField body="parameters.personGeneration" type="string">
  생성된 비디오의 인물을 제어합니다. 기본값: allow_adult

  가능한 값: `dont_allow`, `allow_adult`, `allowAll`
</ParamField>

<ParamField body="parameters.pubsubTopic" type="string">
  진행 상황 업데이트를 위한 Cloud Pub/Sub 토픽 (projects/\{project}/topics/\{topic})
</ParamField>

<ParamField body="parameters.resizeMode" type="string">
  입력 이미지의 크기 조정 방식입니다. 기본값: pad

  가능한 값: `pad`, `crop`
</ParamField>

<ParamField body="parameters.resolution" type="string">
  출력 비디오 해상도입니다. Veo 3.0+ 모델에서 지원됩니다. 기본값: 720p

  가능한 값: `720p`, `1080p`, `4k`
</ParamField>

<ParamField body="parameters.sampleCount" type="integer">
  생성할 비디오 수입니다. 지정하지 않으면 비디오 1개가 생성됩니다.

  범위: `1` ~ `4`
</ParamField>

<ParamField body="parameters.seed" type="integer">
  결정론적 출력을 위한 랜덤 시드입니다. sampleCount > 1인 경우 비디오마다 다른 시드가 사용됩니다.

  형식: `uint32`
</ParamField>

<ParamField body="parameters.storageUri" type="string">
  생성된 비디오를 저장하기 위한 Cloud Storage URI(gs://)
</ParamField>

<ParamField body="parameters.task" type="string">
  비디오 생성 요청의 작업 유형입니다.

  가능한 값: `textToVideo`, `imageToVideo`, `referenceToVideo`, `edit`, `extend`, `upscale`
</ParamField>

Router가 `GET /v2/models/veo/veo-3.0-generate-001/openapi.json`에서 제공하는 스키마를 기반으로 생성되었습니다. 이 문서는 요청이 공급자에 도달하기 이전에 Router가 호출을 검증할 때 기준으로 삼는 것과 동일한 문서입니다.
### 출력

<ResponseField name="done" type="boolean">
  작업이 완료되었는지 여부
</ResponseField>

<ResponseField name="error" type="object">
  오류 세부 정보. 작업이 실패한 경우 제공됩니다.
</ResponseField>

<ResponseField name="error.code" type="integer">
  gRPC 오류 코드
</ResponseField>

<ResponseField name="error.message" type="string">
  오류 메시지
</ResponseField>

<ResponseField name="name" type="string">
  작업 리소스 이름
</ResponseField>

<ResponseField name="response" type="object">
  예측 응답. done이 참일 때 제공됩니다.
</ResponseField>

<ResponseField name="response.@type" type="string">
   
</ResponseField>

<ResponseField name="response.raiMediaFilteredCount" type="integer">
  책임 있는 AI 정책에 의해 필터링된 비디오 수
</ResponseField>

<ResponseField name="response.raiMediaFilteredReasons" type="string[]">
  책임 있는 AI 정책에 의해 비디오가 필터링된 이유
</ResponseField>

<ResponseField name="response.videos" type="object[]">
   
</ResponseField>

<ResponseField name="response.videos[].bytesBase64Encoded" type="string">
  Base64로 인코딩된 비디오 콘텐츠
</ResponseField>

<ResponseField name="response.videos[].gcsUri" type="string">
  생성된 비디오의 Cloud Storage URI
</ResponseField>

<ResponseField name="response.videos[].mimeType" type="string">
  비디오 MIME 유형(video/mp4)
</ResponseField>

## 예시

### 입력

```json
{
  "instances": [
    {
      "prompt": "a single red maple leaf falling onto still water, slow motion"
    }
  ],
  "parameters": {
    "durationSeconds": 4,
    "generateAudio": false,
    "sampleCount": 1
  }
}
```

### 출력

```json
{
  "done": true,
  "name": "projects/example-project/locations/us-central1/publishers/google/models/veo-3.1-fast-generate-001/operations/1a2b3c4d",
  "response": {
    "@type": "type.googleapis.com/cloud.ai.large_models.vision.GenerateVideoResponse",
    "raiMediaFilteredCount": 0,
    "videos": [
      {
        "gcsUri": "https://storage.googleapis.com/EXAMPLE_BUCKET/veo/USER_ID/REQUEST_ID/sample_0.mp4",
        "mimeType": "video/mp4"
      }
    ]
  }
}
```

<RouterCodeFooter />
