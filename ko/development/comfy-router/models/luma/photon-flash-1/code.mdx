...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내기하세요. Python 및 TypeScript 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`)를 사용하며, cURL 스니펫은 동일한 호출을 원시 HTTP로 수행합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/luma/photon-flash-1/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비되는 즉시 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 방법을 자세히 다룹니다.
