...
<ParamField body="contents" type="object[]" required>
  모델과의 현재 대화 콘텐츠입니다. 단일 턴 질의의 경우 단일 인스턴스입니다. 멀티 턴 질의의 경우 대화 기록과 최신 요청을 포함하는 반복 필드입니다.
</ParamField>
...
<ParamField body="generationConfig" type="object">
  생성에 대한 샘플링, 길이 및 출력 설정입니다. 모든 필드는 선택 사항입니다. 아래에서 `default`를 선언한 필드는 생략 시 해당 기본값이 적용되고, 나머지는 모델 자체의 동작을 따릅니다.
</ParamField>
...
<ParamField body="generationConfig.maxOutputTokens" type="integer">
  응답에서 생성할 수 있는 최대 토큰 수입니다. 토큰 하나는 약 4자에 해당합니다. 100개의 토큰은 대략 60~80단어에 해당합니다.
...
<ParamField body="generationConfig.seed" type="integer">
  시드를 특정 값으로 고정하면 모델은 반복 요청에 대해 동일한 응답을 제공하기 위해 최선을 다합니다. 결정론적 출력은 보장되지 않습니다. 또한 temperature와 같은 모델 또는 매개변수 설정을 변경하면 동일한 시드 값을 사용하더라도 응답에 변화가 생길 수 있습니다. 기본적으로 무작위 시드 값이 사용됩니다. 다음 모델에서 사용할 수 있습니다: gemini-2.5-flash, gemini-2.5-pro, gemini-2.5-flash-preview-04-1, gemini-2.5-pro-preview-05-0, gemini-2.0-flash-lite-00, gemini-2.0-flash-001
</ParamField>
...
<ParamField body="generationConfig.temperature" type="number" default="1">
  temperature는 응답 생성 중 샘플링에 사용되며, 이는 topP와 topK가 적용될 때 발생합니다. temperature는 토큰 선택의 무작위성 정도를 제어합니다. 낮은 temperature는 덜 개방적이거나 창의적인 응답이 필요한 프롬프트에 적합하고, 높은 temperature는 더 다양하거나 창의적인 결과를 낼 수 있습니다. temperature가 0이면 확률이 가장 높은 토큰이 항상 선택됩니다. 이 경우 주어진 프롬프트에 대한 응답은 대부분 결정적이지만, 약간의 변동은 여전히 가능합니다. 모델이 너무 일반적이거나 너무 짧은 응답을 반환하거나 대체 응답을 제공한다면 temperature를 높여 보세요
...
<ParamField body="generationConfig.thinkingConfig" type="object">
  선택 사항입니다. 사고(thinking) 기능에 대한 설정입니다. 사고는 모델이 복잡한 작업을 더 작은 단계로 나누어 더 높은 품질의 응답을 생성하는 과정입니다.
</ParamField>
...
<ParamField body="generationConfig.thinkingConfig.thinkingBudget" type="integer">
  선택 사항입니다. 모델의 사고 과정을 위한 토큰 예산입니다. 모델은 이 예산을 넘지 않도록 최선을 다합니다.
</ParamField>
...
<ParamField body="generationConfig.topK" type="integer" default="40">
  Top-K는 모델이 출력을 위해 토큰을 선택하는 방식을 변경합니다. Top-K가 1이면 다음에 선택되는 토큰은 모델 어휘의 모든 토큰 중에서 확률이 가장 높은 토큰입니다. Top-K가 3이면 temperature를 사용하여 확률이 가장 높은 3개의 토큰 중에서 다음 토큰이 선택됩니다.
...
Top-P는 모델이 출력을 위해 토큰을 선택하는 방식을 변경합니다. 토큰은 확률이 가장 높은 것(top-K 참조)부터 가장 낮은 것까지 선택되며, 확률의 합이 top-P 값과 같아질 때까지 진행됩니다. 예를 들어 토큰 A, B, C의 확률이 각각 0.3, 0.2, 0.1이고 top-P 값이 0.5이면, 모델은 temperature를 사용하여 A 또는 B를 다음 토큰으로 선택하고 C는 후보에서 제외합니다.
...
<ParamField body="systemInstruction" type="object">
  모델을 더 나은 성능으로 이끌기 위한 지침입니다. 예를 들어 "가능한 한 간결하게 답변하세요" 또는 "응답에 전문 용어를 사용하지 마세요"와 같습니다. 텍스트 문자열은 토큰 한도에 포함됩니다. systemInstruction의 역할 필드는 무시되며 모델의 성능에 영향을 주지 않습니다. 참고: parts에는 텍스트만 사용해야 하며, 각 part의 콘텐츠는 별도의 단락에 있어야 합니다.
</ParamField>

<ParamField body="systemInstruction.parts" type="object[]" required>
  하나의 메시지를 구성하는 순서가 있는 part 목록입니다. part마다 IANA MIME 유형이 다를 수 있습니다. 최대 토큰 수나 이미지 수와 같은 입력 제한에 대해서는 Google 모델 페이지의 모델 사양을 참조하세요.
</ParamField>
...
<ParamField body="systemInstruction.role" type="string">
  메시지를 생성하는 주체의 정체성입니다. 다음 값이 지원됩니다. user: 메시지가 실제 사람이 보낸 것으로, 일반적으로 사용자가 생성한 메시지임을 나타냅니다. model: 메시지가 모델에 의해 생성되었음을 나타냅니다. model 값은 다중 턴 대화 중에 모델의 메시지를 대화에 삽입할 때 사용됩니다. 다중 턴이 아닌 대화에서는 이 필드를 비워 두거나 설정하지 않을 수 있습니다.

  가능한 값: `user`, `model`
</ParamField>

<ParamField body="tools" type="object[]">
  시스템이 모델의 지식과 범위를 벗어난 작업 또는 일련의 작업을 수행하기 위해 외부 시스템과 상호 작용할 수 있게 해 주는 코드 조각입니다. 함수 호출(Function calling)을 참조하세요.
</ParamField>
...
<ParamField body="videoMetadata" type="object">
  비디오 입력의 경우, 재생 시간 형식으로 된 비디오의 시작 및 종료 오프셋입니다. 예를 들어 1:00에 시작하는 10초 클립을 지정하려면 "startOffset": \{ "seconds": 60 } 및 "endOffset": \{ "seconds": 70 }을 설정하세요. 메타데이터는 비디오 데이터가 inlineData 또는 fileData로 제공되는 동안에만 지정해야 합니다.
</ParamField>
...
<ParamField body="videoMetadata.endOffset.nanos" type="integer">
  나노초 해상도로 표현되는 초의 부호 있는 소수 부분입니다. 소수 부분이 있는 음수 초 값이라도 나노초 값은 음수가 아니어야 합니다.
...
Router가 `GET /v2/models/vertexai/gemini-3.1-flash-image/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 동일한 문서입니다.
...
<ResponseField name="usageMetadata.cachedContentTokenCount" type="integer">
  출력 전용입니다. 입력의 캐시된 부분(캐시된 콘텐츠)에 있는 토큰 수입니다.
</ResponseField>
...
<ResponseField name="usageMetadata.promptTokenCount" type="integer">
  요청에 있는 토큰 수입니다. cachedContent가 설정된 경우에도 이는 총 유효 프롬프트 크기이며, 캐시된 콘텐츠의 토큰 수를 포함합니다.
</ResponseField>
...
기본적으로 생성된 이미지 파트는 `inlineData.data`에 base64 바이트를, `inlineData.mimeType`에 미디어 타입을 담고 있습니다. 바이트를 디코딩하여 파일로 저장하세요. `uploadImagesToStorage: true`를 사용하면 업로드된 이미지는 대신 서명된 URL을 위한 `fileData.fileUri`와 미디어 타입을 위한 `fileData.mimeType`을 사용합니다. 이러한 이미지는 생성된 이후 24시간 뒤에 URL이 만료되므로 그 이전에 다운로드하세요. 업로드가 실패하면 해당 이미지는 인라인으로 남으므로, 각 파트에서 `inlineData` 또는 `fileData`를 확인하세요. 텍스트 파트가 함께 나타날 수도 있으며, 이미지가 첫 번째 파트라고 보장되지는 않습니다.
