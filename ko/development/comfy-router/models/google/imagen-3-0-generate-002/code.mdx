---
title: "Comfy Router로 Imagen 3.0 Generate 002 사용하기"
description: "Comfy Router를 통해 vertexai/imagen-3.0-generate-002를 호출합니다. endpoint, 요청 형태, 그리고 Router가 반환하는 응답을 설명합니다."
sidebarTitle: "Imagen 3.0 Generate 002"
translationSourceHash: 3ee77a31
translationFrom: development/comfy-router/models/google/imagen-3-0-generate-002/code.mdx
translationBlockHashes:
  "_intro": 0286732d
  "Quick start": 23f28650
  "Schema": aa01ed6f
  "Examples": eb746872
---

{/* GENERATED FILE. router-schemas/vertexai/imagen-3.0-generate-002.json에서 `pnpm code-pages:gen`으로 생성됨. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

Google에서 Comfy Router로 제공하는 `vertexai/imagen-3.0-generate-002`의 API 레퍼런스입니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 동일한 호출을 원시 HTTP로 수행한 것입니다.

**모델 ID:** `vertexai/imagen-3.0-generate-002`

**엔드포인트:** `POST https://api.comfy.org/v2/models/vertexai/imagen-3.0-generate-002`

<Tabs>
  <Tab title="결과 기다리기">
<CodeGroup>
```python Python
from comfy_sdk import Comfy

# 환경 변수에서 COMFY_API_KEY를 읽습니다.
# SDK는 자동으로 idempotency key를 생성하고 자동 재시도에 재사용합니다.
with Comfy() as client:
    result = client.models.run(
        "vertexai/imagen-3.0-generate-002",
        {
            "instances": [
                {
                    "prompt": "A single red maple leaf on a plain white background.",
                },
            ],
            "parameters": {
                "sampleCount": 1,
            },
        },
    )

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 자동으로 idempotency key를 생성하고 자동 재시도에 재사용합니다.
const { data } = await comfy.models.run("vertexai/imagen-3.0-generate-002", {
  instances: [
    {
      prompt: "A single red maple leaf on a plain white background.",
    },
  ],
  parameters: {
    sampleCount: 1,
  },
});

console.log(data);
```

```swift Swift
import Foundation
import ComfySwiftSDK

// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 idempotency key를 발급하고 자동 재시도에 재사용합니다.
let client = ComfyCloudClient(apiKey: ProcessInfo.processInfo.environment["COMFY_API_KEY"]!)
let result = try await client.models.run(
    "vertexai/imagen-3.0-generate-002",
    input: [
        "instances": [
            [
                "prompt": "A single red maple leaf on a plain white background.",
            ],
        ],
        "parameters": [
            "sampleCount": 1,
        ],
    ]
)

print(result.output)
```

```bash cURL
curl https://api.comfy.org/v2/models/vertexai/imagen-3.0-generate-002 \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"instances\": [{\"prompt\":\"A single red maple leaf on a plain white background.\"}], \"parameters\": {\"sampleCount\":1}}"
```
</CodeGroup>
  </Tab>
  <Tab title="실행 대기열에 넣고 나중에 수집">
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/imagen-3.0-generate-002/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되면 이 프로세스나 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [대기열 전달](/ko/development/comfy-router/queue)에서 설명합니다.

<CodeGroup>
```python Python
from comfy_sdk import Comfy

# 환경에서 COMFY_API_KEY를 읽습니다.
# 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
with Comfy() as client:
    handle = client.models.submit(
        "vertexai/imagen-3.0-generate-002",
        {
            "instances": [
                {
                    "prompt": "A single red maple leaf on a plain white background.",
                },
            ],
            "parameters": {
                "sampleCount": 1,
            },
        },
    )
    print("request_id:", handle.request_id)  # 모델 ID와 함께라면 다른 프로세스에 필요한 전부입니다

    # 요청이 완료될 때까지 폴링하며, 서버가 지정한 Retry-After만큼 기다립니다.
    for update in handle.iter_events():
        print(update.status, update.queue_position)

    # 공급자 자체의 페이로드이며, models.run()이 반환하는 것과 동일한 값입니다.
    # 실패했거나 취소된 요청은 여기에서 타입이 지정된 Router 오류를 발생시킵니다.
    result = handle.get()

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// 환경에서 COMFY_API_KEY를 읽습니다.
// 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
const handle = await comfy.models.submit("vertexai/imagen-3.0-generate-002", {
  instances: [
    {
      prompt: "A single red maple leaf on a plain white background.",
    },
  ],
  parameters: {
    sampleCount: 1,
  },
});
console.log("requestId:", handle.requestId); // 모델 ID와 함께라면 다른 프로세스에 필요한 전부입니다

// 요청이 완료될 때까지 폴링하며, 서버가 지정한 Retry-After만큼 기다립니다.
for await (const update of handle.events()) {
  console.log(update.status, update.queuePosition);
}

// models.run()이 반환하는 것과 동일한 결과입니다. 실패했거나 취소된 요청은 여기에서 거부됩니다.
const result = await handle.get();

console.log(result.data);
```

```swift Swift
import Foundation
import ComfySwiftSDK

// 환경에서 COMFY_API_KEY를 읽습니다.
// 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
let client = ComfyCloudClient(apiKey: ProcessInfo.processInfo.environment["COMFY_API_KEY"]!)
let handle = try await client.models.submit(
    "vertexai/imagen-3.0-generate-002",
    input: [
        "instances": [
            [
                "prompt": "A single red maple leaf on a plain white background.",
            ],
        ],
        "parameters": [
            "sampleCount": 1,
        ],
    ]
)
print("requestId:", handle.requestId)  // 모델 ID와 함께라면 다른 프로세스에 필요한 전부입니다

// 요청이 완료될 때까지 폴링하며, 서버가 지정한 Retry-After만큼 기다립니다.
for try await update in handle.events() {
    print(update.state.rawValue, update.queuePosition.map(String.init) ?? "unknown")
}

// 공급자 자체의 페이로드이며, models.run()이 반환하는 것과 동일한 값입니다.
// 실패했거나 취소된 요청은 여기에서 타입이 지정된 Router 오류를 발생시킵니다.
let result = try await handle.result()

print(result.output)
```

```bash cURL
# 1. 제출. Router는 request_id, status_url, response_url, cancel_url과 함께 201을 응답합니다.
curl https://api.comfy.org/v2/models/vertexai/imagen-3.0-generate-002/requests \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"instances\": [{\"prompt\":\"A single red maple leaf on a plain white background.\"}], \"parameters\": {\"sampleCount\":1}}"

# 2. 상태가 COMPLETED가 될 때까지 폴링하며, 각 응답이 지정한 Retry-After 초만큼 기다립니다.
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/vertexai/imagen-3.0-generate-002/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 수집. 모델의 네이티브 출력과 함께 200, 아직 실행 중일 때는 상태 본문과 함께 202를 반환합니다.
curl https://api.comfy.org/v2/models/vertexai/imagen-3.0-generate-002/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
</CodeGroup>
  </Tab>
</Tabs>

## 스키마

### 입력

<ParamField body="instances" type="object[]" required>
   
</ParamField>

<ParamField body="instances[].prompt" type="string" required>
  이미지 생성을 위한 텍스트 프롬프트
</ParamField>

<ParamField body="parameters" type="object">
   
</ParamField>

<ParamField body="parameters.addWatermark" type="boolean">
   
</ParamField>

<ParamField body="parameters.aspectRatio" type="string">
  가능한 값: `1:1`, `9:16`, `16:9`, `3:4`, `4:3`
</ParamField>

<ParamField body="parameters.enhancePrompt" type="boolean">
   
</ParamField>

<ParamField body="parameters.includeRaiReason" type="boolean">
   
</ParamField>

<ParamField body="parameters.includeSafetyAttributes" type="boolean">
   
</ParamField>

<ParamField body="parameters.outputOptions" type="object">
   
</ParamField>

<ParamField body="parameters.outputOptions.compressionQuality" type="integer">
  범위: `0` ~ `100`
</ParamField>

<ParamField body="parameters.outputOptions.mimeType" type="string">
  가능한 값: `image/png`, `image/jpeg`
</ParamField>

<ParamField body="parameters.personGeneration" type="string">
  가능한 값: `dont_allow`, `allow_adult`, `allow_all`
</ParamField>

<ParamField body="parameters.safetySetting" type="string">
  가능한 값: `block_most`, `block_some`, `block_few`, `block_fewest`
</ParamField>

<ParamField body="parameters.sampleCount" type="integer" default="4">
  범위: `1` ~ `4`
</ParamField>

<ParamField body="parameters.seed" type="integer">
  형식: `uint32`
</ParamField>

<ParamField body="parameters.storageUri" type="string">
   
</ParamField>

Router가 `GET /v2/models/vertexai/imagen-3.0-generate-002/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에게 전달되기 이전에 호출을 검증할 때 사용하는 것과 동일한 문서입니다.

### 출력

<ResponseField name="predictions" type="object[]">
   
</ResponseField>

<ResponseField name="predictions[].bytesBase64Encoded" type="string (byte)">
  Base64로 인코딩된 이미지 콘텐츠

  형식: `byte`
</ResponseField>

<ResponseField name="predictions[].mimeType" type="string">
  생성된 이미지의 MIME 타입
</ResponseField>

<ResponseField name="predictions[].prompt" type="string">
  이 이미지를 생성하는 데 사용된, 향상되거나 재작성된 프롬프트
</ResponseField>

## 예시

### 입력

```json
{
  "instances": [
    {
      "prompt": "A single red maple leaf on a plain white background."
    }
  ],
  "parameters": {
    "sampleCount": 1
  }
}
```

### 출력

```json
{
  "predictions": [
    {
      "bytesBase64Encoded": "PGJhc2U2ND4=",
      "mimeType": "image/png",
      "prompt": "A lighthouse at the edge of the harbour at dawn, warm low sun"
    }
  ]
}
```

<RouterCodeFooter />
