---
title: "Comfy Router로 Gemini 3.1 Flash Lite 사용하기"
description: "Comfy Router를 통해 vertexai/gemini-3.1-flash-lite를 호출합니다: endpoint, 요청 형태, 그리고 Router가 반환하는 응답을 설명합니다."
sidebarTitle: "Gemini 3.1 Flash Lite"
translationSourceHash: f7fdc005
translationFrom: development/comfy-router/models/google/gemini-3-1-flash-lite/code.mdx
translationBlockHashes:
  "_intro": 063db100
  "Quick start": 869398cb
  "Schema": 032ca94e
  "Examples": 2ebce337
---
...
Google가 Comfy Router를 통해 제공하는 `vertexai/gemini-3.1-flash-lite`의 API 레퍼런스입니다.
...
**모델 ID:** `vertexai/gemini-3.1-flash-lite`

**엔드포인트:** `POST https://api.comfy.org/v2/models/vertexai/gemini-3.1-flash-lite`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-3.1-flash-lite/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집됩니다. 상태, 취소, 수집 방법은 [대기열 전송](/ko/development/comfy-router/queue) 을 참고하세요.
...
<ParamField body="contents[].parts[].fileData.mimeType" type="string">
  data 또는 fileUri 필드에 지정된 파일의 미디어 유형입니다. 허용되는 값은 다음과 같습니다. gemini-2.0-flash-lite와 gemini-2.0-flash의 경우 오디오 파일의 최대 길이는 8.4시간이고 비디오 파일(오디오 없음)의 최대 길이는 1시간입니다. 자세한 내용은 Gemini 오디오 및 비디오 요구 사항을 참조하세요. 텍스트 파일은 UTF-8로 인코딩되어야 합니다. 텍스트 파일의 내용은 토큰 한도에 포함됩니다. 이미지 해상도에는 제한이 없습니다.
...
<ParamField body="contents[].parts[].inlineData" type="object">
  원시 바이트 형식의 인라인 데이터입니다. gemini-2.0-flash-lite와 gemini-2.0-flash의 경우 inlineData를 사용하여 최대 3000개의 이미지를 지정할 수 있습니다.
</ParamField>
...
<ParamField body="generationConfig.seed" type="integer">
  시드를 특정 값으로 고정하면 모델은 반복 요청에 대해 동일한 응답을 제공하기 위해 최선을 다합니다. 결정론적 출력은 보장되지 않습니다. 또한 동일한 시드 값을 사용하더라도 모델이나 temperature와 같은 매개변수 설정을 변경하면 응답이 달라질 수 있습니다. 기본적으로 임의의 시드 값이 사용됩니다. 다음 모델에서 사용할 수 있습니다: gemini-2.5-flash, gemini-2.5-pro, gemini-2.5-flash-preview-04-1, gemini-2.5-pro-preview-05-0, gemini-2.0-flash-lite-00, gemini-2.0-flash-001
</ParamField>
...
<ParamField body="generationConfig.temperature" type="number" default="1">
  temperature는 응답 생성 중 샘플링에 사용되며, 이는 topP와 topK가 적용될 때 발생합니다. temperature는 토큰 선택의 무작위성 정도를 제어합니다. 낮은 temperature는 덜 개방적이거나 창의적인 응답이 필요한 프롬프트에 적합하고, 높은 temperature는 더 다양하거나 창의적인 결과로 이어질 수 있습니다. temperature가 0이면 확률이 가장 높은 토큰이 항상 선택됩니다. 이 경우 주어진 프롬프트에 대한 응답은 대부분 결정적이지만, 약간의 변동은 여전히 가능합니다. 모델이 너무 일반적이거나 너무 짧은 응답을 반환하거나 대체 응답을 제공한다면 temperature를 높여 보세요.
...
<ParamField body="generationConfig.thinkingConfig" type="object">
  선택 사항입니다. 사고(thinking) 기능에 대한 구성입니다. 사고란 모델이 더 높은 품질의 응답을 생성하기 위해 복잡한 작업을 더 작은 단계로 나누는 과정입니다.
</ParamField>
...
<ParamField body="generationConfig.topK" type="integer" default="40">
  Top-K는 모델이 출력할 토큰을 선택하는 방식을 변경합니다. Top-K가 1이면 다음에 선택되는 토큰은 모델 어휘 전체에서 확률이 가장 높은 토큰입니다. Top-K가 3이면 temperature를 사용하여 확률이 가장 높은 3개의 토큰 중에서 다음 토큰을 선택합니다.
...
Top-P는 모델이 출력할 토큰을 선택하는 방식을 변경합니다. 토큰은 확률의 합이 top-P 값과 같아질 때까지 확률이 가장 높은 것부터(top-K 참조) 가장 낮은 것까지 선택됩니다. 예를 들어 토큰 A, B, C의 확률이 각각 0.3, 0.2, 0.1이고 top-P 값이 0.5라면, 모델은 temperature를 사용하여 A 또는 B를 다음 토큰으로 선택하고 C는 후보에서 제외합니다.
...
<ParamField body="systemInstruction" type="object">
  모델을 더 나은 성능으로 이끌기 위한 지침입니다. 예를 들어 "Answer as concisely as possible" 또는 "Don't use technical terms in your response"와 같습니다. 텍스트 문자열은 토큰 한도에 포함됩니다. systemInstruction의 role 필드는 무시되며 모델의 성능에 영향을 주지 않습니다. 참고: parts에는 텍스트만 사용해야 하며, 각 part의 content는 별도의 단락으로 구성해야 합니다.
</ParamField>

<ParamField body="systemInstruction.parts" type="object[]" required>
  하나의 메시지를 구성하는 순서가 있는 parts 목록입니다. part마다 IANA MIME 타입이 다를 수 있습니다. 최대 토큰 수나 이미지 수와 같은 입력 제한에 대해서는 Google 모델 페이지의 모델 사양을 참조하세요.
</ParamField>
...
<ParamField body="systemInstruction.role" type="string">
  메시지를 생성하는 주체의 정체성입니다. 다음 값이 지원됩니다. user: 메시지가 실제 사람이 보낸 것임을 나타내며, 일반적으로 사용자가 생성한 메시지입니다. model: 메시지가 모델에 의해 생성되었음을 나타냅니다. model 값은 멀티턴 대화 중에 모델의 메시지를 대화에 삽입할 때 사용됩니다. 멀티턴이 아닌 대화에서는 이 필드를 비워 두거나 설정하지 않아도 됩니다.

  가능한 값: `user`, `model`
</ParamField>

<ParamField body="tools" type="object[]">
  모델의 지식과 범위를 벗어난 작업 또는 일련의 작업을 수행하기 위해 시스템이 외부 시스템과 상호 작용할 수 있게 하는 코드 조각입니다. 함수 호출(Function calling)을 참조하세요.
</ParamField>
...
<ParamField body="videoMetadata" type="object">
  비디오 입력의 경우, Duration 형식으로 된 비디오의 시작 및 종료 오프셋입니다. 예를 들어 1:00에 시작하는 10초 클립을 지정하려면 "startOffset": \{ "seconds": 60 }, "endOffset": \{ "seconds": 70 }으로 설정합니다. 메타데이터는 비디오 데이터가 inlineData 또는 fileData로 제공되는 동안에만 지정해야 합니다.
</ParamField>
...
<ParamField body="videoMetadata.endOffset.nanos" type="integer">
  나노초 해상도로 표현된 부호 있는 초의 소수부입니다. 소수부를 가진 음수 초 값이라도 나노초 값은 음수가 아니어야 합니다.

  범위: `0` ~ `999999999`
</ParamField>

<ParamField body="videoMetadata.endOffset.seconds" type="integer">
  시간 범위의 부호 있는 초입니다. -315,576,000,000 이상 +315,576,000,000 이하여야 합니다.
...
Router가 `GET /v2/models/vertexai/gemini-3.1-flash-lite/openapi.json`에서 제공하는 스키마에서 생성된 문서이며, 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
...
<ResponseField name="candidates[].content" type="object">
  모델과의 현재 대화 콘텐츠입니다. 단일 턴 쿼리의 경우 단일 인스턴스입니다. 멀티턴 쿼리의 경우 대화 기록과 최신 요청을 포함하는 반복 필드입니다.
</ResponseField>
...
<ResponseField name="usageMetadata.cachedContentTokenCount" type="integer">
  출력 전용입니다. 입력에서 캐시된 부분(캐시된 콘텐츠)에 있는 토큰 수입니다.
</ResponseField>
...
<ResponseField name="usageMetadata.promptTokenCount" type="integer">
  요청에 있는 토큰 수입니다. cachedContent가 설정된 경우에도 이 값은 총 유효 프롬프트 크기이며, 캐시된 콘텐츠의 토큰 수를 포함합니다.
</ResponseField>
