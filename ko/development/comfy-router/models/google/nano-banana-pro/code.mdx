...
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-3-pro-image/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하며, 결과는 준비가 되면 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 설명합니다.
...
`strict_mode`의 기본값은 거짓이므로, Router는 이 페이지에 문서화된 네이티브 요청 본문을 공급자의 자체 스키마로 변환하고 응답을 다시 변환합니다. API 참조의 [`model_provider`, `strict_mode`, `fallback_provider`](/ko/development/comfy-router/reference#post-v2modelsprovidermodel)와, 이 방식으로 라우팅되는 모든 모델에 대한 [제공 공급자](/ko/development/comfy-router/providers)를 참고하세요.
...
<ParamField body="generationConfig.thinkingConfig.includeThoughts" type="boolean">
  선택 사항. 참이면 모델이 응답에 자신의 생각을 포함합니다.
</ParamField>
