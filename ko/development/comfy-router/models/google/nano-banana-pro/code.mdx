...
Router가 `GET /v2/models/vertexai/gemini-3-pro-image/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.### 출력
...
<ResponseField name="promptFeedback.safetyRatings[].probability" type="string">
  콘텐츠가 지정된 안전 카테고리를 위반할 확률입니다.<ResponseField name="responseId" type="string">
  응답의 고유 식별자입니다.
</ResponseField>
