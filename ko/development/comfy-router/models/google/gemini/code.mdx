...
[your Comfy workspace](https://platform.comfy.org/profile/api-keys)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 동일한 호출을 raw HTTP로 수행합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-3.1-pro-preview/requests` 로 전송합니다. Router는 실행이 수락되는 즉시 `request_id`와 함께 `201`을 반환하고, 준비가 완료되면 이 프로세스든 다른 프로세스든 결과를 수집합니다. [큐 전송](/ko/development/comfy-router/queue) 문서에서 상태, 취소, 수집 과정을 설명합니다.
...
```
  ...
  <Tab title="대기열에 넣고 나중에 수집">
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-3.5-flash/requests` 로 전송합니다. Router는 실행이 수락되는 즉시 `request_id`와 함께 `201`을 반환하고, 준비가 완료되면 이 프로세스든 다른 프로세스든 결과를 수집합니다. [큐 전송](/ko/development/comfy-router/queue) 문서에서 상태, 취소, 수집 과정을 설명합니다.
...
```
  ...
  <Tab title="대기열에 넣고 나중에 수집">
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-2.5-pro/requests` 로 전송합니다. Router는 실행이 수락되는 즉시 `request_id`와 함께 `201`을 반환하고, 준비가 완료되면 이 프로세스든 다른 프로세스든 결과를 수집합니다. [큐 전송](/ko/development/comfy-router/queue) 문서에서 상태, 취소, 수집 과정을 설명합니다.
...
```
  ...
  <Tab title="대기열에 넣고 나중에 수집">
동일한 본문을 `POST https://api.comfy.org/v2/models/vertexai/gemini-2.5-flash/requests` 로 전송합니다. Router는 실행이 수락되는 즉시 `request_id`와 함께 `201`을 반환하고, 준비가 완료되면 이 프로세스든 다른 프로세스든 결과를 수집합니다. [큐 전송](/ko/development/comfy-router/queue) 문서에서 상태, 취소, 수집 과정을 설명합니다.
...
<ParamField body="generationConfig.thinkingConfig.includeThoughts" type="boolean">
  선택 사항. true이면 모델이 응답에 자신의 사고 내용을 포함합니다.
</ParamField>
...
Router가 `GET /v2/models/vertexai/gemini-3.1-pro-preview/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 이전에 호출을 검증하는 기준이 되는 것과 동일한 문서입니다.
