...
<ParamField body="stream" type="boolean" default="false">
  Comfy Router는 완전한 JSON 결과를 캡처하기 때문에 명시적으로 전달된 stream 플래그를 false로 설정합니다. v1 프록시에서 이 필드는 스트리밍 출력 모드를 활성화할지 여부를 제어합니다. seedream-5.0-lite, 4.5, 4.0만 이 매개변수를 지원합니다(seedream-5.0-pro는 지원하지 않음). 거짓 = 모든 출력 이미지가 한 번에 반환됩니다. 참 = 각 출력 이미지가 생성된 직후 즉시 반환됩니다.
</ParamField>
