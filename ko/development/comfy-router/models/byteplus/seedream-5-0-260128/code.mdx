...
"seedream-5-0-pro-260628"에서 layer_decomposition 활성화 시: 해상도 수준의 방법만 지원됩니다. 선택 가능한 값: 1K, 1.5K, 2K, auto. 기본값: auto.
  기본 이미지는 원본 입력 이미지의 가로세로 비율로 지정된 해상도에 출력되고, 각 레이어는 원본 이미지에서 가졌던 가로세로 비율을 유지하면서 지정된 해상도에 가깝게 출력됩니다.
  auto: 입력 이미지의 크기와 가로세로 비율을 기준으로 출력합니다. [1280x720, ~2048x2048] 범위 내의 입력은 원본 입력 크기로 출력되고, 1K보다 작은 입력은 1K로, 2K보다 큰 입력은 2K로 출력됩니다.
</ParamField>

<ParamField body="stream" type="boolean" default="false">
  Comfy Router는 완전한 JSON 결과를 캡처하기 때문에 명시적으로 전달된 stream 플래그를 false로 확정합니다. v1 프록시에서 이 필드는 스트리밍 출력 모드를 활성화할지 여부를 제어합니다. seedream-5.0-lite, 4.5, 4.0만 이 매개변수를 지원합니다(seedream-5.0-pro는 지원하지 않음). false = 모든 출력 이미지가 한 번에 반환됩니다. true = 생성된 각 출력 이미지가 생성 직후 즉시 반환됩니다.
</ParamField>
