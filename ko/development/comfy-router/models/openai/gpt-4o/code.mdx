---
title: "Comfy Router와 함께 GPT 4o 사용하기"
description: "Comfy Router를 통해 openai/gpt-4o를 호출합니다: 엔드포인트, 요청 형태, 그리고 Router가 반환하는 응답."
sidebarTitle: "GPT 4o"
...
{/* 생성된 파일. `pnpm code-pages:gen`이(가) router-schemas/openai/gpt-4o.json에서 생성했습니다. */}
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-4o/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비되면 이 프로세스나 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [대기열 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
Router가 `GET /v2/models/openai/gpt-4o/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
