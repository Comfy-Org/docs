---
title: "Comfy Router로 o4-mini 사용하기"
description: "Comfy Router를 통해 openai/o4-mini를 호출합니다: endpoint, 요청 형태, Router가 반환하는 응답을 설명합니다."
...
{/* 생성된 파일. `pnpm code-pages:gen`이 router-schemas/openai/o4-mini.json에서 생성했습니다. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

OpenAI에서 제공하고 Comfy Router가 서비스하는 `openai/o4-mini`의 API 레퍼런스입니다.
...
**모델 ID:** `openai/o4-mini`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/o4-mini`
...
// 환경에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 idempotency key를 생성하고 자동 재시도를 위해 재사용합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/o4-mini/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`를 응답하며, 결과는 준비되는 대로 이 프로세스나 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [실행 대기열 전송](/ko/development/comfy-router/queue)에서 다룹니다.
...
<ParamField body="input" type="string | object[]" required>
  모델에 대한 텍스트, 이미지 또는 파일 입력으로, 응답을 생성하는 데 사용됩니다. 이 계약에서 Router가 제공할 수 없는 유일한 필드이며, 아래 `required`의 유일한 항목입니다.
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  응답을 위해 생성되는 토큰 수의 상한으로, 표시되는 출력 토큰과 추론 토큰을 포함합니다. 추론 id에서는 이 상한이 숨겨진 추론 토큰과 공유되므로, 작은 값을 사용하면 표시되는 텍스트가 나오기 이전에 전체 예산이 소진될 수 있습니다. 이것이 추론 스모크 케이스가 1024를 보내는 반면 채팅 케이스는 16을 보내는 이유입니다.

  범위: `1` ~ `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 모델 식별자입니다. Comfy Router에서 이 필드는 선택 사항(OPTIONAL)이며 Router가 `{model}` 경로 세그먼트에서 채웁니다. 명시적인 `null`도 같은 방식으로 교체됩니다. 경로와 일치하지 않는 값을 보내면 거부됩니다.
</ParamField>
...
<ParamField body="reasoning" type="object">
  추론 등급 전용(REASONING TIER ONLY)입니다. 추론 모델을 위한 구성으로, 예: `{"effort": "medium"}`. 변경 없이 전달됩니다. 허용되는 키는 OpenAI의 추론 가이드를 참조하세요. 채팅 등급 id는 이를 무시합니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이를 보내는 호출자가 거부되지 않도록 선언되었지만, 이 표면에서는 아무 효과가 없습니다(INERT). Router는 디스패치 이전에 이를 `false`로 확정(SETTLES)하는데, 이는 `text/event-stream`을 중계하는 대신 공급자 응답을 캡처하기 때문입니다. openAiResponsesProxy의 ModifyResponse는 이를 디코딩할 수 없으므로, 스트리밍된 생성은 OpenAI에 의해 청구되고 아무도 계량하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하세요.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 temperature입니다. 채팅 등급 전용(CHAT TIER ONLY)입니다. o 시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 파라미터를 거부합니다. Router는 이들에 대해 이를 거부하지 않습니다. 두 등급이 하나의 스키마를 공유하는 이유에 대해서는 이 컴포넌트의 참고 사항을 참조하세요. 따라서 이를 보내는 추론 호출은 OpenAI 자체의 오류로 응답됩니다.
...
<ParamField body="tool_choice" type="string | object">
  모델이 사용할 도구를 어떻게 선택할지 지정합니다. 문자열 모드이거나 도구를 지정하는 객체입니다.
</ParamField>

<ParamField body="tools" type="object[]">
  모델이 호출할 수 있는 도구 정의입니다. Router는 도구 분류 체계를 좁히지 않습니다. 허용되는 형태는 OpenAI의 Responses API 참조를 확인하세요.
</ParamField>

<ParamField body="top_p" type="number">
  누클리어스 샘플링(nucleus-sampling) 컷오프입니다. `temperature`와 동일한 조건으로 채팅 등급 전용입니다.

  범위: `0` ~ `1`
</ParamField>

<ParamField body="truncation" type="string">
  컨텍스트가 모델의 창을 초과할 때의 잘림 전략입니다. 열거형(enum)은 위의 세 가지 어휘와 달리 여기서는 강제됩니다. 이 두 값이 OpenAI가 문서화한 완전한 집합이며 늘어나지 않았기 때문입니다. 명시적 `null`은 위 필드들과 동일한 조건으로 여전히 허용됩니다.

  가능한 값: `auto`, `disabled`
</ParamField>

<ParamField body="usage" type="object">
  토큰 사용량 봉투(envelope)입니다. v1 작업이 요청 본문에 이를 선언하기 때문에 이 계약에 존재합니다. OpenAI는 응답(RESPONSE)에 이를 채우므로, 호출자가 이를 보낼 이유가 없습니다.
</ParamField>

Router가 `GET /v2/models/openai/o4-mini/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력
...
- `auto`: 이 응답과 이전 응답의 컨텍스트가 모델의 컨텍스트 창 크기를 초과하면, 모델이 대화 중간의 입력 항목을 제거하여 컨텍스트 창에 맞도록 응답을 잘라냅니다.
...
<ResponseField name="previous_response_id" type="string">
  모델에 대한 이전 응답의 고유 ID입니다. 이를 사용하여 여러 턴의 대화를 생성합니다. [대화 상태](https://platform.openai.com/docs/guides/conversation-state)에 대해 더 알아보세요.
</ResponseField>
...
<ResponseField name="reasoning.context" type="string">
  이후 턴에서 어떤 추론 항목을 모델에 다시 렌더링할지 제어합니다. 예: `auto`, `current_turn`, `all_turns`.
</ResponseField>

<ResponseField name="reasoning.effort" type="string" default="&quot;medium&quot;">
  **o 시리즈 모델 전용**

[추론 모델](https://platform.openai.com/docs/guides/reasoning)의 추론에 대한 노력 수준을 제한합니다. 현재 지원되는 값은 `low`, `medium`, `high`입니다. 추론 노력을 줄이면 응답이 더 빨라지고 응답에서 추론에 사용되는 토큰 수가 줄어들 수 있습니다.
...
모델이 수행한 추론의 요약입니다. 이는 모델의 추론 과정을 디버깅하고 이해하는 데 유용할 수 있습니다. `auto`, `concise`, `detailed` 중 하나입니다.
...
<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  응답을 생성할 때 모델이 어떤 도구를 사용할지 선택하는 방식입니다. 모델이 호출할 수 있는 도구를 지정하는 방법은 `tools` 파라미터를 참고하세요.
</ResponseField>
...
<ResponseField name="frequency_penalty" type="number">
  지금까지의 텍스트에서 기존 빈도를 기준으로 새 token에 페널티를 부여합니다.
</ResponseField>
...
<ResponseField name="max_tool_calls" type="integer">
  하나의 응답에서 처리할 수 있는 내장 도구에 대한 총 호출의 최대 개수입니다.
</ResponseField>
...
- `output` 배열의 첫 번째 항목에 접근하여 모델이 생성한 콘텐츠를 담은 `assistant` 메시지라고 가정하기보다는, SDK에서 지원되는 경우 `output_text` 속성을 사용하는 것을 고려해 보세요.
</ResponseField>

<ResponseField name="output_text" type="string">
  `output` 배열의 모든 `output_text` 항목에 있는 텍스트 출력을 집계한 값을 담는 SDK 전용 편의 속성입니다(항목이 있는 경우).
...
<ResponseField name="presence_penalty" type="number">
  지금까지의 텍스트에 등장하는지 여부를 기준으로 새 token에 페널티를 부여합니다.
</ResponseField>
...
<ResponseField name="prompt_cache_retention" type="string">
  prompt 캐시의 보존 정책입니다. 예: `in_memory` 또는 `24h`.
</ResponseField>
...
<ResponseField name="tool_usage" type="object">
  내장 도구별로 분류된 token 및 요청 사용량입니다.
</ResponseField>

<ResponseField name="tool_usage.image_gen" type="object">
  이미지 생성 도구의 token 사용량입니다.
</ResponseField>
...
<ResponseField name="top_logprobs" type="integer">
  각 token 위치에서 반환할 가장 가능성이 높은 token의 최대 개수이며, 각각 연관된 log probability를 가집니다.
</ResponseField>

<ResponseField name="usage" type="object">
  입력 token, 출력 token, 출력 token의 세부 분석, 사용된 총 token을 포함한 token 사용량 세부 정보를 나타냅니다.
</ResponseField>
...
<ResponseField name="user" type="string">
  최종 사용자를 위한 지원 중단된 식별자입니다. `safety_identifier` 및 `prompt_cache_key`로 교체되었습니다.
</ResponseField>
