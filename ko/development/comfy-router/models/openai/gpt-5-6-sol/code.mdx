---
title: "Comfy Router와 함께 GPT 5.6 Sol 사용하기"
description: "Comfy Router를 통해 openai/gpt-5.6-sol을 호출하는 방법: endpoint, 요청 형태, 그리고 Router가 반환하는 응답입니다."
...
OpenAI가 제공하는 Comfy Router에서 서비스하는 `openai/gpt-5.6-sol`의 API 레퍼런스입니다.
...
**모델 ID:** `openai/gpt-5.6-sol`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/gpt-5.6-sol`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-5.6-sol/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되면 이 프로세스든 다른 프로세스든 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기열 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
# 2. 상태가 COMPLETED가 될 때까지 폴링하며, 각 응답이 지정한 Retry-After초만큼 기다립니다.
...
<ParamField body="input" type="string | object[]" required>
  모델에 전달되는 텍스트, 이미지 또는 파일 입력으로, 응답을 생성하는 데 사용됩니다. 이 계약에서 Router가 대신 채울 수 없는 유일한 필드이며, 아래 `required`에 있는 유일한 항목입니다.
</ParamField>
...
<ParamField body="reasoning" type="object">
  추론 등급 전용입니다. 추론 모델을 위한 구성으로, 예를 들어 `{"effort": "medium"}`입니다. 변경 없이 전달됩니다. 허용되는 키는 OpenAI의 reasoning 가이드를 참조하세요. 채팅 등급 id는 이를 무시합니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이 필드를 보내는 호출자가 거부되지 않도록 선언되어 있지만, 이 표면에서는 아무 효과가 없습니다. Router는 디스패치 이전에 이를 `false`로 확정하는데, 이는 `text/event-stream`을 중계하는 대신 공급자 응답을 캡처하기 때문입니다. openAiResponsesProxy의 ModifyResponse는 이를 디코딩할 수 없으므로, 스트리밍 생성은 OpenAI가 과금하고 아무도 계량하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하세요.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 temperature입니다. 채팅 등급 전용입니다. o-시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 매개변수를 거부합니다. Router는 이들에 대해 이를 거부하지 않습니다(두 등급이 하나의 스키마를 공유하는 이유는 이 컴포넌트의 설명을 참조하세요). 따라서 이를 보내는 추론 호출은 OpenAI 자체 오류로 응답됩니다.
...
<ParamField body="truncation" type="string">
  컨텍스트가 모델의 창을 초과할 때의 잘라내기 전략입니다. 위의 세 어휘와 달리 여기서는 enum이 실제로 강제됩니다. 이 두 값이 OpenAI가 문서화한 전체 집합이며 늘어나지 않았기 때문입니다. 명시적인 `null`은 위의 필드들과 같은 조건으로 여전히 허용됩니다.
...
`GET /v2/models/openai/gpt-5.6-sol/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력

<ResponseField name="instructions" type="string">
  모델의 컨텍스트에서 첫 번째 항목으로 시스템(또는 개발자) 메시지를 삽입합니다.

`previous_response_id`와 함께 사용하는 경우, 이전 응답의 instructions는
다음 응답으로 이어지지 않습니다. 따라서 새 응답에서 시스템(또는 개발자)
메시지를 간단히 교체할 수 있습니다.
</ResponseField>

<ResponseField name="max_output_tokens" type="integer">
  표시되는 출력 토큰과 [추론 토큰](https://platform.openai.com/docs/guides/reasoning)을 포함하여,
응답에 대해 생성될 수 있는 토큰 수의 상한입니다.
</ResponseField>
...
- `auto`: 이 응답과 이전 응답의 컨텍스트가 모델의 컨텍스트 창 크기를
  초과하면, 모델이 대화 중간의 입력 항목을 삭제하여 컨텍스트 창에
  맞도록 응답을 잘라냅니다.
- `disabled` (기본값): 모델 응답이 모델의 컨텍스트 창 크기를 초과할
  경우, 요청이 400 오류와 함께 실패합니다.

  사용 가능한 값: `auto`, `disabled`
</ResponseField>
...
<ResponseField name="reasoning.context" type="string">
  이후 턴에서 어떤 추론 항목이 모델로 다시 전달되는지 제어합니다. 예: `auto`, `current_turn`, `all_turns`.
</ResponseField>
...
추론 노력(effort) 수준을 제한합니다.
현재 지원되는 값은 `low`, `medium`, `high`입니다. 추론 노력을
줄이면 응답이 더 빨라지고 응답에서 추론에 사용되는 토큰이 줄어들 수
있습니다.

  사용 가능한 값: `low`, `medium`, `high`
</ResponseField>
...
`auto`, `concise`, `detailed` 중 하나입니다.
  ...
  사용 가능한 값: `auto`, `concise`, `detailed`
</ResponseField>
...
모델이 제공된 JSON 스키마와 일치하도록 보장합니다. 자세한 내용은
...
방법입니다. 모델이 호출할 수 있는 도구를 지정하는 방법은 `tools` 파라미터를
참고하세요.
</ResponseField>
...
<ResponseField name="error" type="object">
  모델이 Response 생성에 실패했을 때 반환되는 오류 객체입니다.
</ResponseField>
...
<ResponseField name="frequency_penalty" type="number">
  지금까지의 텍스트에서 해당 토큰이 나타난 빈도를 기준으로 새 토큰에 페널티를 부여합니다.
</ResponseField>
...
- `output` 배열의 첫 번째 항목에 접근하여 모델이 생성한 콘텐츠를 포함한 `assistant` 메시지라고 가정하기보다는, SDK에서 지원되는 경우 `output_text` 속성을 사용하는 것을 고려해 보세요.
</ResponseField>

<ResponseField name="output_text" type="string">
  `output` 배열의 모든 `output_text` 항목에서 집계된 텍스트 출력을 담고 있는 SDK 전용 편의 속성입니다(항목이 있는 경우).
...
<ResponseField name="presence_penalty" type="number">
  지금까지의 텍스트에 해당 토큰이 나타났는지 여부를 기준으로 새 토큰에 페널티를 부여합니다.
</ResponseField>
...
<ResponseField name="usage" type="object">
  입력 토큰, 출력 토큰, 출력 토큰의 세부 분류, 사용된 총 토큰을 포함한 토큰 사용량 세부 정보를 나타냅니다.
</ResponseField>
