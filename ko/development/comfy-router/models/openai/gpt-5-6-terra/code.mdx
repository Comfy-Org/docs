---
title: "Comfy Router에서 GPT 5.6 Terra 사용하기"
description: "Comfy Router를 통해 openai/gpt-5.6-terra를 호출하세요: 엔드포인트, 요청 형태, 그리고 Router가 반환하는 응답입니다."
...
translationBlockHashes:
  ...
  "Examples": 67e6c8f1
---
{/* 생성된 파일. `pnpm code-pages:gen`이 router-schemas/openai/gpt-5.6-terra.json에서 생성했습니다. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

`openai/gpt-5.6-terra`에 대한 API 레퍼런스이며, Comfy Router가 OpenAI로부터 제공합니다.
...
**모델 ID:** `openai/gpt-5.6-terra`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/gpt-5.6-terra`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-5-6-terra/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [Queued delivery](/ko/development/comfy-router/queue) 를 참고하세요.
...
<ParamField body="input" type="string | object[]" required>
  모델에 전달되어 응답을 생성하는 데 사용되는 텍스트, 이미지 또는 파일 입력입니다. 이 계약에서 Router가 제공할 수 없는 유일한 필드이며, 아래 `required`에 있는 유일한 항목입니다.
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  응답을 위해 생성되는 토큰 수의 상한으로, 표시되는 출력 토큰과 추론 토큰을 포함합니다. 추론 id에서는 이 상한을 숨김 추론 토큰과 공유하므로, 값이 작으면 표시되는 텍스트가 나오기 전에 전체 예산이 소진될 수 있습니다. 그래서 추론 스모크 케이스는 1024를 보내고 채팅 케이스는 16을 보냅니다.
...
<ParamField body="reasoning" type="object">
  추론 티어 전용입니다. 추론 모델을 위한 설정으로, 예를 들어 `{"effort": "medium"}`입니다. 변경 없이 전달되며, 허용되는 키는 OpenAI의 추론 가이드를 참조하십시오. 채팅 티어 id는 이를 무시합니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이를 보내는 호출자가 거부되지 않도록 선언되어 있지만, 이 표면에서는 무효입니다. Router는 디스패치 전에 이를 `false`로 확정하는데, `text/event-stream`을 중계하는 대신 공급자 응답을 캡처하기 때문입니다. openAiResponsesProxy의 ModifyResponse는 이를 디코딩할 수 없으므로, 스트리밍 생성은 OpenAI가 과금하고 아무도 사용량을 집계하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하십시오.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 온도입니다. 채팅 티어 전용입니다. o 시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 매개변수를 거부합니다. Router는 이들에 대해 이를 거부하지 않으며(두 티어가 하나의 스키마를 공유하는 이유는 이 컴포넌트의 설명을 참조), 따라서 이를 보내는 추론 호출은 OpenAI 자체 오류로 응답됩니다.

  범위: `0` ~ `2`
</ParamField>

<ParamField body="text" type="object">
  출력 형식 설정으로, 예를 들어 Structured Outputs를 위한 `{"format": {"type": "json_schema", ...}}`입니다. 변경 없이 전달됩니다.
</ParamField>
...
<ParamField body="top_p" type="number">
  누클리어스 샘플링 컷오프입니다. `temperature`와 동일한 조건으로 채팅 티어 전용입니다.

  범위: `0` ~ `1`
</ParamField>

<ParamField body="truncation" type="string">
  컨텍스트가 모델의 창을 초과할 때의 잘라내기 전략입니다. 위의 세 가지 어휘와 달리 여기서는 열거형이 강제되는데, 이 두 값이 OpenAI가 문서화한 완전한 집합이며 늘어나지 않았기 때문입니다. 명시적인 `null`은 위 필드들과 동일한 조건으로 여전히 허용됩니다.

  가능한 값: `auto`, `disabled`
</ParamField>

<ParamField body="usage" type="object">
  토큰 사용량 봉투입니다. 이 계약에 존재하는 이유는 v1 작업이 요청 본문에 이를 선언하기 때문입니다. OpenAI는 이를 응답에서 채우므로, 호출자가 이를 보낼 이유는 없습니다.
</ParamField>

Router가 `GET /v2/models/openai/gpt-5.6-terra/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이 문서는 요청이 공급자에 도달하기 전에 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력
...
<ResponseField name="max_output_tokens" type="integer">
  응답에 대해 생성될 수 있는 토큰 수의 상한이며, 표시되는 출력 토큰과 [추론 토큰](https://platform.openai.com/docs/guides/reasoning)을 포함합니다.
</ResponseField>
...
<ResponseField name="previous_response_id" type="string">
  모델에 대한 이전 응답의 고유 ID입니다. 이를 사용하여 다중 턴 대화를 생성합니다. [conversation state](https://platform.openai.com/docs/guides/conversation-state)에 대해 더 알아보세요.
</ResponseField>
...
<ResponseField name="reasoning.context" type="string">
  이후 턴에서 어떤 reasoning 항목이 모델에 다시 렌더링될지 제어합니다. 예: `auto`, `current_turn`, `all_turns`.
</ResponseField>

<ResponseField name="reasoning.effort" type="string" default="&quot;medium&quot;">
  **o 시리즈 모델 전용**

[추론 모델](https://platform.openai.com/docs/guides/reasoning)의 추론 노력 수준을 제한합니다.
현재 지원되는 값은 `low`, `medium`, `high`입니다. 추론 노력 수준을 낮추면 응답이 더 빨라지고 응답에서 추론에 사용되는 토큰이 줄어들 수 있습니다.
...
모델이 수행한 추론의 요약입니다. 디버깅과 모델의 추론 과정을 이해하는 데 유용할 수 있습니다.
...
<ResponseField name="text.format" type="object">
  모델이 반드시 출력해야 하는 형식을 지정하는 객체입니다.

`{ "type": "json_schema" }`를 설정하면 Structured Outputs가 활성화되어, 모델이 제공한 JSON 스키마와 일치하도록 보장합니다. 자세한 내용은
...
모델이 생성하는 메시지가 유효한 JSON임을 보장합니다. 이를 지원하는 모델에는 `json_schema`
사용이 권장됩니다.
</ResponseField>

<ResponseField name="text.verbosity" type="string">
  모델 응답의 장황함 정도를 제한합니다. `low`, `medium`, `high` 중 하나입니다.
</ResponseField>

<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  응답을 생성할 때 모델이 어떤 도구(또는 도구들)를 사용할지 선택하는 방식입니다. 모델이 호출할 수 있는 도구를 지정하는 방법은 `tools` 매개변수를 참조하세요.
</ResponseField>
...
- `output` 배열의 첫 번째 항목에 접근하여 그것이 모델이 생성한 콘텐츠를 담은 `assistant` 메시지라고 가정하기보다는, SDK에서 지원하는 경우 `output_text` 속성을 사용하는 것을 고려해 보세요.
</ResponseField>

<ResponseField name="output_text" type="string">
  `output` 배열의 모든 `output_text` 항목에 있는 집계된 텍스트 출력을 담는 SDK 전용 편의 속성입니다(존재하는 경우). Python 및 JavaScript SDK에서 지원됩니다.
</ResponseField>
...
<ResponseField name="usage" type="object">
  입력 토큰, 출력 토큰, 출력 토큰의 세부 내역, 사용된 총 토큰 수를 포함한 토큰 사용 세부 정보를 나타냅니다.
</ResponseField>
