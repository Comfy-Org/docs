...
[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript, Swift 스니펫은 Comfy SDK를 사용하며(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지), cURL 스니펫은 동일한 호출을 raw HTTP로 전송합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-5.5/requests`로 보냅니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비되면 이 프로세스든 다른 프로세스든 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기 중 전송](/ko/development/comfy-router/queue)에서 설명합니다.
...
# 3. 수집. 완료되면 모델의 네이티브 출력과 함께 200, 아직 실행 중이면 상태 본문과 함께 202.
