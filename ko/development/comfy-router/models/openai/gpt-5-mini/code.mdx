---
title: "Comfy Router에서 GPT 5 Mini 사용하기"
description: "Comfy Router를 통해 openai/gpt-5-mini 호출하기: 엔드포인트, 요청 형태, Router가 반환하는 응답."
sidebarTitle: "GPT 5 Mini"
translationSourceHash: e70d12dd
translationFrom: development/comfy-router/models/openai/gpt-5-mini/code.mdx
translationBlockHashes:
  "_intro": 02cc5ae0
  "Quick start": 0fe7af70
  "Schema": ce5d9df5
  "Examples": 67e6c8f1
---

{/* 생성된 파일. `pnpm code-pages:gen`이 router-schemas/openai/gpt-5-mini.json에서 생성했습니다. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

OpenAI가 제공하고 Comfy Router가 서비스하는 `openai/gpt-5-mini`의 API 레퍼런스입니다.
...
**모델 ID:** `openai/gpt-5-mini`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/gpt-5-mini`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-5-mini/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 을 참고하세요.
...
# 2. status 가 COMPLETED 가 될 때까지 폴링하며, 각 응답이 지정한 Retry-After 초만큼 기다립니다.
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/openai/gpt-5-mini/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 수집. 200 은 모델의 네이티브 출력과 함께, 202 는 아직 실행 중일 때 상태 본문과 함께 반환됩니다.
...
<ParamField body="input" type="string | object[]" required>
  모델에 전달되는 텍스트, 이미지 또는 파일 입력으로, 응답을 생성하는 데 사용됩니다. 이 계약에서 Router가 제공할 수 없는 유일한 필드이며, 아래 `required`의 유일한 항목입니다.
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  응답을 위해 생성되는 토큰 수의 상한으로, 가시적 출력 토큰과 추론 토큰을 포함합니다. 추론 id에서는 이 상한을 숨김 추론 토큰과 공유하므로, 작은 값은 가시적 텍스트가 나오기 전에 전체 예산을 소진할 수 있습니다. 이것이 추론 스모크 케이스가 1024를 보내고 채팅 케이스가 16을 보내는 이유입니다.

  범위: `1`부터 `…`까지
</ParamField>

<ParamField body="model" type="string">
  OpenAI 모델 식별자입니다. Comfy Router에서 이 필드는 선택 사항(OPTIONAL)이며 Router가 `{model}` 경로 세그먼트에서 이를 채웁니다. 명시적 `null`도 같은 방식으로 교체됩니다. 경로와 일치하지 않는 값을 보내면 거부됩니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이 필드를 보내는 호출자가 거부되지 않도록 선언되어 있지만, 이 표면에서는 아무 효과가 없습니다. Router는 디스패치 이전에 이를 `false`로 확정합니다. 이 엔드포인트는 `text/event-stream`을 중계하는 대신 공급자 응답을 캡처하기 때문입니다. openAiResponsesProxy의 ModifyResponse는 이를 디코딩할 수 없으므로, 스트리밍 생성은 OpenAI가 과금하고 아무도 계량하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하세요.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 온도입니다. 채팅 티어 전용입니다. o 시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 매개변수를 거부합니다. Router는 이들에 대해 이를 거부하지 않습니다. 두 티어가 하나의 스키마를 공유하는 이유에 대해서는 이 컴포넌트의 설명을 참조하세요. 따라서 이를 보내는 추론 호출은 OpenAI 자체의 오류로 응답됩니다.

  범위: `0`부터 `2`까지
</ParamField>

<ParamField body="text" type="object">
  출력 형식 구성입니다. 예를 들어 Structured Outputs의 경우 `{"format": {"type": "json_schema", ...}}`입니다. 변경 없이 전달됩니다.
</ParamField>

<ParamField body="tool_choice" type="string | object">
  모델이 사용할 도구를 어떻게 선택해야 하는지 지정합니다. 문자열 모드이거나 도구를 지정하는 객체입니다.
</ParamField>
...
<ParamField body="truncation" type="string">
  컨텍스트가 모델의 창을 초과할 때의 잘림(truncation) 전략입니다. 위의 세 값 목록과 달리 여기서는 enum이 실제로 강제됩니다. 이 두 값이 OpenAI가 문서화한 완전한 집합이며 그 이후 늘어나지 않았기 때문입니다. 명시적 `null`은 위 필드들과 동일한 조건으로 여전히 허용됩니다.

  가능한 값: `auto`, `disabled`
</ParamField>

<ParamField body="usage" type="object">
  토큰 사용량 봉투(envelope)입니다. v1 작업이 요청 본문에 이를 선언하기 때문에 이 계약에 존재합니다. OpenAI는 RESPONSE에 이를 채우므로, 호출자가 보낼 이유가 없습니다.
</ParamField>

Router가 `GET /v2/models/openai/gpt-5-mini/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
### 출력
...
<ResponseField name="max_output_tokens" type="integer">
  표시되는 출력 토큰과 [추론 토큰](https://platform.openai.com/docs/guides/reasoning)을 포함하여 응답에 대해 생성할 수 있는 토큰 수의 상한입니다.
</ResponseField>
...
- `auto`: 이 응답과 이전 응답들의 컨텍스트가 모델의 컨텍스트 창 크기를 초과하면, 모델은 대화 중간의 입력 항목을 삭제하여 컨텍스트 창에 맞도록 응답을 잘라냅니다.
...
<ResponseField name="previous_response_id" type="string">
  모델에 대한 이전 응답의 고유 ID입니다. 이를 사용하여 다중 턴 대화를 생성합니다.
...
<ResponseField name="reasoning.context" type="string">
  이후 턴에서 어떤 추론 항목이 모델에 다시 제공되는지 제어합니다. 예: `auto`, `current_turn`, `all_turns`.
</ResponseField>
...
모델이 수행한 추론의 요약입니다. 이는 모델의 추론 과정을
디버깅하고 이해하는 데 유용할 수 있습니다.
...
모델에서는 `json_schema`를 사용하는 것이 좋습니다.
</ResponseField>
...
<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  응답을 생성할 때 모델이 사용할 도구(또는 도구들)를 어떻게 선택해야 하는지
지정합니다. 모델이 호출할 수 있는 도구를 지정하는 방법은 `tools` 매개변수를
참조하세요.
</ResponseField>
...
<ResponseField name="frequency_penalty" type="number">
  지금까지의 텍스트에서 기존 등장 빈도를 기준으로 새 token에 페널티를 부여합니다.
</ResponseField>
...
<ResponseField name="moderation" type="object">
  moderations가 요청된 경우 응답 입력 및 출력에 대한 moderation 결과입니다.
</ResponseField>
...
- `output` 배열의 첫 번째 항목에 접근하여 모델이 생성한 콘텐츠를 가진 `assistant` 메시지라고 가정하기보다는, SDK에서 지원되는 경우 `output_text` 속성을 사용하는 것을 고려해 보세요.
</ResponseField>
...
<ResponseField name="prompt_cache_retention" type="string">
  prompt cache의 보존 정책입니다. 예: `in_memory` 또는 `24h`.
</ResponseField>
...
<ResponseField name="top_logprobs" type="integer">
  각 token 위치에서 반환할 가장 가능성이 높은 token의 최대 개수이며, 각각 관련 log probability가 함께 제공됩니다.
</ResponseField>

<ResponseField name="usage" type="object">
  입력 token, 출력 token, 출력 token의 세부 내역, 사용된 총 token을 포함한 token 사용 세부 정보를 나타냅니다.
</ResponseField>
