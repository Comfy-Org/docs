---
title: "Comfy Router로 GPT 5.5 Pro 사용하기"
description: "Comfy Router를 통해 openai/gpt-5.5-pro를 호출합니다: 엔드포인트, 요청 형태, Router가 반환하는 응답입니다."
...
{/* 생성된 파일. `pnpm code-pages:gen`이 router-schemas/openai/gpt-5.5-pro.json에서 생성했습니다. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

Comfy Router가 OpenAI에서 제공하는 `openai/gpt-5.5-pro`에 대한 API 레퍼런스입니다.
...
**모델 ID:** `openai/gpt-5.5-pro`

**엔드포인트:** `POST https://api.comfy.org/v2/models/openai/gpt-5.5-pro`
...
```
  ...
  <Tab title="실행 대기열에 제출하고 나중에 수집하기">
동일한 본문을 `POST https://api.comfy.org/v2/models/openai/gpt-5.5-pro/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
<ParamField body="input" type="string | object[]" required>
  응답 생성에 사용되는 모델에 대한 텍스트, 이미지 또는 파일 입력입니다. 이 계약에서 Router가 제공할 수 없는 유일한 필드이며, 아래 `required`의 유일한 항목입니다.
</ParamField>
...
<ParamField body="max_output_tokens" type="integer">
  응답을 위해 생성되는 토큰 수의 상한으로, 표시되는 출력 토큰과 추론 토큰을 포함합니다. 추론 id에서는 이 상한을 숨김 추론 토큰과 공유하므로, 작은 값은 표시되는 텍스트가 나오기 전에 전체 예산을 소진할 수 있습니다. 이것이 추론 스모크 케이스가 1024를 보내고 채팅 스모크 케이스가 16을 보내는 이유입니다.

  범위: `1` ~ `…`
</ParamField>

<ParamField body="model" type="string">
  OpenAI 모델 식별자입니다. Comfy Router에서 이 필드는 선택 사항이며 Router가 `{model}` 경로 세그먼트에서 채웁니다. 명시적 `null`도 같은 방식으로 교체됩니다. 경로와 일치하지 않는 값을 보내면 거부됩니다.
</ParamField>
...
<ParamField body="reasoning" type="object">
  추론 등급 전용입니다. 추론 모델을 위한 구성입니다(예: `{"effort": "medium"}`). 변경 없이 전달됩니다. 허용되는 키는 OpenAI의 추론 가이드를 참조하세요. 채팅 등급 id는 이를 무시합니다.
</ParamField>
...
<ParamField body="stream" type="boolean">
  이를 보내는 호출자가 거부되지 않도록 선언되어 있지만, 이 표면에서는 아무 효과가 없습니다. Router는 디스패치 이전에 이를 `false`로 확정합니다. `text/event-stream`을 중계하는 대신 공급자 응답을 포착하기 때문입니다. openAiResponsesProxy의 ModifyResponse는 이를 디코딩할 수 없으므로, 스트리밍 생성은 OpenAI가 과금하고 아무도 계량하지 않게 됩니다. 스트림이 필요하면 `POST /proxy/openai/v1/responses`를 사용하세요.
</ParamField>

<ParamField body="temperature" type="number">
  샘플링 온도입니다. 채팅 등급 전용입니다. o 시리즈 추론 id(`o1`, `o1-pro`, `o3`, `o4-mini`)는 OpenAI에서 이 매개변수를 거부합니다. Router는 이들을 위해 이를 거부하지 않습니다. 두 등급이 하나의 스키마를 공유하는 이유에 대해서는 이 컴포넌트의 설명을 참조하세요. 따라서 이를 보내는 추론 호출은 OpenAI 자체 오류로 응답됩니다.
...
<ParamField body="truncation" type="string">
  컨텍스트가 모델의 창을 초과할 때의 잘림 전략입니다. 위의 세 어휘와 달리 여기서는 enum이 실제로 강제됩니다. 이 두 값이 OpenAI가 문서화한 완전한 집합이며 늘어나지 않았기 때문입니다. 명시적 `null`은 위 필드들과 동일한 조건으로 여전히 허용됩니다.

  가능한 값: `auto`, `disabled`
</ParamField>

<ParamField body="usage" type="object">
  토큰 사용량 봉투입니다. v1 작업이 요청 본문에 이를 선언하기 때문에 이 계약에 존재합니다. OpenAI는 이를 응답에 채우므로, 호출자가 보낼 이유가 없습니다.
</ParamField>

`GET /v2/models/openai/gpt-5.5-pro/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 호출을 검증하는 동일한 문서입니다.
### 출력

<ResponseField name="instructions" type="string">
  시스템(또는 개발자) 메시지를 모델 컨텍스트의 첫 번째 항목으로 삽입합니다.

`previous_response_id`와 함께 사용할 때, 이전 응답의 instructions는 다음 응답으로 이어지지 않습니다. 따라서 새 응답에서 시스템(또는 개발자) 메시지를 간단히 교체할 수 있습니다.
</ResponseField>
...
- `auto`: 이 응답과 이전 응답의 컨텍스트가 모델의 컨텍스트 창 크기를 초과하면, 모델은 대화 중간의 입력 항목을 제거하여 응답을 컨텍스트 창에 맞게 잘라냅니다.
- `disabled` (기본값): 모델 응답이 모델의 컨텍스트 창 크기를 초과할 경우, 요청은 400 오류로 실패합니다.
...
다중 턴 대화를 생성합니다. [대화 상태](https://platform.openai.com/docs/guides/conversation-state)에 대해
더 알아보세요.
</ResponseField>
...
[추론 모델](https://platform.openai.com/docs/guides/reasoning)에 대한
구성 옵션입니다.
</ResponseField>

<ResponseField name="reasoning.context" type="string">
  나중에 이어지는 턴에서 어떤 추론 항목이 모델에 다시 전달되는지 제어합니다. 예: `auto`, `current_turn`, `all_turns`.
</ResponseField>

<ResponseField name="reasoning.effort" type="string" default="&quot;medium&quot;">
  **o-시리즈 모델 전용**

[추론 모델](https://platform.openai.com/docs/guides/reasoning)의 추론 노력을
제한합니다. 현재 지원되는 값은 `low`, `medium`, `high`입니다. 추론 노력을
줄이면 응답이 빨라지고 응답에서 추론에 사용되는
토큰 수가 줄어들 수 있습니다.
...
이해에 유용할 수 있습니다. `auto`, `concise`, `detailed`
중 하나입니다.
...
<ResponseField name="tool_choice" type="`none`, `auto`, `required` | object">
  응답을 생성할 때 모델이 사용할 도구를 어떻게 선택해야 하는지
지정합니다. 모델이 호출할 수 있는 도구를 지정하는 방법은 `tools`
매개변수를 참조하세요.
</ResponseField>
...
- `output` 배열의 첫 번째 항목에 접근하여 그것이 모델이 생성한 콘텐츠를 담은 `assistant` 메시지라고 가정하기보다는, SDK에서 지원되는 경우 `output_text` 속성을 사용하는 것을 고려해 보세요.
</ResponseField>

<ResponseField name="output_text" type="string">
  `output` 배열의 모든 `output_text` 항목에 있는 집계된 텍스트 출력을 포함하는 SDK 전용 편의 속성입니다(있는 경우). Python 및 JavaScript SDK에서 지원됩니다.
</ResponseField>
...
<ResponseField name="presence_penalty" type="number">
  지금까지의 텍스트에 나타나는지 여부를 기준으로 새 토큰에 페널티를 부여합니다.
</ResponseField>

<ResponseField name="prompt_cache_key" type="string">
  유사한 요청에 대한 응답을 캐시하여 캐시 적중률을 최적화하기 위해 OpenAI가 사용합니다. `user` 필드를 대체합니다.
</ResponseField>
...
<ResponseField name="top_logprobs" type="integer">
  각 토큰 위치에서 반환할 가장 가능성이 높은 토큰의 최대 개수이며, 각각 연관된 로그 확률을 가집니다.
</ResponseField>
