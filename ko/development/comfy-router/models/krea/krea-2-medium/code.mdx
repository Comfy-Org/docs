---
title: "Comfy Router로 Krea 2 Medium 사용하기"
description: "Comfy Router를 통해 krea/krea-2-medium을 호출합니다: endpoint, 요청 형태, 그리고 Router가 반환하는 응답."
...
{/* 생성된 파일. router-schemas/krea/krea-2-medium.json에서 `pnpm code-pages:gen`으로 생성됨. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

`krea/krea-2-medium`의 API 레퍼런스이며, Krea에서 Comfy Router를 통해 제공됩니다.
...
**모델 ID:** `krea/krea-2-medium`

**엔드포인트:** `POST https://api.comfy.org/v2/models/krea/krea-2-medium`
...
# SDK가 멱등성 키를 자동으로 생성하고 자동 재시도에 재사용합니다.
...
동일한 body를 `POST https://api.comfy.org/v2/models/krea/krea-2-medium/requests`로 전송합니다. Router는 실행이 승인되는 즉시 `201`과 `request_id`를 응답하며, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 다룹니다.
...
Router가 `GET /v2/models/krea/krea-2-medium/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 전에 라우터가 호출을 검증할 때 사용하는 것과 동일한 문서입니다.
...
<ResponseField name="result" type="object" required>
  완료된 생성 결과. `KreaJob`과 달리 여기서는 null이 될 수 없습니다. `result`에 `urls`가 없는 터미널 작업은 Comfy Router 오류로 응답되므로, `200` 응답에는 항상 이 필드가 포함됩니다.
</ResponseField>
...
<ResponseField name="result.urls" type="string (uri)[]" required>
  생성된 이미지의 다운로드 가능한 링크. 최소 한 개 이상입니다. `urls`가 비어 있는 `completed` 작업은 `success_without_output`이며 이 문서에 도달하지 않기 때문입니다.
</ResponseField>

<ResponseField name="status" type="string" required>
  이 문서에서는 항상 `completed`입니다. `classifyKrea`가 `succeeded`로 응답하는 유일한 상태이며, Krea의 어휘는 대소문자 폴딩 없이 매칭됩니다.
