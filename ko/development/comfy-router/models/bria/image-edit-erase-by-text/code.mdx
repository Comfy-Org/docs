...
`bria/image-edit-erase-by-text`의 API Reference로, Comfy Router가 Bria에서 제공합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/bria/image-edit-erase-by-text/requests`로 전송합니다. 라우터는 실행이 접수되는 즉시 `request_id`와 함께 `201`을 응답하며, 결과는 준비되면 이 프로세스에서든 다른 프로세스에서든 수집할 수 있습니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 안내합니다.
