...
동일한 본문을 `POST https://api.comfy.org/v2/models/beeble/switchx/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 응답과 `request_id`를 반환하고, 결과는 준비되는 대로 이 프로세스나 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [대기열 전송](/ko/development/comfy-router/queue)에서 자세히 다룹니다.
