...
동일한 body를 `POST https://api.comfy.org/v2/models/bfl/flux-3-video/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `201`과 `request_id`로 응답하며, 결과는 준비가 완료되면 이 프로세스든 다른 프로세스든 상관없이 수집할 수 있습니다. [대기 중 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 살펴봅니다.
