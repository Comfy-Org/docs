...
`meshy/meshy-7`에 대한 API 레퍼런스이며, Meshy에서 Comfy Router를 통해 제공됩니다.
...
**엔드포인트:** `POST https://api.comfy.org/v2/models/meshy/meshy-7`
...
동일한 본문을 `POST https://api.comfy.org/v2/models/meshy/meshy-7/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
<ParamField body="ai_model" type="string">
  실행할 Meshy 모델입니다. Comfy Router 경로에서는 경로(`/v2/models/meshy/{model}`)로 제공되며 생략하거나 null로 보낼 수 있습니다. Router는 검증 후 경로의 모델을 여기에 기록합니다. v1 /proxy 경로에서는 MeshyTextTo3DPreviewRequest를 참조하세요. 이는 이를 MeshyAiModel로 제한합니다.
</ParamField>

<ParamField body="art_style" type="string" default="&quot;realistic&quot;">
  객체의 원하는 아트 스타일을 설명합니다.
...
<ParamField body="moderation" type="boolean" default="false">
  참일 때 입력 콘텐츠에서 잠재적으로 유해한 콘텐츠를 검사합니다.
</ParamField>
...
<ParamField body="prompt" type="string" required>
  3D 모델이 어떤 객체인지 설명합니다. 최대 600자입니다.
</ParamField>

<ParamField body="should_remesh" type="boolean" default="true">
  리메시 단계를 활성화할지 제어합니다. 거짓이면 최고 정밀도의 삼각 메시를 반환합니다.
</ParamField>
...
<ParamField body="ultra_mode" type="boolean" default="false">
  더 세밀한 표면 디테일과 함께 더 높은 충실도의 지오메트리를 위해 Ultra 생성을 활성화합니다. ai_model이 meshy-7 또는 latest인 경우에만 지원됩니다.
</ParamField>

#### `refine` 변형

<ParamField body="ai_model" type="string">
  실행할 Meshy 모델입니다. Comfy Router 경로에서는 경로(`/v2/models/meshy/{model}`)로 제공되며 생략하거나 null로 보낼 수 있습니다. Router는 검증 후 경로의 모델을 여기에 기록합니다. v1 /proxy 경로에서는 MeshyTextTo3DRefineRequest를 참조하세요. 이는 이를 MeshyAiModel로 제한합니다.
</ParamField>

<ParamField body="enable_pbr" type="boolean" default="false">
  기본 색상 외에 PBR 맵(metallic, roughness, normal)을 생성합니다. Sculpture 스타일을 사용할 때는 enable_pbr를 거짓으로 설정해야 합니다.
</ParamField>
...
<ParamField body="preview_task_id" type="string" required>
  해당 미리보기 작업 id입니다. 지정한 미리보기 작업의 상태가 SUCCEEDED여야 합니다.
</ParamField>
...
<ParamField body="texture_resolution" type="string" default="&quot;2k&quot;">
  생성된 텍스처의 해상도입니다. 2k, 4k, 8k 중 하나입니다. 4k와 8k는 ai_model이 meshy-6, meshy-7 또는 latest여야 합니다.

  가능한 값: `2k`, `4k`, `8k`
</ParamField>

`GET /v2/models/meshy/meshy-7/openapi.json`에서 Router가 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에 도달하기 이전에 Router가 호출을 검증하는 데 사용하는 것과 동일한 문서입니다.
### 출력
...
<ResponseField name="progress" type="integer">
  작업의 진행률입니다. 시작되지 않은 경우 0이며, 성공한 경우 100입니다.
