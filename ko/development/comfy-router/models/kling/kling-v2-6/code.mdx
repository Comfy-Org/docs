...
# SDK는 멱등성 키(idempotency key)를 자동으로 생성하고 자동 재시도에 재사용합니다.
...
// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 멱등성 키(idempotency key)를 발급하고 자동 재시도에 재사용합니다.
...
동일한 본문을 `POST https://api.comfy.org/v2/models/kling/kling-v2-6/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 로 응답하고, 결과는 준비되는 대로 이 프로세스나 다른 프로세스에서 수집합니다. 상태, 취소, 수집 방법은 [큐 전송](/ko/development/comfy-router/queue) 에서 자세히 다룹니다.
