...
동일한 본문을 `POST https://api.comfy.org/v2/models/kling/kling-v2-5-turbo/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기열 전달](/ko/development/comfy-router/queue) 를 참고하세요.
...
Router가 `GET /v2/models/kling/kling-v2-5-turbo/openapi.json`에서 제공하는 스키마를 기반으로 생성되었으며, 이는 요청이 공급자에게 도달하기 전에 호출을 검증하는 데 사용하는 동일한 문서입니다.
### 출력
