---
title: "Comfy Router에서 Patina 사용하기"
description: "Comfy Router를 통해 fal/patina를 호출하는 방법: endpoint, 요청 형태, 그리고 Router가 반환하는 응답."
sidebarTitle: "Patina"
translationSourceHash: 17d60787
translationFrom: development/comfy-router/models/fal/patina/code.mdx
translationBlockHashes:
  "_intro": 325742dd
  "Quick start": c556a4dd
  "Schema": 44320e46
  "Examples": a3177476
---

{/* GENERATED FILE. Generated from router-schemas/fal/patina.json by `pnpm code-pages:gen`. */}

import RouterCodeFooter from "/snippets/ko/comfy-router/model-code-footer.mdx";

`fal/patina`에 대한 API 레퍼런스이며, Comfy Router가 fal에서 제공합니다.

## 빠른 시작

[Comfy 워크스페이스](https://platform.comfy.org/profile/api-keys?onboarding=router)에서 키를 생성하고 `COMFY_API_KEY`로 내보내세요. Python, TypeScript 및 Swift 스니펫은 Comfy SDK(`pip install comfy-sdk`, `npm install @comfyorg/sdk`, 그리고 [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift 패키지)를 사용하며, cURL 스니펫은 원시 HTTP를 통한 동일한 호출입니다.

**모델 ID:** `fal/patina`

**엔드포인트:** `POST https://api.comfy.org/v2/models/fal/patina`

<Tabs>
  <Tab title="결과 대기">
<CodeGroup>
```python Python
from comfy_sdk import Comfy

# 환경에서 COMFY_API_KEY를 읽습니다.
# SDK는 자동으로 idempotency 키를 생성하고 자동 재시도 시 재사용합니다.
with Comfy() as client:
    result = client.models.run(
        "fal/patina",
        {
            "image_url": "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png",
        },
    )

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// 환경에서 COMFY_API_KEY를 읽습니다.
// SDK는 자동으로 idempotency 키를 생성하고 자동 재시도 시 재사용합니다.
const { data } = await comfy.models.run("fal/patina", {
  image_url: "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png",
});

console.log(data);
```

```swift Swift
import Foundation
import ComfySwiftSDK

// 환경에서 COMFY_API_KEY를 읽습니다.
// SDK는 호출마다 idempotency 키를 발급하고 자동 재시도 시 재사용합니다.
let client = ComfyCloudClient(apiKey: ProcessInfo.processInfo.environment["COMFY_API_KEY"]!)
let result = try await client.models.run(
    "fal/patina",
    input: [
        "image_url": "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png",
    ]
)

print(result.output)
```

```bash cURL
curl https://api.comfy.org/v2/models/fal/patina \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"image_url\": \"https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png\"}"
```
</CodeGroup>
  </Tab>
  <Tab title="대기열에 넣고 나중에 수집">
동일한 본문을 `POST https://api.comfy.org/v2/models/fal/patina/requests`로 전송합니다. Router는 실행이 접수되는 즉시 `request_id`와 함께 `201`로 응답하고, 결과는 준비되면 이 프로세스 또는 다른 프로세스에서 수집합니다. [대기열 전달](/ko/development/comfy-router/queue)에서 상태, 취소, 수집 과정을 자세히 안내합니다.

<CodeGroup>
```python Python
from comfy_sdk import Comfy

# 환경 변수에서 COMFY_API_KEY를 읽습니다.
# 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
with Comfy() as client:
    handle = client.models.submit(
        "fal/patina",
        {
            "image_url": "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png",
        },
    )
    print("request_id:", handle.request_id)  # 모델 ID와 함께라면 다른 프로세스에 필요한 전부입니다

    # 서버가 지정한 Retry-After만큼 기다리며 요청이 완료될 때까지 폴링합니다.
    for update in handle.iter_events():
        print(update.status, update.queue_position)

    # 공급자 자체의 페이로드이며, models.run()이 반환하는 것과 동일한 값입니다.
    # 실패하거나 취소된 요청은 여기에서 타입이 지정된 Router 오류를 발생시킵니다.
    result = handle.get()

print(result)
```

```typescript TypeScript
import { comfy } from "@comfyorg/sdk";

// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
const handle = await comfy.models.submit("fal/patina", {
  image_url: "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png",
});
console.log("requestId:", handle.requestId); // 모델 ID와 함께라면 다른 프로세스에 필요한 전부입니다

// 서버가 지정한 Retry-After만큼 기다리며 요청이 완료될 때까지 폴링합니다.
for await (const update of handle.events()) {
  console.log(update.status, update.queuePosition);
}

// models.run()이 반환하는 것과 동일한 결과입니다. 실패하거나 취소된 요청은 여기에서 거부됩니다.
const result = await handle.get();

console.log(result.data);
```

```swift Swift
import Foundation
import ComfySwiftSDK

// 환경 변수에서 COMFY_API_KEY를 읽습니다.
// 각 submit() 호출은 자체 Idempotency-Key를 생성하고 자동 재시도에 재사용합니다.
let client = ComfyCloudClient(apiKey: ProcessInfo.processInfo.environment["COMFY_API_KEY"]!)
let handle = try await client.models.submit(
    "fal/patina",
    input: [
        "image_url": "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png",
    ]
)
print("requestId:", handle.requestId)  // 모델 ID와 함께라면 다른 프로세스에 필요한 전부입니다

// 서버가 지정한 Retry-After만큼 기다리며 요청이 완료될 때까지 폴링합니다.
for try await update in handle.events() {
    print(update.state.rawValue, update.queuePosition.map(String.init) ?? "unknown")
}

// 공급자 자체의 페이로드이며, models.run()이 반환하는 것과 동일한 값입니다.
// 실패하거나 취소된 요청은 여기에서 타입이 지정된 Router 오류를 throw합니다.
let result = try await handle.result()

print(result.output)
```

```bash cURL
# 1. 제출합니다. Router는 request_id, status_url, response_url, cancel_url과 함께 201로 응답합니다.
curl https://api.comfy.org/v2/models/fal/patina/requests \
  -H "X-API-Key: $COMFY_API_KEY" \
  -H "Idempotency-Key: $(uuidgen)" \
  -H "Content-Type: application/json" \
  -d "{\"image_url\": \"https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png\"}"

# 2. 각 응답이 지정한 Retry-After 초만큼 기다리며 status가 COMPLETED가 될 때까지 폴링합니다.
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/fal/patina/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 수집합니다. 모델의 네이티브 출력과 함께 200, 아직 실행 중이면 status 본문과 함께 202를 반환합니다.
curl https://api.comfy.org/v2/models/fal/patina/requests/$REQUEST_ID \
  -H "X-API-Key: $COMFY_API_KEY"
```
</CodeGroup>
  </Tab>
</Tabs>

## 스키마

### 입력

<ParamField body="enable_safety_checker" type="boolean" default="true">
  이미지에 대한 안전 검사기를 활성화합니다.
</ParamField>

<ParamField body="image_url" type="string" required>
  입력 이미지(사진 또는 렌더)의 URL입니다.
</ParamField>

<ParamField body="maps" type="string[]" default="[&quot;basecolor&quot;,&quot;normal&quot;,&quot;roughness&quot;,&quot;metalness&quot;,&quot;height&quot;]">
  예측할 PBR 맵을 지정합니다. 기본값은 다섯 가지 모두입니다.
</ParamField>

<ParamField body="output_format" type="string" default="&quot;png&quot;">
  출력 이미지 형식: jpeg, png, webp.
</ParamField>

<ParamField body="seed" type="integer">
  재현 가능한 디노이징을 위한 랜덤 시드입니다.
</ParamField>

<ParamField body="sync_mode" type="boolean" default="false">
  true이면 이미지를 CDN URL 대신 data URI로 반환합니다.
</ParamField>

Router가 `GET /v2/models/fal/patina/openapi.json`에서 제공하는 스키마에서 생성되었으며, 이는 요청이 공급자에게 도달하기 이전에 호출을 검증하는 데 사용하는 동일한 문서입니다.

### 출력

<ResponseField name="images" type="object[]">
  생성된 타일링 가능한 텍스처 이미지와 예측된 PBR 재질 맵입니다. 기본 텍스처에는 url만 있으며, 각 맵 항목에는 map_type도 함께 포함됩니다.
</ResponseField>

<ResponseField name="images[].map_type" type="string">
  PBR 맵 유형(basecolor, normal, roughness, metalness, height)입니다. 기본 텍스처 이미지에는 없습니다.
</ResponseField>

<ResponseField name="images[].url" type="string">
  생성된 이미지의 URL입니다.
</ResponseField>

<ResponseField name="prompt" type="string">
  텍스처 생성에 사용된 프롬프트(확장되었을 수 있음)입니다.
</ResponseField>

<ResponseField name="seed" type="integer">
  텍스처 생성에 사용된 시드입니다.
</ResponseField>

<ResponseField name="timings" type="object">
  엔드투엔드 타이밍 분석(초)입니다.
</ResponseField>

## 예시

### 입력

```json
{
  "image_url": "https://storage.googleapis.com/falserverless/gallery/patina-blog-hero-render.png"
}
```

### 출력

```json
{
  "images": [
    {
      "map_type": "basecolor",
      "url": "https://example.invalid/fal/patina/basecolor.png"
    },
    {
      "map_type": "normal",
      "url": "https://example.invalid/fal/patina/normal.png"
    },
    {
      "map_type": "roughness",
      "url": "https://example.invalid/fal/patina/roughness.png"
    }
  ],
  "prompt": "weathered copper roof tiles",
  "seed": 1234567,
  "timings": {
    "inference": 6.25
  }
}
```

<RouterCodeFooter />
