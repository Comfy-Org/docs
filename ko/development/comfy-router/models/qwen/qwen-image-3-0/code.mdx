...
동일한 본문을 `POST https://api.comfy.org/v2/models/qwen/qwen-image-3.0/requests` 로 보냅니다. Router는 실행이 접수되는 즉시 `201` 과 `request_id` 를 응답하며, 결과는 준비가 되는 대로 이 프로세스나 다른 프로세스에서 수집할 수 있습니다. 상태, 취소, 수집 방법은 [대기열 전송](/ko/development/comfy-router/queue) 를 참고하세요.
...
with Comfy() as client:
    ...
    print("request_id:", handle.request_id)  # 다른 프로세스에 필요한 것은 모델 ID와 이것뿐입니다
...
console.log("requestId:", handle.requestId); // 다른 프로세스에 필요한 것은 모델 ID와 이것뿐입니다
...
print("requestId:", handle.requestId)  // 다른 프로세스에 필요한 것은 모델 ID와 이것뿐입니다
...
# 2. 상태가 COMPLETED가 될 때까지 폴링하며, 각 응답이 알려준 Retry-After 초만큼 기다립니다.
