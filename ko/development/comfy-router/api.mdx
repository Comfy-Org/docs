...
하나의 Router 호출은 기본적으로 10분 동안 연결을 유지할 수 있습니다. 클라이언트 타임아웃을 그 한도보다 높게 설정하여 불투명한 로컬 중단 대신 타입이 지정된 `504`와 요청 ID를 유지하세요. 애플리케이션이 그렇게 오래 연결을 유지할 수 없다면, [큐 전송](/ko/development/comfy-router/queue)이 즉시 `request_id`를 반환하고 나중에 결과를 수집할 수 있게 해줍니다.
...
- [큐 전송](/ko/development/comfy-router/queue): 연결을 유지하지 않고 요청을 제출하고 상태를 추적하며 결과를 수집하거나 취소합니다.
