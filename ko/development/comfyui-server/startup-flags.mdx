...
| `--models-directory` `PATH` | ComfyUI 루트/models | 모델 디렉토리. `--base-directory`의 models 폴더를 재정의합니다. |
...
<Note>
  `--windows-standalone-build`는 `auto_launch`를 true로 설정합니다. `--disable-auto-launch`가 이를 재정의합니다. 브라우저를 열지 않고 서버로 실행하려면 `--disable-auto-launch`를 사용하세요.
</Note>
...
| 플래그 | 기본값 | 설명 |
|------|---------|-------------|
| `--cuda-device` `DEVICE_ID` | 없음 | 사용할 CUDA 장치 ID, 쉼표로 구분 (예: `0` 또는 `0,1`), 또는 `all`을 지정하면 현재 표시되는 모든 장치를 유지합니다. 다른 장치는 숨겨집니다. |
...
| `--disable-triton-backend` | 비활성화 | comfy-kitchen Triton 백엔드를 강제로 비활성화하며 `--enable-triton-backend`를 재정의합니다. |
...
<Note>
  캐시 모드 플래그는 상호 배타적입니다. `--cache-ram`, `--cache-classic`, `--high-ram`, `--cache-lru`, `--cache-none` 중 하나만 사용해야 합니다.
</Note>

| 플래그 | 기본값 | 설명 |
|------|---------|-------------|
| `--cache-ram` `[GB] [GB]` | 활성화됨 (기본 모드) | RAM 압력 캐싱입니다. 첫 번째 값: 활성 캐시 임계값(GB). 선택적 두 번째 값: 비활성 캐시/고정 임계값(GB). 값이 주어지지 않으면: 활성 = 시스템 RAM의 10% (최소 2 GB, 최대 10 GB); 비활성 = 시스템 RAM의 100% (최대 128 GB). 최대 두 개의 값만 허용합니다. |
| `--cache-classic` | - | 이전의 적극적인 캐싱 방식을 사용합니다. |
| `--high-ram` | 비활성화 | RAM이 충분한 시스템이나 모델을 다시 로드하는 대신 페이지 파일 사용을 선호하는 시스템에서 성능이 약간 향상될 수 있습니다. classic 캐싱도 함께 활성화됩니다. |
...
<Note>
  ComfyUI는 같은 세션에서 캐시된 노드 결과를 재사용하며, 이것이 반복 실행을 빠르게 만드는 이유입니다. 매번 깨끗한 캐시에서 시작하려면 `--cache-none`을 사용하세요. 예를 들어 워크플로를 디버깅할 때나 실행 간 캐시 재사용 여부를 배제할 때 유용합니다. 모든 노드가 다시 실행되므로 생성 시간은 더 길어집니다. 시작 시 지정하는 플래그이므로 적용하려면 ComfyUI를 다시 시작해야 합니다.
</Note>
...
| `--use-ck-attention` | Comfy Kitchen 어텐션을 사용합니다. INT8 커널을 지원하는 하드웨어가 필요합니다. |
...
| `--vram-headroom` `GB` | `0` | 기본 헤드룸에 더해 동적 VRAM이 비워 둘 추가 VRAM(GB)입니다. ComfyUI는 다른 앱이 사용하는 VRAM까지 포함해 이 만큼의 VRAM을 완전히 비워 두려고 시도합니다. |
| `--disable-nvml-pressure` | 비활성화 | 동적 VRAM 메모리 압력을 NVML 대신 CUDA로 보고합니다. |
...
| `--disable-fast-disk` | 비활성화 | 디스크 기반 동적 로딩과 오프로드를 비활성화하고 비고정 RAM을 사용합니다. `--fast-disk`를 재정의합니다. |
| `--disable-cuda-graphs` | 비활성화 | CUDA graphs를 비활성화합니다. |
| `--disable-comfy-compiler` | 비활성화 | Comfy 모델 컴파일러를 비활성화합니다(CUDA graph 하위 기능 포함). |
| `--assert-graph-breaks` | 비활성화 | Comfy 모델 컴파일러의 graph break에서 실패 처리합니다. |
...
| `--debug-hang` | 비활성화 | Ctrl-C 입력 시 스택 트레이스를 덤프하여 멈춤 현상을 디버깅합니다. |
...
| `--enable-manager-legacy-ui` | 레거시 매니저 UI를 사용합니다. `--enable-manager`를 자동으로 활성화합니다. `--disable-manager-ui`와 상호 배타적입니다. |
...
<Note>
  `--disable-api-nodes`는 브라우저가 불러올 수 있는 내용도 바꿉니다. 이 플래그는 Content-Security-Policy를 추가해 페이지를 ComfyUI 자체가 제공하는 리소스로 제한하므로, 다른 출처의 이미지, 폰트, 미디어, 프레임, 네트워크 요청이 차단되고 API 노드(파트너 노드)도 등록되지 않습니다. 재시작 후 평소에 불러오던 미리보기나 썸네일이 보이지 않는다면 노드를 살펴보기 전에 실행 인자에 이 플래그가 있는지 확인하세요.
</Note>
...
| `--enable-asset-hashing` | 비활성화 | 에셋을 스캔할 때 blake3 콘텐츠 해시를 계산합니다. 향후 에셋 이식성 기능(중복 제거, 머신 간 모델 해석)을 가능하게 하지만 시작 시간과 대규모 모델 디렉토리에서 출력마다 해싱하는 비용이 추가됩니다. 기본적으로 비활성화됩니다. |
| `--feature-flag` `KEY[=VALUE]` | — | 서버 기능 플래그를 설정합니다. `KEY`만 지정하면 true로 설정됩니다. 반복하여 사용할 수 있습니다. 불리언과 숫자는 자동 변환됩니다. |
...
| 플래그 | 기본값 | 설명 |
|------|---------|-------------|
| `--verbose` `[LEVEL] [FILE]` | `INFO` | 로깅 수준. 선택: `DEBUG`, `DETAIL`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`. `--verbose`만 사용하면 `DEBUG`로 설정됩니다. 수준 뒤에 파일 경로를 지정하면 해당 수준을 파일에도 기록합니다. 이 플래그는 반복 지정할 수 있어 콘솔과 파일 출력을 함께 설정할 수 있습니다. |
