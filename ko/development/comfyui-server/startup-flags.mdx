...
icon: "terminal"
translationSourceHash: 4cbb463e
...
| `--fast-disk` | 자동 | 비고정 RAM보다 디스크 기반 동적 로딩을 강제합니다. 고속 NVMe에 유용합니다. 모델 파일이 고속으로 감지된 저장소에 있으면 자동으로 활성화됩니다. |
| `--disable-fast-disk` | 비활성화 | 디스크 기반 동적 로딩을 비활성화합니다. `--fast-disk`와 자동 감지보다 우선합니다. |
