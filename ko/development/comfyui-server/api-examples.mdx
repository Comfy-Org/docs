...
이 페이지에서는 간단한 HTTP 제출부터 실시간 이미지 출력을 포함한 완전한 WebSocket 통합까지, ComfyUI 서버 API와 상호작용하는 세 가지 방법을 보여줍니다.
...
<Note>
  이 예제들은 표준 라이브러리 및 `websocket-client` 패키지와 함께 Python을 사용합니다(`pip install websocket-client`). 언어에 관계없이 기본 API 프로토콜은 동일합니다. TypeScript 및 curl에 해당하는 예제는 [클라우드 API 참조](/ko/development/cloud/api-reference)에서 확인하세요.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="방법 2: WebSocket + 히스토리" icon="chart-line" href="#방법-2-websocket-+-히스토리-완료-모니터링">
    **권장.** 실행이 완료될 때까지 기다렸다가 출력물을 다운로드하세요. 간편함과 신뢰성의 최적 조화입니다.
  </Card>
  <Card title="방법 3: SaveImageWebsocket" icon="images" href="#방법-3-saveimagewebsocket을-사용한-websocket-실시간-이미지">
