---
title: "실행 모델 반전 가이드"
description: "ComfyUI 실행 모델 반전 가이드. monkey patching과 선택적 입력 검증의 호환성 깨짐, 지연 평가와 노드 확장 등 새 기능을 다룹니다."
icon: "code-branch"
...
- 비교할 수 없는 타입(예: 딕셔너리)에 `min` 및 `max`와 같은 예약된 [추가 매개변수](/ko/custom-nodes/backend/datatypes#추가-파라미터)를 사용하여 맞춤 위젯을 구성하는 경우.
    ...
    - 이 입력을 포함한 맞춤 [VALIDATE_INPUTS](/ko/custom-nodes/backend/server_overview#validate_inputs) 함수를 정의하여 검증을 건너뛰세요. *(빠른 해결 방법)* 
        ```python
        @classmethod
...
- 복합 타입(예: `CUSTOM_A,CUSTOM_B`) 사용 시
    - (출력으로 사용될 때) `MakeSmartType`과 같은 래퍼를 정의하고 사용하세요. [PR의 단위 테스트에서 확인할 수 있음](https://github.com/Comfy-Org/ComfyUI/pull/2666/files#diff-714643f1fdb6f8798c45f77ab10d212ca7f41dd71bbe55069f1f9f146a8f0cb9R2)
        ...
        class MyCustomNode:
            ...
            # ...
        ```
    - (입력으로 사용될 때) `input_types` 인자를 받는 맞춤 [VALIDATE_INPUTS](/ko/custom-nodes/backend/server_overview#validate_inputs) 함수를 정의하여 타입 검증을 건너뛰세요.
        ```python
        @classmethod
...
위에서 언급한 [선택적 입력 검증](#선택적-입력-검증)의 영향을 줄이기 위해 `VALIDATE_INPUTS` 함수에 여러 기능이 추가되었습니다.
...
자세한 내용은 [VALIDATE_INPUTS](/ko/custom-nodes/backend/server_overview#validate_inputs)에서 확인할 수 있습니다.
