...
<Note>
  API 액세스에는 유료 Comfy Cloud 구독이 필요합니다. 무료 티어에는 포함되지 않습니다. 동시에 실행할 수 있는 작업 수는 티어에 따라 다릅니다. [병렬 실행](/ko/development/deploy/cloud#병렬-실행-동시-작업)을 참조하세요.
</Note>
...
베타 기간 동안 v2 API는 ComfyUI와 함께 실행되는 작은 오픈소스 서비스인 [comfy-api-proxy](https://github.com/Comfy-Org/comfy-api-proxy)가 제공합니다. 설치하고 실행한 다음 `COMFY_BASE_URL="http://127.0.0.1:8189"`로 설정하세요:
...
구성, 인증 및 프록시가 존재하는 이유를 확인하려면 [자체 호스팅 ComfyUI용 API 프록시](/ko/development/comfyui-server/api-proxy)를 참조하세요. 프록시는 임시 방편입니다. v2 API가 안정화되면 ComfyUI 코어로 이동하므로 프록시가 더 이상 필요하지 않습니다.
