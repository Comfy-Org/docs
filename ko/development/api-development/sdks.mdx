...
| **자체 ComfyUI** | `http://127.0.0.1:8189` (로컬 프록시) | 사용하지 않음 |
...
그런 다음 SDK가 `http://127.0.0.1:8189`를 가리키게 하고 API 키는 전달하지 마세요. 프록시는 기본적으로 루프백에만 바인딩됩니다. 설치된 ComfyUI에 모델 파일도 업로드하려면 `--comfyui-base-dir /path/to/ComfyUI`와 함께 실행하세요.
