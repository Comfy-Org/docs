...
translationSourceHash: 915dc0ea
...
모든 예제는 다음과 같은 공통 임포트와 구성 사용합니다:
...
console.log(`KSampler 입력: ${Object.keys(ksampler.input?.required ?? {}).length}`);
```
...
<Info>
  **동시 제출 지원:** 구독 등급에 따라 이전 작업이 완료되기 전에도 여러 워크플로우를 제출할 수 있습니다. 등급 한도 내에서 작업이 병렬로 실행되며, 추가 작업은 자동으로 대기열에 들어갑니다. 자세한 내용과 동시성 한도는 [병렬 실행](/development/cloud/overview#parallel-execution-concurrent-jobs)을 참조하세요.
</Info>
...
워크플로우에 [파트너 노드](/tutorials/api-nodes/overview) (Flux Pro, Ideogram 등 외부 AI 서비스를 호출하는 노드)가 포함된 경우, 요청 페이로드의 `extra_data` 필드에 [Comfy API 키](/development/api-development/getting-an-api-key)를 포함해야 합니다.
...
def submit_workflow_with_partner_nodes(workflow: dict, api_key: str) -> str:
    """파트너 노드를 사용하는 워크플로우를 제출합니다.
    
    Args:
        workflow: API 형식의 ComfyUI 워크플로우
        api_key: platform.comfy.org에서 받은 API 키
...
<Info>
  API 키는 [platform.comfy.org](https://platform.comfy.org/login)에서 생성하세요. 이는 클라우드 API 인증(`X-API-Key` 헤더)에 사용되는 동일한 키입니다. [API 키 얻기](/development/api-development/getting-an-api-key)에서 단계별 지침을 확인하세요.
</Info>
...
<Note>
  각 JSON 메시지 유형의 완전한 스키마 정의는 [OpenAPI 사양](/development/cloud/openapi)을 참조하세요.
</Note>
