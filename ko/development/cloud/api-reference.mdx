...
<Warning>
  **지원 중단됨:** v1 Cloud API는 [Comfy API v2](/ko/api-reference/v2/overview)로 대체되어 지원이 중단되었습니다. 엔드포인트, 요청/응답 형식 및 동작은 사전 통지 없이 수정될 수 있습니다. 일부 엔드포인트는 로컬 ComfyUI와의 호환성을 위해 유지되지만 다른 의미를 가질 수 있습니다(예: 무시된 필드).
</Warning>

이 페이지에서는 일반적인 Comfy Cloud API 작업에 대한 완벽한 예제를 제공합니다.

<Note>
  **구독 필요:** API를 통해 워크플로를 실행하려면 활성 Comfy Cloud 구독이 필요합니다. 자세한 내용은 [가격 페이지](https://www.comfy.org/cloud/pricing?utm_source=docs&utm_campaign=cloud-api)를 참조하세요.
</Note>
...
<Info>
  **동시 제출 지원:** 구독 등급에 따라 이전 작업이 완료될 때까지 기다리지 않고 여러 워크플로우를 제출할 수 있습니다. 등급 한도 내에서 작업이 병렬로 실행되며, 초과 작업은 자동으로 대기열에 추가됩니다. 자세한 내용과 동시성 한도는 [병렬 실행](/ko/development/cloud/overview#병렬-실행-동시-작업)을 참조하세요.
</Info>
