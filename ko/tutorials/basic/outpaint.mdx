...
아래 이미지를 다운로드한 후 **ComfyUI로 드래그하여 워크플로를 로드**하세요:

![ComfyUI 아웃페인팅 워크플로](https://raw.githubusercontent.com/Comfy-Org/example_workflows/refs/heads/main/image/basic/outpaint.png)

<Tip>
메타데이터에 워크플로 JSON이 포함된 이미지는 직접 ComfyUI로 드래그하거나 메뉴 `워크플로` -> `열기 (ctrl+o)`를 사용해 로드할 수 있습니다.
</Tip>

### 아웃페인팅 워크플로 사용 설명

![ComfyUI 아웃페인팅 워크플로 다이어그램](/images/tutorial/basic/outpaint/outpainting_workflow.jpg)

아웃페인팅 워크플로의 핵심 단계는 다음과 같습니다:

1. `체크포인트 로드` 노드에서 로컬에 설치된 모델 파일을 로드하세요
2. `이미지 로드` 노드의 `업로드` 버튼을 클릭해 이미지를 업로드하세요
3. `실행 대기열` 버튼을 클릭하거나 단축키 `Ctrl + Enter`를 사용해 이미지 생성을 실행하세요

이 워크플로에서는 주로 `아웃페인팅을 위한 이미지 패딩` 노드를 사용해 이미지 확장 방향과 범위를 제어합니다. 이 워크플로는 사실 [인페인팅](/ko/tutorials/basic/inpaint) 워크플로이지만, 마스크를 구성하기 위해 다른 노드를 사용합니다.

### 아웃페인팅을 위한 이미지 패딩 노드

![아웃페인팅을 위한 이미지 패딩 노드](/images/comfy_core/image/pad_image_for_outpainting.jpg)
...
`아웃페인팅을 위한 이미지 패딩` 노드 처리 후 출력되는 이미지와 마스크 미리보기는 다음과 같습니다:

![아웃페인팅을 위한 이미지 패딩 노드 결과](/images/tutorial/basic/outpaint/pad_Image_for_outpainting_result.jpg)
