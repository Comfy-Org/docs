...
description: "ComfyUI에서 텍스트 기반 이미지 생성 워크플로 구축: 기본 워크플로 로드, SD1.5 체크포인트 설치, 프롬프트 작성, 각 노드의 역할 이해."
...
이 가이드는 ComfyUI의 텍스트 기반 이미지 생성 워크플로우를 소개하고 다양한 ComfyUI 노드의 기능과 사용법을 이해하도록 돕습니다.
...
## ComfyUI의 텍스트 기반 이미지 생성이란? {#about-text-to-image}

**텍스트 기반 이미지 생성**은 텍스트 설명을 통해 이미지를 생성하는 AI 아트 생성의 핵심 과정으로, 그 중심에는 **확산 모델**이 있습니다.

텍스트 기반 이미지 생성 과정에는 다음과 같은 요소가 필요합니다:
- **아티스트**: 이미지 생성 모델
- **캔버스**: 잠재 공간
- **이미지 요구사항(프롬프트)**: 긍정적 프롬프트(이미지에 포함되기를 원하는 요소)와 부정적 프롬프트(포함되지 않기를 원하는 요소) 포함

이 텍스트 기반 이미지 생성 과정은 간단히 말해, 당신의 요구사항(긍정적·부정적 프롬프트)을 **아티스트(이미지 모델)**에게 전달하면, 아티스트가 이러한 요구사항에 맞춰 원하는 이미지를 만들어내는 것입니다.
...
<img src="https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/text-to-image-workflow.png" alt="ComfyUI-텍스트 기반 이미지 생성 워크플로우" />
...
## 텍스트 기반 이미지 생성은 어떻게 작동하나요? {#text-to-image-working-principles}
...
![ComfyUI 텍스트 기반 이미지 생성 워크플로우 설명](/images/tutorial/basic/text-image-workflow.jpg)
