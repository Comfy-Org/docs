...
translationSourceHash: 6524a7b2
...
### Pixal3D: 멀티뷰를 3D 모델로 {#pixal3d-multi-views}

캐릭터 턴어라운드 시트(한 장의 이미지에 정면, 측면, 후면을 배치)에서 3D 모델을 생성합니다. 멀티뷰 모델은 추가 시점을 활용해 단일 이미지 파이프라인으로는 파악할 수 없는 지오메트리를 재구성하여, 더 정확한 측면과 후면 프로필을 만들어냅니다.

템플릿 라이브러리의 `Character Turnaround Sheet Generator` 템플릿을 사용해 Nano Banana 2로 턴어라운드 시트를 만들거나, 피사체를 배경에서 분리한 멀티뷰 이미지를 직접 준비하세요.

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/3d_pixal3d_multi_views-1.webp" alt="Pixal3D 멀티뷰 워크플로 미리보기" />

<CardGroup cols={2}>
  <Card title="Comfy Cloud에서 실행" icon="cloud" href="https://cloud.comfy.org/?template=3d_pixal3d_multi_views&utm_source=docs&utm_medium=referral&utm_campaign=pixal3d">
    이 워크플로를 Comfy Cloud에서 즉시 실행하세요
  </Card>
  <Card title="워크플로 다운로드" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/3d_pixal3d_multi_views.json">
    JSON을 다운로드하거나 템플릿 라이브러리에서 &quot;Pixal3D: Multi Views to 3D&quot;을 검색하세요
  </Card>
</CardGroup>
...
<CardGroup cols={1}>
  <Card title="character_turnaround_4view_sheet.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/character_turnaround_4view_sheet.png">
    `LoadImage` 노드 364 · `character_turnaround_4view_sheet.png`
  </Card>
</CardGroup>

워크플로는 다음 모델을 사용합니다. 모델을 다운로드하여 해당하는 `models/` 하위 디렉터리에 배치하세요:

<CardGroup cols={2}>
  <Card title="Pixal3D 멀티뷰 UNet" icon="download" href="https://huggingface.co/Comfy-Org/Pixal3D/blob/main/diffusion_models/pixal3d_multiview_int8_convrot.safetensors">
    pixal3d_multiview_int8_convrot.safetensors &#8594; ComfyUI/models/diffusion_models/
  </Card>
  <Card title="쉐이프 VAE" icon="download" href="https://huggingface.co/Comfy-Org/Pixal3D/blob/main/vae/trellis_2_shape_vae_bf16.safetensors">
