...
TRELLIS.2는 Pixal3D와 기본 아키텍처를 공유합니다. 두 모델 모두 동일한 DINOv3 이미지 인코더, 동일한 형태 및 텍스처 VAE, 동일한 구조·형태·텍스처 생성 단계를 사용합니다. 주요 diffusion 모델과 조건화 형식만 다릅니다. [Pixal3D 워크플로](/tutorials/3d/pixal3d)를 실행해 본 적이 있다면 모델 설정은 거의 동일합니다.
