...
## Flux.1 Kontext Dev 워크플로

### Flux Kontext Dev 이미지 편집 (`flux_kontext_dev_basic`)

캐릭터를 일관되게 유지하고, 다른 부분에 영향을 주지 않고 특정 부분만 편집하며, 원본 스타일을 보존하는 스마트 이미지 편집입니다.
...
<CardGroup cols={2}>
  ...
  <Card title="워크플로 다운로드" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/flux_kontext_dev_basic.json">
    JSON 다운로드 또는 템플릿 라이브러리에서 &quot;Flux Kontext Dev&quot; 검색
  </Card>
</CardGroup>
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', alignItems: 'start'}}>
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/flux_kontext_dev_basic_input_image.jpg" alt="입력 이미지" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
  <img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/output/flux_kontext_dev_basic.png" alt="Flux Kontext Dev 예시 출력" style={{width: '100%', height: 'auto', objectFit: 'contain'}} />
</div>

이 워크플로는 `Load Image(from output)` 노드를 사용하여 편집할 이미지를 불러오므로, 편집된 이미지에 접근하여 여러 번 편집을 반복하기 더 편리합니다.

### 1. 워크플로 및 입력 이미지 다운로드

### 2. 단계별로 워크플로 완료하기

![워크플로 단계 가이드](/images/tutorial/flux/flux_1_kontext_dev_basic_step_guide.jpg)
이미지의 번호를 참고하여 워크플로 실행을 완료할 수 있습니다:

1. `Load Diffusion Model` 노드에서 `flux1-dev-kontext_fp8_scaled.safetensors` 모델을 로드합니다.
2. `DualCLIP Load` 노드에서 `clip_l.safetensors`와 `t5xxl_fp16.safetensors` 또는 `t5xxl_fp8_e4m3fn_scaled.safetensors`가 로드되었는지 확인합니다.
3. `Load VAE` 노드에서 `ae.safetensors` 모델이 로드되었는지 확인합니다.
