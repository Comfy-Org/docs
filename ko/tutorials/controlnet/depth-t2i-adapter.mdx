...
ComfyUI에서 T2I 어댑터를 사용하는 방법은 [ControlNet](/tutorials/controlnet/controlnet)과 인터페이스와 작업 흐름이 유사합니다. 이번 예시에서는 깊이 T2I 어댑터를 사용해 실내 장면을 제어하는 방법을 보여드리겠습니다.
...
다양한 T2I 어댑터를 혼합하거나, 다른 제어 방식(예: ControlNet, 지역 프롬프트 등)과 결합하면 창작 가능성을 더욱 확장할 수 있습니다. 혼합을 위해서는 [ControlNet 혼합](/tutorials/controlnet/mixing-controlnets)에서 설명한 것과 동일한 방식으로 여러 `Apply ControlNet` 노드를 연결하기만 하면 됩니다.