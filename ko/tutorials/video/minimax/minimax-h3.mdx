...
세 가지 예제 템플릿은 빙산의 일각에 불과합니다. 네이티브 MiniMax H3 노드는 임의 프레임에 가이드를 고정하는 기능, 프롬프트 임베딩, 잠재 노이즈 마스크도 지원합니다. 이러한 워크플로우를 사용하려면 ComfyUI를 최신 버전으로 업데이트하세요.

### 임의 프레임에 가이드 고정

예제 템플릿은 비디오의 첫 프레임과 마지막 프레임에만 키프레임을 고정합니다. `MiniMaxH3AddGuide` 노드는 이 제한을 제거합니다. 연속적인 시간 축의 임의 프레임에 가이드를 고정할 수 있습니다.

MiniMax H3 노드의 conditioning 출력과 latent 출력을 `MiniMaxH3AddGuide`에 연결한 다음, 가이드 입력을 하나 이상 제공하세요:
...
MiniMax H3는 비디오와 오디오 잠재 표현 모두에서 노이즈 마스크를 지원합니다. 평소처럼 마스크를 샘플러의 `denoise_mask` 입력에 연결하세요. 마스크된 영역은 재생성되고 나머지는 고정된 상태로 유지됩니다. 비디오 마스크는 모델의 2x2 잠재 패치 그리드에 맞춰지고, 오디오 마스크는 전체 잠재 프레임에 맞춰집니다. 부분 인페인팅, 객체 제거, 또는 기존 콘텐츠를 안정적으로 유지하면서 클립을 확장할 때 사용하세요.
