...
ComfyUI는 MiniMax H3를 네이티브로 지원합니다. 템플릿 라이브러리에는 현재 다섯 가지 예제 워크플로가 제공됩니다.
...
- **Fun ControlNet Union**: Canny, Depth, HED, MLSD 또는 Pose 컨트롤 비디오로 H3를 구동하거나, 마스크로 비디오 인페인팅을 수행합니다

이 다섯 가지는 예제 템플릿이며 모델의 전체 기능을 나열한 것이 아닙니다. 네이티브 MiniMax H3 노드를 통해 더 많은 생성 모드를 지원합니다. `MiniMaxH3ImageToVideo` 노드로 첫 번째/마지막 프레임 이미지-비디오 생성(fl2va)을, `MiniMaxH3ReferenceToVideo` 노드로 이미지, 비디오, 오디오 레퍼런스 기반 생성(ref2va)을 수행할 수 있습니다. 이 노드들로 추가 워크플로를 직접 구축할 수도 있습니다.
...
텍스트만이 아니라 컨트롤 비디오로 MiniMax H3 비디오의 모션을 제어합니다. Fun ControlNet Union 패치([alibaba-pai/MiniMax-H3-Fun-Controlnet-Union](https://huggingface.co/alibaba-pai/MiniMax-H3-Fun-Controlnet-Union), [Comfy-Org/MiniMax-H3](https://huggingface.co/Comfy-Org/MiniMax-H3)에 패키징됨)는 단일 체크포인트로, Canny, Depth, HED, MLSD 또는 Pose 컨트롤 비디오를 조건으로 H3에 적용할 수 있으며, 마스크를 사용한 비디오 인페인팅도 지원합니다. `fl2va` 및 `ref2va` transformer 파일 모두에서 작동합니다.
...
다음 파일을 해당 `Load Video` 노드에 업로드하세요:
...
<CardGroup cols={2}>
  <Card title="dancer_field_pose.mp4" icon="video" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/dancer_field_pose.mp4">
    예제 워크플로에서 사용하는 Pose 컨트롤 비디오입니다. 직접 만든 비디오를 사용해도 됩니다.
  </Card>
</CardGroup>

이 템플릿은 ComfyUI 0.35.0 이상이 필요합니다. 예제는 Pose 컨트롤 비디오를 사용합니다. 내장된 SDPose 서브그래프가 입력 비디오에서 포즈 스켈레톤을 추출하여 출력이 댄서의 움직임을 재현합니다. 전처리된 컨트롤 비디오를 `Apply MiniMax H3 Fun ControlNet` 노드의 `control_video` 입력에 직접 연결할 수도 있습니다.
...
<CardGroup cols={2}>
  ...
  <Card title="ControlNet Patch: minimax_h3_fun_controlnet_union_pruned_int8_convrot" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/model_patches/minimax_h3_fun_controlnet_union_pruned_int8_convrot.safetensors">
    <code>ComfyUI/models/model_patches/</code>에 배치
  </Card>
  <Card title="Text Encoder: qwen3vl_32b_minimax_h3_nvfp4_awq" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/text_encoders/qwen3vl_32b_minimax_h3_nvfp4_awq.safetensors">
  ...
  <Card title="LoRA: minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    선택 사항인 Lightning LoRA 터보 모드에 사용합니다. <code>ComfyUI/models/loras/</code>에 배치
  </Card>
  <Card title="Checkpoint: sdpose_wholebody_fp16" icon="download" href="https://huggingface.co/Comfy-Org/SDPose/blob/main/checkpoints/sdpose_wholebody_fp16.safetensors">
    내장 SDPose 서브그래프가 사용하는 포즈 추출기입니다. <code>ComfyUI/models/checkpoints/</code>에 배치
  </Card>
  <Card title="Detector: rt_detr_v4-x-hgnet_fp16" icon="download" href="https://huggingface.co/Comfy-Org/SDPose/blob/main/diffusion_models/rt_detr_v4-x-hgnet_fp16.safetensors">
    내장 SDPose 서브그래프가 사용하는 사람 감지기입니다. <code>ComfyUI/models/diffusion_models/</code>에 배치
  </Card>
</CardGroup>
...
1. **비디오 길이**: 대상 비디오는 5~15초이며 24fps의 17n+5 프레임 그리드에 맞춰집니다(124 프레임 = 5초). 컨트롤 비디오가 대상보다 길면 첫 프레임으로 잘리고, 짧으면 마지막 프레임이 유지됩니다. 출력이 컨트롤 비디오 길이와 정확히 일치하게 하려면 예제에서 계산된 `batch_size` 출력을 `MiniMax H3 Reference to Video` 노드의 length 입력에 연결하세요
2. **가이던스와 강도**: `guidance_scale`은 1.0으로 유지하세요. 출력이 컨트롤에서 벗어나는 경우에만 패치 `strength`를 1 이상으로 올립니다
3. **비디오 인페인팅**: `mask` 입력에 마스크를 연결합니다. `1`로 표시된 영역은 선택적 `source_video` 입력의 제어 아래에서 재생성되고, 프레임의 나머지 부분은 고정됩니다
4. **터보 모드(선택)**: **Lightning LoRA** 체크박스를 활성화하면 4단계 터보 LoRA로 훨씬 빠르게 생성할 수 있으며, 오디오와 모션 품질이 약간 저하됩니다
5. **컨트롤 비디오 준비**: ComfyUI에는 각 컨트롤 유형용 전처리 노드가 내장되어 있습니다. 엣지는 [Detect Edges (Canny) 노드](https://docs.comfy.org/built-in-nodes/Canny)를, 뎁스 맵은 [Depth Anything 3 튜토리얼](https://docs.comfy.org/tutorials/utility/depth-anything-3)을 참조하세요
