...
템플릿은 빠른 미리보기 크기로 제공됩니다. 전체 품질로 출력하려면 16:9에서 메가픽셀을 약 `1.0`으로 올리면 1376x768이 됩니다. 이는 H3의 네이티브 캔버스(단변 768px)입니다.
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA (선택): minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors">
    선택적 Lightning LoRA 터보 모드에서만 필요합니다. 다음 위치에 배치: <code>ComfyUI/models/loras/</code>
  </Card>
</CardGroup>
...
3. **해상도**: H3의 기본 캔버스는 768px 단변, 해상도는 32의 배수로 반올림됩니다.
...
6. **터보 모드(선택)**: 이 워크플로에는 더 빠른 생성을 위한 Lightning LoRA(`minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16`)가 포함되어 있습니다. MiniMax H3 노드에서 `turbo_mode`를 활성화하면 20단계 대신 8단계로 생성되며, 품질이 약간 저하됩니다.
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA (선택): minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    선택적 Lightning LoRA 터보 모드에서만 필요합니다. 다음 위치에 배치: <code>ComfyUI/models/loras/</code>
  </Card>
</CardGroup>
...
6. **터보 모드(선택)**: 워크플로의 **Lightning LoRA** 체크박스를 활성화하면 `ref2va` 모델을 전체 20단계 대신 4단계 터보 LoRA(`minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16`)로 실행할 수 있습니다. 훨씬 빠르며 품질 저하는 미미합니다.
