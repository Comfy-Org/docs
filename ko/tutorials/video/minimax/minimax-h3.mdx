...
예제 템플릿 외에도 네이티브 MiniMax H3 노드는 임의 프레임에 가이드를 고정하는 기능, 프롬프트 임베딩, 잠재 노이즈 마스크를 지원합니다. 이러한 기능은 최근 ComfyUI 업데이트에서 추가되었습니다: [임의 프레임 가이드 고정](https://github.com/Comfy-Org/ComfyUI/pull/15439), [프롬프트 임베딩](https://github.com/Comfy-Org/ComfyUI/pull/15697), [잠재 노이즈 마스크](https://github.com/Comfy-Org/ComfyUI/pull/15375). 사용하려면 ComfyUI를 최신 버전으로 업데이트하세요.

### 임의 프레임에 가이드 고정

예제 템플릿은 비디오의 첫 프레임과 마지막 프레임에만 키프레임을 고정합니다. `MiniMaxH3AddGuide` 노드는 이 제한을 제거합니다. 연속적인 시간 축의 임의 프레임에 가이드를 고정할 수 있습니다.
...
MiniMax H3는 프롬프트에서 ComfyUI 표준 `embedding:` 구문을 지원합니다. 임베딩 파일을 `ComfyUI/models/embeddings/`에 넣고 프롬프트에서 이름으로 참조하세요(예: `embedding:my_embedding`). 임베딩은 다른 ComfyUI 모델과 마찬가지로 로드되어 텍스트 conditioning에 혼합됩니다.

### 잠재 노이즈 마스크로 인페인팅과 확장

MiniMax H3는 비디오와 오디오 잠재 표현 모두에서 노이즈 마스크를 지원합니다. 평소처럼 마스크를 샘플러의 `denoise_mask` 입력에 연결하세요: `0`은 해당 잠재 영역을 보존하고, `1`은 다시 생성합니다. 비디오 마스크는 모델의 2x2 잠재 패치 그리드에 맞춰지고, 오디오 마스크는 전체 잠재 프레임에 맞춰집니다. 부분 인페인팅, 객체 제거, 또는 기존 콘텐츠를 안정적으로 유지하면서 클립을 확장할 때 사용하세요.
