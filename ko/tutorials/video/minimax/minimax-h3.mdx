...
템플릿은 빠른 미리보기 크기로 제공됩니다. 전체 품질로 출력하려면 16:9에서 Resolution Selector의 메가픽셀을 `0.98`로 설정하세요. 이는 H3의 네이티브 캔버스(단변 768px, 16:9에서 1344x768)입니다. 또는 MiniMax H3 노드의 `width`와 `height`에 직접 `1344 x 768`을 입력하세요(기본값). 한 단계 위인 `1.0` 메가픽셀은 1376x768로, 16:9보다 약간 넓습니다.
