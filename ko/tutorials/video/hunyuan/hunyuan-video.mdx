...
Hunyuan 이미지 기반 비디오 생성(Image-to-Video) 모델은 2025년 3월 6일에 오픈소스로 공개되었으며, HunyuanVideo 프레임워크를 기반으로 합니다. 정적 이미지를 부드러운 고품질 비디오로 변환하며, 머리카락 성장, 객체 변환 등 특수 비디오 효과를 사용자 정의할 수 있는 LoRA 학습 코드도 제공합니다.

현재 Hunyuan 이미지 기반 비디오 생성 모델은 두 가지 버전이 있습니다:
- v1 "concat": 움직임 유동성은 더 좋지만 이미지 가이드 대비 충실도가 낮음
- v2 "replace": v1 발표 다음 날 업데이트 되었으며, 이미지 가이드에 더 잘 따르지만 v1에 비해 덜 역동적으로 보임
...
<Card icon="cube" title="CLIP Vision: llava_llama3_vision.safetensors" href="https://huggingface.co/Comfy-Org/HunyuanVideo_repackaged/blob/main/split_files/clip_vision/llava_llama3_vision.safetensors?download=true">
  ComfyUI/models/clip_vision에 저장
</Card>### V1 "concat" 이미지 기반 비디오 생성 워크플로
...
<CardGroup cols={2}>
  <Card title="Comfy Cloud에서 실행" icon="cloud" href="https://cloud.comfy.org/?utm_source=docs&utm_medium=referral&utm_campaign=hunyuan-video">
    Comfy Cloud에서 열기
  </Card>
...
<Card icon="cube" title="Diffusion Model: hunyuan_video_image_to_video_720p_bf16.safetensors" href="https://huggingface.co/Comfy-Org/HunyuanVideo_repackaged/blob/main/split_files/diffusion_models/hunyuan_video_image_to_video_720p_bf16.safetensors?download=true">
  ComfyUI/models/diffusion_models 폴더에 저장
</Card>

다음 모든 모델 파일이 올바른 위치에 있는지 확인하세요.
...
1. `DualCLIPLoader`(이중 CLIP 로드)가 다음 모델을 로드했는지 확인합니다.
   - clip_name1: clip_l.safetensors
   - clip_name2: llava_llama3_fp8_scaled.safetensors
2. `Load CLIP Vision`(CLIP_VISION 로드)이 `llava_llama3_vision.safetensors`를 로드했는지 확인합니다.
3. `Load Image Model`(이미지 모델 로드)이 `hunyuan_video_image_to_video_720p_bf16.safetensors`를 로드했는지 확인합니다.
4. `Load VAE`(VAE 로드)가 `vae_name: hunyuan_video_vae_bf16.safetensors`를 로드했는지 확인합니다.
5. `Load Diffusion Model`(확산 모델 로드)이 `hunyuan_video_image_to_video_720p_bf16.safetensors`를 로드했는지 확인합니다.
6. `Queue`(실행 대기열) 버튼을 클릭하거나 단축키 `Ctrl(Cmd) + Enter`를 눌러 워크플로를 실행합니다.### v2 '대체' 이미지 기반 비디오 생성 워크플로

v2 워크플로는 v1 워크플로와 본질적으로 동일합니다. **대체** 모델을 다운로드하여 `Load Diffusion Model` 노드에서 사용하기만 하면 됩니다.

#### 1. 워크플로 및 자산

아래 워크플로 이미지를 다운로드하여 ComfyUI로 끌어다 놓으면 워크플로가 로드됩니다:
...
<CardGroup cols={2}>
  ...
  <Card title="워크플로 다운로드" icon="download" href="https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/hunyuan-video/i2v/v2_fennec_gril.webp">
    워크플로 이미지를 다운로드하여 ComfyUI로 끌어다 놓으세요
  </Card>
</CardGroup>
...
│   │   └── hunyuan_video_vae_bf16.safetensors                             // 공유 모델
│   └── diffusion_models/
│       └── hunyuan_video_v2_replace_image_to_video_720p_bf16.safetensors  // V2 '대체' 버전 모델
```

#### 3. 워크플로 실행 단계

![ComfyUI Hunyuan Video I2V v2 워크플로](/images/tutorial/advanced/hunyuanvideo/flow_diagram_i2v_v2.jpg)
