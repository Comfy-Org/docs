...
translationSourceHash: b75715b3
...
## PixelDiT 텍스트 기반 이미지 생성 워크플로

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/image_pixeldit_t2i-1.webp" alt="PixelDiT 텍스트 기반 이미지 생성 워크플로" />

<CardGroup cols={2}>
  <Card title="워크플로 다운로드" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/image_pixeldit_t2i.json">
    JSON을 다운로드하거나 템플릿 라이브러리에서 "PixelDiT"를 검색하세요
  </Card>
  <Card title="Comfy Cloud에서 실행" icon="cloud" href="https://cloud.comfy.org/?template=image_pixeldit_t2i&utm_source=docs&utm_medium=referral&utm_campaign=pixeldit">
    클라우드에서 열기
  </Card>
</CardGroup>
...
워크플로는 세 가지 주요 노드로 구성됩니다:

1. **ResolutionSelector**: 원하는 출력 해상도 선택
2. **텍스트 기반 이미지 생성 (PixelDiT) 서브그래프**: 프롬프트, 시드, 모델 선택 및 해상도를 조정할 수 있는 핵심 생성 노드
3. **SaveImage**: 생성된 이미지 저장

<Card title="서브그래프 알아보기" icon="book-open" href="/ko/interface/features/subgraph">
  이 워크플로는 모듈식 처리를 위해 서브그래프 노드를 사용합니다. 서브그래프 문서를 확인하여 워크플로를 사용자 정의하고 확장하는 방법을 배워보세요.
</Card>
### 워크플로 컨트롤
...
<CardGroup cols={2}>
<Card title="텍스트 인코더" icon="download" href="https://huggingface.co/Comfy-Org/PixelDiT/blob/main/text_encoders/gemma_2_2b_it_elm_bf16.safetensors">
    gemma_2_2b_it_elm_bf16.safetensors — Gemma-2-2B-IT 텍스트 인코더
  </Card>
<Card title="확산 모델" icon="download" href="https://huggingface.co/Comfy-Org/PixelDiT/blob/main/diffusion_models/pixeldit_1300m_1024px_bf16.safetensors">
