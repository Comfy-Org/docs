...
> [comfyui_controlnet_aux](https://github.com/Fannovel16/comfyui_controlnet_aux)와 같은 맞춤형 노드를 사용해 깊이맵을 생성할 수도 있습니다.
...
인페인트 모델의 경우, [Mask Editor](/interface/maskeditor)를 사용해 마스크를 그린 후 이를 입력 제어 조건으로 사용해야 합니다.

1. `ModelPatchLoader`가 `qwen_image_inpaint_diffsynth_controlnet.safetensors` 모델을 로드했는지 확인하세요.
2. 이미지를 업로드하고 [Mask Editor](/interface/maskeditor)를 사용해 마스크를 그립니다. 해당 `Load Image` 노드의 `mask` 출력을 `QwenImageDiffsynthControlnet`의 `mask` 입력에 연결해 정확한 마스크가 로드되도록 해야 합니다.
