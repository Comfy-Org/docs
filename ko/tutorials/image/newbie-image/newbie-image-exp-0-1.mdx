...
NewBie Exp0.1의 Next-DiT 아키텍처로 상세한 애니메이션 스타일 이미지를 생성합니다. XML 구조화된 프롬프트를 지원하여 더 나은 다중 캐릭터 장면과 속성 바인딩을 제공합니다.

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/image_newbieimage_exp0_1-t2i-1.webp" alt="NewBie-image 텍스트 기반 이미지 생성 워크플로 미리보기" />
...
<CardGroup cols={2}>
  ...
  <Card title="워크플로 다운로드" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/image_newbieimage_exp0_1-t2i.json">
    JSON 다운로드 또는 템플릿 라이브러리에서 &quot;NewBie-image&quot; 검색
  </Card>
</CardGroup>
...
NewBie-image는 캐릭터 생성에 최적화된 애니메이션 이미지 생성 모델입니다. 학습을 위해 XML 구조화 프롬프트를 사용하며, 각 `<>` 태그는 `<appearance>`, `<clothing>`과 같은 카테고리를 정의하고 `</>`로 닫습니다. 내부의 태그는 표준 Danbooru 태그입니다. 이 구조는 다중 캐릭터 장면에서 더 나은 속성 바인딩을 통해 정밀한 제어를 가능하게 합니다.
...
다중 캐릭터 장면의 경우 XML 구조화 프롬프트를 사용하면 일반적으로 더 나은 어텐션 바인딩과 속성 분리를 통해 더 정확한 이미지 생성 결과를 얻을 수 있습니다.
