...
텍스트 프롬프트와 선택적 참조 이미지를 입력하세요. 텍스트 기반 이미지 생성, 이미지 편집, 주제 기반 개인화를 지원하는 고해상도 이미지(최대 2048x2048)를 생성할 수 있습니다.
...
<CardGroup cols={2}>
  ...
  <Card title="워크플로우 다운로드" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/image_hidream_o1_dev.json">
    JSON을 다운로드하거나 템플릿 라이브러리에서 &quot;HiDream O1 Dev&quot;를 검색하세요.
  </Card>
</CardGroup>
...
**체크포인트(Dev)**: 재패키징 및 양자화되었습니다. 모든 모델은 최악의 이상치에 대해 bf16을 사용하며, 사용되지 않는 딥스택 레이어가 제거되었습니다:

- [hidream_o1_image_dev_fp8_scaled.safetensors](https://huggingface.co/Comfy-Org/HiDream-O1-Image/blob/main/checkpoints/hidream_o1_image_dev_fp8_scaled.safetensors): FP8, 지원되는 하드웨어에서 속도 향상을 위해 안전한 MLP 레이어에서 fp8/mxfp8 행렬 곱셈을 사용합니다.
...
- [hidream_o1_image_dev_bf16.safetensors](https://huggingface.co/Comfy-Org/HiDream-O1-Image/blob/main/checkpoints/hidream_o1_image_dev_bf16.safetensors): 전체 bf16 정밀도(가장 큰)
