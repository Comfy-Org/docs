...
ComfyUI를 최신 버전으로 업데이트한 다음 `워크플로` -> `템플릿 탐색`으로 이동하여 유틸리티 카테고리에서 SDPose 워크플로를 찾으세요.### SDPose: 이미지 다중 인물 감지 (`utility_sdpose_multi_person`)
...
</div>### SDPose: 비디오 다중 인물 감지 (`utility_sdpose_multi_person_video`)
...
</div>### SDPose-OOD: 이미지에서 포즈 맵으로 (`utility_sdpose_ood_image_to_pose`)
...
</div>### SDPose-OOD: 비디오를 포즈 맵으로 (`utility_sdpose_ood_video_to_pose_map`)
