...
어텐션 백엔드는 양자화된 체크포인트에서 검정색이거나 손상된 출력을 조용히 반환할 수 있습니다. SeedVR2 INT8 워크플로가 완전히 검정색인 이미지를 생성한다면 다음을 순서대로 시도해 보세요:

1. **FlashAttention 없이 실행:** `--use-flash-attention` 또는 커스텀 노드 팩을 통해 이를 활성화했다면, 기본 어텐션 백엔드로 ComfyUI를 시작하고 다시 실행하세요. FlashAttention을 비활성화하면 RTX 3090에서 3B 및 7B INT8 워크플로의 완전히 검정색인 출력이 해결되었습니다.
2. **체크포인트 전환:** 동일한 크기의 FP16 또는 FP8 파일을 사용하세요. 이것들이 올바르게 렌더링된다면, INT8 양자화 경로에서 문제가 발생하는 것입니다.
3. **파일 확인:** diffusion 모델과 `seedvr2_ema_vae_fp16.safetensors`가 모두 완전히 다운로드되어 모델 설치에 표시된 대로 배치되어야 합니다. 다운로드가 중단되면 같은 방식으로 실패합니다.
4. **실시간 미리보기 비활성화:** 어떤 경우에는 미리보기만 검정색이고 저장된 파일은 올바르므로, 실시간 미리보기를 끄면 모호함이 사라집니다.

결과가 여전히 검정색이라면, GPU, ComfyUI 버전, 설치 방법(데스크톱, 포터블 또는 수동) 및 콘솔 출력과 함께 3B 및 7B 워크플로가 동일하게 실패하는지 여부를 튜토리얼 토론에 게시하세요.
