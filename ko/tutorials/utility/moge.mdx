...
>
> MoGe-3는 MoGe-2 아키텍처를 유지하면서 자기 유도 방식의 희소 복셀 정밀화 단계를 추가해 세밀한 점 지도 기하를 크게 개선합니다. ViT-L 체크포인트(`moge_3_vitl_fp16`)와 더 큰 ViT-g 체크포인트(`moge_3_vitg_fp16`)로 제공됩니다.
...
- **MoGe-2 (권장)**: [moge_2_vitl_normal_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_2_vitl_normal_fp16.safetensors)
- **MoGe-1 (기준)**: [moge_1_vitl_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_1_vitl_fp16.safetensors)
- **MoGe-3 (최고 디테일)**: [moge_3_vitg_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_3_vitg_fp16.safetensors) (1.25B) 또는 [moge_3_vitl_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_3_vitl_fp16.safetensors) (370M)
...
MoGe-3 체크포인트 전용: `Run MoGe Inference` 및 `Run MoGe Panorama Inference` 노드에는 예측 깊이에 대해 희소 복셀 정밀화를 수행하는 고급 설정 `refine_steps`(0~8, 기본값 3)가 있습니다. `0`으로 설정하면 정밀화가 비활성화되고, 횟수를 늘릴수록 거의 선형적인 비용으로 세부 묘사와 가장자리가 선명해집니다. MoGe-1 및 MoGe-2 체크포인트에서는 이 설정이 무시됩니다.
...
### MoGe 3: 기하 추정 {#utility_moge3_geometry_estimation}

MoGe 3로 단일 이미지에서 세밀한 기하 구조를 추정합니다. MoGe-3 체크포인트는 거친 포인트 맵을 희소 복셀 셸로 끌어올리고 경량 희소 3D 합성곱으로 깊이를 정제하므로, 이미지 공간 디코더가 흐릿하게 만드는 얇은 구조와 깊이 불연속을 유지합니다.

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/utility_moge3_geometry_estimation-1.webp" alt="MoGe 3 기하 추정 워크플로 미리보기" />

<CardGroup cols={2}>
  <Card title="Comfy Cloud에서 실행" icon="cloud" href="https://cloud.comfy.org/?template=utility_moge3_geometry_estimation&utm_source=docs&utm_medium=referral&utm_campaign=moge">
    Comfy Cloud에서 열기
  </Card>
  <Card title="워크플로 다운로드" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_moge3_geometry_estimation.json">
    JSON 다운로드 또는 템플릿 라이브러리에서 &quot;Geometry Estimation: MoGe 3&quot; 검색
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="cosmic_suitcase.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/cosmic_suitcase.png">
    `LoadImage` 노드 3 · `cosmic_suitcase.png`
  </Card>
</CardGroup>

### 4.1 실행 단계

1. `Geometry Estimation (MoGe 3)` 서브그래프에 MoGe 3 체크포인트가 선택되어 있는지 확인합니다: 템플릿은 `moge_3_vitg_fp16.safetensors`를 사용하며, 같은 드롭다운에서 `moge_3_vitl_fp16.safetensors`도 선택할 수 있습니다
2. `Load Image` 노드에 이미지를 불러옵니다
3. (선택 사항) 서브그래프 노드에서 `output`을 변경하면 다른 맵을 렌더링할 수 있습니다: 기본값은 `mask`이며, `depth`, `depth_colored`, `normal_opengl`, `normal_directx`도 사용할 수 있습니다
4. `Queue`를 클릭하거나 `Ctrl(cmd) + Enter`를 눌러 실행합니다.
