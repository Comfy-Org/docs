...
MCP 설정, 설치, 도구 목록 및 인증 방법은 [Comfy MCP](/ko/agent-tools/mcp)를 참조하세요.
...
CI 또는 스크립트에서 비대화형 설치를 원하면 `-y`를 전달하세요. 전체 `comfy setup` 플래그 표는 [Comfy CLI 시작하기](/ko/comfy-cli/getting-started#빠른-설정-권장)를 참조하세요.
...
| [Comfy MCP](/ko/agent-tools/mcp) | MCP를 통해 에이전트를 연결하여 채팅 기반 생성 |
