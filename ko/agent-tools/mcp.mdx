...
<AccordionGroup>
  <Accordion title="지원되는 클라이언트는 무엇인가요?">
    MCP와 호환되는 모든 클라이언트를 지원합니다.

    **클라우드 연결**에는 원격 HTTP 지원이 필요합니다. **Claude Code**, **Claude Desktop**, **Cursor**, **Codex**, **OpenClaw**는 위에서 안내된 일급(first-class) 설정을 제공하며, **Windsurf**, **Amp** 및 기타 도구는 OAuth 또는 API 키와 함께 동일한 URL을 사용합니다.

    **로컬 연결**은 로컬 stdio 서버를 하위 프로세스로 실행할 수 있는 클라이언트가 필요합니다. 따라서 브라우저 기반 클라이언트는 제외됩니다. [claude.ai](https://claude.ai)와 ChatGPT는 원격 커넥터만 지원합니다.
  </Accordion>
  ...
  <Accordion title="로컬 ComfyUI와 함께 사용할 수 있나요?">
    네. [로컬 Comfy MCP 연결](#local-comfy-mcp-connection)을 사용하면 됩니다. 사용자 컴퓨터에 설치된 ComfyUI를 구동하므로 에이전트가 실제 보유한 모델, LoRA, 커스텀 노드를 확인할 수 있으며, 사용자의 GPU에서 실행됩니다.
  </Accordion>
  <Accordion title="클라우드와 로컬 연결을 동시에 사용할 수 있나요?">
    네, 로컬에서 ComfyUI를 실행한다면 두 연결을 모두 사용하는 것을 권장합니다. 대부분의 클라이언트는 두 개의 MCP 서버를 문제없이 호스팅하며, 에이전트가 두 연결을 명확히 구분합니다. 각 연결은 자체 워크플로를 실행하고 자체 결과를 반환합니다.

    단, 두 로그인은 **별개**입니다. 동일한 Comfy 계정이더라도 한쪽에 로그인해도 다른 쪽에는 로그인되지 않습니다.
  </Accordion>
  <Accordion title="내 컴퓨터가 로컬 연결을 실행할 수 있는지 어떻게 알 수 있나요?">
    에이전트에게 물어보세요. 에이전트는 무거운 작업을 시작하기 전에 하드웨어를 확인합니다.

    **Mac**에서는 생성 작업에 클라우드 연결을 사용하세요. 현재의 오픈 웨이트 모델은 Apple GPU에서 실용적인 속도로 실행하기에는 너무 큽니다. **전용 그래픽 카드가 있는 PC**의 경우 VRAM이 24GB 이상이면 비디오를 포함한 대부분의 작업을 처리할 수 있습니다. 8~24GB는 이미지에는 충분하지만 비디오는 느리거나 메모리에 맞지 않을 수 있습니다. 8GB 미만이면 클라우드를 사용하세요.
  </Accordion>
  <Accordion title="정식 출시되었나요?">
    클라우드 연결은 **공개 베타** 상태입니다. 서비스를 개선하는 동안 API, 도구 및 동작이 변경될 수 있습니다. 로컬 연결은 로컬 ComfyUI 설치 환경에서 사용할 수 있습니다. 문제를 신고하려면 [피드백](#feedback)을 참조하세요.
  </Accordion>
</AccordionGroup>### 비용 및 액세스

<AccordionGroup>
  <Accordion title="비용이 발생하나요?">
    두 연결 모두에서 검색은 무료입니다. 템플릿, 모델, 노드를 검색하려면 Comfy 계정만 있으면 됩니다.

    **클라우드 연결**에서는 이미지 생성 실행에 활성 Comfy Cloud 구독이 필요하며, 신규 사용자에게는 무료 실행 5회가 제공됩니다. **로컬 연결**에서는 자체 하드웨어에서 실행되므로 실행이 무료입니다. 단, 한 가지 예외가 있습니다. 파트너 모델은 파트너 인프라에서 실행되며 크레딧을 사용합니다.
  </Accordion>
  <Accordion title="API 키가 필요한가요?">
    OAuth를 지원하는 대화형 클라이언트(Claude Code, Claude Desktop, Codex, OpenClaw 포함)에는 API 키가 필요하지 않습니다.

    **Cursor**는 MCP 구성에 Comfy Cloud API 키가 필요합니다. 아직 MCP OAuth가 없습니다. 브라우저가 없는 헤드리스 및 CI 환경에서도 API 키가 필요합니다. [클라우드 연결 설정](#set-up-the-cloud-connection) 아래의 **Cursor** 및 **기타 클라이언트** 탭을 참조하세요.
  </Accordion>
</AccordionGroup>### 사용 방법

<AccordionGroup>
  <Accordion title="연결되면 에이전트는 무엇을 할 수 있나요?">
    MCP 도구를 직접 호출하지 않습니다. 에이전트가 사용자의 요청에 따라 도구를 선택합니다. 일반적으로 사용 가능한 항목을 **탐색**하고(`search_templates`, `search_models`, `search_nodes`), 생성을 **실행**한 다음, 출력을 **대기하고 가져옵니다**. [에이전트가 수행할 수 있는 작업](#what-your-agent-can-do)을 참조하세요.
  </Accordion>
  <Accordion title="내 출력은 어디에 저장되나요?">
    **클라우드 연결**에서 서버는 사용자의 컴퓨터에 파일을 기록하지 않습니다. `get_output`은 임시 서명 URL과 에이전트가 셸에서 실행할 수 있는 즉시 실행 가능한 다운로드 명령을 반환합니다. [업로드 및 다운로드](#uploads-and-downloads)를 참조하세요.

    **로컬 연결**에서 ComfyUI는 워크스페이스의 `output/` 디렉터리에 파일을 기록하며, `fetch_outputs(prompt_id, out_dir)`는 완료된 작업의 파일을 사용자가 지정한 위치에 복사합니다.
  </Accordion>
  <Accordion title="한 연결로 시작했는데 이제 다른 연결이 필요합니다. 어떻게 해야 하나요?">
    실행 취소할 작업은 없습니다. 첫 번째 연결과 함께 두 번째 연결을 추가하면 됩니다.

    **로컬 → 클라우드** 전환(Cloud GPU 또는 파트너 모델이 필요한 경우): 에이전트에게 로그인 처리를 요청한 다음, 클라이언트에 `https://cloud.comfy.org/mcp`를 추가하세요.

    **클라우드 → 로컬** 전환(자체 모델과 커스텀 노드를 사용하려는 경우): ComfyUI와 로컬 서버를 설치한 다음, 클라이언트가 해당 서버를 가리키도록 설정하세요. 에이전트가 대부분의 작업을 대신 처리할 수 있습니다.
  </Accordion>
  <Accordion title="로컬 연결과 클라우드 연결 사이를 어떻게 전환하나요?">
    에이전트에게 요청하기만 하면 됩니다. 두 연결을 모두 추가한 상태에서 작업을 실행할 위치를 말하세요. "이 작업은 Comfy Cloud에서 실행해 줘", "이건 로컬에서 실행해 줘"라고 말하면 에이전트가 올바른 연결을 사용합니다. 전환할 모드도 없고 실행 간에 재구성할 항목도 없습니다.

    워크플로가 사용자의 컴퓨터에서 실행하기에 너무 무거운 경우, 에이전트가 이를 알리고 Comfy Cloud에서 대신 실행하도록 제안할 수 있습니다. 연결이 하나만 설정된 경우, 다른 연결을 추가하도록 요청하세요. [클라우드 연결 설정](#set-up-the-cloud-connection) 또는 [로컬 Comfy MCP 연결](#local-comfy-mcp-connection)을 참조하세요.
  </Accordion>
  <Accordion title="Comfy MCP를 어떻게 업데이트하나요?">
    **클라우드 연결**에서는 할 일이 없습니다. 호스팅 방식이므로 항상 최신 버전을 사용하게 됩니다.

    **로컬 연결**에서는 에이전트에게 처리를 요청하세요. 완료된 후에는 **클라이언트를 재시작**하거나 새 세션을 시작하세요. MCP 서버는 세션이 시작될 때 로드되므로, 재시작할 때까지 실행 중인 서버는 이전 버전을 계속 제공합니다.
  </Accordion>
</AccordionGroup>### 문제 해결

<AccordionGroup>
  <Accordion title="슬래시 명령이 Claude Desktop에서 작동하나요?">
    아니요. 슬래시 명령은 Claude Code 플러그인에 포함되어 있습니다. Claude Desktop은 동일한 MCP 서버에 연결됩니다. 일반 언어로 요청하거나 프롬프트 선택기를 사용하면 도구가 작동하지만, Claude Code 플러그인이나 슬래시 명령은 지원하지 않습니다.
  </Accordion>
  <Accordion title="/comfy 또는 /cloud를 입력했는데 아무것도 표시되지 않습니다.">
    `/comfy` 또는 `/cloud` 명령은 없습니다. 명령은 연결 방법에 따라 다음 두 접두사 중 하나로 표시됩니다.

    - **플러그인(권장):** `/comfy-cloud:generate-image`, `/comfy-cloud:generate-video`, … 전체 목록을 보려면 `/comfy-cloud:`를 입력하세요.
    - **직접 연결(플러그인 없음):** `/mcp__comfy-cloud__generate-image`, … 이 명령들을 보려면 `/mcp__`를 입력하세요.

    어느 쪽이든 일반 언어로 그냥 요청하면 됩니다("…의 이미지를 생성해 주세요"). MCP 도구는 모델이 호출하며 슬래시 명령이 필요하지 않습니다.
  </Accordion>
  <Accordion title="로그인 시 브라우저가 열리지 않았습니다.">
    Claude Code에서 `/mcp`를 실행하고 **comfy-cloud**를 선택한 다음 **인증**을 선택하세요. Claude Desktop에서는 **사용자 정의 → 커넥터**에서 커넥터를 다시 열고 로그인을 시작하세요.
  </Accordion>
</AccordionGroup>
