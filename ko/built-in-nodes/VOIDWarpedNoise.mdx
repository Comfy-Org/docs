...
description: "VOID 비디오 개선 프로세스의 두 번째 패스(pass)를 위한 시간적 상관관계가 있는 노이즈를 생성합니다. 패스 1의 출력 비디오를 가져와 광학 흐름 벡터를 따라 가우시안 노이즈를 워핑(warping)하여 비디오 콘텐츠와 일관되게 움직이는 노이즈를 만듭니다. 이 워핑된 노이즈는 패스 2의 시작 잠재 변수(latent)로"
