...
description: "CLIPMergeAdd 노드는 두 개의 CLIP 모델을 결합하여 첫 번째 모델에 두 번째 모델의 패치를 추가합니다. 첫 번째 CLIP 모델의 복사본을 생성하고, 위치 ID 및 로짓 스케일 매개변수를 제외한 두 번째 모델의 주요 키 패치를 선택적으로 통합합니다. 이를 통해 기본 모델의 구조를 유지하면서 CLIP 모델 구성"
