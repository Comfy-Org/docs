...
description: "Kling 텍스트-비디오 카메라 제어 노드는 텍스트를 실제 영화 촬영 기법을 시뮬레이션하는 전문적인 카메라 움직임이 있는 시네마틱 비디오로 변환합니다. 이 노드는 줌, 회전, 패닝, 틸트 및 1인칭 시점을 포함한 가상 카메라 동작을 제어하면서 원본 텍스트에 대한 초점을 유지합니다. 지속 시간, 모드 및 모델 이름은 하드코"
