...
description: "UNetSelfAttentionMultiply 노드는 UNet 모델의 셀프 어텐션 메커니즘에서 쿼리(query), 키(key), 값(value) 및 출력(output) 구성 요소에 곱셈 계수를 적용합니다. 이를 통해 어텐션 계산의 여러 부분을 조정하여 어텐션 가중치가 모델 동작에 미치는 영향을 실험할 수 있습니다."
