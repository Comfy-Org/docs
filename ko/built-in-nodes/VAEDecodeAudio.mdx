...
description: "VAEDecodeAudio 노드는 변이형 오토인코더(Variational Autoencoder)를 사용하여 잠재 표현을 다시 오디오 파형으로 변환합니다. 인코딩된 오디오 샘플을 입력받아 VAE를 통해 처리하여 원본 오디오를 재구성하며, 일관된 출력 레벨을 보장하기 위해 정규화를 적용합니다. 결과 오디오는 표준 샘플 레이트"
