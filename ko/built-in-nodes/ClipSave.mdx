...
description: "CLIPSave 노드는 CLIP 텍스트 인코더 모델을 SafeTensors 형식으로 디스크에 저장합니다. 고급 모델 병합 워크플로우를 위해 설계되었으며, 모델의 내부 구조에 따라 CLIP 모델을 구성 요소(예: CLIP-L, CLIP-G 또는 T5XXL)로 자동 분리하여 각 구성 요소를 별도의 파일로 저장합니다."
