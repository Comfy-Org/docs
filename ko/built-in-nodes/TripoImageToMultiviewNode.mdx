...
|-----------|-------------|-----------|----------|-------|
| `이미지` | Tripo가 정면, 좌측, 후면 및 우측 뷰를 생성하는 데 사용하는 피사체의 원본 이미지입니다. 요청에는 정확히 하나의 이미지가 사용됩니다. | IMAGE | 예 | 단일 이미지 |
...
| 출력 이름 | 설명 | 데이터 타입 |
|-------------|-------------|-----------|
| `다중 뷰 task_id` | Tripo가 멀티뷰 이미지 생성 요청에 대해 반환한 작업 식별자입니다. 완료된 작업을 참조하는 데 사용할 수 있는 문자열 식별자입니다. | MULTIVIEW_TASK_ID |
| `앞쪽` | 생성된 피사체의 정면 뷰입니다. | IMAGE |
| `왼쪽` | 생성된 피사체의 좌측 뷰입니다. | IMAGE |
| `뒤쪽` | 생성된 피사체의 후면 뷰입니다. | IMAGE |
| `오른쪽` | 생성된 피사체의 우측 뷰입니다. | IMAGE |
