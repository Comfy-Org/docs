...
포터블 버전에는 독립적인 Python 환경(`python_embeded`)이 내장되어 있습니다. 추가 Python 패키지(어텐션 최적화, 커스텀 노드 의존성, 기타 도구 등)를 설치하려면 명령줄에서 내장 Python의 pip를 사용하세요:

```bash
# 먼저 ComfyUI 포터블 폴더로 이동
cd ComfyUI_windows_portable

# 내장 Python을 사용하여 패키지 설치
.\python_embeded\python.exe -m pip install <패키지명>
```

시작 플래그가 필요한 패키지를 설치한 후 `.bat` 파일(예: `run_nvidia_gpu.bat`)에 해당 플래그를 추가하세요:

```bat
.\python_embeded\python.exe -s ComfyUI\main.py --<시작-플래그> --windows-standalone-build
```

커스텀 노드 의존성 설치에 대한 자세한 내용은 [커스텀 노드 의존성 가이드](/ko/development/core-concepts/custom-nodes)를 참조하세요.
