...
<Tab title="데스크탑">
Comfy 데스크탑은 Comfy Engine을 패키지 매니저로 사용하여 ComfyUI를 관리합니다. ComfyUI 엔진을 업데이트하려면 [관리 패널](/ko/installation/desktop/usage/manage#업데이트-탭)의 업데이트 탭을 열고 **안정판**(기본값) 또는 **GitHub 최신판** 업데이트 채널을 선택할 수 있습니다.

데스크탑 앱 자체의 업데이트는 자동으로 이루어지며, ComfyUI 엔진 업데이트와는 별개입니다.
</Tab>
<Tab title="수동 설치">
...
또한 업데이트 과정에서 해당 의존성들이 성공적으로 업데이트되었는지 확인하세요. 업데이트 후에도 문제가 지속된다면 [의존성 업데이트 문제 해결](#핵심-의존성-업데이트-문제-해결) 섹션을 참고해 문제를 진단하세요.
