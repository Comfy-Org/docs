...
<Tip>Python 모듈은 `__init__.py` 파일을 포함한 디렉터리입니다. 
...
<Warning>이전 버전의 Comfy에서는 `__init__.py`를 통해 자바스크립트 파일을 메인 Comfy 웹 하위 디렉터리로 복사해야 했습니다. 아직도 그런 코드를 볼 수 있을 것입니다. 그러지 마세요.</Warning>