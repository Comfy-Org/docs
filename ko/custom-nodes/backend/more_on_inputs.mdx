---
title: "숨겨진 및 유연한 입력"
description: "커스텀 노드를 만들 때 사용하는 ComfyUI의 숨겨진 입력(UNIQUE_ID, PROMPT, EXTRA_PNGINFO, DYNPROMPT)과 유연한 입력. 커스텀 데이터 타입, 와일드카드 입력, 동적으로 생성되는 입력을 포함합니다."
translationSourceHash: aa268adb
translationFrom: custom-nodes/backend/more_on_inputs.mdx
translationBlockHashes:
  "Hidden inputs": 98a727dc
  "Flexible inputs": 8e99af72
---
...
### UNIQUE_ID
`UNIQUE_ID`는 노드의 고유 식별자로, 클라이언트 측 노드의 `id` 속성과 일치합니다. 이는 일반적으로 클라이언트-서버 통신에 사용됩니다(참고: [메시지](/ko/development/comfyui-server/comms_messages#node_id-얻기)).

### PROMPT
`PROMPT`는 클라이언트가 서버로 전송하는 전체 프롬프트입니다. 자세한 설명은 [프롬프트 객체](/ko/custom-nodes/js/javascript_objects_and_hijacking#프롬프트)를 참조하세요.
...
프론트엔드에서는 `*`를 사용해 어떤 소스와도 연결 가능한 입력을 표시할 수 있습니다. 이는 백엔드에서 공식적으로 지원되지 않으므로, `VALIDATE_INPUTS` 함수에 `input_types`라는 매개변수를 받아들여 백엔드의 타입 검증을 건너뛸 수 있습니다. (자세한 내용은 [VALIDATE_INPUTS](./server_overview#validate_inputs) 참조.) 노드가 전달된 데이터를 이해하는 것은 노드의 몫입니다.
