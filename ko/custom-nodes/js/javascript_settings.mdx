---
title: "설정"
description: "app.registerExtension으로 ComfyUI 확장 설정을 추가, 읽기, 쓰기하고 변경에 반응하는 방법. boolean, text, number, slider, combo, color, image 등 모든 설정 타입을 다룹니다."
...
[카테고리를 제공하지 않으면](#카테고리) `id`가 `.`로 분리되어 설정 패널의 위치를 결정합니다.
