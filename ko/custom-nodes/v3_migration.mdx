...
# 이미지를 저장하고 UI 데이터를 반환 (가장 일반적인 패턴)
return io.NodeOutput(
    ui=ui.ImageSaveHelper.get_save_images_ui(
        images=images,
        filename_prefix=filename_prefix,
        cls=cls,  # 메타데이터를 위해 히든 prompt/extra_pnginfo를 전달
    )
)
...
class MyCustomType:
    Type = torch.Tensor  # Python 타입 힌트
...
# 방법 2: Custom 헬퍼 사용
# 편의를 위해 변수에 먼저 저장하지 않고 헬퍼를 바로 사용할 수도 있습니다
...
```python
# 소켓 전용 멀티 타입 입력 (위젯 없음)
io.MultiType.Input("input", types=[io.Image, io.Mask])

# 위젯 폴백이 있는 멀티 타입 입력 (아무것도 연결되지 않으면 String 위젯 표시)
io.MultiType.Input(
...
def define_schema(cls):
    # 템플릿 생성 - 같은 템플릿을 공유하는 모든 입력/출력은 타입이 일치합니다
...
def execute(cls, images: io.Autogrow.Type) -> io.NodeOutput:
    # 'images'는 입력 이름을 값에 매핑하는 딕셔너리입니다
...
def execute(cls, image, resize_type: dict) -> io.NodeOutput:
    # resize_type은 선택된 옵션 키와 그 입력을 담은 딕셔너리입니다
...
async def execute(cls, images, **kwargs) -> io.NodeOutput:
    ...
    for i, image in enumerate(images):
        ...
        await api.execution.set_progress(
            value=i + 1,
            max_value=total,
            preview_image=image,  # 선택 사항: 진행 중 미리보기 표시
        )
    return io.NodeOutput(result)
```
...
class MyExtension(ComfyExtension):
    async def on_load(self) -> None:
        await api.node_replacement.register(io.NodeReplace(
            ...
            old_widget_ids=["param1", "param2"],  # 위치 기반 매핑을 위한 정렬된 위젯 ID
            input_mapping=[
                {"new_id": "image", "old_id": "input_image"},       # 입력 이름 변경
                {"new_id": "method", "set_value": "lanczos"},       # 고정 값 설정
            ],
            output_mapping=[
                {"new_idx": 0, "old_idx": 0},  # 인덱스로 출력 매핑
            ],
        ))
...
# 여러 출력 값 반환 (순서는 schema의 outputs 목록과 일치)
...
class InvertImage(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="MyPack_InvertImage",  # 충돌 방지를 위한 접두사
