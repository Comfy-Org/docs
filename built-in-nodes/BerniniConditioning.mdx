...
The Bernini Conditioning node prepares video and image conditioning data for the Wan2.2-A14B model. It encodes source videos, reference videos, and reference images using the provided VAE, then attaches them to the conditioning data as in-context tokens. The task is automatically inferred from which inputs are connected.
...
| `source_video` | Source video to edit or restyle (v2v, rv2v). Resized to width/height and trimmed to `length` frames. | IMAGE | No | - |
| `reference_video` | Video to insert into the source video (ads2v). Trimmed to `length` frames and resized with preserved aspect ratio with the long edge capped at `ref_max_size`. | IMAGE | No | - |
| `reference_images` | Reference images injected as in-context tokens (r2v, rv2v). Each image is encoded independently at its own native aspect ratio with the long edge capped at `ref_max_size`. Up to 8 reference image inputs can be connected. | IMAGE | No | 0 to 8 inputs |
