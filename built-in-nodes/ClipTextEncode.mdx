...
> **Note:** Unlike code or some third-party prompt nodes, the native `CLIP Text Encode (Prompt)` node **does not** support comment syntax (`//` or `/* */`). Every character you enter will be tokenized and encoded by the CLIP model.
>
> To temporarily disable part of your prompt, consider:
> - Using the **Note** node to store alternative prompt text.
> - Disconnecting (or Muting) the conditioning link.
> - Splitting prompt sections across multiple `CLIPTextEncode` nodes and using `Conditioning (Combine)` to merge selectively.
