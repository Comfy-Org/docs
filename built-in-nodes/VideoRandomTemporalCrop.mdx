...
Randomly crop a continuous range of frames from an input video. The number of frames to keep is set by `length`, and the start position is chosen randomly using `seed`. The node operates lazily, meaning it does not process the entire video until the output is used downstream.
