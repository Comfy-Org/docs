...
description: "This legacy node generates finished 3D models from a text description using Tripo's API."
sidebarTitle: "TripoTextToModelNode"
icon: "circle"
mode: wide
---

This legacy node generates finished 3D models from a text description using Tripo's API. It waits for generation to complete and then returns the model file, optionally with textures and PBR materials. It is marked as deprecated and is kept for older workflows.
...
**Notes:**
- This node is deprecated and marked as a legacy node. It is kept for backward compatibility with older workflows.
...
|-------------|-------------|-----------|
| `model_file` | The generated 3D model filename in the format `<task_id>.<format>`, kept for backward compatibility only. | STRING |
