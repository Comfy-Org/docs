...
description: "This node generates videos with the Kling V3 model."
sidebarTitle: "KlingVideoNode"
icon: "circle"
mode: wide
---

This node generates videos with the Kling V3 model. It supports text-to-video, where a video is created from a text description, and image-to-video, where an existing image is animated. It can also create multi-segment videos using storyboard prompts and optionally generate audio.
...
| `generate_audio` | When enabled, the node generates audio for the video. Note: `"kling-3.0-turbo"` always generates native audio, so the audio toggle is ignored for that model. Default is True. | BOOLEAN | Yes | True<br />False |
