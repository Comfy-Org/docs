...
|-----------|-------------|-----------|----------|-------|
| `model` | The ByteDance model to use for video generation (default: `"seedance-1-0-pro-fast-251015"`). | COMBO | Yes | `"seedance-1-5-pro-251215"`<br />`"seedance-1-0-pro-250528"`<br />`"seedance-1-0-pro-fast-251015"` |
...
| `camera_fixed` | Specifies whether to fix the camera. The platform appends an instruction to fix the camera to your prompt, but does not guarantee the actual effect (default: False). | BOOLEAN | No | `False`<br />`True` |
| `watermark` | Whether to add an "AI generated" watermark to the video (default: False). | BOOLEAN | No | `False`<br />`True` |
| `generate_audio` | This parameter is ignored for any model except `seedance-1-5-pro-251215` (default: False). | BOOLEAN | No | `False`<br />`True` |
