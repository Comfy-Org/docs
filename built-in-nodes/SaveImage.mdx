...
The file name for each saved image is built from `filename_prefix`, an optional `%batch_num%` placeholder that is replaced with the image's position in the batch, and a five-digit counter, for example `ComfyUI_00001_.png`. Images are written with PNG compression level 4.
