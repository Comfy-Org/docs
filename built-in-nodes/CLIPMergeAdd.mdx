...
description: "The CLIPMergeAdd node combines two CLIP models by adding patches from the second model to the first model."
...
Keys ending in `.position_ids` or `.logit_scale` from `clip2` are skipped, so those parameters keep the values from `clip1`.
