...
The audio-video latent is created as an empty pair matching the requested `width`, `height`, and the snapped frame count. The audio portion is sized from the same frame count at 40 audio frames per second.
