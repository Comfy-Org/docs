...
|-----------|-------------|-----------|----------|-------|
| `prompt` | Text prompt for the video generation (default: empty). | STRING | Yes | 1 to 6000 characters |
...
**Note:** The keyframe sequence must contain at least 2 keyframes and at most 64 keyframes. Each keyframe must resolve to a distinct position on the timeline; two keyframes that map to the same output frame are rejected. Keyframe positions are resolved to output frame indices based on the selected duration (120 frames for 5s, 240 frames for 10s). When using seconds mode, keyframe positions must not go beyond the total video length. Keyframes can be connected in any order — their positions on the timeline determine the sequence.
