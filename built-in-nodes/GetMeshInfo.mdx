...
| `info` | A multiline text report with vertex count, face count, and detected attributes (uvs, vertex_colors, normals, tangents, texture, metallic_roughness, normal_map). Texture-like attributes are shown with their height × width dimensions. Large counts are comma-formatted, for example "1,234,567 (1.23M)" or "12,345 (12.3K)". The same text is displayed on the node. | STRING |
