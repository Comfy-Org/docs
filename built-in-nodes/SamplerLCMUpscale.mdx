...
description: "This node provides a specialized sampling method that combines Latent Consistency Model (LCM) sampling with progressive image upscaling."
sidebarTitle: "SamplerLCMUpscale"
icon: "circle"
mode: wide
---

This node provides a specialized sampling method that combines Latent Consistency Model (LCM) sampling with progressive image upscaling. During sampling, the image is upscaled step by step toward a target scale ratio using a chosen interpolation method, allowing higher-resolution results in a single sampling pass. The node outputs a configured sampler object that can be connected to a sampling node.
