...
| --- | --- | --- |
| `tracks` | The concatenated motion tracking data, containing the combined `track_path` and `track_visibility` from the inputs. Returns `tracks_1` unchanged when `tracks_2` is not connected. | TRACKS |
