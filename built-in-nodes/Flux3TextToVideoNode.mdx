...
description: "Generates a video with synchronized audio from a text prompt using FLUX 3."
...
Note: `safety_tolerance` is an advanced input. The `seed` input includes Control After Generate controls in the UI. The displayed price is based on `resolution` and `duration`: HD (720p) is charged at $0.2431 per second and FHD (1080p) at $0.4147 per second. When a fixed duration is chosen, the total estimated cost for the clip is shown; when `duration` is "auto", the per-second rate is shown.
