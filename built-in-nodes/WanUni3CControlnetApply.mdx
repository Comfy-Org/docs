...
This node applies a Uni3C ControlNet to a Wan video diffusion model, using a rendered guidance video to influence the generated video. The guidance video is typically a warped point cloud render of the input image that follows a camera trajectory. The control signal is only active during the portion of the denoising process defined by the start and end percentages.
...
| `model_patch` | A Uni3C ControlNet patch. Must be a Uni3C ControlNet instance; otherwise the node raises an error. | MODEL_PATCH | Yes | – |
...
- The `render_video` input is expected to be in RGB format (only the first 3 channels are used).
