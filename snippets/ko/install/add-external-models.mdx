...
<Tabs>
  <Tab title="휴대용/수동 설치">
  [휴대용](/installation/comfyui_portable_windows) 및 [수동](/installation/manual_install) 버전의 ComfyUI에서는 ComfyUI 루트 디렉터리에 `extra_model_paths.yaml.example`라는 예제 파일을 찾을 수 있습니다:
  ```
  ...
  [ComfyUI 데스크톱](/installation/desktop/windows) 애플리케이션을 사용하는 경우, 아래 이미지를 따라 추가 모델 구성 파일을 열 수 있습니다:
