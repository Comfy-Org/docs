...
`ComfyUI/models/checkpoints` 폴더에 SD1.5 모델 파일이 최소 하나 이상 포함되어 있는지 확인해 주세요. 모델 설치 방법이 확실하지 않은 경우, [ComfyUI의 AI 드로잉 여정 시작하기](/get_started/first_generation#3-설치_드로잉_모델) 섹션을 참조하여 모델 설치 지침을 확인해 주세요.
