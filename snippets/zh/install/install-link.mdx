...
<AccordionGroup>
  <Accordion title="Comfy 桌面版">
    Comfy 桌面版目前支持 **Windows 及 MacOS(Apple Silicon)** 的独立安装，目前仍在 Beta 版本
    ...
    <Tabs>
      <Tab title="Windows">
        <Card title="ComfyUI桌面版(Windows)安装指南" icon="link" href="/zh/installation/desktop/windows">
        适合带有 **Nvdia** 显卡 **Windows** 版本的 Comfy 桌面版
        </Card>
      </Tab>
      <Tab title="MacOS(Apple Silicon)">
        <Card title="ComfyUI桌面版(MacOS)安装指南" icon="link" href="/zh/installation/desktop/macos">
        适合带有 **Apple Silicon** 的 MacOS Comfy 桌面版
        </Card>
      </Tab>
      <Tab title="Linux">
        <Note>ComfyUI桌面版，**暂时没有 Linux 的预构建**，请访问[手动安装](/zh/installation/manual_install)部分进行 ComfyUI 的安装</Note>
      </Tab>
    </Tabs>
  </Accordion>
  ...
  <Accordion title="手动安装 ComfyUI">
    <Card title="ComfyUI 手动安装教程" icon="link" href="/zh/installation/manual_install">
