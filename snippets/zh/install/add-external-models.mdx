...
<Tabs>
  ...
  如果你使用的是 ComfyUI 桌面应用程序，点击桌面应用顶部的胶囊菜单，打开 **存储（Storage）** 标签页，在其中添加模型目录。胶囊菜单仅桌面应用内存在，浏览器访问的 Web 版没有此项
...
或者你也可以参考默认的 [extra_model_paths.yaml.example](https://github.com/Comfy-Org/ComfyUI/blob/master/extra_model_paths.yaml.example) 来配置，保存之后， 需要 **重启 ComfyUI** 才能生效。
