...
- Local networks only allow access from `127.0.0.1`. We do not support accessing addresses without `https` in local area networks, which may mean you cannot use API Nodes in a ComfyUI service started with the `--listen` parameter in a local area network environment.
- Access using `https` addresses. If you have deployed ComfyUI on a Linux server, you may need to bind a service domain name for the corresponding server and apply for an SSL certificate to ensure that the requests are secure.
...
You need to be logged into your ComfyUI with a [Comfy account](/zh-CN/interface/user) and have a credit balance of [credits](/zh-CN/interface/user/credits) greater than 0.
...
- [Credits](/zh-CN/interface/user/credits): After logging in, the settings interface will show a credits menu where you can purchase credits. We use a prepaid system, so there will be no unexpected charges.
