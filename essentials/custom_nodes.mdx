...
A custom node is a combination of some Python code and potentially some models that you must download. Custom nodes are extremely powerful, and allows the Comfy community to build their own functionality into ComfyUI.
...
Every custom node must implement the following methods:
...
Bool. Default is False. Set to true if your node will output a result/image from the graph.
