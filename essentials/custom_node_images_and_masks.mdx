...
`torch.Tensor` represents a tensor, which is the mathematical generalisation of a vector or matrix to any number of dimensions.
A tensor's _rank_ is the number of dimensions it has (so a vector has _rank_ 1, a matrix _rank_ 2); its _shape_ descibes the
...
<Warning>When returing a tuple, which just contains a tensor, if you forget the trailing comma, 
