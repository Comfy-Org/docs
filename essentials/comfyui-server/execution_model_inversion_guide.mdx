...
- Use of reserved [Additional Parameters](/custom-nodes/backend/datatypes) like `min` and `max` on types that aren't comparable (e.g. dictionaries) in order to configure custom widgets.
