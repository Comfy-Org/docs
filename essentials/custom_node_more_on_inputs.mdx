...
### DYNPROMPT
`DYNPROMPT` is an instance of `comfy.graph.DynamicPrompt`. It differs from `PROMPT` in that it may mutate during the course of execution in response to [Node Expansion](custom_node_expansion).
