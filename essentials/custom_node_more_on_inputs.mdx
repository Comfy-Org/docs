...
{/* TODO sometime there should be a page with all the options, and we should link to it */}
...
arbitrary names. Since the Comfy server validates the inputs being sent to a node by checking if
the `required` or `optional` dictionaries contain a key with the matching name, a node can signal 
that it is able to receive any input by providing an `optional` dictionary that contains every possible key:
