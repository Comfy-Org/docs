...
You will need to import `app` (at least) from the Comfy system files:
```Javascript
import { app } from "../../scripts/app.js";
```

The basic deployment is to include a call in your `.js` file to `app.registerExtension`, passing a dictionary that
_must_ contain a unique `name`, and _may_ contain any of a number of functions to be called by hooks in the Comfy code.

A complete but trivial extension might look like this:
```Javascript
import { app } from "../../scripts/app.js";
...
`#invokeExtensionsAsync` or `#invokeExtensions` with a method name (such as `setup` in the example above). 
These invoke, on all registered extenstions, the appropriately named method (if present).
This provides a variety of hooks for custom extension code to use to modify Comfy client behaviour.
...
A few of the most significant hooks are discussed below. 
...
Examples of the sequence in which hooks are invoked can also be found below.
...
In development you may find it helpful to insert logging code into the Comfy `app.js` file, such as:
```Javascript
...
The following are typical call sequences
