...
- **Civitai**：登录后访问 [https://civitai.com/user/account](https://civitai.com/user/account) 生成 API 密钥。
- **Hugging Face**：访问 [https://huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) 生成 API token。

### 2. 保存 API 密钥

1. 前往 [Comfy Cloud 设置](https://cloud.comfy.org)，进入 **Secrets** 部分。
2. 添加你的 API 密钥：
   - Hugging Face：添加你的 Hugging Face API token（密钥名称可以自由命名）
   - Civitai：添加你的 Civitai API key（密钥名称可以自由命名）
...
1. 从 Hugging Face 或 Civitai 获取模型链接（参见下方[如何获取导入模型链接](#如何获取导入模型链接)）。
2. 在导入弹窗中粘贴链接。
