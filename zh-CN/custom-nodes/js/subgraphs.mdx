...
Subgraphs let users group nodes into reusable, nestable components. Each subgraph is its own `LGraph` with a UUID. For the user-facing guide, see [Subgraphs](/interface/features/subgraph). For backend expansion, see [Node Expansion](/custom-nodes/backend/expansion).
...
Internal traversal utilities are not importable from extensions. Use a simple recursive helper instead:
...
- Internal traversal utilities (`forEachNode`, `collectAllNodes`, etc.) are not importable from extensions
...
- [Node Expansion (Backend)](/custom-nodes/backend/expansion)
