...
注意，与接下来几个属性不同，`INPUT_TYPES` 是一个 `@classmethod`。这样做的目的是让下拉小部件中的选项（比如要加载的 checkpoint 名称）可以在运行时由 Comfy 动态计算。详见 [数据类型](./datatypes#combo) 中关于 COMBO 类型的说明。
