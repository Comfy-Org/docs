...
**全新3D生成支持**
- **Rodin3D Gen-2**：新增 Rodin3D Gen-2 3D模型生成的API节点，改进了文件命名方式，使用UUID系统替代时间戳，提供更好的工作流组织

**增强音频功能**
- **新增音频节点**：扩展了音频处理能力，新增多个节点用于增强音频驱动的工作流和多模态内容创作

**API节点扩展**
- **WAN图生图**：新增WanImageToImageApi节点用于增强图像转换工作流，优化了VRAM使用，减少了推理过程中的峰值内存消耗

**昇腾NPU硬件支持**
- **NPU兼容性**：ComfyUI现在可以在指定的昇腾NPU硬件上运行，为专业AI硬件环境扩展了部署选项

**节点架构迁移进展（V3）**
- **大规模V3更新**：将众多节点类别迁移到V3架构，包括rebatch、Fresca、SD upscale、TCFG、SAG、后处理、CLIP文本编码、HiDream、BFL、Luma、PixArt、PhotoMaker、Qwen、Lotus、Lumina2、HyperTile、Mochi、PerpNeg和Mahiro节点，提升性能和一致性

**视频处理改进**
- **LatentCompositeMasked**：增强了与基础视频潜变量的兼容性，提升了视频工作流的灵活性和合成能力

**模型兼容性修复**
- **Qwen2.5VL模板处理**：改进了Qwen2.5VL模型在提示词中已包含模板时的模板管理
- **HuMo视图操作**：修复了HuMo模型中的.view()操作问题，提高视频生成的稳定性

**内存和性能优化**
- **内存泄漏修复**：通过显式分离模型终结器解决了内存泄漏问题，提高长时间运行工作流的稳定性
- **采样器CFG增强**：在采样器CFG函数参数中添加了'input_cond'和'input_uncond'参数，提供更灵活的条件控制

**开发和工作流更新**
- **模板跟踪**：在系统状态响应中添加了工作流模板版本跟踪，便于调试和版本管理
- **PyTorch更新**：更新了安装最新PyTorch每夜构建版本的命令
- **本地化模板缓存**：改进了基于本地化的模板入口文件缓存行为
