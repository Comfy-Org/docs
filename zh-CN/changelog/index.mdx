...
**主要模型支持与 VAE 改进**
- 新增 Qwen Image Layered 扩散模型支持，配备专用空潜在向量节点，增强图像分层工作流
- 改进 Wan/Qwen 模型的 VAE 兼容性，支持自动通道裁剪/填充，实现不同模型架构的无缝集成
- 增强去噪掩码和自定义采样器节点的嵌套张量支持，提供对复杂去噪操作的更好控制

**新节点和工作流工具**
- 新增 LatentCutToBatch 节点，用于高级批处理工作流
- 引入 Flux2MaxImage API 节点，用于高分辨率图像生成
- 实现统一作业 API，提供 /api/jobs 端点，改善工作流监控和管理

**文本编码器和注意力机制增强**
- 实现 Jina CLIP v2 和 NewBie 双 CLIP 支持，提升文本编码能力
- 在 Gemma3 模型中添加滑动注意力机制，改善长序列处理
- 在 NewBie 文本编码器上禁用提示词权重，防止现有工作流出现兼容性问题

**性能和内存优化**
- 重构训练器实现，添加分辨率分桶功能，提高训练工作流效率
- 将 skip_load_model 参数更改为 force_full_load，使模型加载行为更加清晰
- 增强 AMD GPU 支持，通过环境变量提供可选的 MIOpen 加速

**API 和外部服务更新**
- 扩展 Kling O1 StartEndFrame 节点持续时间范围，提供更灵活的视频生成
- 更新 Topaz 4k 视频超分辨率功能
- API 节点默认禁用水印生成，输出更清洁
- 集成新的 Nano Banana 自定义端点，提升处理能力

**错误修复和稳定性**
- 修复在 --gpu-only 模式下 ZImageFunControlNet 的掩码连接问题
- 解决 NewBie 模型集成的批处理错误
- 修复可能中断工作流执行的 Context Windows 日志记录错误
- 更正 Gemma 量化配置，仅应用于相关模型

**工作流模板**
- 工作流模板更新至 v0.7.63，包含最新节点配置和最佳实践
...
<Update label="v0.5.0" description="2024年12月17日">
