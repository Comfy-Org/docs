...
**新模型支持**
- Ovis图像模型实现，支持高级图像理解工作流
- Kandinsky5模型支持，具备增强的生成能力
- 支持Z Image Alipai ControlNets，改善图像生成控制
- Newbie扩散模型加载器，简化模型集成流程

**主要显存优化**
- 时间滚动VAE实现，显著减少Hunyuan和Kandinsky模型的显存占用
- 减少大量LoRA预留，特别有利于Flux2工作流
- 修复Flux2内存不足错误，改进反量化卸载计算
- 优化文本编码器GPU利用率，防止不必要的CPU回退
- 通过FP16处理增强LoRA计算速度并降低内存使用

**V3架构迁移**
- 将多个节点库转换为V3架构，包括3D节点、音频节点、freelunch节点和遮罩节点
- 为V3架构添加MatchType、DynamicCombo和Autogrow支持，提升工作流灵活性

**工作流增强**
- ComfyUI-Manager支持pip安装版本
- 上下文窗口修复和新功能，改善时序处理
- Z-Image FP16兼容性改进，提升性能
- 增强Kling Omni API节点中的@image引用格式支持

**API与集成**
- 为CORS标头添加PATCH方法，提升API兼容性
- Banana API节点的系统指令支持和视觉强化
- API工作流中支持seedance-1-0-pro-fast模型
- 添加Chroma-radiance-x0模式，扩展创意选项

**模型兼容性**
- Transformer LoRA前缀支持Z-Image工作流
- Qwen Image LoRA训练修复，改进微调工作流
- HunyuanVideo1.5 meanflow distil兼容性
- 修复缩放FP8格式与混合量化操作的兼容性
