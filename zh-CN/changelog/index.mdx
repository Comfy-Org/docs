...
**内存与性能优化**
- **默认启用锁页内存**适用于NVIDIA和AMD GPU，为模型加载和推理提供显著的性能改进
- **减少VRAM使用量**，通过优化内存管理降低LTXV、Qwen和Flux模型的显存占用
- 增强模型卸载功能，具备智能VRAM监控，能在运行间使用量增加时自动释放内存
- 修复异步卸载黑屏问题，改进卸载流上的权重转换性能

**节点与工作流改进**
- **ScaleROPE节点现已支持Flux模型**，将绳索缩放功能扩展到更多架构
- 统一所有模型的RoPE函数实现，以获得更好的一致性和性能
- 修复影响工作流可靠性的Qwen ControlNet回归问题
- 通过改进的模型重载逻辑解决Upscaler和LoRA工作流的错误输出

**API节点更新**
- 更新Rodin3D节点以使用新的V3客户端架构，移除传统API客户端
- 弃用OpenAI DALL-E 2和DALL-E 3节点（为兼容性临时恢复）
- 在/history和/queue端点的提示数据中添加create_time字段，以便更好地跟踪工作流

**开发与稳定性**
- 增强量化操作，提供更好的稳定性和性能
- 改进自定义节点导入错误处理，为第三方集成提供更可靠的支持
- 为分词器添加左填充支持，用于高级文本处理工作流
- 更好的内存锁定保护，防止重复锁定/解锁操作

**平台更新**
- 更新Intel Arc GPU安装说明，移除IPEX
- 添加Python 3.14兼容性说明
- 将ROCm安装更改为使用7.1版本，以便更好地支持AMD GPU
