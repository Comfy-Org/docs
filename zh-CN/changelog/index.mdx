...
- **面向未来的架构**：流线化的实现框架使未来版本更容易添加新的 Qwen ControlNet

**节点架构和 API 演进**
- **V3 架构迁移**：字符串节点、Google Veo API 和 Ideogram API 节点升级到 V3 架构，提供更好的性能和一致性
- **增强的 API 节点**：OpenAI Chat 节点重命名为"OpenAI ChatGPT"以提高清晰度，Gemini Chat 节点现在包含复制按钮功能
...
- **导航模式**：禁用令人烦恼的默认导航模式以改善用户体验

**工作流优势**
- **精确控制**：扩展的 Qwen ControlNet 选项为艺术家提供对图像生成和编辑的更精确控制
- **稳定的视频工作流**：LTXV 和 3D 条件控制修复确保复杂视频生成管道的可靠运行
- **增强性能**：新的缓存系统和逐元素融合减少了计算密集型工作流的执行时间
- **更好的用户体验**：改进的 API 节点界面和文件名处理创造更流畅的工作流创建体验
- **面向未来**：V3 架构升级和 Python 3.13 支持保持工作流与不断发展的标准兼容
