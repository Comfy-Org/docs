...
**性能优化**
- 显著提升工作流取消速度，在开发和实验过程中提供更好的响应性
- 修复了在使用 NVIDIA GPU 的 PyTorch 2.9 上 VAE 消耗3倍内存的关键问题，为现代设置提供改进的内存效率解决方案
- 增强色度辐射处理速度，修复批量大小超过1时的问题，使多批次操作更加可靠

**硬件兼容性**
- 添加 Python 3.14 支持，更新安装说明供开发者使用最新的 Python 功能
- 为 AMD gfx942 GPU 禁用 FP8 操作以防止兼容性问题
- 改进在使用 --fast 自动调优模式时的 CUDA 内存管理，默认禁用 CUDA malloc 以获得更好的性能

**新节点和 API 更新**
- 添加 TemporalScoreRescaling 节点，用于视频生成工作流中的高级时间控制
- 在 API 节点中引入 Veo 3.1 模型支持，扩展视频生成能力
- 修复动态定价格式，在 API 节点中将 comfy_io 重命名为 IO 以获得更好的一致性

**节点架构迁移 (V3)**
- 将 ControlNet 节点转换为 V3 架构，改进工作流兼容性和节点标准化

**缓存和执行改进**
- 增强 EasyCache，在 apply_cache_diff 中正确处理 batch_slice，使缓存行为更加可靠
- 改进 merge_nested_dicts 功能，具有正确的输入排序和重构的手动补丁合并

**前端更新**
- 更新到前端版本 1.28.7，包含已弃用 API 警告，帮助用户过渡到更新的 API
- 通过更好的 API 弃用反馈增强用户体验

**开发者体验**
- 为未使用的文件添加弃用警告，帮助维护更清洁的代码库
- 通过禁用导致问题的特定函数编译，改进 torch 编译器兼容性
- 更新到最新的 PyTorch 稳定版本，支持 CUDA 13.0，提供更好的 GPU 加速
