...
**前端和 API 更新**
- 前端更新至 v1.34.9，增强了工作流处理和文件端点安全性
- 支持每队列实时预览方法覆盖，允许在不同工作流中自定义预览体验
- 工作流模板更新至 v0.7.59，提高与现代节点结构的兼容性

**内存和性能优化**
- 改进了 Z-Image 和 SD3 模型的内存估算，减少 VRAM 分配问题
- 增强了 VAE 加载要求以更好地管理 GPU 内存，特别有利于 Qwen-Image VAE 工作流
- 修复了混合操作中的数据类型处理，防止模型执行过程中出现偏差相关错误
- 优化了色度辐射 VRAM 估算，提升渲染工作流效率

**模型支持增强**
- 新增 WanMove 模型支持，用于高级运动处理工作流
- 新增 Qwen Edit 2511 参考方法支持，基于状态字典键自动检测
- Z-Image Fun Control Union 2.0 基础实现，通过 ZImageFunControlnet 节点支持修复功能
- 改进了 Z-Image 在新版 PyTorch 上的 FP16 兼容性

**API 节点更新**
- Kling Omni Image 节点现已可用于图像生成工作流
- 新增 Kling TextToVideoWithAudio 和 ImageToVideoWithAudio 节点，实现全面的视频生成功能
- 集成 Wan2.6 模型，增强视频处理能力
- 弃用了旧版 Pika API 节点和传统 Kling 模型，优化用户体验

**采样和生成**
- 新增 SEEDS_2 phi_2 变体和专用采样器节点，增强生成控制能力
- 添加 exp_heun_2_x0 采样器系列，为精细调整输出提供更多采样选项
- 支持上下文窗口回调，在高级工作流中实现自定义条件处理

**3D 和外部集成**
- 新增 Tripo3.0 支持，用于 3D 模型生成工作流
- 增强 Lumina 和 Wan 模型块信息，改善工作流调试和优化

**错误修复和稳定性**
- 修复了 V3 节点实现中 Node V1 字典结果处理问题
- 解决了便携式更新器在未合并存储库状态下的问题
- 修正了内部文件端点的隐藏文件过滤
- 修复了 PyTorch 警告和已弃用类型使用，确保更清洁的操作
- 通过在需要时禁用 transformer_options 的保护机制，改进了 torch.compile 兼容性
