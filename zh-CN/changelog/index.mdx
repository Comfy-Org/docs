...
**性能与内存优化**
- 默认启用 Nvidia GPU 的异步卸载功能以提升性能。通过原地权重反量化增强 VRAM 管理，显著减少 Flux2 内存使用量，并在内存计算中考虑权重卸载成本。

**新模型支持与功能**
- 添加了全面的 Z Image 支持，包括 LoRA 训练、格式兼容性和 ScaleRope 节点功能。集成了 3D 动画节点以增强创意工作流，并为 Flux 2 模型添加了廉价潜在预览支持。

**API 节点增强**
- 添加了 Veo3 First-Last-Frame 节点和 Kling O1 模型支持用于视频生成工作流。改善了图像处理的批量上传用户体验，并通过优化文件处理增强了 Gemini 集成（前10张图像作为URL，其余作为base64）。

**训练与数据集处理**
- 引入了数据集处理节点，改进了支持多分辨率训练的 LoRA 训练器节点。增强了 LoRA 与 Z Image 模型的兼容性，并移除了冗余的训练节点以简化工作流。

**视频处理**
- 添加了对视频微型 VAE 的支持，以在减少内存需求的情况下实现更高效的视频处理工作流。

**开发工具与用户体验**
- 为 Flux 模型添加了 BlockInfo 支持以改进节点组织。使用 Qwen 3 令牌更新了 Qwen 分词器，以提高语言模型兼容性。通过系统用户保护改进了安全性，并修复了各种 UI/UX 问题。

**错误修复与稳定性**
- 修复了 CSP 离线功能与最新前端的兼容性，解决了量化张量连续性问题，并通过适当的流上下文处理改进了内存管理。修复了用户数据管理验证，并禁用了与 torch compile 冲突的卸载流。
