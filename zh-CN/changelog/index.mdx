...
**前端界面与用户体验增强**
- **节点 2.0 公测版**：推出全新设计的节点界面系统，提升工作流创建体验
- **线性模式测试版**：新增线性工作流执行模式（可通过快捷键设置中的热键启用）
- **工作流进度面板**：实时可视化工作流执行进度
- **资产侧边栏**：新增专用侧边栏用于管理工作流资产
- **子图改进**：增强子图功能，包含错误修复和稳定性改进
- **缺失节点体验优化**：改进处理缺失节点的工作流时的用户体验

**3D 功能增强**
- 在 3D 节点中添加全景图像支持，实现沉浸式 360° 内容工作流
- 将 3D 动画功能直接合并到主 3D 节点中，简化工作流程

**LoRA 训练能力**
- **多分辨率训练**：原生 LoRA 训练器现支持同时在多个分辨率下进行训练
- **Z Image LoRA 训练**：添加全面的 Z Image LoRA 训练支持，具备格式兼容性

**性能与模型支持**
- **视频微型 VAE**：添加视频微型 VAE 支持，以更少的内存需求实现更高效的视频处理
- **Z Image 格式支持**：完整支持 Z Image LoRA 格式和模型兼容性
- **异步卸载**：默认在 Nvidia GPU 上启用异步卸载，提升性能
- **内存优化**：通过原地权重反量化增强 VRAM 管理，显著减少 Flux2 内存使用量

**API 节点增强**
- **Veo3 First-Last-Frame 节点**：新增用于 Veo3 高级视频生成控制的节点
- **Kling V2.5 Turbo**：在 Kling FirstLastFrame 节点中添加 Kling v2.5 turbo 模型，实现更快处理
- **Kling O1 模型支持**：集成 Kling O1 模型，增强视频生成工作流
- **批量上传体验优化**：改善图像处理的批量上传体验
- **Gemini 集成优化**：增强 Gemini 集成，优化文件处理（前10张图像作为URL，其余作为base64）

**开发工具与系统改进**
- 为 Flux 模型添加 BlockInfo 支持以改进节点组织
- 使用 Qwen 3 令牌更新 Qwen 分词器，提高语言模型兼容性
- 通过系统用户保护改进安全性
- 为 Z Image 模型添加 ScaleRope 节点功能
- 为 Flux 2 模型添加廉价潜在预览支持

**错误修复与稳定性**
- 修复 CSP 离线功能与最新前端的兼容性
- 解决量化张量连续性问题
- 通过适当的流上下文处理改进内存管理
- 修复用户数据管理验证
- 禁用与 torch compile 冲突的卸载流
