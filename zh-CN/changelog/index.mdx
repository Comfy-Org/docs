...
**节点架构演进 (V3)**
- **全面 V3 迁移**: 将核心节点类别迁移到 V3 架构，包括模型缩放、LoRA 提取、合成、潜在操作、SD3/SLG、Flux、放大模型和 HunyuanVideo 节点。增强的 Enum 类对 Combo 选项的支持提供了更好的类型安全性和工作流可靠性。

**音频工作流增强**
- **MMaudio 16K VAE 支持**: 实现了新的 MMaudio 16K VAE，用于高保真音频生成工作流，扩展了多模态内容创建能力。修复了音频保存节点中单声道音频错误保存为立体声的问题。

**模型采样和性能改进**
- **重构模型采样**: 简化了模型采样 sigmas 代码，提高了性能和可维护性。修复了当 FP8 操作不可用时 FP8 缩放 LoRA 应用问题，确保在不同硬件配置下模型行为的一致性。

**AMD GPU 优化**
- **增强 AMD 性能**: 对 AMD 用户的显著性能改进，包括更好的 SD/Flux VAE 操作内存估计。在 ROCm 7.0 及更高版本上启用 RDNA4 PyTorch 注意力机制，提高推理速度。

**API 节点扩展**
- **增强外部集成**: 添加了价格提取器功能并改进了 Kling/Pika API 节点。增强 Gemini Image API 节点，支持 aspect_ratio 和最新模型版本，为更灵活的 AI 图像生成工作流提供支持。

**模型兼容性修复**
- **旧版支持**: 修复了在较新 NumPy 版本上加载旧版 Stable Diffusion 检查点文件的问题，确保与现有模型集合的向后兼容性。改进了跨工作流扩散模型评估模式的一致性。

**开发者体验**
- **代码质量改进**: 在日志函数中实现了惰性格式化，并在"Preview Any"节点中添加了带有适当缩进的 JSON 美化打印。修复了 WAN2.2 缓存 VRAM 泄漏问题，并更新了模型路径示例以提高清晰度。

**模板和文档更新**
- 将模板更新到 0.1.95 版本，节点文档更新到 0.3.0 版本，为创作者提供最新的工作流示例和节点参考资料。
</Update>
