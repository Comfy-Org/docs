...
**混元图像模型生态系统**
- **混元图像蒸馏模型**: 全面支持新的蒸馏模型，具备更快的生成能力
- **混元图像精炼器**: 新的精炼器模型实现，支持噪声增强以提升图像质量
- **快速预览支持**: 为混元图像工作流添加了快速预览功能
- **VAE兼容性**: 混元精炼器现在可以与分块VAE处理无缝协作，实现高分辨率精炼
- **色彩校正**: 修复了精炼器在噪声增强级别低于0.25时出现的过曝色彩问题

**新模型支持**
- **HuMo模型**: 添加了对1.7B和17B HuMo模型的支持，用于高级多模态工作流
- **Chroma Radiance**: 新模型支持，增强色彩处理能力
- **Qwen Edit Plus**: 支持Qwen edit plus模型，配备新的TextEncodeQwenImageEditPlus节点
- **Omnigen2 UMO LoRA**: 添加了对Omnigen2 UMO LoRA模型的兼容性
- **Wav2vec基础模型**: 通过wav2vec基础模型支持扩展了音频处理能力
- **Whisper Large V3**: 添加了编码器部分支持，用于高级音频编码工作流

**WAN动画增强**
- **WAN动画模型**: 基本支持WAN动画模型和视频生成能力
- **WanAnimateToVideo节点**: 新节点，便于视频扩展工作流，支持角色替换输入
- **内存优化**: 减少了WAN推理的峰值显存使用量，在消费级硬件上实现更好的性能
- **实验状态**: WAN节点不再标记为实验性功能，表明已达到稳定的生产就绪状态

**API节点扩展**
- **Kling v2.1支持**: 在KlingStartEndFrame节点中添加了kling-v2-1模型，增强视频帧控制
- **超时改进**: 将默认API超时时间从7天减少到2小时，提高可靠性
- **Seedream4增强**: 添加了在部分成功时忽略错误的标志，使工作流更加稳健
- **WAN API节点**: 新的文本到图像、文本到视频和图像到视频API节点，用于基于云的生成

**节点架构迁移 (V3)**
- **核心节点更新**: 将多个节点类别迁移到V3架构，包括Minimax API、Cosmos、条件、CFG和Canny节点
- **增强的组合类型**: 在V3组合类型中添加了Output以匹配V1功能，提高工作流兼容性
- **Moonvalley API**: 将Moonvalley API节点转换为V3架构，改进功能

**性能和技术改进**
- **运行时注意力选择**: 启用了注意力函数的动态选择，优化性能
- **FP8操作**: 在gfx1200硬件上默认启用FP8操作，实现更快的处理速度
- **LoRA训练器修复**: 解决了LoRA训练工作流中FP8模型兼容性的错误
- **差分扩散**: 添加了强度输入参数，实现对差分扩散效果的更精确控制

**UI和前端更新**
- **前端版本更新**: 更新到版本1.26.13，带来各种界面改进
- **图像锐化修复**: 纠正了图像锐化节点的问题，确保可靠的图像增强
- **模板更新**: 将工作流模板更新到版本0.1.86，采用最新最佳实践

</Update>
<Update label="v0.3.59" description="2025年9月10日">

**字节跳动 Seedream 4.0 集成**
- **新增 Seedream 节点**：新增 ByteDanceSeedream (4.0) 节点.
