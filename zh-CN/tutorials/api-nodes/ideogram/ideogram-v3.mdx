---
title: "ComfyUI Ideogram 3.0 API 节点官方示例"
description: "本文将介绍在 ComfyUI 中使用 Ideogram 3.0 API 节点的相关功能"
...
当同时提供了图像和蒙版输入时，节点将使用图像编辑模式。

> 注意：图像和蒙版必须同时提供，并且使用相同的尺寸
...
【】
你可参考图片中的序号来完成最基础的工作流运行
1. 首先我们需要在 `Load Image` 节点中通过右键`Open in MaskEditor` 来打开遮罩编辑器，如果你不熟悉如何使用，可以参考 [遮罩编辑器](/zh-CN/interface/maskeditor) 部分的说明
2. 在 `Ideogram V3` 节点的 `prompt` 输入你对遮罩区域想要被修改的描述
