...
## 使用 ComfyUI 账户 API Key 集成来调用付费模型 API 节点

目前我们支持通过 ComfyUI 账户 API Key 集成来访问我们的服务来调用付费模型 API 节点，请参考 API Key 集成章节了解如何使用 API Key 集成来调用付费模型 API 节点

<Note>
  **重要提示:** 此处讨论的 API 密钥是您的 **ComfyUI 账户 API Key**(用于在工作流中访问付费 API 节点)。这与开发者用于将自定义节点发布到注册表的 **注册表发布 API Key** **不同**。如果您想发布自定义节点,请参阅[发布节点](/zh-CN/registry/publishing)。
</Note>
...
<Card title="API 节点计费" icon="coin" href="/zh-CN/tutorials/api-nodes/pricing">
请参考定价页面了解对应的 API 定价
</Card>
