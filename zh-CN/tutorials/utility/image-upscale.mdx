---
title: "ComfyUI 图像放大"
description: "学习如何在 ComfyUI 中使用本地模型和合作伙伴节点进行图像放大"
sidebarTitle: "图像放大"
---

本指南介绍 ComfyUI 中的图像放大工作流，包括本地模型和合作伙伴节点选项，适用于各种使用场景。

<Tip>
如需更全面的深入分析、基准测试和视觉对比，请参阅 ComfyUI 博客上的 [The Complete AI Upscaling Handbook](https://blog.comfy.org/p/upscaling-in-comfyui)。
</Tip>

## 核心概念

### 放大 vs. 增强

- **放大（Upscaling）**：在重建细节的同时提高分辨率。
- **增强（Enhancement）**：改善感知质量（降噪、锐化、修复、色彩、面部等）。

AI 工作流的最佳实践通常是将两个步骤结合使用。

### 创意放大 vs. 保守放大

| | 保守放大 | 创意放大 |
|---|---|---|
| **方式** | 保留原始内容 | 重新想象并增强 |
| **优势** | 高精度、一致性、适合生产环境 | 细节丰富、更锐利、视觉效果更突出 |
| **局限** | 可能显得平淡，对低质量输入改善有限 | 可能产生幻觉或结构偏移 |
| **模型** | Magnific Precise、SeedVR2、FlashVSR、Topaz Fast、HitPaw | Wan 2.2、Magnific Creative、Topaz Astra、HitPaw Creative |

## 使用场景

### 人像和皮肤增强

人像放大的关键是在保持角色一致性的同时实现逼真的皮肤细节。最佳模型能够添加纹理、毛孔和自然的皮肤瑕疵。

**推荐**：Magnific Image Skin Enhancer、Nano Banana Pro Portrait、HitPaw Creative Portrait

<CardGroup cols={2}>
  <Card title="Magnific 皮肤增强" icon="cloud" href="https://links.comfy.org/4cbvbE7">
    人像专用皮肤增强
  </Card>
  <Card title="Nano Banana Pro 人像" icon="cloud" href="https://links.comfy.org/3O9XtF7">
    快速人像放大
  </Card>
  <Card title="HitPaw 创意人像" icon="cloud" href="https://links.comfy.org/4kkoula">
    创意人像模式
  </Card>
</CardGroup>

### 产品摄影

保持材质、产品标签边缘和小字的忠实呈现需要使用保守放大模型。

**推荐**：HitPaw、Magnific Precise、WaveSpeed SeedVR2、Nano Banana Pro Product

<CardGroup cols={2}>
  <Card title="Nano Banana Pro 产品" icon="cloud" href="https://links.comfy.org/4qUntD0">
    针对产品摄影优化
  </Card>
  <Card title="SeedVR2（开源）" icon="cloud" href="https://links.comfy.org/4ko2sOG">
    高保真度保守放大
  </Card>
</CardGroup>

### 风景和环境

模型选择取决于你的需求：
- **创意模型**：让环境照片细节丰富、氛围突出。
- **保守模型**：保持建筑和结构的一致性。

创意模型可能会重新想象伪影，而保守模型则不会。

<CardGroup cols={2}>
  <Card title="Topaz 风景" icon="cloud" href="https://links.comfy.org/4r1utxS">
    针对风景和环境图像优化
  </Card>
  <Card title="Topaz 创意" icon="cloud" href="https://links.comfy.org/4kjT12P">
    带主体检测的创意放大
  </Card>
</CardGroup>

### 风格化艺术/插画

对于独特或特殊风格的图像，保守模型效果最佳，可避免添加不需要的细节。对于有细节空间的图像，创意模型可以改善风格。

**提示**：调整 Magnific Creative 和 Topaz Image Enhance 中的 `creativity` 参数以找到合适的平衡点。

<CardGroup cols={2}>
  <Card title="Topaz 插画" icon="cloud" href="https://links.comfy.org/46zi8c1">
    最适合风格化艺术和插画
  </Card>
  <Card title="Nano Banana Pro 风格化艺术" icon="cloud" href="https://links.comfy.org/3O0Maiz">
    最适合风格化艺术作品
  </Card>
  <Card title="Recraft 创意" icon="cloud" href="https://links.comfy.org/4afKfOJ">
    带细节增强的创意放大
  </Card>
  <Card title="Recraft 清晰" icon="cloud" href="https://links.comfy.org/3O0M4Yf">
    锐利、清晰的放大效果
  </Card>
</CardGroup>

### AI 生成图像

创意放大无法可靠地修复常见的 AI 问题，如多余的手指、伪影或不正确的解剖结构。最佳做法是在放大之前使用图像编辑模型或传统工具解决这些问题。

<CardGroup cols={2}>
  <Card title="SeedVR2（开源）" icon="cloud" href="https://links.comfy.org/4ko2sOG">
    AI 图像的保守放大
  </Card>
</CardGroup>

## 可用模型

### 本地模型（ESRGAN）

有关使用 ESRGAN 模型进行基础本地放大，请参阅[基础放大教程](/zh-CN/tutorials/basic/upscale)。

| 模型 | 最适合 |
|---|---|
| RealESRGAN | 通用放大 |
| BSRGAN | 文字和锐利边缘 |
| SwinIR | 自然纹理、风景 |

<CardGroup cols={2}>
  <Card title="传统非 AI 放大" icon="cloud" href="https://links.comfy.org/45Nu9um">
    在 Comfy Cloud 上运行传统非 AI 放大工作流
  </Card>
</CardGroup>

### 合作伙伴节点

合作伙伴节点通过 API 提供高级放大模型访问。

| 模型 | 类型 | 最大分辨率 | 特性 |
|---|---|---|---|
| Topaz Image Enhance (Bloom) | 创意 | 8K | 主体检测、面部增强、色彩保留 |
| Magnific Precise | 保守 | - | 高精度、适合生产环境 |
| Magnific Creative | 创意 | - | 细节重新想象、皮肤增强 |
| Magnific Skin Enhancer | 创意 | - | 人像专用，添加逼真皮肤纹理 |
| Nano Banana Pro | 保守 | - | 快速，适合产品摄影 |
| WaveSpeed SeedVR2 | 保守 | - | 高保真度 |
| HitPaw | 两者 | - | 保守和创意模式 |

## 基准测试：1K 到 4K 放大时间

| 模型 | 时间 |
|---|---|
| Nano Banana Pro | ~80秒 |
| Topaz Image Enhance | ~100秒 |

## 技巧

- 对于 SeedVR2，在放大之前使用 `ImageScaleToTotalPixels` 节点将图像缩小到 0.35 百万像素可获得更好的效果。
- 链接多个放大节点（例如 2x → 4x）以实现超高倍率放大。
- 在生成后连接放大节点，实现"生成 + 增强"工作流。
- 在特定图像类型上测试多个模型以找到最佳匹配。
