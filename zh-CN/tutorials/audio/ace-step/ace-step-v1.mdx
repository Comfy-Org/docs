...
1. 确保 `Load Checkpoints` 节点加载了 `ace_step_v1_3.5b.safetensors` 模型
2. 在 `LoadAudio` 节点上传前面提供的音频文件
3. （可选）在 `TextEncodeAceStepAudio` 的 `tags` 输入对应的音乐风格等等
4. (可选) 在 `TextEncodeAceStepAudio` 的 `lyrics` 中输入对应的歌词，可以参考文本提示词指南部分（仍在更新中）或 ACE-Step 项目页面中的歌词示例
