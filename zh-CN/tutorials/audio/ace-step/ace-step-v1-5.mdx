---
title: "ComfyUI ACE-Step 1.5 指南"
description: "ACE-Step 1.5 简介以及如何在 ComfyUI 中运行音乐生成工作流。"
...
ACE-Step 1.5 是开源音乐生成模型的重大更新，将商业级质量带到你的本地机器上。它引入了一种新颖的混合架构，其中语言模型作为全能规划器，将简单的用户查询转换为完整的歌曲蓝图。

**模型亮点：**
- **商业级质量**：在音乐连贯性方面达到 4.72 分，超越大多数商业音乐模型
- **极速生成**：在 RTX 5090 上约 1 秒生成完整的 4 分钟歌曲，在 RTX 3090 上不到 10 秒
- **50+ 语言支持**：对英语、中文、日语、韩语、西班牙语、德语、法语、葡萄牙语、意大利语和俄语有强大支持
- **LoRA 微调**：支持通过 LoRA 训练进行轻量级个性化定制
...
<CardGroup cols={2}>
  <Card title="AIO 整合模型工作流" icon="download" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/refs/heads/main/templates/audio_ace_step_1_5_checkpoint.json">
    下载一体化 checkpoint 工作流（只需下载单个模型）。
  </Card>
  <Card title="分离模型工作流" icon="download" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/refs/heads/main/templates/audio_ace_step_1_5_split.json">
    下载分离模型工作流（需要分别下载模型文件）。
  </Card>
</CardGroup>
