...
description: "本文将使用 Flux.1 fill dev 来完成 Inpainting 和 Outpainting 的工作流。"
...
Flux.1 fill dev 是 [Black Forest Labs](https://blackforestlabs.ai/) 退出的 ​FLUX.1 Tools 套件中的核心工具之一。

以下是简单的 Flux.1 fill dev 核心特点：
- 生成效果仅次于 FLUX.1 Fill [pro]。
- 良好的提示词跟随能力，能够很好地保持与原图像的一致性。
- 采用引导蒸馏训练，使 FLUX.1 Fill [dev] 更加高效。
- 生成的输出可用于个人、科学和商业目的，具体请参见 [FLUX.1 [dev] 非商业许可证](https://huggingface.co/black-forest-labs/FLUX.1-dev/blob/main/LICENSE.md)。
...
如果你不太了解 Inpainting 和 Outpainting 的工作流可以参考 [ComfyUI 布局重绘示例](/zh-CN/tutorials/basic/inpaint) 和 [ComfyUI 布局扩图示例](/zh-CN/tutorials/basic/outpaint)，部分的相关说明。


## Flux.1 Fill dev 工作流模型安装

在开始之前，让我们先完成 Flux.1 Fill dev 模型文件的安装， inpainting 和 outpainting 的工作流中会使用完全相同的模型文件，如果你之前使用过完整版本的 [Flux.1 文生图工作流](/zh-CN/tutorials/flux/flux1-fill-dev)，那么在这个部分你仅需要下载 flux1-fill-dev.safetensors 这个模型文件。

不过由于下载对应模型需要同意对应的使用协议，所以请访问 [](https://huggingface.co/black-forest-labs/FLUX.1-Fill-dev)，确保你参照下图同意了对应的协议。
...
<Note>
对应的图片已经包含 aplha 通道，所以你不需要额外进行蒙版的绘制, 如果你想要自己进行蒙版的绘制请[点击这里](https://raw.githubusercontent.com/Comfy-Org/example_workflows/main/flux/inpaint/flux_fill_inpaint_input_original.png)获取不带蒙版的版本，并参考 [ComfyUI 布局重绘示例](/zh-CN/tutorials/basic/inpaint) 中的 MaskEditor 的使用部分来了解如何在`Load Image`节点中绘制蒙版。
</Note>
...
1. 确保在`Load Diffusion Model`加载了`flux1-fill-dev.safetensors`
2. 确保在`DualCLIPLoader`中下面的模型已加载：
