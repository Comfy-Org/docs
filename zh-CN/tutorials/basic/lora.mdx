...
在本示例中，我们将完成以下内容来学习[ComfyUI](https://github.com/comfyanonymous/ComfyUI) 中加载并使用 LoRA 模型。
...
1. 确保`Load Checkpoint` 加载了 `dreamshaper_8.safetensors` 模型
2. 确保`Load LoRA` 加载了 `blindbox_V1Mix.safetensors` 模型
...
| 参数名称  | 作用                        |
|----------|----------------------------|
| `model`  | 输出应用了LoRA 调整的模型      |
| `clip`   | 输出经过 LoRA 调整的 CLIP 模型 |
...
试着修改提示词，或者调整 `Load LoRA`  节点的不同参数，比如 `strength_model` ，来观察生成图片的变化，熟悉对应节点。
或访问 [CivitAI](https://civitai.com/models) 网站，下载其它风格的 LoRA 模型，尝试使用。