...
这是因为专门为 inpainting 设计的模型，它可以帮助我们更好地控制生成区域，从而获得更好的生成效果。
...
首先在上一步工作流中的`Save Image` 节点上右键，你可以在右键菜单中看到`复制(Clipboard)` 选项，点击后会复制当前图片到剪贴板

![ComfyUI 局部重绘 - 复制图片](/images/tutorial/basic/inpaint/inpaint_copy_clipspace.png)

然后在 **加载图像(Load Image)** 节点上右键，你可以在右键菜单中看到`Paste(Clipboard)` 选项，点击后会从剪贴板中粘贴图片
