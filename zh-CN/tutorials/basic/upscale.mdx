...
在放大产品图像时，必须保持材质、产品标签边缘和小文字的忠实呈现。因此需要使用保守型放大模型。我们推荐测试 HitPaw、Magnific Precise、WaveSpeed SeedVR2 和 Nano Banana Pro。
...
当放大具有典型 AI 问题的图像时（如手指过多、伪影、解剖结构错误和变形），人们通常假设创意型放大可以修复这些问题。有时确实如此，但有时并非如此。最佳实践是在放大之前使用图像编辑模型或传统工具解决这些问题（或者重新生成一张图像）。

### 传统非 AI 放大

对于需要简单、确定性放大而不需要 AI 增强的情况：

<CardGroup cols={2}>
  <Card title="传统非 AI 放大 (开源)" icon="cloud" href="https://links.comfy.org/45Nu9um">
    在 Comfy Cloud 上试用或下载工作流
  </Card>
</CardGroup>
