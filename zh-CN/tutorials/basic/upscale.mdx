...
比如默认通过 SD1.5 模型对于大尺寸的图片生成表现不佳，如果需要高分辨率，我们通常会先生成尺寸的图形，然后使用图像放大来提升图片的分辨率。

当然本文介绍的只是诸多 ComfyUI 中图像放大中的一种，在这篇讲解中，我们将带你完成以下内容：
- 下载并安装放大模型
- 使用放大模型进行一次简单的放大
- 结合文生图工作流，完成图像的放大
...
2. 将模型文件（.pth）放入 `ComfyUI\models\upscale_models` 目录
</Step>
</Steps>
...
在完成了简单的放大工作流后，我们就可以尝试结合[文生图](/zh-CN/tutorials/basic/text-to-image.mdx)的工作流来完成一个完整放大工作的流程，关于文生图的基础部分及相关模型要求，请参考[文生图](/zh-CN/tutorials/basic/text-to-image.mdx)的部分的说明完成。
