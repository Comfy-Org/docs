...
文生图（Text to Image），是 AI 绘图的基础，通过输入文本描述来生成对应的图片，是 AI 绘图最常用的功能之一，它的核心是 **扩散模型**。

在文生图过程中我们需要以下条件：
- 画家：绘图模型
- 画布：潜在空间
- 对画面的要求：提示词，包括正向提示词（希望在画面中出现的元素）和负向提示词（不希望在画面中出现的元素）

这个文本到图片图片生成过程，可以理解成你把你的**绘图要求(正向提示词、负向提示词)**告诉一个**画家(绘图模型)**，画家会根据你的要求，画出你想要的内容。
...
另外由于扩散模型并不能直接理解我们的自然语言，所以需要使用 CLIP 模型将我们的自然语言转换为模型可以理解的特征，然后输入到扩散模型中。
...
### C. 空Latent图像（Empty Latent Image）节点

![空Latent图像](/images/comfy_core/lantent/empty_latent_image.jpg)

定义一个潜在空间（Latent Space）,它输出到 Ksampler 节点

它的具体的作用你可以理解为定义画布尺寸的大小，也就是我们最终生成图片的尺寸

### B. CLIP文本编码器（CLIP Text Encoder）节点
...
| **denoise**              | 每次降噪强度系数                                                     | 控制每次降噪的强度，0.0保留原始输入特征，1.0完全重构新图像               |
...
![VAE 解码](/images/comfy_core/decoders/vae_decode.jpg)
