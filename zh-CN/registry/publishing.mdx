...
<Note>
**正在寻找 API 节点 API 密钥？** 如果您想要在工作流中使用 API 节点（而不是发布自定义节点），您需要不同的 API 密钥。请参阅 [API 节点 API 密钥集成](/zh-CN/development/comfyui-server/api-key-integration)文档了解如何在无头模式下使用 API 节点的详细信息。
</Note>
