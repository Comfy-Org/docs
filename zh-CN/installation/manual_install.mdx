...
<AccordionGroup>
  <Accordion title="不支持 GGUF 格式模型">
    ComfyUI 原生并不支持 GGUF 格式模型。如需使用 GGUF 模型，您需要安装社区自定义节点来获取支持，例如 [ComfyUI-GGUF](https://github.com/city96/ComfyUI-GGUF)。
  </Accordion>
  
  <Accordion title="为什么找不到模型？">
    如果安装模型后找不到模型，请尝试以下解决方案：
