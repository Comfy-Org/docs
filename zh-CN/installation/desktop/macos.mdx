...
**ComfyUI 桌面版（Desktop）** 是一个独立的安装版本，可以像常规软件一样安装，支持快捷安装自动配置 **Python环境及依赖** ，支持一件导入已有的 ComfyUI 设置、模型、工作流和文件
...
然后在**启动台(Lanchpad)**找到对应的 **omfyUI 图标** 点击将进入 ComfyUI 的初始化设置
...
![ComfyUI 安装失败-错误日志](/images/desktop/win-comfyui-desktop-8.png)
![ComfyUI 安装失败-GPT 反馈](/images/desktop/win-comfyui-desktop-9.png)
