...
| prompt_upsampling | 布尔值 | True | 是否使用提示词上采样技术增强细节 |
| steps | 整数 | 30 | 生成步数，影响质量和生成时间 |
| guidance | 浮点数 | 7.0 | 提示词引导强度，较高值更严格遵循提示词 |
| seed | 整数 | 0 | 随机种子值 |
...
根据 Black Forest Labs 官方介绍：

- **超高分辨率**：支持高达 4MP（标准 Flux 1.1 [pro] 的 4 倍）
- **极速生成**：每个样本仅需 10 秒，比同类高分辨率产品快 2.5 倍以上
- **出色的提示词遵循性**：即使在高分辨率下仍保持对提示词的准确理解和表现
、
## 工作原理
