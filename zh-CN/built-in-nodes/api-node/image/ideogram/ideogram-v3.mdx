...
Ideogram V3 节点提供对Ideogram最新一代图像生成技术的访问，带来显著提升的图像质量、文字渲染和创意控制选项。

## 节点功能

此节点连接到由前Google Brain团队成员开发的Ideogram V3 API，代表了该平台最新的技术进步。V3版本提供了更强大的文字渲染能力、更高的图像质量和更多的创意控制选项，为用户提供更精确和灵活的创作体验。
...
魔法提示功能使用智能算法自动增强用户提供的提示词，以实现更好的结果。此外，V3还支持更多的创意控制参数，如精确的宽高比设置和渲染质量选项，使创作者能够获得更符合其设计愿景的输出。

## 常见问题

**Q: 为什么我的局部重绘（Inpainting）不工作？**  
A: 局部重绘模式要求同时提供图像和遮罩输入。如果只提供其中一个，节点将抛出错误。另外，遮罩必须与图像具有完全相同的尺寸。

**Q: 如何使用风格参考功能？**  
A: 当前版本的Ideogram V3节点不支持单独的风格参考功能。图像输入仅用于局部重绘，不能单独用作风格参考。

**Q: 为什么我的遮罩不起作用？**  
A: 确保遮罩是正确的格式（白色区域表示要替换的部分），并且与原始图像具有完全相同的尺寸。
