...
description: "使用Minimax AI将文本描述转换为视频的节点"
...
Minimax Text to Video 节点允许你使用Minimax AI的视频生成技术，通过文本描述创建高质量、流畅的视频内容。

## 节点功能

此节点连接到Minimax的文本到视频API，让用户能够通过详细的文本提示词生成动态视频内容。Minimax AI提供多种视频生成模型，能够创建各种风格和类型的短视频片段，满足不同创意需求。
...
Minimax Text to Video 节点分析用户提供的文本提示词，通过Minimax AI的生成模型创建相应的视频内容。该过程利用深度学习技术理解文本描述并将其转换为连贯的动态视觉序列。

节点会将用户的请求发送到Minimax的API，服务器处理生成请求并返回视频结果。用户可以通过调整提示词的详细程度和选择不同的模型来控制生成结果的风格和内容。

用户可以选择"T2V-01"模型进行基础的文本到视频转换，或选择"T2V-01-Director"模型获取更高级的导演级视频生成能力，后者可能提供更复杂的镜头语言和场景转换。
