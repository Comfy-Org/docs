...
Kling Text to Video 节点允许你使用Kling的视频生成技术，通过文本描述创建高质量、连贯的视频内容。

## 节点功能

此节点连接到Kling的文本到视频API，让用户能够通过详细的文本提示词生成动态视频内容。节点提供多种参数控制，包括视频时长、宽高比和模型选择，以满足不同的创作需求。
...
| model_name      | 选择项 | "ModelName.text2video" | 使用的视频生成模型               |
...
Kling Text to Video 节点分析用户提供的文本提示词，通过Kling的生成模型创建相应的视频序列。节点将请求发送到Kling的API服务器，处理完成后返回生成的视频结果。

用户可以通过调整各种参数来控制生成结果，包括负面提示词、配置缩放值、视频持续时间和宽高比等。不同的模型和模式选项可以产生不同风格和质量的视频输出。
