...
Kling Start-End Frame to Video 节点允许你提供开始和结束图像，生成在两者之间平滑过渡的视频序列。

## 节点功能

此节点连接到Kling的视频生成API，专门用于创建从一个图像状态到另一个图像状态的平滑过渡。节点会自动创建所有中间帧，产生流畅的变换效果。该功能特别适合创建变形、场景转换和对象演变等视觉效果。
...
| mode | 选择项 | "standard" | 视频生成模式和持续时间设置 |
...
Kling Start-End Frame to Video 节点分析起始帧和结束帧图像，理解两者之间的视觉差异，然后创建连接这两个状态的平滑过渡序列。节点将图像和参数发送到Kling的API服务器，后者生成所有必要的中间帧，创建流畅的变换效果。

用户可以通过提示词引导过渡效果的风格和内容，而负面提示词则帮助避免不需要的元素。不同的模式选项提供了各种预设的持续时间和过渡风格，以满足不同的创意需求。
