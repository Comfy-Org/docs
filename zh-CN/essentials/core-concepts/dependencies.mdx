...
随着 ComfyUI 的发展，我们可能也会调整相应的依赖，比如添加新的依赖，或者删除一些不再需要的依赖, 比如随着迭代发展，目前 [ComfyUI Manager 也被将被纳入到ComfyUI 的核心依赖中](https://blog.comfy.org/p/comfyui-manager-joins-comfy-org)。
所以如果你是使用 Git 来更新 ComfyUI 那么你需要在拉取最新的更新之后在对应的环境下使用

```bash
pip install -r requirements.txt
```
从而来安装 ComfyUI 最新的依赖以保证 ComfyUI 的正常运行。
