...
- 为了配置自定义小部件（widget），在不适合进行比较的类型（如字典）上使用了保留的[附加参数](/custom-nodes/backend/datatypes#additional-parameters)（例如 `min` 和 `max`）。
    ...
    - 为该输入定义一个自定义的 [VALIDATE_INPUTS](/custom-nodes/backend/server_overview#validate-inputs) 函数，从而跳过对其的验证。*（快速方案）*
        ```python
        @classmethod
...
- 使用了复合类型（例如 `CUSTOM_A,CUSTOM_B`）
    - （作为输出时）定义并使用类似 `MakeSmartType` 的包装器 [见于此 PR 的单元测试](https://github.com/comfyanonymous/ComfyUI/pull/2666/files#diff-714643f1fdb6f8798c45f77ab10d212ca7f41dd71bbe55069f1f9f146a8f0cb9R2)
        ...
        class MyCustomNode:
            ...
            # ...
        ```
    - （作为输入时）定义一个自定义的 [VALIDATE_INPUTS](/custom-nodes/backend/server_overview#validate-inputs) 函数，使其接受 `input_types` 参数，从而跳过类型验证。
        ```python
        @classmethod
...
为了缓解前述[可选输入验证](#optional-input-validation)变更带来的影响，`VALIDATE_INPUTS` 函数新增了若干特性。
...
更多详情请参阅 [VALIDATE_INPUTS](/custom-nodes/backend/server_overview#validate-inputs) 文档。

### Lazy Evaluation

输入现支持 `Lazy Evaluation` （惰性求值），即可以先判断是否确实需要某个输入值，再决定是否执行其连接的上游节点及其所有依赖节点。更多信息请参见[惰性求值](/custom-nodes/backend/lazy_evaluation)。

### Node Expansion

在运行时，节点可以动态扩展为一个子图（subgraph）。该机制使得通过尾递归（tail-recursion）实现循环等复杂逻辑成为可能。更多信息请参见[节点扩展](/custom-nodes/backend/expansion)。
