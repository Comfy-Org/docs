...
<Note>
  API 提供两个端点用于检查任务状态：
  - **`GET /api/jobs/{id}`** - 返回用户友好的状态（`pending`、`in_progress`、`completed`、`failed`、`cancelled`）以及输出。**推荐用于轮询。**
  - **`GET /api/job/{id}/status`** - 返回原始内部状态（如 `success`、`executing`、`queued_limited`）。适用于调试，但不建议用于轮询逻辑。
</Note>
