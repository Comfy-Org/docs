...
便携包本身并不带有任何的绘图模型，所以你需要自己下载对应的绘图模型，如果你不知道从哪里开始，你可以访问下面的链接并下载 **stable diffusion 1.5** 的绘图模型
- [点击前往下载 table diffusion 1.5](https://huggingface.co/Comfy-Org/stable-diffusion-v1-5-archive/blob/main/v1-5-pruned-emaonly-fp16.safetensors)

请将下载到的`v1-5-pruned-emaonly-fp16.safetensors` 文件保存到 `ComfyUI_windows_portable/ComfyUI/models/checkpoints` 文件夹下
...
当你浏览器成功打开 ComfyUI 的页面后，你可以看到类似下图的界面

![ComfyUI GUI](/images/comfyui-gui.png)

参考图片序号请按照下面的步骤来完成第一次图片生成：
1. 请在`Load Checkpoint`节点选择之前下载到的`v1-5-pruned-emaonly-fp16.safetensors` 或者你本地的模型文件
2. 点击 `Queue` 按钮，或者使用快捷键 `Ctrl + enter(回车)` 来执行图片生成

常见问题解决：
- 如果 `Load Checkpoint` 节点没有任何模型可以选择，请先确认你的模型安装位置正确，或者尝试刷新浏览器或者重启 ComfyUI 使得对应文件夹下的模型可以被检测到

## 其它 ComfyUI 便携版相关说明

### 1. ComfyUI 便携版升级

你可以使用 update 文件下的相关批处理命令完成 ComfyUI 便携版的升级
...
如果你同事有使用 [A1111](https://github.com/AUTOMATIC1111/stable-diffusion-webui) 或者想要自定义模型存储位置，你可以通过修改下面的文件来完成配置
```
ComfyUI_windows_portable
└─ 📂ComfyUI
