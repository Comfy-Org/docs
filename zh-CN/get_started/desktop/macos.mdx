...
**ComfyUI 桌面版（Desktop）** 是一个独立的安装版本，支持一件快捷安装自动配置 **Python环境及依赖** ，支持一件导入已有的 ComfyUI 设置、模型、工作流和文件

ComfyUI 桌面版是一个开源项目，完整代码请访问：https://github.com/Comfy-Org/desktop

<Note>ComfyUI 桌面版（MacOS） 仅支持 ARM 架构即 M 系列芯片</Note>
...
![ComfyUI 安装包](/images/desktop/mac-comfyui-desktop-0.jpg)

然后再**启动台(Lanchpad)**找到对应的 **omfyUI 图标** 点击将进入 ComfyUI 的初始化设置
...
<Steps>
  <Step title="开始界面">
   ![ComfyUI 安装步骤 - 起始](/images/desktop/mac-comfyui-desktop-2.jpg)

   点击 **Get Started** 开始初始化步骤
  </Step>
  <Step title="Select GPU(GPU选择)">
  ![ComfyUI 安装步骤 - GPU 选择](/images/desktop/mac-comfyui-desktop-3.jpg)
  ...
  ![ComfyUI 安装步骤 - 安装位置设置](/images/desktop/mac-comfyui-desktop-4.jpg)

  在这一步将选择 ComfyUI 的安装位置相关内容如：
  - **Python 环境**
  - **Models 模型文件**
  - **Custom Nodes 自定义节点**

  选择默认的文件夹或新建一个单独的空白文件夹作为 ComfyUI 的安装目录，点击 **Next** 进入下一步
  </Step>
  <Step title="Migrate from Existing Installation（从已有安装迁移 - 可选）">
  ![ComfyUI 安装步骤 - 文件迁移](/images/desktop/mac-comfyui-desktop-5.jpg)
  ...
  ![ComfyUI 安装步骤 - 桌面版设置](/images/desktop/mac-comfyui-desktop-6.jpg)
   ...
   ![ComfyUI 安装步骤 - 镜像设置](/images/desktop/mac-comfyui-desktop-7.jpg)
...
![ComfyUI 安装失败](/images/desktop/mac-comfyui-desktop-8.jpg)
