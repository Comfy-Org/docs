...
<Steps>
  ...
  在这一步你可以将你已有的 ComfyUI 安装内容迁移到 ComfyUI 桌面版中，如图所示，我选择了我原本的 **D:\ComfyUI_windows_portableI** 安装目录，安装程序会自动识别对应目录下的：
  - User Files 用户文件
  ...
  ![ComfyUI 安装步骤 - 桌面版设置](/images/desktop/win-comfyui-desktop-5.jpg)
   ...
   1. 查找可用的镜像源头，更新镜像
   2. 手动下载对应镜像文件，使用路径名称，来直接读取对应的文件进行安装

   如上图，对应示例将需要的镜像文件下载到本地后使用 `file:///C:/Users/YourName/Downloads/package.tar.gz ` 来让安装程序读取对应的依赖文件
  </Step>
  <Step title="Desktop Setting(桌面版设置)">
...
以上的两个操作并不会删除以下你的以下文件夹，如果你需要删除对应文件的话，请手动删除：
- models 模型文件
- custom nodes 自定义节点
