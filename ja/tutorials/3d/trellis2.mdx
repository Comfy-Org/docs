...
2. **TRELLIS.2 パイプラインに切り替え**: `Boolean (Switch to Trellis2)` を `true` に設定し、`UNETLoader` ノードで `trellis_2_bf16.safetensors` を読み込みます
...
4. **結果を表示する**：`Preview3DAdvanced` ノードでメッシュを確認します。GLBは `ComfyUI/output/3d/pixal3d/` に保存されます。
