...
このワークフローには TRELLIS.2 パイプラインも含まれています。`Boolean (Switch to Trellis2)` ノードはデフォルトで `false` となり、ここで説明する Pixal3D パイプラインが実行されます。
