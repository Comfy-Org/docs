...
### Z-Image-Turbo 2K アップスケーラー（ローカル）

ローカルの拡散モデルによるアップスケーリングでは、Z-Image-Turbo 2K アップスケーラーがまず RealESRGAN で画像を事前にアップスケールし、その後 Z-Image-Turbo で低い denoise 値により再サンプリングして 2K 解像度のディテールを再構築します。

サンプラーの `denoise` 値は、モデルが入力からどれだけ離れてよいかを決めます。`0.15`〜`0.25` は元画像に近いまま、`0.25`〜`0.35` は再解釈によってディテールを追加し、`0.35` を超えると、短いタグ列ではなく詳細なキャプションをプロンプトに使用しない限りアーティファクトが生じることがあります。テンプレートの既定値は `denoise` = `0.33` です。

<CardGroup cols={2}>
  <Card title="Comfy Cloud で実行" icon="cloud" href="https://cloud.comfy.org/?template=utility_z_image_turbo_2k_upscaler.app&utm_source=docs&utm_medium=referral&utm_campaign=image-upscale">
    Comfy Cloud で開く
  </Card>
  <Card title="ワークフローをダウンロード" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_z_image_turbo_2k_upscaler.app.json">
    JSON をダウンロードするか、テンプレートライブラリで &quot;Image Upscale: Z-Image-Turbo 2K&quot; を検索してください
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="fighter_jet.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/fighter_jet.png">
    `LoadImage` ノード 77 · `fighter_jet.png`
  </Card>
</CardGroup>
