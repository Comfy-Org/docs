...
>
> MoGe-3 は MoGe-2 のアーキテクチャを維持しつつ、自己誘導型のスパース volumetric リファインメント段を追加し、細粒度のポイントマップ ジオメトリを大幅に改善します。ViT-L チェックポイント（`moge_3_vitl_fp16`）と、より大きな ViT-g チェックポイント（`moge_3_vitg_fp16`）の2種類があります。
...
- **MoGe-2（推奨）**：[moge_2_vitl_normal_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_2_vitl_normal_fp16.safetensors)
- **MoGe-1（ベースライン）**：[moge_1_vitl_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_1_vitl_fp16.safetensors)
- **MoGe-3（最高ディテール）**：[moge_3_vitg_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_3_vitg_fp16.safetensors)（1.25B）または [moge_3_vitl_fp16.safetensors](https://huggingface.co/Comfy-Org/MoGe/blob/main/geometry_estimation/moge_3_vitl_fp16.safetensors)（370M）
...
MoGe-3 チェックポイント限定：`Run MoGe Inference` と `Run MoGe Panorama Inference` ノードには、予測深度に対してスパース volumetric リファインメントを実行する詳細設定 `refine_steps`（0〜8、デフォルト 3）があります。`0` でリファインメントを無効化でき、回数を増やすほどおおむね線形のコストで細部とエッジがシャープになります。MoGe-1 / MoGe-2 チェックポイントではこの設定は無視されます。
...
### MoGe 3: ジオメトリ推定 {#utility_moge3_geometry_estimation}

MoGe 3 で1枚の画像から細部まで精細なジオメトリを推定します。MoGe-3 のチェックポイントは粗いポイントマップをスパースボクセルシェルに持ち上げ、軽量なスパース3D畳み込みで深度を精細化するため、画像空間のデコーダではぼやけてしまう薄い構造や深度の不連続を保つことができます。

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/utility_moge3_geometry_estimation-1.webp" alt="MoGe 3ジオメトリ推定ワークフローのプレビュー" />

<CardGroup cols={2}>
  <Card title="Comfy Cloudで実行" icon="cloud" href="https://cloud.comfy.org/?template=utility_moge3_geometry_estimation&utm_source=docs&utm_medium=referral&utm_campaign=moge">
    Comfy Cloudで開く
  </Card>
  <Card title="ワークフローをダウンロード" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/utility_moge3_geometry_estimation.json">
    JSONをダウンロードするか、テンプレートライブラリで「Geometry Estimation: MoGe 3」を検索
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="cosmic_suitcase.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/cosmic_suitcase.png">
    `LoadImage`ノード3 · `cosmic_suitcase.png`
  </Card>
</CardGroup>

### 4.1 実行手順

1. `Geometry Estimation (MoGe 3)`サブグラフに MoGe 3 チェックポイントが選択されていることを確認します：テンプレートは`moge_3_vitg_fp16.safetensors`を使用し、同じドロップダウンで`moge_3_vitl_fp16.safetensors`も選択できます
2. `Load Image`ノードに画像を読み込みます
3. （任意）サブグラフノードの`output`を変更すると別のマップをレンダリングできます：デフォルトは`mask`で、他に`depth`、`depth_colored`、`normal_opengl`、`normal_directx`があります
4. `Queue`をクリックするか、`Ctrl(⌘)+Enter`を押して実行します
