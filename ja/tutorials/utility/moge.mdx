...
このチュートリアルのワークフロー例（PNG画像）にはワークフローJSONがメタデータとして含まれています：
- ComfyUI に直接ドラッグ＆ドロップできます
- またはメニュー `Workflows` -> `Open (ctrl+o)` で開けます

ワークフローを読み込むと、必要なモデルのダウンロードが促されます。生成された `.glb` モデルは `ComfyUI/output/mesh` フォルダに出力されます。
</Tip>
