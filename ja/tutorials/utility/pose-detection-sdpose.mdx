...
ComfyUIを最新バージョンに更新してから、`ワークフロー` -> `テンプレートを参照` に移動し、ユーティリティカテゴリのSDPoseワークフローを見つけてください。### SDPose: 画像内複数人物検出 (`utility_sdpose_multi_person`)
...
</div>### SDPose: ビデオ複数人物検出 (`utility_sdpose_multi_person_video`)
...
</div>### SDPose-OOD: 画像からポーズマップ (`utility_sdpose_ood_image_to_pose`)
...
</div>### SDPose-OOD: 動画からポーズマップへ (`utility_sdpose_ood_video_to_pose_map`)
