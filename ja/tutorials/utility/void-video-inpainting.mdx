...
description: "Netflix がオープンソース化した VOID ビデオ修復モデルを使って、動画からオブジェクトを削除する方法を学びます"
---
...
VOID（Video Object Inpainting and Deletion）は、Netflix が 2026年5月14日にオープンソース化した強力なビデオ修復モデルです。2パス拡散パイプラインを採用し、動画からオブジェクトを削除し、時間的に一貫性のあるコンテンツで穴を埋めます。
...
[VOID モデル - GitHub](https://github.com/Netflix/void-model)
...
VOID は、動画から不要なオブジェクトを削除しながら、自然な動き、照明、シーンの一貫性を維持します。上の例は、シーンからオブジェクトを削除した後の処理結果を示しています。

### 主な強み

- **単なる単一フレームの修正ではない真のオブジェクト削除** — クリップ全体で一貫性のある動きと照明を生成
- **インタラクション認識シーム修復** — オブジェクト（および近隣要素への影響）が**最初から存在しなかった**かのように遮蔽ピクセルが補完され、影や重なりなどの因果的手がかりが単純な消去よりも自然に見える
...
- **SAM3 オブジェクトプロンプト** — **削除したいもの**の短いラベル
