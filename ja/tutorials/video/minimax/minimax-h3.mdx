...
3つのサンプルテンプレートは氷山の一角にすぎません。ネイティブの MiniMax H3 ノードは、任意のフレームへのガイドアンカー、プロンプトエンベディング、潜在ノイズマスクにも対応しています。これらのワークフローを使用するには、ComfyUI を最新バージョンに更新してください。

### 任意のフレームにガイドをアンカーする

サンプルテンプレートは、動画の最初と最後のフレームにのみキーフレームをアンカーします。`MiniMaxH3AddGuide` ノードはこの制限を解除します。連続した時間軸上の任意のフレームにガイドをアンカーできます。

MiniMax H3 ノードの conditioning 出力と latent 出力を `MiniMaxH3AddGuide` に接続し、ガイド入力を少なくとも1つ指定します：
...
MiniMax H3 は、ビデオとオーディオの両方の潜在表現でノイズマスクをサポートしています。通常どおりマスクをサンプラーの `denoise_mask` 入力に接続します。マスクされた領域は再生成され、残りは固定されたままです。ビデオマスクはモデルの2x2潜在パッチグリッドに、オーディオマスクは潜在フレーム全体にスナップされます。部分的なインペイント、オブジェクトの削除、または既存のコンテンツを安定させたままクリップを拡張する場合に使用します。
