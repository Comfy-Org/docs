...
バックエンドを切り替えるには、組み込みの **Model Attention Backend** ノード（カテゴリ `model/patch`）をワークフローに追加し、`UNETLoader` と `BasicGuider` ノードの間に接続して、バックエンドを `comfy kitchen attention` に設定します。または、`--use-ck-attention` フラグ付きで ComfyUI を起動してください。このバックエンドは Nvidia および AMD GPU で利用可能で、ComfyUI に同梱されているため追加インストールは不要です。該当ブロックを bfloat16 アテンションで実行することでもアーティファクトは解消されますが、レンダリング時間が長くなります。
