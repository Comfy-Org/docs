...
サンプルテンプレート以外にも、ネイティブの MiniMax H3 ノードは任意のフレームへのガイドアンカー、プロンプトエンベディング、潜在ノイズマスクをサポートしています。これらの機能は最近の ComfyUI アップデートで追加されました：[任意フレームへのガイドアンカー](https://github.com/Comfy-Org/ComfyUI/pull/15439)、[プロンプトエンベディング](https://github.com/Comfy-Org/ComfyUI/pull/15697)、[潜在ノイズマスク](https://github.com/Comfy-Org/ComfyUI/pull/15375)。使用するには ComfyUI を最新バージョンに更新してください。

### 任意のフレームにガイドをアンカーする

サンプルテンプレートは、動画の最初と最後のフレームにのみキーフレームをアンカーします。`MiniMaxH3AddGuide` ノードはこの制限を解除します。連続した時間軸上の任意のフレームにガイドをアンカーできます。
...
MiniMax H3 は、プロンプトでの ComfyUI 標準の `embedding:` 構文をサポートしています。エンベディングファイルを `ComfyUI/models/embeddings/` に配置し、プロンプトで名前を指定して参照します（例：`embedding:my_embedding`）。エンベディングは、他の ComfyUI モデルと同様に読み込まれ、テキスト conditioning に混ぜられます。

### 潜在ノイズマスクによるインペイントと拡張

MiniMax H3 は、ビデオとオーディオの両方の潜在表現でノイズマスクをサポートしています。通常どおりマスクをサンプラーの `denoise_mask` 入力に接続します：`0` は対応する潜在領域を保持し、`1` は再生成します。ビデオマスクはモデルの2x2潜在パッチグリッドに、オーディオマスクは潜在フレーム全体にスナップされます。部分的なインペイント、オブジェクトの削除、または既存のコンテンツを安定させたままクリップを拡張する場合に使用します。
