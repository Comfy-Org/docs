...
これら 3 つはサンプルテンプレートであり、網羅的なリストではありません。モデルは、ネイティブの MiniMax H3 ノードを通じて、より多くの生成モードをサポートしています：`MiniMaxH3ImageToVideo` ノードによる先頭/末尾フレームの画像からビデオ生成（fl2va）、`MiniMaxH3ReferenceToVideo` ノードによる画像・ビデオ・オーディオ参照駆動の生成（ref2va）。これらのノードで追加のワークフローを構築することもできます。

<UpdateReminder/>
<Note>
H3 のオープンウェイトを使用すると、モデルをローカルで実行できます。ローカルで生成された出力の商用利用には、[MiniMax 商用ライセンス](https://comfy.org/minimax/license)が必要です。このライセンスは、唯一の公式リセラーである Comfy から入手できます。Comfy Cloud での生成には、商用利用権がすでに含まれています。
</Note>
