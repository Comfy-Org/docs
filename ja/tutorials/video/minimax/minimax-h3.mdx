...
translationSourceHash: 07164c68
...
translationBlockHashes:
  ...
  "ComfyUI Native Workflows": 64bad94a
...
ComfyUIは MiniMax H3をネイティブにサポートします。テンプレートライブラリには現在、5 つのサンプルワークフローが用意されています：
...
- **Fun ControlNet Union**: Canny、Depth、HED、MLSD、Pose のコントロールビデオで H3 を動かす、またはマスクによるビデオインペイント

これら 5 つはサンプルテンプレートであり、モデルの全機能を網羅したものではありません。ネイティブの MiniMax H3 ノードを使うと、より多くの生成モードに対応できます：`MiniMaxH3ImageToVideo` ノードによる先頭/末尾フレームの画像からビデオ生成（fl2va）、`MiniMaxH3ReferenceToVideo` ノードによる画像・ビデオ・オーディオ参照駆動の生成（ref2va）。これらのノードで追加のワークフローを構築することもできます。
...
テキストだけではなくコントロールビデオで MiniMax H3 動画のモーションを制御します。Fun ControlNet Union パッチ（[alibaba-pai/MiniMax-H3-Fun-Controlnet-Union](https://huggingface.co/alibaba-pai/MiniMax-H3-Fun-Controlnet-Union)、[Comfy-Org/MiniMax-H3](https://huggingface.co/Comfy-Org/MiniMax-H3) にパッケージ済み）は単一のチェックポイントで、Canny、Depth、HED、MLSD、Pose のコントロールビデオを条件として H3 に与えられるほか、マスクによるビデオインペイントにも対応します。`fl2va` と `ref2va` の両方の transformer ファイルで動作します。
...
以下のファイルを対応する `Load Video` ノードにアップロードしてください：
...
<CardGroup cols={2}>
  <Card title="dancer_field_pose.mp4" icon="video" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/dancer_field_pose.mp4">
    サンプルワークフローで使用する Pose コントロールビデオ。自分のビデオでもかまいません。
  </Card>
</CardGroup>

このテンプレートには ComfyUI 0.35.0 以降が必要です。サンプルは Pose コントロールビデオを使用します。内蔵の SDPose サブグラフが入力ビデオからポーズスケルトンを抽出し、出力がダンサーの動きを再現します。前処理済みのコントロールビデオを `Apply MiniMax H3 Fun ControlNet` ノードの `control_video` 入力に直接接続することもできます。
...
<CardGroup cols={2}>
  ...
  <Card title="ControlNet Patch: minimax_h3_fun_controlnet_union_pruned_int8_convrot" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/model_patches/minimax_h3_fun_controlnet_union_pruned_int8_convrot.safetensors">
    <code>ComfyUI/models/model_patches/</code> に配置
  </Card>
  <Card title="Text Encoder: qwen3vl_32b_minimax_h3_nvfp4_awq" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/text_encoders/qwen3vl_32b_minimax_h3_nvfp4_awq.safetensors">
  ...
  <Card title="LoRA: minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    オプションの Lightning LoRA ターボモード用。<code>ComfyUI/models/loras/</code> に配置
  </Card>
  <Card title="Checkpoint: sdpose_wholebody_fp16" icon="download" href="https://huggingface.co/Comfy-Org/SDPose/blob/main/checkpoints/sdpose_wholebody_fp16.safetensors">
    内蔵 SDPose サブグラフが使用するポーズ抽出器。<code>ComfyUI/models/checkpoints/</code> に配置
  </Card>
  <Card title="Detector: rt_detr_v4-x-hgnet_fp16" icon="download" href="https://huggingface.co/Comfy-Org/SDPose/blob/main/diffusion_models/rt_detr_v4-x-hgnet_fp16.safetensors">
    内蔵 SDPose サブグラフが使用する人物検出器。<code>ComfyUI/models/diffusion_models/</code> に配置
  </Card>
</CardGroup>
...
1. **ビデオの長さ**：ターゲットビデオは 5 〜 15 秒で、24fps の 17n+5 フレームグリッドに揃えられます（124 フレーム = 5 秒）。コントロールビデオがターゲットより長い場合は先頭フレームにトリミングされ、短い場合は最終フレームが保持されます。コントロールビデオの長さに正確に合わせるには、サンプルで計算された `batch_size` 出力を `MiniMax H3 Reference to Video` ノードの length 入力に接続します
2. **ガイダンスと強度**：`guidance_scale` は 1.0 のままにしてください。出力がコントロールから逸れる場合のみ、パッチの `strength` を 1 以上に上げます
3. **ビデオインペイント**：`mask` 入力にマスクを接続します。`1` とマークされた領域はオプションの `source_video` 入力の制御下で再生成され、フレームの残りの部分は固定されます
4. **ターボモード（オプション）**：**Lightning LoRA** チェックボックスを有効にすると、4 ステップのターボ LoRA で生成が大幅に高速化されます。オーディオとモーションの品質はわずかに低下します
5. **コントロールビデオの準備**：ComfyUI には各コントロールタイプ用の前処理ノードが内蔵されています。エッジは [Detect Edges (Canny) ノード](https://docs.comfy.org/built-in-nodes/Canny)、デプスマップは [Depth Anything 3 チュートリアル](https://docs.comfy.org/tutorials/utility/depth-anything-3) を参照してください
