---
title: "ComfyUI で MiniMax H3 を使用：T2V、I2V、R2V ビデオワークフロー"
description: "オープンウェイトのMiniMax H3をComfyUIで使用する方法を学びます。テキストから動画、画像から動画、参照から動画へのネイティブワークフローを備え、ステレオオーディオに対応。"
...
テンプレートは高速なプレビューサイズになっています。フル品質で出力するには、16:9 でメガピクセルを約 `1.0` に上げると、1376x768 になります。これは H3 のネイティブキャンバス（短辺 768px）です。
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA (オプション): minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors">
    オプションの Lightning LoRA ターボモードでのみ必要です。<code>ComfyUI/models/loras/</code> に配置してください
  </Card>
</CardGroup>
...
3. **解像度**: H3 のネイティブキャンバスは短辺 768px で、解像度は 32 の倍数に丸められます
...
6. **ターボモード（オプション）**: このワークフローには、高速生成用の Lightning LoRA（`minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16`）が含まれています。MiniMax H3 ノードで `turbo_mode` を有効にすると、20 ステップではなく 8 ステップで生成されます。画質はわずかに低下します。
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA (オプション): minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    オプションの Lightning LoRA ターボモードでのみ必要です。<code>ComfyUI/models/loras/</code> に配置してください
  </Card>
</CardGroup>
...
6. **ターボモード（オプション）**: ワークフローの **Lightning LoRA** チェックボックスを有効にすると、`ref2va` モデルをフルの 20 ステップではなく 4 ステップのターボ LoRA（`minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16`）で実行できます。大幅に高速化されますが、画質はわずかに低下します。
