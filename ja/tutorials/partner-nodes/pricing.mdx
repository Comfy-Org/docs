...
| BytePlus Video Generation Product（Kトークンあたり）                    | model: dreamina-seedance-2-0, resolution: 1080P, video_type: text-to-video (動画入力なし：テキストまたは画像)               | 2.323 / Kトークン      | Video    |
| BytePlus Video Generation Product（Kトークンあたり）                    | model: dreamina-seedance-2-0, resolution: 1080P, video_type: image-to-video (動画入力なし：テキストまたは画像)              | 2.323 / Kトークン      | Video    |
| BytePlus Video Generation Product（Kトークンあたり）                    | model: dreamina-seedance-2-0, resolution: 1080P, video_type: video-to-video (動画入力あり)                             | 1.418 / Kトークン      | Video    |
...
| BytePlus Video Generation Product（Kトークンあたり）                    | model: dreamina-seedance-2-0-mini, resolution: 720P, video_type: text-to-video (動画入力なし：テキストまたは画像)          | 1.056 / Kトークン      | Video    |
| BytePlus Video Generation Product（Kトークンあたり）                    | model: dreamina-seedance-2-0-mini, resolution: 720P, video_type: image-to-video (動画入力なし：テキストまたは画像)         | 1.056 / Kトークン      | Video    |
| BytePlus Video Generation Product（Kトークンあたり）                    | model: dreamina-seedance-2-0-mini, resolution: 720P, video_type: video-to-video (動画入力あり)                         | 0.634 / Kトークン      | Video    |

> **Seedance 2.0 トークン計算方式：** 出力トークン = (入力ビデオ時間 + 出力ビデオ時間) × 出力ビデオの長さ × 出力ビデオの幅 × 出力ビデオのフレームレート ÷ 1024。テキスト、画像、またはオーディオが入力の場合、入力時間は 0 として計算します。Seedance 2.0 の出力フレームレートは 24fps です。動画から動画への入力には、最低トークン使用量の制限が適用されます。**Kトークンあたりのクレジット**は、モデル、出力解像度、および参照ビデオが使用されるかどうかによって異なります。`Seedance 2.0 Fast` および `Seedance 2.0 Mini` は 480P と 720P をサポートしています。標準の `Seedance 2.0` はさらに 1080P と 4K をサポートしています。480P は 720P と同じ Kトークンあたりのクレジットレートを使用します。
