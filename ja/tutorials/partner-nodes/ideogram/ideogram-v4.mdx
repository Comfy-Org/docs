...
2. **構造化 JSON**: スタイルパレット、要素配置（`bbox`）、画像内テキストレンダリングを細かく制御。ポスター風の構図に推奨
...
- **ネガティブプロンプトは不要**: Ideogram 4.0 は非対称の分類器不要ガイダンス（asymmetric classifier-free guidance）を使用しており、無条件パスではテキストトークンを破棄するため、別途ネガティブプロンプト文字列は必要ありません。
