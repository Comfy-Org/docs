...
2. **モデルバリアントを選択** : **Krea 2 Medium**（イラスト）または **Krea 2 Large**（フォトリアリズム）から選択
3. **パラメータを調整** : アスペクト比、解像度、クリエイティビティレベル、シード値を設定
4. **キューをクリック** : `Ctrl+Enter` またはキューボタンをクリックして生成
...
スタイル参照ワークフローでは、1 枚以上の参照画像をアップロードして美的スタイル、ムード、ビジュアルの方向性を定義します。生成される画像はプロンプトに沿いながら、参照画像のスタイルが適用されます。
...
1. **参照画像をアップロード** — **LoadImage** ノードでスタイル参照画像を読み込みます（最大 10 枚）
2. **プロンプトを入力** — **Krea2ImageNode** で生成したいシーンを説明
3. **モデルバリアントを選択** — Medium または Large を選択
4. **スタイル強度を調整** — **Krea2StyleReferenceNode** ノードでスタイル参照の強度を調整
5. **キューをクリック** — `Ctrl+Enter` で生成
