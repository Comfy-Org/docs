...
- `background` オプションによる透明背景の出力（合成用）

GPT-Image-2 は、ノードライブラリの **OpenAI GPT Image 2.5** ノードで `model` オプションとして選択できます。このノードは OpenAI の画像生成 API を同期的に呼び出し、説明に一致する画像を返します。
...
2. ノードライブラリで **OpenAI GPT Image 2.5** を検索し、ノードを追加します。
