...
[Beeble SwitchX](/ja/built-in-nodes/BeebleSwitchXImageEdit) パートナーノードを使用すると、元の被写体を保持しながら、既存の画像や動画の環境、照明、スタイルを変換できます。被写体画像と参照環境をアップロードするだけで、SwitchX が被写体を新しいシーンにリアルな照明と影で合成します。

## Beeble SwitchXが得意なこと

- **環境置換**: 画像またはビデオの背景環境を参照シーンに置き換えつつ、オリジナルの被写体を保持します
- **リアルなリライティング**: 適切な照明、影、パースペクティブを伴って、新しい環境に被写体を合成します
- **モーションを保持したビデオ編集**: ビデオ内のマスクされた被写体領域にリライティングを適用し、マスクされていない領域はオリジナルのピクセルとモーションを維持します
- **合成対応の出力**: 画像ワークフローはマスク済みの切り抜きを保存し、ビデオワークフローは、合成を容易にするための独立したアルファマットを生成します
