...
ノードは被写体を新しい環境に適切な照明、影、パースペクティブで自然に合成した出力を生成します。

### 実行手順

1. **被写体画像をアップロード** : 最初の **LoadImage** ノードで被写体画像を読み込み（例：ヘッドフォンを着けた人物）
2. **参照画像をアップロード** : 2 つ目の **LoadImage** ノードでターゲット環境/スタイルを読み込み
3. **プロンプトを入力** : **BeebleSwitchXImageEdit** ノードでシーンを説明
4. **解像度を設定** : 出力解像度を選択
5. **シードを調整** : シードを変更して異なる結果を試す
6. **キューをクリック** : `Ctrl+Enter` で生成
...
3. **動画範囲を設定** — **Video Slice** ノードで処理するフレーム範囲を指定（開始、終了を含む）
4. **プロンプトを入力** — **BeebleSwitchXVideoEdit** ノードで希望の環境と照明を説明
5. **解像度を設定** — 出力解像度を選択
6. **シードを調整** — シードを変更して異なる結果を試す
7. **キューをクリック** — `Ctrl+Enter`
