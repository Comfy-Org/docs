...
Ideogram 4.0 には安全フィルターが組み込まれています。JSON 以外（プレーンテキスト）のプロンプトは、誤検出率が高く、NSFW として誤ってフラグ付けされる可能性が高くなります。プロンプトがブロックされる可能性を減らすには：

- **構造化 JSON プロンプトを使用する** — モデルは JSON でトレーニングされているため、より正確に理解され、誤検出が減少します。
- プロンプトがブロックされた場合は、JSON プロンプトに書き直してみてください。
