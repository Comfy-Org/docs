...
2. `File → Export Workflow (API)` でエクスポートする

自動化が必要な場合は、各ノードの `x`、`y`、`幅` フィールドを削除し、ルート JSON から `グループ` と `extra` セクションを取り除くスクリプトを作成できます。

## 関連ページ

- [API 概要](/ja/development/api-development/overview) — クラウドAPI と サーバーAPI の比較
...
- [APIキーの取得](/ja/development/api-development/getting-an-api-key) — クラウドAPI と パートナーノードに必須
