...
構造に注目：
- **トップレベルのキー**は数値ノード ID（`"3"`、`"4"`、`"8"`）
- 各ノードは `inputs`（リンクは `[node_id, output_index]` 形式）と `class_type` を含む
- `x`、`y`、`width` フィールドなし — UI のみのデータは削除済み

API 形式のワークフローをフロントエンドに読み込むこともできます（`File → Load` またはドラッグ＆ドロップ）が、ノード位置は自動配置されます。
