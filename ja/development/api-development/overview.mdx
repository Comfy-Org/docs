...
| **ベースURL** | `https://cloud.comfy.org` | `http://localhost:8188` |
| **プロトコル** | REST + WebSocket | REST + WebSocket |
| **料金** | サブスクリプション制 | 無料（自分の計算リソース）|
| **用途** | 迅速な統合、インフラ不要 | 完全制御、カスタムハードウェア・モデル |

両 API は同じワークフロー形式（"API 形式"）を使用するため、ローカルで開発・テストしてからクラウドに移行しても変更は不要です。
...
<CardGroup cols={3}>
  ...
  <Card title="ComfyUI Server API" icon="server" href="/ja/development/comfyui-server/server-guide">
