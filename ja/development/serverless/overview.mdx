...
icon: "cloud-arrow-up"
translationSourceHash: 16a4f4cc
...
<Warning>
  Comfy API はベータ版です。[platform.comfy.org](https://platform.comfy.org) からサインアップしてアクセスをリクエストしてください。
</Warning>

Comfy API は、ComfyUI ワークフローにマネージド URL とオンデマンドの GPU キャパシティを提供します。新しい Build and Deploy CLI は、ビルド定義をプロジェクト内に保持し、その定義からリリースを作成し、トラフィックを受け付ける準備ができたらリリースをデプロイします。
