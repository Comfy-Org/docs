...
<ParamField body="stream_options.include_usage" type="boolean">Deprecated: このフィールドは効果がありません。完全な使用状況の詳細は常に含まれます。
</ParamField>
...
Router が `GET /v2/models/openrouter/chat-completions/openapi.json` で提供するスキーマ、つまりリクエストがプロバイダーに到達する前に呼び出しを検証するのと同じドキュメントから生成済みです。### 出力
