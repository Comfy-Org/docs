...
```
  ...
  </Tab>  <Tab title="キューに送信し、後で収集">
同じボディを `POST https://api.comfy.org/v2/models/recraft/recraftv4_styles_pro_vector/requests` に送信します。Router は実行が受け付けられ次第 `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスからでも別のプロセスからでも取得できます。ステータス、キャンセル、収集の流れは [Queued delivery](/ja/development/comfy-router/queue) で説明しています。
...
# 3. 取得します。モデル固有の出力とともに 200、まだ実行中はステータスボディとともに 202 を返します。
