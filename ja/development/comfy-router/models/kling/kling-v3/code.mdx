...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router) でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用します。cURL のスニペットは、raw HTTP 経由の同じ呼び出しです。
...
同じボディを `POST https://api.comfy.org/v2/models/kling/kling-v3/requests` に送信します。Router は実行が受け付けられ次第 `201` と `request_id` を返し、結果は準備が整った時点で、このプロセスからでも別のプロセスからでも取得できます。[キュー配信](/ja/development/comfy-router/queue) では、ステータス、キャンセル、結果の取得について説明しています。
...
```bash cURL
# 1. 送信。Router は request_id、status_url、response_url、cancel_url を添えて 201 を返します。
...
- **Comfy**（デフォルト）: `POST https://api.comfy.org/v2/models/kling/kling-v3`
- **Higgsfield**、`higgsfield/higgsfield-kling-3-std` として: `POST https://api.comfy.org/v2/models/kling/kling-v3?model_provider=higgsfield`

`strict_mode` のデフォルトは false であるため、Router はこのページに記載されているネイティブのリクエストボディをプロバイダー独自のスキーマに変換し、レスポンスを元の形式に変換して返します。このようにルーティングされるすべてのモデルについては、API リファレンスの [`model_provider`、`strict_mode`、`fallback_provider`](/ja/development/comfy-router/reference#post-v2modelsprovidermodel) と、[提供プロバイダー](/ja/development/comfy-router/providers) を参照してください。
