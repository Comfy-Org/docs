...
同じボディを `POST https://api.comfy.org/v2/models/byteplus/seedream-5-0-260128/requests` に送信します。Router は実行が受け付けられ次第 `201` と `request_id` を返し、結果は準備が整った時点で、このプロセスからでも別のプロセスからでも取得できます。ステータス、キャンセル、結果の取得の詳細は [Queued delivery](/ja/development/comfy-router/queue) を参照してください。
...
# 2. 各レスポンスが指定する Retry-After の秒数だけ待機しながら、ステータスが COMPLETED になるまでポーリングします。
...
<ParamField body="size" type="string">
  "seedream-4-0-250828": 生成する画像の仕様を設定します。2 つの方法が利用可能ですが、併用はできません。
...
layer_decomposition を有効にした "seedream-5-0-pro-260628": 解像度による方法のみがサポートされます。指定可能な値: 1K、1.5K、2K、auto。デフォルト: auto。
  ベース画像は、オリジナルの入力画像のアスペクト比で、指定した解像度で出力されます。各レイヤーは、オリジナル画像でのアスペクト比を維持しつつ、指定した解像度に近いサイズで出力されます。
...
<ParamField body="stream" type="boolean" default="false">
  Comfy Router は、完全な JSON 結果を取得するため、明示的に指定された stream フラグを false に確定します。v1 プロキシでは、このフィールドはストリーミング出力モードを有効にするかどうかを制御します。このパラメータをサポートするのは seedream-5.0-lite、4.5、4.0 のみです (seedream-5.0-pro ではサポートされていません)。false = すべての出力画像が一度に返されます。true = 各出力画像は生成され次第すぐに返されます。
</ParamField>
...
`GET /v2/models/byteplus/seedream-5-0-260128/openapi.json` で Router が提供するスキーマから生成されています。これは、リクエストがプロバイダーに到達する前に呼び出しを検証する際に使用されるのと同じドキュメントです。### 出力
