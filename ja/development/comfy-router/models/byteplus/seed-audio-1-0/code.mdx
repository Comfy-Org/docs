...
# 3. 収集。200 はモデルのネイティブ出力、まだ実行中は 202 とステータスボディを返します。
...
<ParamField body="model" type="string">
  モデル識別子。対応モデル: seed-audio-1.0 および seed-audio-1.0-multilingual。POST /proxy/byteplus/api/v3/tts/create への直接の v1 呼び出しでは必ず指定する必要があります。プロキシはそれ以外の値や省略された値を 400 で拒否します。このスキーマの `required` リストに含まれていないのは、Comfy Router が /v2/models/byteplus/\{model} の `{model}` パスセグメントからこれを設定するためであり、Router の呼び出し元は省略します。
</ParamField>

<ParamField body="references" type="object[]">
  参照リソース。テキストのみの生成では省略します。最大 3 件のオーディオ参照（それぞれ最大 30 秒、デコード後 10 MB。wav、mp3、pcm または ogg_opus）またはちょうど 1 件の画像参照（デコード後最大 10 MB。jpeg、png または webp）。画像参照とオーディオ参照は混在できません。これらの上限は BytePlus のもので、デコード後のバイト数で示されています。base64 はアセットを約 4/3 に膨らませ、Comfy Router の呼び出しは JSON ドキュメント全体で 100 MiB に制限されているため、/v2/models/byteplus/\{model} に送信されるインラインの `audio_data`/`image_data` 参照は、転送制限で拒否されるまでにおよそ 75 MB（デコード後）を運ぶことができます。したがって、単一のインライン参照については BytePlus 自身の参照あたり 10 MB という上限が最初に効きます。そして、その上限の 3 件のオーディオ参照は 1 つのボディに収まります。大きなアセットをリクエストから完全に除外するには `audio_url`/`image_url` を使用してください。
</ParamField>
...
<ParamField body="references[].speaker" type="string">
  ボイス ID。サポートされている Doubao TTS ボイス、またはボイスクローン ID を指定できます。
</ParamField>
...
Router が `GET /v2/models/byteplus/seed-audio-1.0/openapi.json` で提供するスキーマから生成済みです。これは、プロバイダーにリクエストが到達する前に Router が呼び出しを検証する際に使用するものと同じドキュメントです。
