...
HappyHorse 1.0 T2V の API リファレンス。HappyHorse 1.0 のテキストから動画への生成は、プロンプトのみから同期したオーディオ付きの短いビデオを作成します。
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力ありの場合は
  ...
  モデルが適切な再生時間を選ぶスマート再生時間モードが有効になります
...
スキーマから生成されたもので、Router が `GET /v2/models/wan/happyhorse-1.0-t2v/openapi.json` で提供するドキュメントと同じです。これはリクエストがプロバイダーに到達する前に、呼び出しを検証する際に使用されるものと同じドキュメントです。
### 出力
