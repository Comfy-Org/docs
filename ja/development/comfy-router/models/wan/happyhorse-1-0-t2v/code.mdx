...
キーを[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys)で作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、raw HTTP 経由で同じ呼び出しを行います。
