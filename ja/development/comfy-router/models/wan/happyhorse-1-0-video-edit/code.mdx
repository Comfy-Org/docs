---
title: "Comfy Router で HappyHorse 1.0 Video Edit を使う"
description: "Comfy Router 経由で wan/happyhorse-1.0-video-edit を HTTP で呼び出すための Python、TypeScript、cURL スニペット、およびリクエストフィールドと結果の形状"
sidebarTitle: "HappyHorse 1.0 Video Edit"
translationSourceHash: 7de4abf1
translationFrom: development/comfy-router/models/wan/happyhorse-1-0-video-edit/code.mdx
translationBlockHashes:
  "_intro": 43ae28bb
  "Quick start": 8e9f5e21
  "Schema": 932e3f12
  "Examples": a41df668
---
...
HappyHorse 1.0 Video Edit の API リファレンス。HappyHorse 1.0 video edit は、テキスト指示に基づいて既存のクリップを書き換えます。任意で参照画像によるガイドも利用できます。

## クイックスタート

[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python、TypeScript、Swift のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`、および [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift パッケージ）を使用しています。cURL スニペットは、生の HTTP で同じ呼び出しを行うものです。

**モデル ID:** `wan/happyhorse-1.0-video-edit`

**エンドポイント:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-video-edit`
...
同じボディを `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-video-edit/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集されます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集について順を追って説明しています。
...
<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセット一覧。ビデオ生成のための参照素材（画像、オーディオ、ビデオ）
を指定します。各要素には type と url フィールドが含まれます。
...
- wan3.0-video: first_frame（最大1）、last_frame（最大1）、reference_image（最大10）、
  ...
  合計再生時間 \<= 15秒）、file（最大1、link とは併用不可）、link（最大1、file とは併用
  不可）。reference_*/file/link タイプと first_frame/last_frame タイプは、同じリクエスト内で
  同時に使用できません。配列の順序が、プロンプト内でのアセットの参照順序（画像1、ビデオ1、オーディオ1、...）
  を定義します。
...
複数の参照ビデオを指定する wan2.6-r2v では、'character1'、'character2' などを使って
参照ビデオの順に被写体を参照します。例:「キャラクター1が道端で歌い、キャラクター2がその横で踊る」
wan3.0-video の参照モードでは、'Image 1'、'Video 1'、'Audio 1' などを使って、
media 配列内の対応する順序のメディアアセットを参照します。
</ParamField>
...
- 本数: 1～3本
- 1本あたりのビデオ長: 2～30秒
- 1ファイルあたりのファイルサイズ: 最大30MB
- audio_url とは併用できません
参照可能な再生時間: ビデオ1本の場合は最大5秒、2本の場合はそれぞれ最大2.5秒、3本の場合は比例して短くなります。
課金: 実際に使用した参照再生時間に基づきます。
</ParamField>

<ParamField body="input.template" type="string">
  ビデオエフェクトのテンプレート名。任意です。現在サポートされているのは squish、flying、carousel です。使用した場合、prompt パラメータは無視されます。
</ParamField>

<ParamField body="model" type="string">
  呼び出すモデルのID。このコンポーネントでは制約されません: Comfy Router が `POST /v2/models/wan/{model}` の `{model}` パスセグメントから設定するため、Router の呼び出し元は省略します。`POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` への直接の v1 呼び出しでは、必ず指定する必要があります。受け付けられる表記の enum は、そのオペレーション自身のコンポーネントである `WanVideoGenerationRequest` に定義されています。
</ParamField>
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力ありの場合は、入力ビデオの再生時間 +
  出力ビデオの再生時間の合計が 30 秒を超えてはなりません。-1 を指定するとスマート再生時間モードが有効になり、
  モデルが適切な再生時間を選択します
...
wan3.0-video の場合、adaptive（デフォルト）が入力メディアの比率と意図に基づいて適切な
アスペクト比を自動的に推奨します。
...
このプロキシは、resolution と size のどちらも指定していないビデオ生成リクエストを拒否します。
...
wan2.6 および wan2.7-r2v モデル向け。
- single: シングルショットのビデオ（デフォルト）
- multi: マルチショットのビデオ
...
Router が `GET /v2/models/wan/happyhorse-1.0-video-edit/openapi.json` で提供するスキーマから生成されています。これは、リクエストがプロバイダーに到達する前に Router が呼び出しを検証する際に対象とするドキュメントと同じものです。
...
<ResponseField name="code" type="string">
  失敗したリクエストのエラーコード。`output` の下ではなくエンベロープのROOTで報告されます（リクエストが成功した場合は返されません）。
</ResponseField>

<ResponseField name="message" type="string">
  失敗したリクエストの詳細情報。`output` の下ではなくエンベロープのROOTで報告されます（リクエストが成功した場合は返されません）。`output.message` にフォールバックする前にこちらを確認してください。
</ResponseField>
