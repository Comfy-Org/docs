...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しています。cURL スニペットは、生の HTTP で同じ呼び出しを行うものです。
...
インラインでも余裕をもって収まります。したがって、1 つのアセットについてはパートナーの
ルールが効きます。転送上限は複数のアセットにまたがって効きます。100 MiB の Router の
上限は各要素ではなくリクエスト全体を制限するためです。これらの上限に近いものは
公開 URL として送信してください。
</ParamField>
...
<ParamField body="parameters.ratio" type="string">
  生成されるビデオのアスペクト比率。wan2.7 および wan3.0 モデル専用です。
wan2.7 モデルの場合、指定されなければ解像度ティアに基づいてデフォルトが決まります。
wan3.0-video の場合、adaptive（デフォルト）は入力メディアの比率と意図に基づいて
適切なアスペクト比率を自動的に推奨します。
