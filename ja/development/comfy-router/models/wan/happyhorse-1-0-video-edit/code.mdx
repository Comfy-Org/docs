...
translationSourceHash: c30d92df
...
- wan3.0-video: ビデオ入力がない場合は [2, 30] の整数。ビデオ入力がある場合は、入力ビデオの
  合計再生時間 + 出力ビデオの再生時間が30秒を超えてはなりません。-1 はスマート再生時間モードを
  有効にし、モデルが適切な再生時間を選択します
...
wan2.7モデルでは、指定しない場合、解像度ティアに基づいてデフォルトが決まります。
...
- wan2.7モデル（i2v、t2v、r2v、videoedit）: 720P、1080P（デフォルトは 1080P）
...
wan2.6 および wan2.7-r2v モデルで使用できます。
