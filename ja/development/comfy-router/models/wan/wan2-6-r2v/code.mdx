---
title: "Comfy Router で Wan 2.6 R2V を使う"
description: "Comfy Router 経由で HTTP から wan/wan2.6-r2v を呼び出すための Python、TypeScript、cURL スニペットと、リクエストフィールドおよび結果の形状"
...
Wan 2.6 R2V の API リファレンス。Wan 2.6 の参照から動画は、1～3 本の参照ビデオの被写体を新しいシーンへ持ち込みます。プロンプト内では character1、character2 などのように指定します。

## クイックスタート

[お使いの Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは同じ呼び出しを raw HTTP で行うものです。

**モデル ID:** `wan/wan2.6-r2v`

**エンドポイント:** `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v`
...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan2.6-r2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスまたは別のプロセスから収集されます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集について説明します。
...
# 3. 収集。モデル固有の出力とともに 200、まだ実行中はステータスボディとともに 202。
...
- wan3.0-video: first_frame（最大1）、last_frame（最大1）、reference_image（最大10）、
  ...
  reference_*/file/link タイプと first_frame/last_frame タイプは、同じリクエスト内で
  同時に使用できません。配列の順序が、プロンプト内でのアセットの参照順
...
転送上限にも達します。Comfy Router の POST body は合計で 100 MiB が上限で、
超えると 413 が返ります。base64 はペイロードを約 4/3 に膨らませるため、およそ
75 MB を超える単一のインラインアセットは、ここに示すパートナーのルールに到達する
前に拒否されます。そのサイズであれば、パートナー自身の上限である 20MB ちょうどの
単一アセットでもインラインで問題なく収まるため、単一のアセットではパートナーの
ルールが支配的になります。一方、複数のアセットにまたがる場合は転送上限が支配的に
なります。100 MiB の Router 上限は各要素ではなくリクエスト全体を制限するためです。
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または（上記 `media` の説明にあるそのモデルの項目が対応すると述べている場合、たとえば happyhorse-1.x の i2v と r2v の表記のように）インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、およびインラインペイロードを全体として制限する 100 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>

<ParamField body="input.negative_prompt" type="string">
  ネガティブプロンプトは、ビデオ画面に表示させたくないコンテンツを記述するために使用します
</ParamField>
...
- 1ファイルあたりのサイズ: 最大30MB
- audio_url とは併用できません
参照の再生時間: 1本の場合は最大5秒、2本の場合はそれぞれ最大2.5秒、3本の場合は比例してさらに短くなります。
課金: 実際に使用された参照再生時間に基づきます。
</ParamField>

<ParamField body="input.template" type="string">
  ビデオエフェクトのテンプレート名。任意です。現在サポートされている値: squish、flying、carousel。使用した場合、prompt パラメータは無視されます。
</ParamField>

<ParamField body="model" type="string">
  呼び出すモデルのID。このコンポーネントでは制約されません: Comfy Router が `POST /v2/models/wan/{model}` の `{model}` パスセグメントから設定するため、Router の呼び出し側は省略します。`POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` への直接の v1 呼び出しでは必ず指定する必要があり、受け付けられる表記の enum はそのオペレーション自身のコンポーネント `WanVideoGenerationRequest` にあります。
</ParamField>
...
スキーマから生成された、Router が `GET /v2/models/wan/wan2.6-r2v/openapi.json` で提供するドキュメントです。これは、リクエストがプロバイダーに到達する前に呼び出しを検証する際に使用されるものと同じドキュメントです。
