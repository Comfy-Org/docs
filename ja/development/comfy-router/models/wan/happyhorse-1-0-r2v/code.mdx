...
HappyHorse 1.0 R2V の API リファレンスです。HappyHorse 1.0 参照から動画は、1 つ以上の参照画像の被写体を新しいシーン全体で一貫させます。プロンプトでは character1、character2 などとして指定します。
...
<ParamField body="parameters.ratio" type="string">
  生成されるビデオのアスペクト比。wan2.7およびwan3.0モデル専用。
...
スキーマから生成。Router が `GET /v2/models/wan/happyhorse-1.0-r2v/openapi.json` で提供するドキュメントで、リクエストがプロバイダーに到達する前に呼び出しを検証する際に使用されるものと同じです。
### 出力
