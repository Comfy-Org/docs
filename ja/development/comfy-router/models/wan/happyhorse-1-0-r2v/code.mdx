...
[お使いの Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、raw HTTP 経由で同じ呼び出しを行うものです。
...
<ParamField body="input.img_url" type="string">
  先頭フレーム画像のURL、またはBase64エンコードされたデータ。
wan2.5-i2v-preview および wan2.6-i2v モデルでのみ必須です。happyhorse-1.x の i2v 表記は、ここでは先頭フレームを受け取りません。先頭フレームは type が `first_frame` の `media` 要素として渡します (下記 `media` を参照)。wan2.6-i2v で成功する `img_url` のボディも、happyhorse-1.0-i2v および happyhorse-1.1-i2v ではプロバイダーに拒否されます。画像フォーマット: JPEG、JPG、PNG、BMP、WEBP。解像度: 360～2000ピクセル。ファイルサイズ: 最大10MB。
</ParamField>
...
上記のアセットごとの「最大 20MB」は、最終的に受け取る画像に対するパートナーの
上限です。アセットが公開 URL の場合は記載どおりに適用されます。そのバイト列が
...
前に拒否されます。そのサイズであれば、パートナー自身の 20MB 上限にあるアセット
1 つは余裕をもってインラインに収まるため、1 つのアセットについてはパートナーの
ルールが効きます。一方、100 MiB の Router 上限は各要素ではなくリクエスト全体を
制限するため、複数のアセットにまたがる場合は転送上限のほうが効きます。これらの
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数、ビデオ入力ありの場合は入力ビデオの再生時間 + 出力ビデオの再生時間の合計が 30 秒を超えてはなりません。-1 を指定すると、モデルが適切な再生時間を選択するスマート再生時間モードが有効になります
...
wan3.0-video の場合、adaptive（デフォルト）は入力メディアの縦横比と意図に基づいて適切なアスペクト比を自動的に推奨します。
...
このプロキシは、resolution も size も指定されていないビデオ生成リクエストを拒否します。解像度ティアが課金レートを決定するためです。
