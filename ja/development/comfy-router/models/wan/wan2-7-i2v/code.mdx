...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan2.7-i2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集できます。[キューの配信](/ja/development/comfy-router/queue) では、ステータス、キャンセル、収集について順を追って説明します。
...
// models.run() が返すものと同じ結果です。失敗またはキャンセルされたリクエストは、ここで reject されます。
...
- wan3.0-video: ビデオ入力がない場合は [2, 30] の整数。ビデオ入力がある場合は、入力
  ビデオの再生時間と出力ビデオの再生時間の合計が 30 秒を超えてはなりません。-1 を指定すると
...
このプロキシは、resolution も size も指定していないビデオ生成リクエストを拒否します。
...
Router が `GET /v2/models/wan/wan2.7-i2v/openapi.json` で提供する schema から生成されています。これは、リクエストがプロバイダーに到達する前に呼び出しを検証する対象となるドキュメントと同じものです。
