...
前に拒否されます。このサイズであれば、パートナー自身の 20MB 上限にある単一の
アセットはインラインで余裕をもって収まります。そのため 1 つのアセットでは
パートナーのルールが適用されます。転送上限は複数のアセットにまたがって適用されます。
...
<ParamField body="parameters.audio_setting" type="string" default="&quot;auto&quot;">```
  wan2.7-videoedit モデル用のビデオオーディオ設定です。
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力ありの場合は、入力ビデオの再生時間 + 出力ビデオの再生時間が 30 秒を超えてはなりません。-1 を指定するとスマート再生時間モードが有効になり、モデルが適切な再生時間を選択します
...
wan2.7 モデルの場合、未指定であれば解像度ティアに基づいてデフォルトが決まります。
wan3.0-video の場合、adaptive（デフォルト）が入力メディアの比率と意図に基づいて適切なアスペクト比を自動的に推奨します。
