...
- wan3.0-video: first_frame（最大1）、last_frame（最大1）、reference_image（最大10）、
  ...
  同一リクエスト内では相互に排他的です。配列の順序がプロンプト内のアセットの参照順
...
アセットはインラインでも余裕をもって収まるため、1 つのアセットについてはパートナーの
ルールが効きます。転送上限は複数のアセットにまたがって効きます。100 MiB の Router
上限は各要素ではなくリクエスト全体を制限するためです。これらの上限に近い
...
このプロキシは、解像度とサイズのどちらも指定しないビデオ生成リクエストを拒否します。
