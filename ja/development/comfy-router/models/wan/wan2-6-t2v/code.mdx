...
description: "Comfy Router 経由で HTTP を通じて wan/wan2.6-t2v を呼び出すための Python、TypeScript、cURL スニペット、およびリクエストフィールドと結果の形状"
sidebarTitle: "Wan 2.6 T2V"
translationSourceHash: 8929e2a2
translationFrom: development/comfy-router/models/wan/wan2-6-t2v/code.mdx
translationBlockHashes:
  "_intro": 2f5a31f7
  "Quick start": ab58d10a
  "Schema": 144c40ee
  "Examples": 12f77722
---
...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python、TypeScript、Swift のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`、および [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift パッケージ）を使用しており、cURL のスニペットは同じ呼び出しを生の HTTP で実行するものです。
...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan2.6-t2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集できます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集の流れを説明しています。
...
# 3. 収集。実行中の場合は 202 とステータスボディ、完了時は 200 とモデルのネイティブ出力。
...
<ParamField body="input.img_url" type="string">
  先頭フレーム画像のURLまたはBase64エンコードデータ。wan2.5-i2v-preview および wan2.6-i2v モデルでのみ必須です。happyhorse-1.x の i2v 表記は、ここでは先頭フレームを受け取りません。先頭フレームは type が `first_frame` の `media` 要素として渡します (下記 `media` を参照)。wan2.6-i2v で成功する `img_url` のボディは、happyhorse-1.0-i2v および happyhorse-1.1-i2v ではプロバイダーに拒否されます。画像フォーマット: JPEG、JPG、PNG、BMP、WEBP。解像度: 360～2000ピクセル。ファイルサイズ: 最大10MB。
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセットリスト。ビデオ生成用の参照素材（画像、オーディオ、ビデオ）
...
Router が `GET /v2/models/wan/wan2.6-t2v/openapi.json` で提供するスキーマから生成されたもので、リクエストがプロバイダーに到達する前に Router が呼び出しを検証する際に使用する同一のドキュメントです。
...
<ResponseField name="output.video_url" type="string">
  完了したビデオ生成タスクのビデオURL。リンクの有効期間は 24 時間です
</ResponseField>
