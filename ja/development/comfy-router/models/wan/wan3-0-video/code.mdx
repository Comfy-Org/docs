---
title: "Comfy RouterでWan 3.0 Videoを使用する"
description: "Comfy Router経由でHTTPでwan/wan3.0-videoを呼び出すためのPython、TypeScript、cURLスニペット、およびリクエストフィールドと結果の形状"
...
Wan 3.0 Video の API リファレンスです。Wan 3.0 Video は、プロンプトのみから、または最初と最後のフレーム、参照画像、ビデオ、オーディオを含むメディアリストから生成します。これらはプロンプト内で Image 1、Video 1、Audio 1 として参照します。

## クイックスタート

[ご利用の Comfy ワークスペース](https://platform.comfy.org/profile/api-keys?onboarding=router)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk` および `npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、同じ呼び出しを raw HTTP で実行するものです。

**モデル ID:** `wan/wan3.0-video`

**エンドポイント:** `POST https://api.comfy.org/v2/models/wan/wan3.0-video`
...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan3.0-video/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスまたは別のプロセスから収集されます。[キュー配信](/ja/development/comfy-router/queue)で、ステータス、キャンセル、収集の流れを説明します。
...
async def main():
    async with AsyncComfy() as client:
        ...
        # リクエストが完了するまでポーリングし、サーバーが指定する Retry-After のあいだ待機します。
...
# 2. ステータスが COMPLETED になるまでポーリングし、各レスポンスが指定する Retry-After 秒のあいだ待機します。
REQUEST_ID="<request_id from the 201 body>"
curl -i https://api.comfy.org/v2/models/wan/wan3.0-video/requests/$REQUEST_ID/status \
  -H "X-API-Key: $COMFY_API_KEY"

# 3. 収集。実行中は 202 とステータスボディ、完了後は 200 とモデル本来の出力が返ります。
...
このモデルは、リクエストで別のプロバイダーを指定しない限り、Comfy Router が直接サービングします。以下のプロバイダーも、同じエンドポイントで同じモデル ID を使用してこのモデルをサービングしており、`model_provider` クエリパラメータで選択します。

- **Comfy** (デフォルト): `POST https://api.comfy.org/v2/models/wan/wan3.0-video`
- **Higgsfield**、`higgsfield/higgsfield-wan-3` として: `POST https://api.comfy.org/v2/models/wan/wan3.0-video?model_provider=higgsfield`

`strict_mode` のデフォルトは false であるため、Router はこのページに記載されているネイティブのリクエストボディをプロバイダー独自のスキーマに変換し、レスポンスを元の形式に戻します。API リファレンスの [`model_provider`、`strict_mode`、`fallback_provider`](/ja/development/comfy-router/reference#post-v2modelsprovidermodel) を参照し、この方法でルーティングされるすべてのモデルについては[サービングプロバイダー](/ja/development/comfy-router/providers)を参照してください。
...
<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセットリスト。ビデオ生成のための参照素材
（画像、オーディオ、ビデオ）を指定します。各要素は type フィールドと url フィールドを含みます。
...
- wan3.0-video: first_frame（最大1）、last_frame（最大1）、reference_image（最大10）、
  ...
  同一リクエスト内で相互排他的です。配列の順序がプロンプト内のアセットの参照順序
...
パートナー側の上限です。アセットが公開 URL の場合は記載どおりに適用されます。
そのバイト列が Comfy を経由しないためです。一方、インラインの data: URL は
Comfy を経由するため、転送上限にも直面します。Comfy Router の POST body は
合計 100 MiB が上限で、超えると 413 が返ります。また base64 はペイロードを約 4/3 に
膨らませるため、およそ 75 MB を超える単一のインラインアセットは、ここに示す
パートナーのルールに到達する前に拒否されます。そのサイズであれば、パートナー自身の
20MB 上限に収まる単一アセットはインラインでも問題なく収まるため、1 つのアセットに
ついてはパートナーのルールが効きます。転送上限が効くのは複数のアセットを送る
場合です。100 MiB の Router 上限は各要素ではなくリクエスト全体を制限するためです。
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または (上記 `media` の説明にあるモデルの項目が対応すると述べている場合、たとえば happyhorse-1.x の i2v と r2v の表記のように) インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、および合計でインラインペイロードを制限する 100 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>
...
<ParamField body="input.prompt" type="string">
  テキストプロンプトワード。中国語と英語に対応、長さは800文字以内
...
<ParamField body="input.reference_video_urls" type="string[]">
  wan2.6-r2v モデル専用の参照ビデオURL。1～3本のビデオURLの配列です。
入力制限:
- 形式: mp4、mov
- 本数: 1～3本
- 単一ビデオの長さ: 2～30秒
...
<ParamField body="input.template" type="string">
  ビデオエフェクトテンプレート名。任意。現在サポート: squish、flying、carousel。使用時は prompt パラメータは無視されます。
</ParamField>

<ParamField body="model" type="string">
  呼び出すモデルのID。このコンポーネントでは制約されません: Comfy Router が `POST /v2/models/wan/{model}` の `{model}` パスセグメントから設定するため、Router の呼び出し側は省略します。`POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` への v1 直接呼び出しでは必ず指定する必要があり、受け入れられる表記の列挙はそのオペレーション自身のコンポーネント `WanVideoGenerationRequest` にあります。
</ParamField>
...
<ParamField body="parameters.size" type="string">
  ビデオの解像度を width*height 形式で指定します。対応する解像度はモデルによって異なります:
...
Router が `GET /v2/models/wan/wan3.0-video/openapi.json` で提供するスキーマから生成されています。これは、リクエストがプロバイダーに到達する前に Router が呼び出しを検証する際に使用する同一のドキュメントです。
