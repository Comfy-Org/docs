...
translationSourceHash: 33b9ae2b
...
- wan2.6-r2v: 5秒または10秒のみ（15秒はサポートなし）
- wan2.7-i2v、wan2.7-t2v: [2, 15] の整数
- wan2.7-r2v、wan2.7-videoedit: [2, 10] の整数
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力ありの場合は入力
  ビデオの合計再生時間 + 出力ビデオの再生時間が30秒を超えてはなりません。-1 は
...
<ParamField body="parameters.ratio" type="string">
  生成されるビデオのアスペクト比率。wan2.7 および wan3.0 モデル専用。
...
このプロキシは resolution と size のどちらも指定しないビデオ生成リクエストを
拒否します。これは解像度ティアが課金レートを決定するためです。
