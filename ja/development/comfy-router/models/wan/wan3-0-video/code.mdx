...
[ご利用の Comfy ワークスペース](https://platform.comfy.org/profile/api-keys)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python と TypeScript のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`）を使用しています。cURL のスニペットは、同じ呼び出しを raw HTTP で実行するものです。
...
同じボディを `POST https://api.comfy.org/v2/models/wan/wan3.0-video/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスまたは別のプロセスから収集されます。[キューの配信](/ja/development/comfy-router/queue)で、ステータス、キャンセル、収集の流れを説明します。
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数、ビデオ入力ありの場合は
  入力ビデオの再生時間 + 出力ビデオの再生時間の合計が 30 秒を超えてはなりません。
