...
上記のアセットごとの「最大 20MB」は、最終的に受け取る画像に対するパートナーの上限です。
アセットが公開 URL の場合は、そのバイト列が Comfy を経由しないため、記載どおりに適用されます。
インラインの data: URL は Comfy を経由するため、転送時の上限にも当てはまります。Comfy Router の
POST body は合計 100 MiB が上限で、超えると 413 が返ります。base64 はペイロードを約 4/3 に
膨らませるため、およそ 75 MB を超える単一のインラインアセットは、ここに示すパートナーの
ルールに到達する前に拒否されます。そのサイズであれば、パートナー自身の上限である 20MB の
単一アセットはインラインでも余裕をもって収まるため、1 つのアセットについてはパートナーの
ルールが支配的になります。転送上限のほうが支配的になるのは複数のアセットにまたがる場合です。
100 MiB の Router 上限は各要素ではなくリクエスト全体を制限するためです。これらの上限に
近いものは公開 URL として送信してください。
</ParamField>
...
<ParamField body="input.media[].url" type="string" required>
  メディアファイルの URL。公開 HTTP/HTTPS URL、OSS の一時 URL、または (上記 `media` の説明にあるモデルの項目が対応すると述べている場合、たとえば happyhorse-1.x の i2v と r2v の表記のように) インラインの `data:{MIME_type};base64,...` URL です。モデルごとのサイズとピクセルの下限、およびインラインのペイロードを全体として制限する 100 MiB の Router リクエストボディ上限については、その説明を参照してください。
</ParamField>
...
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の整数。ビデオ入力がある場合、入力
...
このプロキシは、解像度と size のどちらも指定されていないビデオ生成リクエストを拒否します。
