---
title: "HappyHorse 1.1 R2V を Comfy Router で使用する"
description: "Comfy Router 経由で HTTP を通して wan/happyhorse-1.1-r2v を呼び出すための Python、TypeScript、cURL スニペット、およびリクエストフィールドと結果の形状"
sidebarTitle: "HappyHorse 1.1 R2V"
translationSourceHash: 305139d4
translationFrom: development/comfy-router/models/wan/happyhorse-1-1-r2v/code.mdx
translationBlockHashes:
  "_intro": 9cfab6c4
  "Quick start": f6b88207
  "Schema": c0c72613
  "Examples": a91cb6b1
---
...
HappyHorse 1.1 R2V の API リファレンスです。HappyHorse 1.1 の参照から動画は、最大9枚の参照画像に写る被写体を新しいシーン全体で一貫させます。プロンプト内では character1、character2 のように指定します。

## クイックスタート

お使いの Comfy ワークスペース (https://platform.comfy.org/profile/api-keys) でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python、TypeScript、Swift のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`、および [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift パッケージ）を使用しており、cURL のスニペットは同じ呼び出しを生の HTTP で行うものです。

**モデル ID:** `wan/happyhorse-1.1-r2v`

**エンドポイント:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-r2v`
...
同じボディを `POST https://api.comfy.org/v2/models/wan/happyhorse-1.1-r2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができた時点で、このプロセスからでも別のプロセスからでも収集できます。[キューの配信](/ja/development/comfy-router/queue) では、ステータス、キャンセル、収集の流れを説明します。
...
<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセットリスト。ビデオ生成用の参照素材（画像、オーディオ、ビデオ）を指定します。
各要素は type と url フィールドを持ちます。
...
- wan3.0-video: first_frame（最大1）、last_frame（最大1）、reference_image（最大10）、
  ...
  同一リクエスト内では同時に使用できません。配列の順序が、プロンプト内でのアセットの
  参照順序（画像1、ビデオ1、オーディオ1、...）を定義します。
...
20 MiB の Router 上限は各要素ではなくリクエスト全体を制限するためです。これらの上限に近い
...
複数の参照ビデオを使う wan2.6-r2v では、'character1'、'character2' などを使って
...
- 単一ファイルサイズ: 最大30MB
- audio_url とは併用不可
参照時間: 単一ビデオは最大5秒、2本の場合は各最大2.5秒、3本の場合はそれに比例して短くなります。
課金: 実際に使用した参照時間に基づきます。
</ParamField>
...
<ParamField body="model" type="string">
  呼び出すモデルのID。このコンポーネントでは制約されません: Comfy Router が `POST /v2/models/wan/{model}` の `{model}` パスセグメントからこれを設定するため、Router の呼び出し元は省略します。`POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` を直接 v1 呼び出しする場合は必ず指定する必要があり、受け付けられる表記の enum はそのオペレーション自身のコンポーネント `WanVideoGenerationRequest` にあります。
</ParamField>
...
- wan2.6-r2v: 5 秒または 10 秒のみ (15 秒はサポート対象外)
- wan2.7-i2v、wan2.7-t2v: [2, 15] の範囲の整数
- wan2.7-r2v、wan2.7-videoedit: [2, 10] の範囲の整数
- wan3.0-video: ビデオ入力なしの場合は [2, 30] の範囲の整数。ビデオ入力がある場合は、
...
- wan2.6-i2v: 720P、1080P のみ (480P はサポート対象外)
...
<ParamField body="parameters.size" type="string">
  幅*高さ形式のビデオ解像度。サポートされる解像度はモデルによって異なります:
wan2.5 T2V の場合: 480P (480*832、832*480、624*624)、720P、1080P のサイズ
...
Router が `GET /v2/models/wan/happyhorse-1.1-r2v/openapi.json` で提供するスキーマから生成されており、リクエストがプロバイダーに到達する前に呼び出しを検証する際に使用されるドキュメントと同じものです。
...
<ResponseField name="message" type="string">
  失敗したリクエストの詳細情報。`output` の下ではなく、エンベロープのルートで報告されます（リクエストが成功した場合は返されません）。`output.message` にフォールバックする前にこれを確認してください。
</ResponseField>
...
ビデオの URL は 24 時間有効です。保存する必要がある場合は、速やかにビデオをダウンロードしてください。
