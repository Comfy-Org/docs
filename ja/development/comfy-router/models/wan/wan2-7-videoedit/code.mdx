...
<ParamField body="input.img_url" type="string">
  先頭フレーム画像のURLまたはBase64エンコードデータ。wan2.5-i2v-preview および wan2.6-i2v モデルでのみ必須です。happyhorse-1.x の i2v 表記は、ここでは先頭フレームを受け取りません。先頭フレームは type が `first_frame` の `media` 要素として受け取ります (下記 `media` を参照)。wan2.6-i2v で成功する `img_url` のボディは、happyhorse-1.0-i2v および happyhorse-1.1-i2v ではプロバイダーに拒否されます。画像形式はJPEG、JPG、PNG、BMP、WEBP。解像度は360～2000ピクセル。ファイルサイズは最大10MBです。
</ParamField>
