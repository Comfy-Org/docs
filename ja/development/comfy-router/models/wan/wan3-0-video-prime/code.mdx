...
[Comfy ワークスペース](https://platform.comfy.org/profile/api-keys)でキーを作成し、`COMFY_API_KEY` としてエクスポートします。Python、TypeScript、Swift のスニペットは Comfy SDK（`pip install comfy-sdk`、`npm install @comfyorg/sdk`、および [`ComfySwiftSDK`](https://github.com/Comfy-Org/comfy-swift-sdk) Swift パッケージ）を使用しています。cURL のスニペットは、同じ呼び出しを raw HTTP で行うものです。
...
- auto（デフォルト）: モデルがプロンプトの内容に基づいてインテリジェントに判断
...
wan2.7 モデルの場合、指定しないと解像度ティアに基づいてデフォルトが設定されます。
wan3.0-video の場合、adaptive（デフォルト）は入力メディアの比率と意図に基づいて適切なアスペクト比を自動的に推奨します。
...
このプロキシは、解像度もサイズも指定されていないビデオ生成リクエストを拒否します。解像度ティアによって課金レートが決まるためです。
...
Router が `GET /v2/models/wan/wan3.0-video-prime/openapi.json` で提供するスキーマから生成されたもので、リクエストがプロバイダーに到達する前に呼び出しを検証する対象となる同一のドキュメントです。
