...
# 3. 収集。200 はモデル固有の出力、実行中は 202 とステータスボディを返します。
...
<ParamField body="input.img_url" type="string">
  先頭フレーム画像のURL、または Base64 エンコードデータ。wan2.5-i2v-preview および
wan2.6-i2v モデルでのみ必須です。happyhorse-1.x の i2v 表記では、先頭フレームを
ここでは受け取りません。先頭フレームは type が `first_frame` の `media` 要素として
渡します (下記 `media` を参照)。wan2.6-i2v で成功する `img_url` のボディも、
happyhorse-1.0-i2v および happyhorse-1.1-i2v ではプロバイダーに拒否されます。
画像形式: JPEG、JPG、PNG、BMP、WEBP。解像度: 360〜2000 ピクセル。
ファイルサイズ: 最大 10MB。
</ParamField>

<ParamField body="input.media" type="object[]">
  wan2.7、wan3.0、happyhorse-1.x モデル用のメディアアセットリスト。ビデオ生成用の参照素材
(画像、オーディオ、ビデオ) を指定します。各要素は type フィールドと
url フィールドを含みます。
...
上記のアセットごとの「最大 20MB」という数値は、パートナーが最終的に受け取る画像に
対する上限であり、アセットが公開 URL の場合はそのまま適用されます。そのバイト列が
...
前に拒否されます。そのサイズでは、パートナー自身の 20MB 上限に収まる単一の
アセットはインラインでも余裕をもって収まるため、アセット 1 つについては
パートナーのルールが制約となります。転送上限が制約となるのは、複数のアセットを
送る場合です。100 MiB の Router 上限は各要素ではなくリクエスト全体を制限する
ためです。これらの上限に近いものは公開 URL として送信してください。
</ParamField>
