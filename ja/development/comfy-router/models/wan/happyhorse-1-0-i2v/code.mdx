---
title: "Comfy Router で HappyHorse 1.0 I2V を使用する"
description: "Comfy Router 経由で HTTP により wan/happyhorse-1.0-i2v を呼び出すための Python、TypeScript、cURL スニペット、およびリクエストフィールドと結果の形状"
...
HappyHorse 1.0 I2V の API リファレンスです。HappyHorse 1.0 の画像から動画への変換は、静止した最初のフレームをアニメーション化し、同じ処理の中で同期したオーディオをレンダリングします。
...
**モデル ID:** `wan/happyhorse-1.0-i2v`

**エンドポイント:** `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-i2v`
...
同じボディを `POST https://api.comfy.org/v2/models/wan/happyhorse-1.0-i2v/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集できます。[キュー配信](/ja/development/comfy-router/queue)では、ステータス、キャンセル、収集について説明します。
...
- wan3.0-video: first_frame (最大 1)、last_frame (最大 1)、reference_image (最大 10)、
  ...
  参照順序 (画像 1、ビデオ 1、オーディオ 1、…) を定義します。
...
先に低いほうの転送上限に達します。Comfy Router の POST ボディは合計 20 MiB が上限で、
超えると 413 が返ります。base64 はペイロードを約 4/3 に膨らませるため、およそ
15 MB を超える単一のインラインアセットは、ここに示すパートナーのルールに到達する
前に拒否されます。ボディが複数のインラインアセットを含む場合はさらに早くなります。
...
<ParamField body="input.prompt" type="string">
  テキストプロンプト。中国語と英語に対応し、長さは 800 文字以内です
(wan3.0-video では最大 20,000 文字。制限を超えた内容は切り捨てられます)。
複数の参照ビデオを使用する wan2.6-r2v の場合は、参照ビデオの順序に従って
被写体を 'character1'、'character2' などで参照します。例: 「Character1 が道端で歌い、Character2 がその横で踊る」
...
- 単一ファイルサイズ: 最大 30MB
- audio_url とは併用不可
参照再生時間: 単一ビデオは最大 5 秒、2 本の場合は各最大 2.5 秒、3 本の場合は比例して短くなります。
課金: 実際に使用した参照再生時間に基づきます。
</ParamField>

<ParamField body="input.template" type="string">
  ビデオエフェクトテンプレート名。任意。現在サポートされているもの: squish、flying、carousel。使用すると prompt パラメータは無視されます。
</ParamField>

<ParamField body="model" type="string">
  呼び出すモデルのID。このコンポーネントでは制約されません: Comfy Router が `POST /v2/models/wan/{model}` の `{model}` パスセグメントから設定するため、Router の呼び出し元は省略します。`POST /proxy/wan/api/v1/services/aigc/video-generation/video-synthesis` への直接の v1 呼び出しでは必ず指定する必要があり、受け付けられる表記の enum はそのオペレーション自身のコンポーネント `WanVideoGenerationRequest` にあります。
</ParamField>
...
- wan3.0-video: ビデオ入力がない場合は [2, 30] の整数。ビデオ入力がある場合は、
...
<ParamField body="parameters.ratio" type="string">
  生成されるビデオの比率。wan2.7 および wan3.0 モデルでのみ使用できます。
...
<ParamField body="parameters.watermark" type="boolean" default="false">
  透かしロゴを追加するかどうか。透かしは右下隅に配置されます
</ParamField>

これは Router が `GET /v2/models/wan/happyhorse-1.0-i2v/openapi.json` で提供するスキーマから生成されたものです。これは、リクエストがプロバイダーに到達する前に呼び出しを検証する対象となるドキュメントと同じものです。
...
ビデオ URL は 24 時間有効です。保持する必要がある場合は、速やかにビデオをダウンロードしてください。
