...
同じボディを `POST https://api.comfy.org/v2/models/bria/image-edit-replace-background/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスからでも別のプロセスからでも収集できます。[Queued delivery](/ja/development/comfy-router/queue) では、ステータス、キャンセル、収集の流れを説明します。
