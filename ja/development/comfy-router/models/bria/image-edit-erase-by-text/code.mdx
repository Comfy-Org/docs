...
```
  ...
  </Tab><Tab title="キューに投入して後で収集">
同じボディを `POST https://api.comfy.org/v2/models/bria/image-edit-erase-by-text/requests` に送信します。Router は実行が受け付けられるとすぐに `201` と `request_id` を返し、結果は準備ができ次第、このプロセスまたは別のプロセスから収集されます。[キュー配信](/ja/development/comfy-router/queue) では、ステータス、キャンセル、収集について順を追って説明します。
...
# 3. 収集。200 はモデルのネイティブ出力を伴い、202 はまだ実行中の場合にステータスボディを伴います。
