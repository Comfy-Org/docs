...
`bfl/flux-2-pro` は 1 つの ID にすぎず、残りはカタログにあります。`GET /v1/models` は Router が実行できるすべてのモデルを 1 ページずつ一覧表示し、各エントリには呼び出しに必要な情報がそのまま含まれています。パスに入れる `id`、個別に保持される `provider` と `model` のセグメント、そして何かを消費する前に分岐判断に使える `billing` ブロックです。
...
- [Comfy Router API リファレンス](/ja/development/comfy-router/reference): すべてのエンドポイント、すべてのパラメータ、そして 15 種類すべてのエラー分類を網羅しています。
- [Comfy Router の制限事項](/ja/development/comfy-router/limitations): 現在 Router が対応していない機能と、その代わりに使用すべきものを説明しています。
