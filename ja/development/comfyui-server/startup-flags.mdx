...
| `--fast-disk` | 自動 | 非ピン RAM よりディスクベースの動的読み込みを強制。高速 NVMe に有用。モデルファイルが高速と判定されたストレージ上にある場合は自動で有効になります。 |
| `--disable-fast-disk` | 無効 | ディスクベースの動的読み込みを無効化します。`--fast-disk` と自動検出より優先されます。 |
