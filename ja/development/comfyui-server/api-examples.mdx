---
title: "API 例"
description: "ComfyUI サーバー API を呼び出す3つの共通パターン"
...
if __name__ == "__main__":
    ...
    print(f"取得した画像を持つ出力ノード数: {len(images)}")

    # 画像を表示する場合（Pillow が必要）:
...
if __name__ == "__main__":
    ...
    print(f"WebSocket 経由で {len(images)} 枚の画像を受信しました。")
...
<CardGroup cols={3}>
  <Card title="方法1: HTTP のみ" icon="paper-plane">
    **送って忘れる。** 即時の出力が必要ない場合、または後で結果を取得しても構わない場合に使用します。
  </Card>
  <Card title="方法2: WebSocket + History" icon="chart-line" href="#method-2-websocket--history-monitor-completion">
    **推奨。** 完了を待ち、出力をダウンロードします。シンプルさと信頼性の最良のバランス。
  </Card>
  <Card title="方法3: SaveImageWebsocket" icon="images" href="#method-3-websocket-with-saveimagewebsocket-real-time-images">
