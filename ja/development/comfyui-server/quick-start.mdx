...
すべての例は[デフォルトの SD1.5 ワークフロー](https://github.com/Comfy-Org/ComfyUI/blob/master/script_examples)を使用しています。ComfyUI フロントエンドから任意のワークフローを API 形式でエクスポートできます（`File → Save (API Format)`）。
