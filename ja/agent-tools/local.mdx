...
[**comfy-local-mcp**](https://github.com/Comfy-Org/comfy-local-mcp) は Comfy の **ファーストパーティのローカル MCP サーバー** — AI エージェント (Claude Code、Claude Desktop、Cursor、およびその他の MCP クライアント) から **ローカル**の ComfyUI インストールを操作する公式の方法です。これは [comfy-cli](https://github.com/Comfy-Org/comfy-cli) の薄いラッパーであり、各ツールは `comfy` コマンドをシェルアウトし、comfy-cli がエンジンとして機能します。[Comfy Cloud MCP](/ja/agent-tools/mcp) とのコード共有はありません。
