...
<Info>
本ページのノード定義は旧式の V1 スキーマ（`INPUT_TYPES` / `RETURN_TYPES`）を使用していますが、現在も完全にサポートされています。新しいノードはモダンな V3 スキーマ（`io.ComfyNode` と `define_schema`、`comfy_entrypoint` 関数）でも書けます。ComfyUI に同梱されている [`custom_nodes/example_node.py.example`](https://github.com/comfyanonymous/ComfyUI/blob/master/custom_nodes/example_node.py.example) はそのように書かれています。違いと移行方法については [V3 マイグレーションガイド](/custom-nodes/v3_migration) を参照してください。
</Info>
