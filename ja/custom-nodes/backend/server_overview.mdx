---
title: "カスタムノードのプロパティ"
description: "V1 スキーマで ComfyUI カスタムノードを定義します: INPUT_TYPES、RETURN_TYPES、CATEGORY、FUNCTION、実行制御、入力検証について説明します。"
translationSourceHash: c0b12b3e
translationFrom: custom-nodes/backend/server_overview.mdx
---


<Info>
本ページのノード定義は旧式の V1 スキーマ（`INPUT_TYPES` / `RETURN_TYPES`）を使用していますが、現在も完全にサポートされています。新しいノードはモダンな V3 スキーマ（`io.ComfyNode` と `define_schema`、`comfy_entrypoint` 関数）でも書けます。ComfyUI に同梱されている [`custom_nodes/example_node.py.example`](https://github.com/comfyanonymous/ComfyUI/blob/master/custom_nodes/example_node.py.example) はそのように書かれています。違いと移行方法については [V3 マイグレーションガイド](/ja/custom-nodes/v3_migration) を参照してください。
</Info>
...
`hidden` 入力についての詳細は、[隠し入力](./more_on_inputs#隠蔽入力) を参照してください。
