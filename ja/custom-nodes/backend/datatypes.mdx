...
translationBlockHashes:
  "_intro": 80373b23
  "Comfy datatypes": fcd90f30
  "Python datatypes": 37b2f2ea
  "Tensor datatypes": cfddd57a
  "Custom Sampling datatypes": 3f4ac4eb
  "Model datatypes": 26b3c960
  "Additional Parameters": cd3afb90
---
...
<Tip>`seed` は `SamplerCustomAdvanced` の `sample` ガイダーに渡されますが、標準のガイダーのいずれでも使用されていないようです。オプションなので、通常は None に設定できます。</Tip>
...
`GUIDER` は、プロンプトや他の形式の条件付けによって「導かれる」ノイズ除去プロセスの一般化です。Comfy では、ガイダーは `sample` によって呼び出される `__call__(*args, **kwargs)` メソッドを提供する呼び出し可能な Python オブジェクトによって表現されます。
