...
---
ノード置換 API を使用すると、カスタムノード開発者は非推奨ノードから新しい同等のノードへの移行パスを定義できます。ノードを更新または名前を変更すると、ユーザーはワークフローを自動的にアップグレードできます。
...
| `old_node_id` | str | 非推奨ノードのクラス名 |
| `old_widget_ids` | list[str] \| None | ウィジェットIDを相対インデックスにバインドする順序付きリスト |
...
ワークフローに非推奨のノードが含まれている場合、フロントエンドは以下を行います：
