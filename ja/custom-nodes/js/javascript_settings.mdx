---
title: "設定"
description: "app.registerExtension を使って ComfyUI 拡張の設定を追加・読み取り・書き込みし、変更に反応する方法。boolean、text、number、slider、combo、color、image などすべての設定タイプをカバーします。"
...
[カテゴリを指定](#カテゴリ) しない場合、`id` は `.` で分割され、設定パネル内の表示位置が決定されます。
