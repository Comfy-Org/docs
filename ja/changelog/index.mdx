...
**メディア読み込みの大幅改善**
* **音声／動画の統合読み込み：** 動画ローダーが音声と動画を同時に読み込み、メディアワークフローを簡素化
* **PyAV 統合：** Pillow から PyAV への移行により JPEG のメモリ使用量を削減、tRNS 付き PNG 透過にも対応
* **メタデータ処理：** PyAV で自動メタデータ回転を扱い、画像の向きを正しく表示

**新モデル対応**
* **CogVideoX：** CogVideoX 動画生成モデルに対応
* **Gemma4：** Gemma4 向けテキスト生成を追加
* **Void：** パス 1／2 を備えた Void モデルに対応
* **BiRefNet：** 高度マスク用の背景除去モデルを追加
* **Wan-Dancer：** Wan-Dancer モデルに対応

**パフォーマンス**
* **ブロック先読みと LoRA 非同期読み込み：** LTX などで処理が大幅高速化
* **動的 VRAM 最適化：** `--cache-ram 2` などにより動画処理のピーク VRAM を削減
* **Triton ComfyUI kitchen：** 高度な最適化を有効化する CLI 引数を追加

**動画生成機能**
* **自己回帰動画生成：** 長いシーケンス向けの新機能
* **因果モデル向け I2V：** 因果アーキテクチャでの画像から動画生成に対応
* **コンテキストウィンドウ：** 長尺でのブレンド改善のため `causal_window_fix` を追加
* **フレーム補間：** メモリ見積もりとオーバーヘッドを改善

**パートナーノードの追加**
* **Luma UNI-1：** Luma Uni-1 統合画像モデル（作成／編集ワークフロー）のパートナー対応
* **Topaz Astra 2：** Topaz 動画強化（Astra 2 クリエイティブアップスケール）モデルを統合
* **Tripo3D 3.1：** 3D 生成機能を更新
* **GPT-5.5：** GPT-5.5／GPT-5.5-pro LLM バリアントに対応
* **NanoBanana2：** DynamicCombo／Autogrow を備えた新規ノード

**ノード改善**
* **Flux2 潜在プレビュー：** Flux2 潜在の高品質プレビューに対応
* **画像ブレンド：** `--gpu-only` 時のデバイス不一致やチャンネル数処理を修正
* **バッチブロードキャスト：** JoinImageWithAlpha のバッチ処理を強化
* **数式：** Boolean を追加し複雑な計算に対応
* **タイル処理：** SplitImageToTileList／ImageMergeTileList の縦ストライド計算を修正

**開発者・API**
* **機能フラグ：** `--feature-flag` とレジストリによる汎用拡張
* **OpenAPI：** クラウドランタイム操作と実験用エンドポイントを仕様に追加
* **Blueprint：** サブグラフに説明フィールドを追加
* **ControlNet：** 読み込み順を決定的にし結果を安定化

**バグ修正・安定性**
* メタテンソル未初期化による CPU TE クラッシュを修正
* SolidMask／MaskComposite のデバイス不一致を解消
* LTX-2.x が出力した音声潜在との VAEDecodeAudio 互換性を修正
* 起動時のポート使用中エラーの扱いを改善
* 末尾アンダースコアなしの保存ファイル名に対応
