...
* [**Text**](https://github.com/Comfy-Org/ComfyUI/pull/14870): コア Text ノードを復元
...
**新しいオープンソースモデル対応**
* [**Ideogram 4**](https://github.com/Comfy-Org/ComfyUI/pull/14259)：Ideogram のオープンウェイトテキスト→画像モデルをリリース日に対応。NextDiT/Lumina2 系シングルストリーム DiT、Qwen3-VL-8B（13 層隠れ状態タップ）をテキストエンコーダーに採用。新ノードとして **Ideogram4Scheduler**・**DualModelGuider**（条件／無条件パスを別モデルで実行）・**CFGOverride** を追加
...
* **DualModelGuider：** ユーザーに適切な期待値を設定するため、実験的としてマーク
...
* [**NVIDIA PixelDiT & PiD**](https://github.com/Comfy-Org/ComfyUI/pull/14103)：NVIDIA の PixelDiT 1300M テキスト→画像モデルと PiD ファミリー（SDXL、QwenImage バリアントは v0.24.0 で追加）に対応
...
* [**MultiGPU ワークユニット**](https://github.com/Comfy-Org/ComfyUI/pull/7063)：条件付けのワークユニットを複数 GPU に分散してサンプリングを高速化。ポジティブ＋ネガティブ条件、マスク条件もワークユニットとして対応
* **XPU MultiGPU CFG 分割**：Windows で有効化（CFG > 1）
* **確率的丸め：** 浮動小数点演算に comfy-kitchen の CUDA 確率的丸めカーネルを使用し、数値精度を向上
* **`--use-flash-attention` フラグ**：xformers と両方インストールされている場合に正しく上書きするよう修正
...
* **Rodin2.5 ノード：** 新しい 3D 生成ノード（Rodin 製）
* **Krea2 Image ノード：** 新しい画像生成パートナーノード（Krea 製）
* **Beeble SwitchX ノード：** 新しいリライティング／背景スワップノード（Beeble 製）
* **Tripo3D P1 モデル：** Tripo3D の P1 生成モデル用の新ノード
* **Flux バーチャル試着 & 消去：** Flux ベースのバーチャル試着とオブジェクト消去ノード
* **SeeDance 2：** 動画リファレンスのアップロード改善と実行時入力バリデーション追加
* **ByteDance2Reference：** 自動アップスケールウィジェットを追加
* **Grok動画生成：** First Frame モードに `grok-imagine-video-1.5` モデルを追加。Grok ノードからベータ版モデルを削除
...
**ノード改善**
* **Math Expression：** ブール対応でより複雑な計算が可能に
* **Flux2 潜在プレビュー：** Flux2 モデル潜在の高品質プレビュー
* **コア ブループリント：** 新しい組み込みブループリントテンプレート
...
* 高度な最適化のための Triton ComfyUI kitchen 用 CLI オプション
* 自己回帰動画、因果モデル向け画像から動画へ、`causal_window_fix`、フレーム補間のメモリ／オーバーヘッド改善
...
**開発者・API**
* `--feature-flag` レジストリ、OpenAPI（クラウドランタイムと実験エンドポイント）、ブループリントサブグラフの説明、コントロールネットの決定的な読み込み順序
...
**新しいモデル対応**
* **SUPIR モデル対応：** 高品質なアップスケーリングワークフローのための強化型超解像機能を追加
* **RIFE / FILM フレーム補間：** スムーズなビデオ遷移のための高度なフレーム補間モデルを導入
* **SAM 3.1 対応：** Segment Anything Model を更新し、セグメンテーション精度を向上
* **LTXV オーディオ VAE 改善：** 単体の LTXV オーディオ VAE に対応し、ネイティブオーディオ処理を強化
...
**ビデオ処理の強化**
* より高ビット深度のビデオ読み込みに対応し、品質保持を改善
* ビデオ読み込みワークフローでアルファチャンネルに対応
* 大容量ビデオファイルを扱うため、ByteDance 2 ノードに自動ダウンスケール機能を追加
...
**パートナーノード**
- SeeDance 2.0 モデルで 1080p 解像度に対応し、より高品質なビデオ生成を実現
...
**基盤アップデート**
- マネージャーをバージョン4.1に更新し、機能を改善
- フロントエンドをバージョン1.42.10に更新し、安定性を強化
...
- Grok 参照動画生成
- Grok 動画拡張
- [ブログ](https://blog.comfy.org/p/grok-imagine-model-feature-updates)
</Update>
...
- タイル化デコード時のピークメモリ使用量を改善し、ビデオ VAE タイラーのフォールバックにおける VRAM リークを修正
...
- AMD gfx1150 (Strix Point) GPU 向けに PyTorch アテンションを有効化
- FP4、FP8、FP16 のネイティブ dtype 対応および量子化リニア自動微分関数により、トレーニング機能を強化
- テキストエンコーダの互換性問題を修正し、Hunyun3D v2.1 DiT モデル向け SageAttention を無効化
...
- ノードおよびブループリント向けの「必須」カテゴリ対応を強化
- ワークフローテンプレートをバージョン 0.9.26 へ更新

**バグ修正および安定性向上**
- ピクセル空間 VAE の互換性問題および終了時のモデルファイナライザ実行を修正
- 拡散モデルを読み込むノードにおける重み dtype 処理を改善（高度な機能入力としてマーク）
...
本リリースは安定性およびバグ修正に焦点を当てています。完全な技術詳細については、GitHub 上の[完全な変更履歴](https://github.com/Comfy-Org/ComfyUI/compare/v0.17.1...v0.17.2)をご覧ください。
...
**新規ノードおよび機能**
- 画像サイズ設定を容易にするため、アスペクト比プリセット付きの `ResolutionSelector` ノードを追加
- 高度なパラメータ制御のための `CURVE` 型サポートを導入
- ワークフローモニタリング向上のため、ジョブ API にテキストプレビューサポートを追加
- 画像処理機能強化のための LongCat-Image ネイティブ実装

**モデルおよびフォーマットサポート**
- LoKR 互換性のため、ACE-Step 1.5 lycoris キーエイリアスマッピングを追加
- SDPose-OOD モデルのサポート
- SCAIL WanVideo モデルのサポート
- zeta chroma 重みの読み込みサポート
- LTXAV 2.3 モデルのサポート
- Z-image ピクセル空間のサポート実装

**メモリとパフォーマンス**
- 動的 VRAM モードをデフォルトに変更し、メモリ効率を改善
- 非 QuantizedTensor fp8 向けの LoRA 再量子化を実装
- パフォーマンス向上のため動的オフロードヒューリスティクスを強化
- アロケータ処理改善のため `comfy-aimdo` をバージョン 0.2.6 に更新
- 動的 VRAM と重みフックおよび WSL 互換性との競合を修正

**API と UI の改善**
- NanoBanana2 API ノードに `"IMAGE+TEXT"` サポートを追加
- 明確化のため Mahiro CFG を `Similarity-Adaptive Guidance` に名称変更
- GLSL ノードの入力次元要件を修正
- `VAEDecodeAudioTiled` を強化し、`tile_size` 入力を適切に使用

**バグ修正**
- WanVAE の `feat_map` 処理におけるエンコーダ／デコーダの混同を修正
- `lm_metadata` 不足時の ACE-1.5 メモリ推定問題を解決
- サブステップシグマのコンテキストウィンドウステップ処理を修正
- `VideoFromComponents` が `BytesIO` に書き込む際のクラッシュを防止
- 画像タイルの重複を制限し処理問題を防止

**ワークフローテンプレート**
- ワークフローテンプレートをバージョン 0.9.7 に更新
...
- torch コンパイラー使用時に動的 VRAM を無効化し、競合を防止

**モデルおよび API 更新**
- フォールバック処理の改善のため、`comfy aimdo` をバージョン 0.2.2 へ更新
- 高度なビデオ処理のため、WanVideo ベースのセグメンテーションモデル FlowRVS のサポートを追加
- 機能拡張のため、NanoBanana2 API ノードを追加
- ゼロコピー sft 問題を防止するための Aimdo フォールバック処理を修正
...
**新規機能**
- 自己アテンションにおけるガイドごとのアテンション強度制御を追加し、細かいワークフロー制御を実現
...
**新規ノードおよび機能**
- 画像ワークフローにおける正確なバウンディングボックス選択のための `BBox` ウィジェットを追加
- 音声処理ワークフロー向けの3バンドイコライザーノードを導入
- Gemma3 および Qwen 3 を初期対応モデルとして、ネイティブモデルによる基本的なテキスト生成をサポート
- 高度な視覚効果のための PyOpenGL を用いた GLSL シェーダーノードを新規追加
- テキスト読み上げ機能のための ElevenLabs API ノードを追加
- FLOAT 入力向けの、改善された UI を備えた新しいグラデーションスライダーディスプレイモードを追加
- タイル処理ワークフロー向けの `SplitImageToTileList` および `ImageMergeTileList` ノードを追加

**API ノード**
- Rodin Gen-2 ノードに価格バッジを追加
- アバター生成のための `KlingAvatar` ノードを追加
- ByteDance Seedream-5 モデルをサポート
- Gemini 画像 MIME タイプ処理をグロブマッチングで修正
- 高品質化のため、Gemini が非圧縮画像を返すよう強制

**パフォーマンスおよびバグ修正**
- 再量子化の返却を制限することで、FP8 動的 VRAM ワークフローのパフォーマンス問題を修正
- 動画保存操作における非連続オーディオ波形のクラッシュを解決
- fp8 チェックポイント対応および埋め込みコネクタ dtype 問題を含む LTXAV モデル読み込み問題を修正
- PyOpenGL バージョン 3.1.4 未満への互換性を向上

**UI 改善**
- 折りたたみ可能な UI のために、429 個のウィジェットを高度な設定としてマーク
- ノードの整理を容易にするため、`essentials_category` を追加
- 分類を改善した必須サブグラフブループリントを強化
- フロントエンドをバージョン 1.39.16 へ更新

**モデル対応**
- トークン化の改善および最小長の強制により、テキスト生成を強化
- LTXAV の av 埋め込みコネクタ統合を更新
...
- パフォーマンス向上のため、cu130 へのアップグレードを推奨する古いPyTorchバージョンに対する警告を追加
...
**モデル対応**
- Ovis Image
- Kandinsky 5.0（T2V/I2V/T2I バリアント）
...
- キューボタンおよびパンくずリストに不足しているノード警告 UI を追加
- モバイル修正
- 各種バグ修正
...
- リニアモードのベータ版（キーバインディング設定でホットキーを設定可能）
- サブグラフの改善およびバグ修正
- 不足ノードの UX を改善
- 新しいワークフロー進行パネル
- 新しいアセットサイドバー
...
**新規モデル対応**
- Flux 2 モデルの包括的対応（Flux 2 Pro APIノードを含む）
...
**モデル互換性および機能強化**
- **HunyuanVideo 1.5 サポート**：最新版HunyuanVideoモデルの互換性を追加し、ビデオ生成機能を拡張
- **LLAMAテキストエンコーダの改善**：LLAMAベースのテキストエンコーダモデルにおける最終正規化の無効化を可能にし、ワークフローのカスタマイズ性を向上
- **HunyuanV3Dスキーマ移行**：パフォーマンスおよび互換性向上のため、Hunyuan3DノードをV3スキーマへ更新

**APIノードの拡張**
- **新規Topaz APIノード**：ComfyUI内で直接Topazビデオ強化ワークフローを実行可能に
- **Nano Banana Pro統合**：処理能力を拡張するためのNano Banana Pro APIノード群を追加
- **Klingリップシンクの改善**：`KlingLipSyncAudioToVideoNode`のオーディオフォーマット変換の問題を修正し、適切なMP3フォーマット処理を確保

**画像処理およびワークフローの改善**
- **画像バッチ処理の強化**：異なるチャンネル数の画像を処理し、必要に応じてアルファチャンネルを自動追加するようImageBatchノードを修正
- **プレビューノードの改善**：ワークフローの整理を明確にするため、PreviewAnyノードを「プレビュー任意」に名称変更
- **ワークフローテンプレートの更新**：マルチパッケージ配布に対応するため、サーバーテンプレートハンドラーを強化

**パフォーマンスおよびシステムの最適化**
- **CUDA最適化**：GPUパフォーマンス向上のため、新しいCUDNNバージョンでの不要な回避策を無効化
...
**フロントエンドの更新**
- **サブグラフウィジェット編集**: サブグラフ内部に入らずに、新しいパラメータパネルから直接サブグラフパラメータを編集可能に
...
**ノードスキーマ移行 (V3)**
- コアノードカテゴリをV3スキーマに移行しました（モデルダウンスケール、LoRA抽出、合成、潜在操作、SD3/SLG、Flux、アップスケールモデル、HunyuanVideoノードを含む）
...
- **キャンバスナビゲーションモードのロールバック**: スタンダードキャンバスナビゲーションモードがデフォルトで有効になっていたことによるユーザーエクスペリエンスの問題を回避するため、キャンバスナビゲーションのデフォルトを従来のレガシーモードに戻しました。スタンダードキャンバスナビゲーションモードは引き続き設定で有効にできます。
...
- **スタンダードキャンバスモード**: スタンダードキャンバスモードを追加、`Lite Graph` > `キャンバス` > `キャンバスナビゲーションモード` で切り替え可能
...
- **5B I2Vモデルサポート**: WAN 2.2 5B 画像から動画へのモデルのメモリ最適化により、これらのモデルをより利用しやすく
