...
translationBlockHashes:
  ...
  "Hardware-Specific Issues": adb5cdc2
...
Intel GPU システムでは、このエラーは CUDA の問題を示すものではなく、PyTorch の再インストールも必要ありません。Intel GPU は CUDA ではなく XPU バックエンドを使用します。上記の NVIDIA CUDA 向け再インストール手順は Intel ハードウェアには適用されないため、実行しないでください。

セットアップが正しく動作しているか確認するには:
```bash
python -c "import torch; print('XPU available:', torch.xpu.is_available())"
```

`torch.xpu.is_available()` が `True` を返す場合、ComfyUI は自動的に XPU バックエンドを使用します。`False` を返す場合は、以下のいずれかのオプションで XPU 対応 PyTorch をインストールしてください。
