...
icon: "lightbulb"
sidebarTitle: "概要"
translationSourceHash: ea8deef0
...
translationBlockHashes:
  ...
  "Hardware-Specific Issues": 99ea13bb
...
アーキテクチャの不一致、不足しているモデル、読み込みエラーを含む包括的なモデルのトラブルシューティングについては、専用の [モデル問題](/ja/troubleshooting/model-issues) ページを参照してください。
...
**Windows AMD Portable - ハードコードされたパスエラー:**
AMD ポータブル配布は CI 環境で `D:\a\ComfyUI\` にビルドされています。別のドライブ（例: `C:\ComfyUI_windows_portable\`）に展開すると、`offload-arch.exe` などのツールが `"Fatal error in launcher: Unable to create process"` または `access violation` で失敗する場合があります。

優先順位の高い順の解決策:

1. **ポータブルを再展開** — `ComfyUI_windows_portable_amd` フォルダー全体を削除します。[7-ZIP](https://7-zip.org/) を使用して `.7z` ファイルを CI ビルドパスと同じドライブに再展開します。最も簡単な回避策は、利用可能であれば `D:\` を使用することです。
2. **代わりに DirectML を使用** — 再展開が機能しない場合や D ドライブがない場合は、ポータブル環境内で torch-directml をインストールします:
   ```bat
   .\python_embeded\python.exe -m pip install torch-directml
   .\python_embeded\python.exe -s ComfyUI\main.py --directml
   ```
3. **ポータブルを再ダウンロード** — [リリースページ](https://github.com/Comfy-Org/ComfyUI/releases) から最新のリリースを再度ダウンロードします。新しいビルドでパス依存関係が修正されている可能性があります。

> **注意:** `torch-directml` はネイティブの ROCm ビルドよりも大幅に遅く、1 年以上更新されていません。動作する場合は AMD ROCm ポータブル（デフォルト）を強く推奨します。
