...
git URL 経由でカスタムノードをインストールする際、または ナイトリー バージョンを使用する際に `This action is not allowed with this security level configuration` エラーが発生した場合は、`config.ini` ファイルでセキュリティレベルを調整する必要があります。
...
<Tip>
git URL 経由または ナイトリー バージョンでカスタムノードをインストールするには、`config.ini` で `security_level = weak` を設定してください。
</Tip>
