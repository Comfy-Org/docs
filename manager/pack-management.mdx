...
3. Or click the version badge on the node card or in the detail panel, then pick a release in **Select Version**

The version list shows the newest release first, and releases the registry has flagged carry a **Flagged** badge. When a pack has no Active release, the picker says so, and you can still select a Flagged release to request installation, subject to your Manager security settings.
...
2. Click **Update** to move the pack to its latest Active release
3. Use the dropdown next to **Update** to choose another update target:
   - **Latest Active** (default): the newest release the registry marks as Active
   - **Latest Installable**: also considers releases the registry marks as Flagged or Pending, subject to your Manager security settings
4. To move a pack to one exact release, pick it in **Select Version** instead
