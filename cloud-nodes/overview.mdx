...
This list is the `get_node_list()` return in `comfy_api_nodes/nodes_comfy_cloud.py` in the ComfyUI repository, which is the only thing that decides what registers. Count the entries there, not the classes in the file, and not any planning document: the file defines a ninth node (`ComfyCloudMiniMaxH3ReferenceToVideoNode`) that is commented out of that list, and planning documents for this feature have drifted from what actually shipped at least once.
...
- No reference-to-video. A MiniMax H3 reference-to-video node exists in the source but is not registered in this release, pending a server-side fix, so it will not appear in the node library.
