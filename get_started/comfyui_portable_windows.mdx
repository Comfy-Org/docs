---
title: "ComfyUI Portable (Windows) Installation Guide"
...
ComfyUI Portable is a standalone packaged complete ComfyUI Windows version that has integrated an independent Python (python_embeded) required for ComfyUI to run. You only need to extract it to use it. Currently, the portable version supports running through Nvidia graphics cards or CPU.
...
├── run_cpu.bat                 // Double click to start ComfyUI (for AMD GPU or CPU only)
└── run_nvidia_gpu.bat          // Double click to start ComfyUI (for Nvidia GPU)
```

## Getting Started with Your First Image Generation

### 1. Download and Install Required Models

The portable package does not come with any drawing models, so you need to download the corresponding models yourself. If you don't know where to start, you can visit the link below and download the **stable diffusion 1.5** model
- [Click here to download Stable Diffusion 1.5](https://huggingface.co/Comfy-Org/stable-diffusion-v1-5-archive/blob/main/v1-5-pruned-emaonly-fp16.safetensors)

Please save the downloaded `v1-5-pruned-emaonly-fp16.safetensors` file to the `ComfyUI_windows_portable/ComfyUI/models/checkpoints` folder
...
When your browser successfully opens the ComfyUI page, you will see an interface similar to the image below

![ComfyUI GUI](/images/comfyui-gui.png)

Following the numbered reference in the image, please follow these steps to complete your first image generation:
1. In the `Load Checkpoint` node, select the previously downloaded `v1-5-pruned-emaonly-fp16.safetensors` or your local model file
2. Click the `Queue` button, or use the shortcut `Ctrl + enter` to execute the image generation

Troubleshooting common issues:
- If the `Load Checkpoint` node shows no models to select from, please first verify that your model installation location is correct, or try refreshing the browser or restarting ComfyUI to detect the models in the corresponding folder
