...
<CardGroup cols={2}>
  ...
  <Card title="Team collaboration" icon="users" href="/cloud/workspace">
...
## Firewall access for generated outputs

If a corporate firewall blocks previews or downloads, allow outbound HTTPS on port 443 from users' browsers to `storage.googleapis.com`. Comfy Cloud's `/api/view` endpoint redirects to a short-lived Google Cloud Storage URL, so allowing `cloud.comfy.org` alone is not enough. Allow WebSocket upgrades to `cloud.comfy.org` if the app connects but workflow progress does not appear. Storage URLs use the `storage.googleapis.com` hostname directly. A wildcard subdomain rule is not needed.
