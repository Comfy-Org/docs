...
Comfy Cloud stores generated images and videos in Google Cloud Storage. The `/api/view` endpoint redirects previews and downloads to short-lived URLs on `storage.googleapis.com`, so browsers need outbound HTTPS access to that hostname on port 443. Allow WebSocket upgrades to `cloud.comfy.org` for workflow progress updates. Storage URLs use the exact `storage.googleapis.com` hostname, so you do not need to allow `*.storage.googleapis.com`.
