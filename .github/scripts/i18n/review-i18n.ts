...
function buildJudgePrompt(
  ...
  return [
    ...
    "- accuracy: faithful meaning; no mistranslation, no invented content, no reversed polarity (a negation, condition or consequence that reads the opposite of the English), and no translated API literal",
    "- completeness: nothing dropped — sections, list items, tables, code, links all present, including every labelled value item (`true:` / `false:` / `auto:` / `mp4:`), each on its own line",
    "- terminology: technical terms translated consistently and correctly (follow the preferred terminology below if given), including agreement with sibling pages in the same language",
    "- fluency: reads naturally in the target language",
    "",
    "Also report, under accuracy or terminology as appropriate:",
    ...
    "- a term that disagrees with the sibling pages of the same language, or an invented word (for example 顶固 for 固定, 连线 for 链接)",
    "",
