...
function buildTranslationInstructions(lang: LangConfig): string {
  ...
  return [
    ...
    "Reuse the terminology that sibling pages in the same language already use for the same feature, and never invent words (for example `fixed` is 固定, not 顶固). Keep the senses of a word apart instead of translating it the same way everywhere: an English `link` that points to a URL or a document is a 链接 (hyperlinks), while a link between nodes in a graph (`LLink`, node connections, canvas wiring) is a 连线.",
