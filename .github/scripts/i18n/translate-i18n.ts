...
function buildTranslationInstructions(lang: LangConfig): string {
  ...
  return [
    ...
    "Reuse the terminology that sibling pages in the same language already use for the same feature; never invent words (for example `fixed` is 固定, not 顶固; `link` is 链接, not 连线).",
