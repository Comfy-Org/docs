...
The **Image Style Reference (Krea-2 Turbo)** subgraph exposes the following controls:

| Control | Description |
|---------|-------------|
| **Image Input** | Connect your reference images via the LoadImage node (supports up to 10 images) |
| **Text String (User Prompt)** | The text prompt describing the image you want to generate |
| **prompt_enhance** | Toggle LLM-powered prompt expansion on/off |
| **LLM_max_token** | Maximum token length for prompt enhancement |
| **Width / Height** | Output resolution (controlled by ResolutionSelector) |
...
The style reference workflow uses the **INT8** model variant optimized for style reference tasks:

| Model | File | Download |
|-------|------|----------|
| Diffusion Model (INT8) | `krea2_turbo_int8_convrot.safetensors` | [Download](https://huggingface.co/Comfy-Org/Krea-2/blob/main/diffusion_models/krea2_turbo_int8_convrot.safetensors) |

Place the model file in `ComfyUI/models/diffusion_models/`.
