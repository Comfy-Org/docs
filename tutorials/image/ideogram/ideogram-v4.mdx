...
Ideogram 4.0 has a built-in safety filter. Non-JSON (plain-text) prompts have a higher false-positive rate — they are more likely to be incorrectly flagged as NSFW. To reduce the chance of your prompt being blocked:

- **Use structured JSON prompts** — the model was trained on JSON, so it understands them more precisely, resulting in fewer false positives.
- If your prompt is blocked, try rewriting it as a JSON prompt.
