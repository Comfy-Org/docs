...
All three workflows sample with `steps` 25, `cfg` 1, the `euler` sampler, and the `simple` scheduler. The text to image workflow keeps the seed fixed, and the image edit and background removal workflows randomize it.
