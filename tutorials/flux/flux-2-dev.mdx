---
title: "ComfyUI Flux.2 Dev Workflow Example"
...
To run the workflows in this guide successfully, you first need to download the following model files.
...
1. In the `Load Diffusion Model` node, load the `flux2_dev_fp8mixed.safetensors` model
2. In the text encoder loader node, ensure `mistral_3_small_flux2_bf16.safetensors` is loaded
3. In the `Load VAE` node, ensure `flux2-vae.safetensors` model is loaded
4. Modify the prompts in the text encode node
