...
| MiniMax H3 Reference to Video               | MiniMax H3 Max                       | 480P       | 15.09 / sec, plus 24.14 / 1K reference tokens beyond 4,096 free tokens |
| MiniMax H3 Reference to Video               | MiniMax H3 Max                       | 768P       | 24.14 / sec, plus 24.14 / 1K reference tokens beyond 4,096 free tokens |
...
**MiniMax H3 Max Reference to Video** reference billing is token-based. Reference tokens are pooled across every image, video, and audio clip in the request. The first 4,096 tokens are free; each 1K tokens beyond that costs 24.14 credits. Image tokens count as (width × height) / 1024: a 1024×1024 image is 1,024 tokens, so the first 4 reference images are free and each additional one costs 24.14 credits; a 2048×2048 image is 4,096 tokens, so the first one is free and each additional one costs 96.58 credits. Reference videos cost 2,886 tokens per second at 480P and 7,459 tokens per second at 768P (the rate follows the output resolution, not the resolution of the reference clip; 69.68 credits/sec at 480P, 180.09 credits/sec at 768P). Audio costs about 80 tokens per second (1.93 credits/sec), regardless of output resolution.
