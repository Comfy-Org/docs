...
OpenRouter partner nodes bill at per-model token rates. **Credits = USD × 211** (see page header). Prompt and completion USD rates (per token) match `comfy_api_nodes/nodes_openrouter.py`. Final charges follow metered token usage from the chat completion response.
