...
| Seedream 5.0 Pro Layer Separation                   | 6.75 (1K, 1.5K) / 6.75 – 13.50 (2K) |
...
Billed per frame. Total credits = **(credits / frame) × (24 × `duration` + 1)**, where `duration` is 4–30 seconds (24 fps output). Resolution is 480p, 720p, or 1080p. Rates are approximate.

| Model                    | Resolution | Credits / 5s | Credits / 10s |
| :----------------------- | :--------- | :----------- | :------------ |
| Seedance 2.5 (no video)  | 480p       | 156.4        | 311.5         |
| Seedance 2.5 (no video)  | 720p       | 351.6        | 700.3         |
| Seedance 2.5 (no video)  | 1080p      | 865.0        | 1722.8        |

**Reference to Video** with a reference video connected uses a lower per-frame rate. Reference videos must be at least 2 seconds long, and their total duration is limited to 15 seconds. Billing is based on the source clip length plus any generated extension, up to 30 seconds.

| Resolution | Credits / frame | Approx. credits / second (24 fps) |
| :--------- | --------------: | --------------------------------: |
| 480p       | 0.773           | 18.55                             |
| 720p       | 1.738           | 41.71                             |
| 1080p      | 4.277           | 102.65                            |

Total credits = **(credits / frame) × (24 × `total_duration` + 1)**, where `total_duration` is the source video duration plus the generated extension. The extension can be up to 30 seconds. The per-second figures are approximate because billing includes one additional frame and aspect ratio adjustments.

For reference, the resulting charges are:

| Resolution | Credits / 5s source | Credits / 10s source |
| :--------- | ------------------: | -------------------: |
| 480p       | 93.5 – 650.1        | 186.3 – 742.9        |
| 720p       | 210.3 – 1461.6      | 418.8 – 1670.2       |
| 1080p      | 517.5 – 3597.0      | 1030.8 – 4110.2      |

Aspect ratio has a small effect on the per-frame rate: 4:3 and 3:4 add ~2.7% (480p), ~0.6% (720p), and ~0.1% (1080p); 21:9 adds ~4.5% (480p), ~0.5% (720p), and ~0.6% (1080p) versus 16:9.
