...
`smart_low_poly` adds **+21.1 / run** on every generation node and caps `face_limit` at 20,000 (10,000 with `quad`).
...
`quad` add-on (+10.55 / run) applies to **Tripo: Text to Model** and **Tripo: Image to Model** only. `smart_low_poly` adds **+21.1 / run** and caps `face_limit` at 20,000 (10,000 with `quad`).
