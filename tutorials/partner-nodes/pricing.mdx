...
Billed per frame. Total credits = **(credits / frame) × (24 × `duration` + 1)**, where `duration` is 4–30 seconds (24 fps output). Resolution is 480p or 720p. Rates are approximate.
...
Aspect ratio has a small effect on the per-frame rate: 4:3 and 3:4 add ~0.6% (480p) to ~1% (720p), 21:9 adds ~1.3% (480p) to ~0.4% (720p) versus 16:9.
