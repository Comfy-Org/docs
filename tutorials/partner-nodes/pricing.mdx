...
The **GPT Image 2.5** models (`gpt-image-2.5-flare`, `gpt-image-2.5-sunburst`) share the same rates and add `xhigh` and `max` quality tiers. Both image and text input are billed; the table uses the image-input rate for input.

| model                        | token type     | Credits / 1M |
| :--------------------------- | :------------- | :----------- |
| gpt-image-2.5-flare          | input          | 2413.84      |
| gpt-image-2.5-flare          | output (image) | 9051.9       |
| gpt-image-2.5-sunburst       | input          | 2413.84      |
| gpt-image-2.5-sunburst       | output (image) | 9051.9       |

Output token counts scale with size and quality (for example, 1024x1024 at `low` is about 196 output tokens, roughly 1.86 credits). Each 1024x1024 reference image adds about 2.47 credits of image input. Multiplying images (`n`) scales the total.
