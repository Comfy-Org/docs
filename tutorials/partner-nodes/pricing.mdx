...
OpenRouter partner nodes bill at the per-model rates listed on [OpenRouter Models](https://openrouter.ai/models). **Credits = USD × 211** (see page header). Prompt and completion USD rates (per token) match `comfy_api_nodes/nodes_openrouter.py` and the OpenRouter API `pricing` fields for each model slug. Final charges follow metered token usage from the chat completion response.

### LLM (`OpenRouterLLMNode`)

Models selectable in the node are listed below. Per 1M tokens.
...
Optional reference images and videos are sent as URLs; vision-capable models bill input tokens for multimodal content at the same input rate.
