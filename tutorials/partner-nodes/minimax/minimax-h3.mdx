...
**MiniMax H3 Max** is the faster companion model added to the same API nodes in ComfyUI 0.34.2. It generates faster than real time, accepts a prompt of up to 50,000 characters, and offers 480P and 768P resolutions with an optional two-stage graph: prompt enhancement through Context IR, then regeneration to 2K. Unlike the base model, H3 Max does not support the AIGC watermark.
...
The template library ships with three API example workflows for the base H3 model and four for H3 Max:
...
- **Match durations across nodes**: Generation, prompt enhance, and upscale must use the same duration, or the stages drift out of sync
...
- **No watermark**: H3 Max does not support the AIGC watermark option
