...
- `s1` (legacy): accepts a single optional voice input and uses the parenthesis emotion tag syntax described later in this guide

### Connecting voices

The `voice` output of the Voice Selector or the Instant Voice Clone connects to the `voice` input of the Text to Speech node. With one connected voice, the node passes it internally as `reference_id`. With two or more, the text must mark speakers with `@Voice1`, `@Voice2`, and so on, and every connected voice must appear in the text at least once or the node raises an error. For the full tag reference, see the sections below.

### Voice cloning limits

The Instant Voice Clone node accepts 1 to 20 recordings, recommends 10 to 30 seconds per clip, and validates that the total duration stays under 270 seconds. The `enhance_audio_quality` option is enabled by default.
...
Generate speech from your script with a preset voice. Write your text with the inline tags from the sections above, then pick another preset in the Voice Selector or connect a cloned voice.
...
Transcribe an audio clip to text. Language is detected automatically across 13+ languages, and enabling `precise_timestamps` returns per-segment start and end times as JSON, which is handy for subtitles.
