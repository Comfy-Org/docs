...
Edit an input image with Flare: keep the scene, camera, and lighting fixed while changing outfit and hairstyle. Good for quick outfit swaps and style iterations.
...
1. **State what to keep**: The template prompt opens with "Keep the background environment... completely unchanged" before describing the edit. This anchoring is what keeps the rest of the frame stable
2. **One change per run**: Describe a single edit (outfit, then hairstyle in the example) rather than a list of unrelated changes
3. **Mask for local edits**: Connect a mask to limit the edit to specific regions; white areas are replaced. A mask requires exactly one reference image
...
Edit an input image with Sunburst's high fidelity: the template changes only the hairstyle and makeup while keeping head, face, body, clothing, background, and lighting unchanged. Use it for edits where identity preservation matters.
...
1. **Keep-list before change-list**: The template prompt names everything to preserve first ("Keep head, face, body, flowers, sweater, background, lighting unchanged"), then the single change. Follow that order
2. **Strong subject consistency**: Sunburst keeps identity across sequential edits, which makes it suitable for frame-to-frame generation with the same character
3. **Transparent backgrounds**: Both variants can return `transparent` backgrounds; useful for compositing the result into another scene downstream
