...
Comfy Router runs FLUX Video Upscale behind one host, one credential and one route. The request body is Black Forest Labs' own native JSON input. The response is their native JSON output, unwrapped. A call written against the Black Forest Labs API becomes a Router call by changing the host.
