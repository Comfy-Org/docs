...
Comfy Router runs Nano Banana 2 behind one host, one credential and one route. The request body is Google's own native JSON input. The response is their native JSON output, unwrapped. A call written against the Google API becomes a Router call by changing the host.
