...
<Note>
Router has not published an authored input schema for this model yet: `GET /v2/models/vertexai/gemini-3-pro-image/openapi.json` returns an open object with `x-comfy-input-schema-authored: false`. The fields below are from the provider's own API documentation and are not yet validated server side.
</Note>

This is the Gemini `generateContent` body. `contents` is required: a list of turns, each with a `role` and `parts` (text, or `inlineData` with a base64 image and `mimeType` for image editing). Ask for an image with `generationConfig.responseModalities: ["IMAGE"]` and set the shape with `imageConfig.aspectRatio` and `imageConfig.imageSize` (`1K`, `2K` or `4K`). `seed` and `safetySettings` are also accepted.
