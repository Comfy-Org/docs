...
The [Grok Imagine Video 1.5](/built-in-nodes/GrokVideoNode) Partner Node enables high-quality video generation with native audio from a single image input. Powered by xAI's latest Grok model, it delivers **#1 ranked performance on the Image-to-Video Arena leaderboard**, beating Sora 2, Veo 3.1, and Seedance 2.0 in blind user testing.

The model is available in two variants:

- **Grok Imagine Video 1.5** — full quality mode, produces rich motion dynamics with synchronized audio
- **Grok Imagine Video 1.5 Fast** — optimized for speed, generating a 6-second 720p video in approximately **25 seconds** (down from 40+ seconds in the previous generation)

Both variants support **native audio generation** — sound effects, ambience, and even dialogue are synthesized in the same pass, eliminating the need for a separate audio pipeline. Output resolutions go up to **720p**, with configurable aspect ratios and duration up to 30 seconds.
