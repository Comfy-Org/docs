...
## Kling 3.0: Motion Control {#api_kling_motion_control3}

Apply the motion of a reference video to a character image. Upload a character image plus a motion reference video and the model drives your character through the action in the clip while keeping their identity. This workflow runs the Kling 3.0 model, which improves facial consistency over previous versions.

<img src="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/templates/api_kling_motion_control3-1.webp" alt="Kling 3.0: Motion Control workflow preview" />

<CardGroup cols={2}>
  <Card title="Run on Comfy Cloud" icon="cloud" href="https://cloud.comfy.org/?template=api_kling_motion_control3&utm_source=docs&utm_medium=referral&utm_campaign=kling-3-0">
    Open in Comfy Cloud
  </Card>
  <Card title="Download Workflow" icon="download" href="https://github.com/Comfy-Org/workflow_templates/blob/main/templates/api_kling_motion_control3.json">
    Download JSON or search &quot;Kling 3.0: Motion Control&quot; in Template Library
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="future_robot.png" icon="image" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/future_robot.png">
    `LoadImage` node 2 · `future_robot.png`
  </Card>
  <Card title="boxing_man.mp4" icon="video" href="https://raw.githubusercontent.com/Comfy-Org/workflow_templates/main/input/boxing_man.mp4">
    `LoadVideo` node 3 · `boxing_man.mp4`
  </Card>
</CardGroup>
...
- `model`: keep `kling-v3` for Kling 3.0. The node also accepts `kling-v2-6` for the previous generation.
- `character_orientation`: `video` follows the orientation of the motion reference video, `image` keeps the orientation of the character image. This setting also sets how long the reference clip can be: 3 to 10 seconds for `image`, 3 to 30 seconds for `video`.
- `mode`: `pro` for higher quality output, `std` for faster and cheaper tests.
- `keep_original_sound`: keeps the audio of the reference video in the result.

This workflow uses the same `Kling Motion Control` node as the [Kling 2.6 Motion Control](/tutorials/partner-nodes/kling/kling-motion-control) example, so the input tips on that page apply here too.
