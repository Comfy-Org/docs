---
title: "Kling 3.0 Motion Control API Node ComfyUI Official Example"
description: "Learn how to use the Kling 3.0 Motion Control Partner node in ComfyUI with Element Binding for facial consistency across angles, emotions, and occlusions"
sidebarTitle: "Kling 3.0 Motion Control"
---

import ReqHint from "/snippets/tutorials/partner-nodes/req-hint.mdx";
import UpdateReminder from "/snippets/tutorials/update-reminder.mdx";

Kling 3.0 Motion Control is a specialized multimodal model developed by Kuaishou that enables precise motion transfer from reference videos to character images. Building on Kling 2.6 Motion Control, this upgrade introduces **Element Binding** — a new facial consistency system that keeps your character's face stable and recognizable across every angle, emotion, and occlusion in the generated video.

By combining a **Reference Image** (your character) and a **Motion Reference Video** (the action), the AI applies the movement, expression, and pacing of the video to your static character while maintaining their identity. Unlike standard image-to-video generation that guesses motion from text prompts, Motion Control uses a reference video as the movement blueprint.

## What's new in 3.0

- **Facial consistency from any angle**: Stable features and smooth expressions even in complex, multi-angle, or long-duration motion
- **Accurate complex emotions**: Upload multiple face reference images to improve expression fidelity across your character's full emotional range
- **Face restoration through occlusions**: Hats, fans, hands, and other partial obstructions no longer break character identity
- **Steady clarity during camera movement**: Face quality stays consistent as the camera zooms, pans, or tracks
...
1. Update ComfyUI to the latest version or visit [Comfy Cloud](https://links.comfy.org/4lf2SaC)
2. Search for the **"Kling Motion Control"** node
3. Prepare your reference image and reference motion video, and run the example workflow
...
- **Multiple face references**: For complex emotions, upload multiple face reference images showing different expressions to improve fidelity.
