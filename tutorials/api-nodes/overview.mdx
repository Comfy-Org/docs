...
<Note>
  **Note:** The API key discussed here is your ComfyUI Account API Key, which is different from the Registry Publishing API Key used by developers to publish custom nodes. See [Understanding Different API Keys](/registry/overview#understanding-different-api-keys-in-comfyui) for clarification.
</Note>
