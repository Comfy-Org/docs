...
description: "This guide covers how to use the Ideogram 3.0 API node in ComfyUI"
...
When both image and mask inputs are provided, the node operates in Image Editing mode.

> Note: Both image and mask must be provided and have the same dimensions
...
1. First, right-click the `Load Image` node and select `Open in MaskEditor`. If you're unfamiliar with the mask editor, check the [Mask Editor](/interface/maskeditor) guide
2. Enter your description for the masked area in the `prompt` field of the `Ideogram V3` node
