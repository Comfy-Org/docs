...
ComfyUI natively supports MiniMax H3. The template library currently ships with five example workflows:
...
These five are example templates, not an exhaustive list. The model supports more generation modes through the native MiniMax H3 nodes: first/last-frame image-to-video (fl2va) via the `MiniMaxH3ImageToVideo` node, and reference-driven generation with images, videos, and audio (ref2va) via the `MiniMaxH3ReferenceToVideo` node. You can build additional workflows with these nodes.
