...
Beyond the example templates, the native MiniMax H3 nodes support guide anchoring at any frame, prompt embeddings, and latent noise masks. These capabilities were added in recent ComfyUI updates: [guide anchoring at any frame](https://github.com/Comfy-Org/ComfyUI/pull/15439), [prompt embeddings](https://github.com/Comfy-Org/ComfyUI/pull/15697), and [latent noise masks](https://github.com/Comfy-Org/ComfyUI/pull/15375). Update ComfyUI to the latest version to use them.

### Anchoring guides at any frame

The example templates anchor keyframes only at the first and last frame of the video. The `MiniMaxH3AddGuide` node removes that restriction: it anchors a guide at any frame, on a continuous time axis.
...
MiniMax H3 supports ComfyUI's standard `embedding:` syntax in prompts. Place an embedding file in `ComfyUI/models/embeddings/` and reference it in the prompt by name, for example `embedding:my_embedding`. The embedding is loaded and mixed into the text conditioning just like with any other ComfyUI model.

### Inpainting and extension with latent noise masks

MiniMax H3 supports noise masks on both the video and audio latents. Connect a mask to the sampler's `denoise_mask` input as usual: a value of `0` preserves the corresponding latent region, while `1` regenerates it. Video masks snap to the model's 2x2 latent patch grid, and audio masks to whole latent frames. Use it for local inpainting, object removal, or extending a clip while keeping the existing content stable.
