...
The three example templates only scratch the surface. The native MiniMax H3 nodes also support guide anchoring at any frame, prompt embeddings, and latent noise masks. Update ComfyUI to the latest version to use these workflows.
...
Connect the conditioning and latent outputs of a MiniMax H3 node to `MiniMaxH3AddGuide`, then provide at least one guide input:
...
MiniMax H3 supports noise masks on both the video and audio latents. Connect a mask to the sampler's `denoise_mask` input as usual: masked-out regions are regenerated while the rest stays fixed. Video masks snap to the model's 2x2 latent patch grid, and audio masks to whole latent frames. Use it for local inpainting, object removal, or extending a clip while keeping the existing content stable.
