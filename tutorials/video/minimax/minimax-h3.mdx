...
The template ships with a fast preview size. For full-quality output, raise Megapixels to about `1.0` at 16:9, which yields 1376x768: H3's native canvas, a 768px short edge.
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA (Optional): minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16.safetensors">
    Only needed for the optional Lightning LoRA turbo mode. Place in <code>ComfyUI/models/loras/</code>
  </Card>
</CardGroup>
...
3. **Resolution**: H3's native canvas is a 768px short edge, and resolutions are rounded to a multiple of 32
...
6. **Turbo mode (optional)**: The workflow includes a Lightning LoRA (`minimax_h3_fl2v_turbo_8step_v1.0_comfyui_bf16`) for faster generation. Enable `turbo_mode` on the MiniMax H3 node to generate at 8 steps instead of 20; quality drops slightly
...
<CardGroup cols={2}>
  ...
  <Card title="LoRA (Optional): minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16" icon="download" href="https://huggingface.co/Comfy-Org/MiniMax-H3/blob/main/loras/minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16.safetensors">
    Only needed for the optional Lightning LoRA turbo mode. Place in <code>ComfyUI/models/loras/</code>
  </Card>
</CardGroup>
...
6. **Turbo mode (optional)**: Enable the **Lightning LoRA** checkbox in the workflow to run the `ref2va` model with the 4-step turbo LoRA (`minimax_h3_ref2v_turbo_4step_v0.1_comfyui_bf16`) instead of the full 20 steps. Much faster, with a small quality tradeoff
