...
MiniMax H3 is a large multimodal model, so local generation uses a lot of VRAM. Each workflow loads the diffusion model, a 32B text encoder (NVFP4-AWQ quantized, no Blackwell GPU required), and two VAEs into memory at once.

- **24 GB or more of VRAM** (for example an RTX 4090) runs all three workflows comfortably at full quality
- **Less VRAM**: the workflows can still run, but keep the Megapixels setting at the fast preview size, use the INT8 pruned checkpoint, and expect slower generation. If you hit out-of-memory errors, lower the resolution further, or run the workflow on Comfy Cloud instead

Which workflow should you pick?

- **T2V** is the lightest. It loads the `fl2va` diffusion model and generates from a text prompt
- **I2V** uses the same `fl2va` model with an input image, and optionally first/last-frame keyframes
- **R2V** is the heaviest. It loads a separate `ref2va` diffusion model plus up to 9 reference images, 3 reference videos, and 3 reference audio clips

On any GPU, the Sage Attention section below roughly doubles generation speed.
