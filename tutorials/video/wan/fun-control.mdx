...
- We still use `SaveAnimatedWEBP` because we currently don't support embedding workflow information in **mp4**. To preserve the workflow in the video, we use WebP images
- You can replace `DWPose Estimator` with other preprocessors from the `ComfyUI-comfyui_controlnet_aux` node package

## Other Wan2.1 Fun Control or Video-Related Custom Nodes
