...
Each `Motion Transfer (Wan Animate 2)` subgraph generates 81 frames, roughly 3.4 seconds at 24 fps. Anything longer is built by chaining segments, and the template ships the pieces for it.

The template already contains a second, bypassed `Motion Transfer (Wan Animate 2)` subgraph. Select it and press `Ctrl + B` to enable it, set its prompt and resolution to match the first segment, then connect its `image` output to the `Batch Images` node.

To add further segments, select a `Motion Transfer (Wan Animate 2)` subgraph and press `Ctrl + C` followed by `Ctrl + Shift + V` to duplicate it, then wire the new segment to the previous one:

- Connect the previous subgraph's `IMAGE` output to the new subgraph's `continue_motion` input, which uses the last generated frame as a temporal anchor.
- Connect the previous subgraph's `video_frame_offset` output to the new subgraph's `video_frame_offset` input, so the new segment keeps reading the driving video where the previous one stopped.
- Connect the new subgraph's `image` output to the `Batch Images` node to extend the total output.

Consecutive segments overlap by one frame by design, which keeps the motion continuous. If a seam shows a one-frame hiccup, remove the new segment's first frame using that subgraph's `trim_image` output.

You need roughly one segment per 81 frames of driving video. The expression node in the template works that number out from the driving video length.

Looping is not available as a built-in feature yet, so extending a video stays a manual step.
