...
Longer videos are handled inside the workflow: the `Motion Transfer (Wan Animate 2)` subgraph loops (`StartLoop` / `EndLoop`) and keeps generating until it has covered the whole driving video, so one subgraph is all you need. Nothing has to be duplicated or rewired.

One loop iteration generates 81 frames, about 3.4 seconds at 24 fps. That is the `Int (fragment_length)` value inside the subgraph. You can raise it, but it has to stay a legal Wan length: 81, 85, 89, and so on, which is `4n + 1`.

The loop math reads the driving video's frame count, works out how many continuations are needed, and the tail is trimmed so the output has exactly as many frames as the driving video. Each continuation repeats the previous segment's last frame and that duplicate is removed, so every extra segment adds `fragment_length - 1` new frames.

Looping uses the core loop nodes, so the workflow needs ComfyUI 0.36.0 or newer.

The `Video Stitch` subgraph is the comparison preview: it places the generated video next to the driving video. It is not part of the extension path.
