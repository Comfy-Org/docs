...
1. Ensure the `Load Diffusion Model` node loads the `kandinsky5lite_t2v_sft_5s.safetensors` model.
2. Ensure the `DualCLIPLoader` node loads the `qwen_2.5_vl_7b_fp8_scaled.safetensors` and `clip_l.safetensors` models.
3. Ensure the `Load VAE` node loads the `hunyuan_video_vae_bf16.safetensors` model.
4. (Optional) Modify the prompts in the `CLIP Text Encoder` node.
...
1. Ensure the `Load Diffusion Model` node loads the `kandinsky5lite_i2v_sft_5s.safetensors` model.
2. Ensure the `DualCLIPLoader` node loads the `qwen_2.5_vl_7b_fp8_scaled.safetensors` and `clip_l.safetensors` models.
3. Ensure the `Load VAE` node loads the `hunyuan_video_vae_bf16.safetensors` model.
4. In the `Load Image` node, upload the image to be used as the initial frame.
5. (Optional) Modify the prompts in the `CLIP Text Encoder` node.
