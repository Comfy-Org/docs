...
Here is the overall approach for troubleshooting custom node issues:
 ...
 ```mermaid
 flowchart TD
    A[Problem Encountered] --> B{Does the issue disappear after disabling all custom nodes?}
...
In this article, we will introduce the binary search approach for troubleshooting custom node issues, which involves checking half of the custom nodes at a time until we locate the problematic node.

Please refer to the flowchart below for the specific approach - enable half of the currently disabled nodes each time and check if the issue appears, until we identify which custom node is causing the problem
   ...
   <Step title="Use Binary Search to Locate Custom Nodes">
      ...
      <Tabs>
         ...
         <Steps>
            ...
            Before starting, enter the `<your installation location>\ComfyUI\` folder
                  <Tabs>
                     <Tab title="Windows">
