...
If after we have disabled all the custom nodes, the issue is gone, then it is a custom node problem.

We have provided few way that allow you quickly diagnose if the issue is caused by custom nodes.
