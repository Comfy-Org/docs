...
**Windows AMD Portable - hardcoded path errors:**
The AMD portable distribution is built in a CI environment at `D:\a\ComfyUI\`. If you extract to a different drive (e.g. `C:\ComfyUI_windows_portable\`), tools like `offload-arch.exe` may fail with `"Fatal error in launcher: Unable to create process"` or `access violation`.

Solutions in order of preference:

1. **Re-extract the portable** — Delete the entire `ComfyUI_windows_portable_amd` folder. Re-extract the `.7z` file to the **same drive as the CI build path** using [7-ZIP](https://7-zip.org/). The simplest workaround is to use `D:\` if available.
2. **Use DirectML instead** — If re-extracting does not work or you do not have a D drive, install torch-directml inside the portable environment:
   ```bat
   .\python_embeded\python.exe -m pip install torch-directml
   .\python_embeded\python.exe -s ComfyUI\main.py --directml
   ```
3. **Redownload the portable** — Download the latest release again from the [releases page](https://github.com/Comfy-Org/ComfyUI/releases). A newer build may have fixed the path dependency.

> **Note:** `torch-directml` is significantly slower than the native ROCm build and has not been updated in over a year. The AMD ROCm portable (the default) is strongly recommended when it works.
