...
# For AMD GPUs (Linux only - ROCm 6.4)
...
```bash
# Install stable ROCm PyTorch (6.4 at the time of writing)
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/rocm6.4

# For nightly builds (ROCm 7.1 at the time of writing, which might have performance improvements)
