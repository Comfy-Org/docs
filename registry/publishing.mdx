...
<Note>
  **Important:** This API key is specifically for publishing custom nodes to the Registry. This is different from:
  - The ComfyUI Account API Key (used for accessing paid API nodes in workflows)
  - The ComfyUI Server API (used for remote workflow execution)
  
  See [Registry Overview](/registry/overview#understanding-different-api-keys-in-comfyui) for details on the different types of API keys.
</Note>
