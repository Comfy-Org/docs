...
<Note>
**Looking for the API node API key?** If you're trying to use API nodes in your workflows (not publish custom nodes), you need a different API key. See the [API node API key integration](/development/comfyui-server/api-key-integration) documentation for details on using API nodes in headless mode.
</Note>
