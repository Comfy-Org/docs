...
<Icon icon="rotate" iconType="solid" size={20}/> **Automated Testing** : Authors can continuously test new versions of their custom nodes against workflows that use their nodes. Our CI/CD dashboard automatically checks for regressions in image quality, runtime, or if a workflow stops running completely.

<Icon icon="shield" iconType="solid" size={20}/> **Stable Upgrades**: Developers frequently publish new versions of their custom nodes which often break workflows that rely on them. With registry nodes being semantically versioned, users can now choose to safely upgrade, deprecate, or lock their node versions in place, knowing in advance how their actions will impact their workflows.

<Icon icon="user-group" iconType="solid" size={20}/> **Reproducibility**: Creators will be able to specify which node versions they used when sharing workflows. This will create a more consistent user experience for users trying to reproduce workflows.
