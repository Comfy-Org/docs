...
Add this file to the root of your repository. Make sure your publisher id and name are filled in correctly. Check the [specifications](/registry/specifications) for more information on the pyproject.toml file.
...
Automate the node publishing process through github actions.
...
<Warning>If your working branch is named something besides `main`, such as `master`, change the branch value in the yaml file accordingly.</Warning>
</Step>
<Step title="Test the Github Action">

Push a change to your `pyproject.toml` file by updating the version number. You should see your node automatically update on the registry.

Now whenever you make a change to your `pyproject.toml` on your `main` branch, our github action will automatically publish your updated node to the registry. 

<Tip>To debug if the github action is properly running, go to the `Actions` tab in your repository. </Tip>
</Step>
